...
const server = http.createServer((req,res)=>{
  ...
  if (p==='/node-city/index.html'){let t=fs.readFileSync(path.join(ROOT,'node-city/index.html'),'utf8');const before=t.length;t=t.replace('horizontal-overflow guard the bar actually measures. */','horizontal-overflow guard the bar actually measures.');if(t.length===before)throw new Error('PATCH DID NOT APPLY');res.writeHead(200,{'Content-Type':'text/html'});return res.end(t);}
...
if(rost) await rost.screenshot({path:path.join(OUT,'rosterFIXED-'+VP.width+'.png')});
const card = await frame.$('#campcard');
if(card) await card.screenshot({path:path.join(OUT,'cardFIXED-'+VP.width+'.png')});
