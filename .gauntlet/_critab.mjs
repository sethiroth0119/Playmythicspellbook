...
const OUT='/tmp/claude-0/-home-user-Playmythicspellbook/40854a97-ff53-55db-aa08-6d67184d4a8e/scratchpad/ab';
fs.mkdirSync(OUT,{recursive:true});
const report=[];
for(const s of SHOTS){
  const cover = await page.evaluate(([c,t])=>{
    ...
    /* UNBUILT-LAND FRACTION: project every tile centre (empty vs occupied)
       through THIS camera and count which land on screen. */
    const THREE=nc.three().THREE||window.THREE;
    const occ=new Set(Object.keys(nc.game.tiles));
    const HALF=12,GRID=24; let onEmpty=0,onOcc=0;
    const V=nc.three().scene; // need a Vector3; use camera.project via plain object
    const proj=(x,y,z)=>{ const v={x,y,z};
      ...
      const px=p[0]*ex+p[4]*ey+p[8]*ez+p[12], py=p[1]*ex+p[5]*ey+p[9]*ez+p[13],
            pz=p[2]*ex+p[6]*ey+p[10]*ez+p[14], pw=p[3]*ex+p[7]*ey+p[11]*ez+p[15];
      if(pw<=0)return null; return [px/pw,py/pw,pz/pw]; };
    for(let gx=0;gx<GRID;gx++)for(let gz=0;gz<GRID;gz++){
      const wx=gx-HALF+.5, wz=gz-HALF+.5;
      const q=proj(wx,0,wz); if(!q)continue;
      if(q[0]<-1||q[0]>1||q[1]<-1||q[1]>1)continue;
      // exclude the region the UI panel covers (right 20%) and HUD bands
      const sx=(q[0]*.5+.5), sy=(1-(q[1]*.5+.5));
      if(sy<0.13||sy>0.92)continue;
      if(occ.has(gx+','+gz))onOcc++; else onEmpty++;
    }
    return {onEmpty,onOcc};
  },[s.cam,s.tgt]);
  await page.waitForTimeout(1200);
  await page.evaluate(()=>{const g=window.__nc.three().scene.getObjectByName('wild'); if(g)g.visible=true;});
  await page.waitForTimeout(900);
  const on=await page.screenshot({path:OUT+'/'+s.n+'-on.png'});
  await page.evaluate(()=>{const g=window.__nc.three().scene.getObjectByName('wild'); if(g)g.visible=false;});
  await page.waitForTimeout(900);
  const off=await page.screenshot({path:OUT+'/'+s.n+'-off.png'});
  await page.evaluate(()=>{const g=window.__nc.three().scene.getObjectByName('wild'); if(g)g.visible=true;});
  report.push({shot:s.n, tilesOnScreen:cover});
}
/* NIGHT: same three framings at 22:00-ish by driving the day/night amount. */
