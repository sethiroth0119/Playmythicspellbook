...
async function _selfCheck() {
  ...
  try {
    ...
    /* ⚠ `truth` IS A STRUCTURAL COUNT, not the claim. It is how many
       ...
       taking it 106 → 130, and v121v167 added the 🌐 Global-radius toggle to the
       targeted block, taking it 130 → 131 (measured: the on-play block went 65
       → 66 .editor-field, and ed-onplay-global is the one new id), and the v121
       turn-effects round added the ⛔ Required box to the on-play block and to
       each of its six extra slots, taking it 131 → 138 (measured 138: exactly
       1 + 6 new .editor-field wrappers, every other invariant unchanged).
       The Card Filter persistence round (v121v177+) took it 138 → 137: the one
       shared, never-saved filter block under the six extra slots was an
       .editor-field and is gone; each slot's own cost row and card picker sit
       inside that slot's existing field (measured 137, same invariants).
       Purity Pact "keeps checking" (v121v178) took it 137 → 138: the
       🔥⚖️ pactbox is one .editor-field in the on-play block (measured 138,
       same invariants).
       The Fetch round (2026-09-19) took it 138 → 145: the 🌟🔍 Fetch box is
       injected as one .editor-field into the on-play block and each of its six
       extra slots (1 + 6, measured 145, same invariants).
       The purge round (2026-09-19) took it 145 → 152: the 💀 Destroy box is
       injected the same way (1 + 6, measured 152, same invariants).
       What this pin is actually for is the line below it:
    ...
    ok(dupId.editors === 1 && dupId.outside === 0 && dupId.hosts === 0
       && dupId.byIdFields === 0 && dupId.truth === 152 && !!dupErr
