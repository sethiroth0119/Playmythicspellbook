/* 🏙 DISTRICT SPECIALISATION DRIVER — layer 2, driven against the real page.
   shot.mjs discards the eval's return value and clips console lines at 400
   chars, so this variant PRINTS what the eval returns.
   Usage: node .gauntlet/drive-districts.mjs [out.png] [eval.js]                */
...
const out = process.argv[2] || '.gauntlet/shots/z-districts.png';
...
console.log('[scene]', JSON.stringify(built).slice(0, 400));
await page.waitForTimeout(3000);

const script = fs.readFileSync(process.argv[3] || '.gauntlet/districts-eval.js', 'utf8');
let r;
try { r = await page.evaluate(script); } catch (e) { r = { EVALERR: e.message, stack: String(e.stack).slice(0, 600) }; }
console.log('══ RESULT ══');
console.log(JSON.stringify(r, null, 1));
fs.mkdirSync(path.dirname(out), { recursive: true });
await page.screenshot({ path: out });
console.log('══ LOGS ══');
console.log(logs.filter(l => /District|Progress|Zoning|LandValue|error|Error/.test(l)).slice(-30).join('\n'));
await browser.close();
server.close();
