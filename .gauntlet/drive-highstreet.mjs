...
/* 🔴 STILL REPORTED, NOT ASSERTED — AND THE REASON MOVED. Before the
   ...
   🔴 AND THE SECOND CAUSE THIS NOTE USED TO NAME WAS NOT REAL — DO NOT ACT
   ON IT. It read: "measured with .gauntlet/_chainprobe, a Refinery with full
   input availability (fill 1.00, avail 1.00, no bottleneck) produced ZERO
   over 400 days, because productionTargets() plans from REALISED demand", and
   concluded that a freshly built deep chain cannot start itself.

   That measurement was an INSTRUMENT ARTEFACT. _chainprobe and its
   descendants read `f.lifetimeProduced || f.lastProduced`, and firms.js has
   no lifetimeProduced field at all — only lifetimeRevenue/lifetimeProfit — so
   the "400-day total" was one day's rate, and production is lumpy. Summed per
   tick instead, the shipped code runs the whole eight-stage clothing chain:
   73,887 clothing on 387 of 400 days, chemicalFeedstock on 339. See
   .gauntlet/drive-bootstrap.mjs, which asserts exactly that.

   A `derivedDemand` bootstrap was written against the false reading and
   reverted: it bankrupted gauntlet3's print works (printedCards cap 52 →
   cap 0) by sizing a crew off a plan the shop could never sell. The header of
   drive-bootstrap.mjs carries the full post-mortem.

   So this stays a report, and the remaining blocker is a LEDGER one, not a
   demand one: the 21 materials-tier ids the city now makes are still absent
   from index.html's RESOURCES, so round0p is red and a player cannot see the
   stock. That is what moves the bar. */
