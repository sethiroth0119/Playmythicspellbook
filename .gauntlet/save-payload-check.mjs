/* Reconciled-save check: does the ONE payload the four systems now share
   ...
   ⚠ THIS IS THE SHELF'S OWN CONTRACT, IN ISOLATION — NOT THE CITY'S SAVE.
     It mounts /src/naming and /src/palette against a three-tile stub and runs
     a 12-line STAND-IN for node-city's serialize() that writes the four fields
     the shelf owns (transit, paint, ext, meta). It has never called the
     iframe's serialize() or loadState(): measured, node-city's serialize()
     writes 37 top-level keys and this file writes 6, so `fin`, `economy`,
     `demog`, `pop`, `hh`, `power`, `water`, `pollution`, `raid` and every
     per-tile field could stop round-tripping and this file would stay green.
     The REAL cycle — the shipped file booted in Chromium, saved through
     saveNow(), reloaded through boot() → loadState(), every field diffed,
     plus the module-absent and tileless-payload paths — is
       node .gauntlet/drive-city-roundtrip.mjs
     Run that when the question is "does the city come back"; run this one
     when the question is "does the shelf still honour its erase guards". */
...
// ── a STAND-IN for node-city's serialize(): only the four shelf fields, with
//    the same erase guards the real one carries on them (see the header) ─────
function serialize(prevPaintDisk, prevTransitRaw) {
  const payload = { v: 5, tiles: game.tiles, savedAt: 1 };
  // Read the globals every time, exactly as node-city does — the whole point
...
console.log('the shelf only — the real serialize() → loadState() cycle is node .gauntlet/drive-city-roundtrip.mjs');
