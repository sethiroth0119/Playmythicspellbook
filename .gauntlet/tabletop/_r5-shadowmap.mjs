/* BUILDER round 5, piece `table-and-shadow`.
...
const shot = async n => { const p = OUT + '_r5-' + n + '.png'; await page.screenshot({ path: p }); return p; };
...
const dayMap = await map(arms['on-day'], arms['off-day'], 'r5-map-day.png');
const duskMap = await map(arms['on-dusk'], arms['off-dusk'], 'r5-map-dusk.png');
const aaMap = await map(arms['on-day'], arms['aa-day'], 'r5-map-AAfloor.png');
