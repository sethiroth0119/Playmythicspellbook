/* ══════════════════════════════════════════════════════════════════════════
   ...
   Run:  node .gauntlet/unreachable-scan.mjs [file] [--all]
         file defaults to public/index.html. public/node-city/index.html is
         the other page with doors to lose (its ops and mayor panels reach the
         player only through the iframe) and it declares its own top-level
         functions, so it is scanned as its own file and never merged into the
         hub's graph: a node-city function named from index.html is NOT
         reachable there — window.parent.App is undefined in the iframe, and
         the two pages share no scope.
   ══════════════════════════════════════════════════════════════════════════ */
import { readFileSync } from 'fs';

/* The first argument that is not a flag is the file. A missing file is left
   to readFileSync's own throw: a scan that silently fell back to the hub
   would report the wrong page's doors as this page's. */
const FILE = process.argv.slice(2).find((a) => !a.startsWith('--')) || 'public/index.html';
...
/* With --all every dead function is listed, tagged or not, so calling that
   count "feature-shaped" would overstate it. */
console.log('  UNREACHABLE: ' + dead.length + '  (of those, ' + (ALL ? 'listed with --all: ' : 'feature-shaped: ') + findings.length + ')\n');

for (const f of findings) {
  console.log('🚪 ' + f.name + '()   line ' + f.line + '  · ' + f.len + ' lines');
  console.log('     ' + (f.tags.length ? f.tags.join(' · ') : 'helper-shaped (no feature marker)'));
...
console.log('  Confirm with:  grep -n "<name>" ' + FILE);
process.exit(0);
