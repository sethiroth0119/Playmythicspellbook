...
- Modules read the game only through `window.MythicBridge` (+ `MythicFarmBridge`): `slots`, `files`, `battle`, `ui`, `miniGames`, `hubs`, `guides`, `avatarPick`, `closet` all live there.
- 👕 `public/src/closet/` is the **Player Closet** (`window.MythicCloset`): clothing fitted to characters by measurement (`closet.rig.js`), brands, the creator players use, the studio admins use. The outfit rides `Profile.closet` and the hub position packet (`w`); purchases spend ONLY through `MythicBridge.closet.charge` → `spendGems()`. **Any scene that draws the player (hub, courthouse, camp, roguelite map) gets the character through `MythicCloset.character.spawn / replace` — never a second avatar loader.** Migration `sql/132`. Bump `?v=` on `src/closet/index.js` (pc…) too. `docs/athena-engine.md` → Player Closet. Harness: `node tools/athena-harness/serve.mjs` then the `pw-test*.mjs` suites with `PLAYWRIGHT_PKG` set.
