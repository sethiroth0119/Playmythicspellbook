...
const WH_RPC_ALLOW = {
  ...
  wh_my_resources: 1, wh_seed_resources: 1,
...
function _whReason(r) {
  ...
  const m = {
    ...
    // ⚠ This one means the SERVER ledger disagrees with what the inventory
    // screen shows — see §DIVERGENCE in the migration. Say that, rather than
    // "not enough", or the player reads it as the game losing their goods.
...
async function _whLoadConfig() {
  ...
  if (c) {
    WH_WEIGHTS = c.weights || {};
    WH_MAXKG = +c.max_shipment_kg || WH_MAXKG;
    WH_DEFKG = +c.default_weight || 1;
  }
  return c;
}
// 🌱 One-time bootstrap of the SERVER resource ledger from the profile blob.
// public.user_resources is authoritative for everything the warehouse does, and
// it starts empty — so without this, a player's first send is refused with
// `insufficient_resources` no matter what their inventory says. The RPC itself
// refuses to run twice, so this is safe to call as often as we like.
let _whSeeded = false;
...
async function _whOpenSendModal(kind, nodeId, label) {
  ...
  const body = `
    ...
    <!-- ⚠ A running weight total, live. The server enforces max_shipment_kg AND
         the destination's free space, but with unit counts only those limits
         were invisible until after the player pressed Send. -->
    <div id="wh-weigh" style="display:flex;align-items:center;gap:0.7rem;padding:0.55rem 0.8rem;margin-bottom:0.8rem;background:rgba(0,0,0,0.4);border:1px solid rgba(212,175,55,0.3);border-radius:9px;font-family:'Roboto Mono',monospace;font-size:0.82rem">
      <span style="color:#bfae87">LOAD</span><strong id="wh-kg" style="color:#ffd166">0.0 kg</strong>
      <span style="flex:1;height:8px;border-radius:99px;background:rgba(255,255,255,0.09);overflow:hidden"><i id="wh-kgbar" style="display:block;height:100%;width:0%;background:linear-gradient(90deg,#7ce8a8,#ffd166)"></i></span>
      <span id="wh-kglim" style="color:#bfae87"></span>
    </div>
      ...
      ${have.map(r => `
        <div style="display:flex;align-items:center;gap:0.55rem;padding:0.5rem 0.65rem;background:rgba(0,0,0,0.35);border:1px solid rgba(212,175,55,0.22);border-radius:9px">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="color:#bfae87;font-size:0.72rem;font-family:'Roboto Mono',monospace">have ${(S[r.id] | 0).toLocaleString()} · ${WH_WEIGHTS[r.id] || 1} kg each</div>
          </div>
          <input class="wh-qty" data-res="${escapeHtml(r.id)}" data-kg="${(WH_WEIGHTS[r.id] || 1)}" type="number" min="0" max="${S[r.id] | 0}" value="0"
  ...
  // Live weight readout: total, the ceiling, and the CHOSEN BAY's free space.
  const _whWeigh = () => {
    let kg = 0;
    root.querySelectorAll('.wh-qty').forEach(i => {
      kg += Math.max(0, Math.floor(+i.value || 0)) * (+i.dataset.kg || WH_DEFKG);
    });
  ...
  root.querySelectorAll('.wh-qty').forEach(i => { i.oninput = _whWeigh; i.onchange = _whWeigh; });
  const _bs = root.querySelector('#wh-bay'); if (_bs) _bs.onchange = _whWeigh;
  _whWeigh();
...
function _whCreditPayload(payload, why) {
  ...
  try {
    ...
    // ⚠ CHECK THE RETURN VALUE, NOT AN EXCEPTION. saveProfile() never throws:
    // its own outer handler swallows everything and returns false, and its FIRST
    // statement is a silent early return when this tab is not the MultiTab
    // writer. Wrapping it in try/catch meant the exact failure this warning
  ...
  if (n <= 0) { showToast('📦 Nothing to collect.', 2400); return 0; }
  if (saved) {
    showToast('📦 ' + (why || 'Returned') + ' — ' + n.toLocaleString() + ' units back in your stores.', 3600);
  } else {
    showToast('⚠ ' + (why || 'Returned') + ' — ' + n.toLocaleString() + ' units are back, but the save did NOT go through. '
      + 'Keep this tab open and do not reload until your stores show them.', 9000);
  }
  try { if (typeof render === 'function') render(); } catch (e) {}
  return n;
}
...
async function _whOpen(warehouseId) {
  ...
  fr.id = 'wh-frame';
  fr.src = 'warehouse/index.html?v=v121b1';
