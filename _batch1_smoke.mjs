...
ok(/\} else if \(a\.kind === 'corpDissolve'\) \{/.test(SRC) && /rpc\('corp_dissolve', \{ p_corp_id: Corp\.mine\.id \}\)/.test(SRC) && /Corp\.mine = null; Corp\.roster = \[\]; Corp\.requests = \[\]; Corp\.vault = \[\]; Corp\.amOwner = false;/.test(SRC.slice(SRC.indexOf("a.kind === 'corpDissolve'"), SRC.indexOf("a.kind === 'corpDissolve'") + 7000 /* bug-mu17gpcz: the withdraw-on-close step now precedes the reset */)), 'the parent action calls the RPC and resets the corp state like Leave does');
