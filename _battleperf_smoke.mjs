...
/* ⚠ RE-DATED 2026-09-14 (v121v162). This matched the return line VERBATIM, so
   adding a field to it failed the gate while the property it guards — the
   decoded bitmap is handed over as soon as it exists, the lazy <img> until then
   — was untouched. `crop` joined the spec when the elemental paintings started
   sampling their interior instead of being drawn whole. Anchored on the bitmap
   handover itself now, rather than on the exact shape of the object literal
   around it. */
