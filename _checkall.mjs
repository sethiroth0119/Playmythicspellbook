...
const SUITES = [
  ...
  { id: 'prnhome',       cmd: ['node', '_prnhome_smoke.mjs'],    expect: 0, minPasses: 64, tier: 'fast',
    why: 'every PRN a player owned rang EVERY city they owned (bug-mucvogzk, bug-mucu5m51): a second city inherited population cap and link from nodes that were never its own, and both cities\' economies stood on the same anchors[0] ground; city_profiles was keyed by that unstable PRN id (bug-mtqasoy6). A PRN now anchors only in the city named by meta.site.nodeId (unsited = nowhere, with an honest message); the construction ceiling and thrive level stay player-wide; the economy grounds on the server ground id (pinned in the save, owner only); city_profiles keys by the TW node and the corp roster reads tw_node_owners, with a legacy transition. sql/192 re-keys the old rows' },
