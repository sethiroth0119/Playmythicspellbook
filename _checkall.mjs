...
const SUITES = [
  ...
  { id: 'foodchain',     cmd: ['node', '_foodchain_smoke.mjs'],  expect: 0, minPasses: 32, tier: 'fast',
    why: 'bug-mu2oz3ve (owner decision): Fast Food, Food Truck and Restaurant firms sat on No feedstock for ever because nothing in ECO_BUILDING_MAP made cookingOil or meat. Adds the Oil Press (soybeans -> cookingOil, tile eats corn) and the Abattoir (livestock -> meat), promotes cookingOil into RESOURCES, and drives the shipped ecoBuildings() against the real economy: without the two buildings every kitchen-day is NO_FEEDSTOCK, with them oil is made and the kitchens make meals, and given livestock the Abattoir makes and sells meat.' },
  { id: 'seamspread',    cmd: ['node', '_seamspread_smoke.mjs'], expect: 0, minPasses: 13, tier: 'fast',
    why: 'bug-mu2tq6op: every tile of a multi-seam extractor used to work the ONE best seam of the node, so on a gas-graded node no Fuel Rig made crude and the Gas Station read Nobody makes it. ecoBuildings() now gives the first tile the best seam and each further tile the least-worked supported seam, stamped on the tile (t.eco, saved) so builds, demolitions and reloads re-found nothing. Drives the shipped functions against the real economy.' },
  { id: 'lumberreserve', cmd: ['node', '_lumberreserve_smoke.mjs'], expect: 0, minPasses: 7, tier: 'fast',
    why: 'bug-mttyizit: firm upkeep and municipal procurement run after production and bought every unit of lumber left in stock, so availabilityMap() found 0 the next morning and every business needing lumber read Lumber Supply 0% beside a working Sawmill. Both sinks now take only stock above one day of committed B2B demand. Drives the shipped ecoBuildings() against the real economy.' },
  ...
  { id: 'cityload',      cmd: ['node', '_cityload_smoke.mjs'],   expect: 0, minPasses: 30, tier: 'fast',
    why: 'bug-mtr5xz8t / bug-mu6bxip6: every city_state read is bounded (a timeout is a refusal, so the refresh-and-retry runs) and only the newest read may write the version record, so a stalled open answering late can no longer cause a false save conflict; node-city bounds the anchor ring, the city read and the delete probe in boot. Drives the lifted cityStateLoad and the shipped bridge against a fake host that never answers. bug-mu424ej6: a full localStorage origin evicts, oldest-opened first, only city copies the server has confirmed (index nc_city_lru), week-old conflict/retired side copies, and week-old unproven copies of OTHER players cities - never the city being saved, never an unproven own city, never an ecoDefer copy; driven through the shipped bridge on a quota-limited fake localStorage. bug-mu2oevqf: node-city toasts raised inside the offline catch-up are dropped (history, on the away report and the log), everything else toasted while boot() loads is held and shown once, deduped, when the city is up; the population milestone re-baselines after the catch-up' },
  ...
  { id: 'prnlate',       cmd: ['node', '_prnlate_smoke.mjs'],    expect: 0, minPasses: 20, tier: 'fast',
    why: 'PRN nodes did not spawn in the city (bug-mtwtxvg8, bug-mtr5bze0): spawnAnchors asked for the nodes once at boot, and in parent mode that is FoundationReserve.nodes, which nodeFetch fills on its own schedule, so a city opened before the read landed rang no anchors all session. The late ring asks the parent to fetch (cityRefreshNodes) and re-reads a bounded number of times, never displacing a building' },
  { id: 'cityboot',      cmd: ['node', '_cityboot_smoke.mjs'],   expect: 0, minPasses: 10, tier: 'fast',
    why: 'bug-mtr5xz8t round 2: after 4f769e51 bounded the anchor ring and the city read, node-city boot() still awaited fetchCards (managed city: city_owner_cards_get) and fetchNeighborCities (city_node_links) with no bound; a read that never answered held everything after it, including setAnimationLoop, so the spinner lifted over a canvas that never drew. Reproduced in Chromium under a stub host; both are now bounded in the bridge, asked together, and late cards are adopted' },
  { id: 'citycatchup',   cmd: ['node', '_citycatchup_smoke.mjs'], expect: 0, minPasses: 9, tier: 'fast',
    why: 'bug-mtr5xz8t round 3, "the game crashes a lot when loading the cities": on a full 24x24 grid a 3 h absence took 65 s to load with the main thread blocked up to 38 s, behind a loading screen the 9 s failsafe had already hidden. tileMult asked kalonCityBoost and finCount (and the production loop stockCap) once per tile, each a walk over every tile; they are now kept for one synchronous pass. The catch-up also yields at least every 100 ms (it yielded every 400 slices, sized for 0.4 ms slices) and shows its progress overlay during boot. Same Cinder and resources to the unit on a seeded 1 h A/B' },
  { id: 'boeboot',       cmd: ['node', '_boeboot_smoke.mjs'],    expect: 0, minPasses: 11, tier: 'fast',
    why: 'City Hall still needed a trip through the Bank of Ethos first (bug-mtr5bze0): its back office is gated on BankEthos.ready, which only boeFetch sets and only openBankOfEthos called. City Hall and the start-up warm pass now load the account (boeEnsureReady, bounded, no request once loaded) and boeFetch is single-flight so the new callers cannot race the bank into a double INSERT' },
  { id: 'prnlevel',      cmd: ['node', '_prnlevel_smoke.mjs'],   expect: 0, minPasses: 12, tier: 'fast',
    why: 'PRN levels in the city did not match the Foundation Reserve (bug-mtwuhbwu): the city levels a node into meta.level through city_set_node_level (server clamps to 50), but the Reserve PRN tab printed the level COLUMN, which nothing writes above 1; and the city kept levelling past 50, announcing levels the server never stored. Display reads _nodeShownLevel; the payout still prices the clamped column' },
  { id: 'prnhome',       cmd: ['node', '_prnhome_smoke.mjs'],    expect: 0, minPasses: 68, tier: 'fast',
    why: 'every PRN a player owned rang EVERY city they owned (bug-mucvogzk, bug-mucu5m51): a second city inherited population cap and link from nodes that were never its own, and both cities\' economies stood on the same anchors[0] ground; city_profiles was keyed by that unstable PRN id (bug-mtqasoy6). A PRN now anchors only in the city named by meta.site.nodeId (unsited = nowhere, with an honest message); the construction ceiling and thrive level stay player-wide; an existing city keeps its economy ground (pinned in the save); only a new city gets its own (server id, owner only); city_profiles keys by the TW node and the corp roster reads tw_node_owners, with a legacy transition. sql/192 re-keys the old rows' },
  ...
  { id: 'azareward',     cmd: ['node', '_azareward_smoke.mjs'],   expect: 0, minPasses: 11, tier: 'fast',
    why: 'AZA from a Resource Run showed in the debrief and the wallet and was gone at the next boot (bug-mu7bk9gy). _covertCollect bumped Profile.sovereigns locally and printed "+1 Aza", then fired sov_reward without reading the answer; a refusal carries no aza, so nothing corrected the bump until the canonical adopt at boot. The weekly gate counted by startedAt while the server counts reward-log rows written at COLLECT time, so the client let players redeploy into a refusal. Pins: nothing credited locally, the debrief reports what the server actually paid and why not, one RPC per collect, and the gate counts the way the server does' },
  ...
  { id: 'claimonce',     cmd: ['node', '_claimonce_smoke.mjs'],    expect: 0, minPasses: 17, tier: 'fast',
    why: 'a daily/weekly mission and a season-pass tier pay ONCE: Profile.missions and Profile.seasonPass were saved but on neither the local-load whitelist nor the cloud row, so every reload returned empty claimed maps over the same seeded mission set and the reward could be claimed again. Both now load and sync, and the cloud half is MERGED (claims unioned, progress only rises) so an older copy cannot re-open a claim' },
  { id: 'aitrade',       cmd: ['node', '_aitrade_smoke.mjs'],      expect: 0, minPasses: 15, tier: 'fast',
    why: 'AI-corp trading has ONE per-corp per-day allowance (_aiTradesLeft / AI_TRADE_DAILY) that the spot trade and every contract delivery spend, signing checks, and the panel counter prints. Before it the only limit was the spot flag: sign, deliver x3, re-sign, forever, each delivery minting Cinder (30 paid deliveries in one day headless). Also pins that the allowance survives a reload (aiTrade was not on the local-load whitelist) and that an older cloud copy cannot hand spent trades back (_aiTradeMerge)' },
  { id: 'aitradesrv',    cmd: ['node', '_aitradesrv_smoke.mjs'],   expect: 0, minPasses: 34, tier: 'fast',
    why: 'the AI-trade allowance is enforced SERVER-side: _aiSpotTrade/_aiDeliver settle through the ai_trade_settle RPC (sql/191: 2 per corp per UTC day, pay bounded by a server-derived price, credited via wallet_credit) instead of addGems. Pins: the server balance is adopted tax-exempt with no extra wallet_credit mirror; a limit/pay/network refusal refunds the goods exactly once and pays nothing; a network failure retries once with the SAME ref; a double click cannot spend twice; with the RPC missing the old client-only path pays once and the miss is remembered' },
  { id: 'faucets',       cmd: ['node', '_faucets_smoke.mjs'],      expect: 0, minPasses: 47, tier: 'fast',
    why: 'four Cinder faucets settle through their own RPCs (sql/196) instead of the generic wallet_credit: Fuel Command sales, Refinery spot sales, City builder income, reconcile_local_gain_on_fetch. Pins per source: ok credits what the server paid with no extra mirror; a refusal returns the goods exactly once and pays nothing; a network failure retries once with the SAME ref and is NOT refunded (unknown outcome) but credited locally + reconcile-flagged; RPC missing → the old addGems path once, remembered. Also models the SQL bounds against the client constants (demand curve, FC_NPC_MAX, refinery prices, haircut, index clamp, tank size) so a balance change fails here before the server refuses honest sales' },
  { id: 'faucets2',      cmd: ['node', '_faucets2_smoke.mjs'],     expect: 0, minPasses: 89, tier: 'fast',
    why: 'phase 2 of the Cinder faucets (sql/198): the mayor ledger clamp (per-call remainder re-queued, day clamp said once), keyed refunds (wallet_charge_ref + wallet_refund: a synchronous spend+refund nets to zero on the server, a refund runs once, missing → the old billing + a named Refund credit), Fuel Command NPC sale / hedge / insurance (a forced event pays nothing) / positions / loans, and Season Pass tiers through faucet_season_pass — ok / refused / missing / network per door. Models the SQL against the client (SEASON_*_REWARDS vs the seeded tier table, FC_NPC_*, tank ladder, insurance, reasons the server keys on, faucet_city_income ceilings) and fails on any anonymous addGems/addCinders in index.html' },
  { id: 'gymbadges',     cmd: ['node', '_gymbadges_smoke.mjs'],    expect: 0, minPasses: 12, tier: 'fast',
    why: 'bug-mu0lf3vw: Core Trial badges reset on every reload because Profile.gymWars was in none of the three persistence whitelists (loadForge, the forgeSmall upload, the cloud hydration). There is no timed reset. Pins the real loader restoring them and a hydration merge that can only ADD a badge' },
  { id: 'persistaudit',  cmd: ['node', '_persistaudit_smoke.mjs'], expect: 0, minPasses: 31, tier: 'fast',
    why: 'the whole class behind gymWars / missions / campWorkforce: saveProfile writes the WHOLE Profile but loadForge restores a whitelist, and a same-device reload skips hydration and re-uploads local, so ~50 more fields (bought furniture, gems, crafted-weapon defs, trader/Dojo licences, JB ops, Kitchen, rentals, convoys in flight, reset stamps, cooldowns) reset and blanked the cloud. Pins the reload, the race end to end, merges that cannot undo a purchase, and a GUARD that fails on any new Profile field that is not restored, not documented transient and not own-key, plus a runtime check that every whitelisted field lands when alone (the dilemma restore hid inside the bunkhouse `if`)' },
  { id: 'packreward',    cmd: ['node', '_packreward_smoke.mjs'],   expect: 0, minPasses: 37, tier: 'fast',
    why: 'a claimed reward pack is a pack: Season Pass pack tiers and coupon packs called grantUnopenedPack, which did not exist, and fell back to Profile.packInventory, which nothing read, so every claimed reward pack vanished. grantUnopenedPack now lands them in Profile.unopenedPacks (openable, reloaded, synced), keyed per claim so a claim is one pack; _packRewardMakeGood rebuilds packs claimed before the fix from the synced claim flags plus any leftover packInventory, once, with the same entry ids on every device' },
  { id: 'webpurchases',  cmd: ['node', '_webpurchases_smoke.mjs'], expect: 0, minPasses: 9, tier: 'fast',
    why: 'website pack receipts are written as ONE KEY of user_profiles.forge: redeemWebPurchases used update({ forge: { webPurchases } }), which replaced the whole forge column (~80 synced keys) with one key and slipped past up_guard. Now the sql/195 web_purchases_set RPC, and until it is applied a read-merge-write conditional on updated_at; a failed read or an unrelated RPC error writes nothing' },
  { id: 'auctionescrow', cmd: ['node', '_auctionescrow_smoke.mjs'], expect: 0, minPasses: 58, tier: 'fast',
    why: 'cloud auction bids were soft-locked in Profile.lockedGems and released by a loop keyed to the in-memory CardMarket.myBids; a reload dropped the lock, so a bidder could spend the escrowed Cinder and still win, paying less than the bid (Math.max clamp) while the seller got all of it. sql/193 moves the escrow to the server: the bid is debited into auction_escrow, an outbid is refunded in the same transaction, sync rebuilds holds after a reload, the winner pays the full bid and the seller is paid once. Drives the real client against a model of the SQL, reproduces the old bug on the RPC-missing fallback, and cross-checks client RPC names against the migration' },
  { id: 'gameclock',     cmd: ['node', '_gameclock_smoke.mjs'],    expect: 0, minPasses: 11, tier: 'fast',
    why: 'bug-mty87zhi: the Bunker showed a hard-coded DAY 047 and a mock clock from 02:14:32, the Camp console showed account age and device local time, and neither matched the website. Both now read gameClock() (world day from the 2026-05-23T16:00Z reset, UTC), which reproduces the website reading of Day 112 at the reported instant' },
  { id: 'tablebadge',    cmd:['node', '_tablebadge_smoke.mjs'],   expect: 0, minPasses: 12, tier: 'fast',
    why: 'bug-mucgw4pi: the camp Table badge was computed once at renderCamp and the Table overlay never re-renders, so it never counted down; and a unit that asked for an item counted in the badge although the Table could not fill a request. Pins the repaint hook and the shared _lqFulfilRequest path' },
  { id: 'stadiumstash',  cmd: ['node', '_stadiumstash_smoke.mjs'], expect: 0, minPasses: 13, tier: 'fast',
    why: 'bug-mucr6azr: the Stadium planner read 2 rations while the stash held hundreds. Rations and remedies are city stock AND stash resources and the only mover ran city to stash, so bought rations could never reach an event. Pins stockPullFromStash (spend first, clamp to the shelf, nothing minted on a refused spend) and the stadium row that offers it' },
  { id: 'campkeep',      cmd: ['node', '_campkeep_smoke.mjs'],     expect: 0, minPasses: 26, tier: 'fast',
    why: 'paid camp staff stay paid for: a Reconstruction hire is charged ONCE (the adopted server balance used to be read by the spend watcher as a legacy spend and debited again), the hired trade lands in its own row, and campWorkforce plus the rest of the Reconstruction state survive a reload (they were in the two cloud lists and missing from the local loader, so every reload dropped paid staff); Camp Research levels ride the cloud row and merge as high-water marks, since they were local-only and any other device or the desktop app came back at Lv 0; and a research cost prints every resource it charges, including salvage ids like Scrap Metal that sit outside RESOURCE_IDS (Bulwark Protocol was disabled for a cost the card never showed)' },
  { id: 'salvagesync',   cmd: ['node', '_salvagesync_smoke.mjs'],  expect: 0, minPasses: 22, tier: 'fast',
    why: 'donated resources stay donated (bug-mu83ph17): the cloud hydration took MAX(local, cloud) per resource, so a second device still holding the pre-donation count out-voted the cloud and uploaded it back; the newer ledger by __salvageAt__ now wins whole (a stamp-less legacy row keeps MAX), and a read-only tab cannot donate, dispatch or deliver a convoy, or bank resources, because its debit is never saved while the server credit is real' },
  { id: 'salvagemerge',  cmd: ['node', '_salvagemerge_smoke.mjs'], expect: 0, minPasses: 40, tier: 'fast',
    why: 'the stash merges on the server (bug-mu8vnos5, sql/190): a second device left open uploaded its stale whole ledger over a market purchase; the upload now carries base + current and the user_profiles trigger adds each device\'s change, a lost reply is applied once (per-device write ids), and before sql/190 is applied the client keeps the old whole-map path' },
  { id: 'corpwithdraw',  cmd: ['node', '_corpwithdraw_smoke.mjs'], expect: 0, minPasses: 10, tier: 'fast',
    why: 'a founder can empty the corporation treasury into their own wallet before shutting it down (bug-mu17gpcz): founder-only button, the existing corp_pay_member_from_treasury RPC with the signed-in id as payee, an append-only negative corp_treasury row, and the local mirror moved by qty without a second server credit' },
