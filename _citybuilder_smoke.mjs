...
console.log('\n=== 6. posts filled — 10 to 200, by the size of the business ===');
{
  ...
  /* bug-mu4mc2c8 retune: the 10–200 figure is now RESIDENT seats only
     (seatsAt → npcSeatsAt). Card posts moved to cardPostCapacity (def.crew +
     level) — pinned in _crewposts_smoke.mjs. So the pin follows the count to
     its new caller, and asserts slotsAt no longer reaches it. */
  const CSRC = (await import('fs')).readFileSync('./public/src/work/crew.city.js', 'utf8');
  ok(/return postCapacity\(t, defOf\(t\.type\), k\);/.test(CSRC), 'seatsAt() hands the resident-seat count to postCapacity');
  ok(/return cardPostCapacity\(t, def\);/.test(CSRC) && !/return postCapacity\(t, def, k\);/.test(CSRC), 'slotsAt() (card posts) no longer uses the 10–200 seat scale');
}
...
console.log('\n=== 9. NPC seats follow the posts, and the names on the books earn ===');
{
  ...
  const env = {
    ...
    // bug-mu4mc2c8: npcSeatsAt reads CREW.seatsAt now (slotsAt is the card-post count).
    CREW: { seatsAt: (k) => (k === '1,1' ? 20 : k === '2,2' ? 150 : 0), slotsAt: () => 2 },
  };
