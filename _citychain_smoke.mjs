...
/* ── 7. THE TOPBAR GROWS ONLY WITH ITS CHIPS ──────────────────────────────── */
{
  /* updateHUD writes $('r-'+r).textContent UNGUARDED, so an id on the DISPLAY
     list with no chip throws and kills the whole HUD. That — not the size of
     the list — is the hazard, and the `missing` check below is what actually
     catches it.
     📌 THE COUNT MOVED 8 → 9 ON 2026-09-19, and the reason is the difference
        this section exists to police. The mirror grew by ~28 ids with NO
        chips, which is why the two lists were split in the first place; planks
        is the opposite case — it was added to the display list AND given its
        own chip (id="r-planks", verified by the next check) in the same change,
        which is the only sanctioned way onto this list.
     ⚠ The number stays pinned rather than being replaced by "every id has a
       chip". That weaker rule would pass while somebody quietly moved all 28
       mirror-only ids across, and the per-id fan-out this file spends section 6
       proving we escaped is exactly what that would cost. Growth here is
  ...
  ok(display.length === 9,
    'the topbar list grew only by ids that brought a chip with them (8 → 9: planks)',
    display.join(','));
