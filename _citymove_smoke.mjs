...
console.log('\n=== 2. what refuses to move, and where it may go ===');
{
  ...
  ok(F.refusal({ type: 'road' }, '1,1') && F.refusal({ type: 'farm', bld: { k: 0 } }, '1,1') && F.refusal({ type: 'farm', bld: { k: 1 } }, '1,1'), 'a road, a construction site and an upgrade in progress all refuse');
  /* THE PRN CAME OFF THAT LIST, for the same reason the anchor did
     (bug-mtyc3btu: "I don't have the ability to move or delete my PRNs ... no
     longer provide the option to re-arrange"). Taking a node up has worked
     since v121v32 and the node card has offered it in as many words since
     09-01, and the player still reported three weeks later that they could
     not move one. This refusal is why: Move was VISIBLE and DISABLED, and its
     tooltip sent them to "its node panel", where the only control is a
     TEAR-DOWN. Nobody trying to shift a node one tile over goes hunting for
     the button that takes it away. Two working features, and the seam between
     them read as a refusal.
     Asserted the other way now, exactly as the anchor is, so it cannot quietly
     go back - and the re-site wiring is asserted in the wiring section. */
  ok(!F.refusal({ type: 'farm' }, '9,9'), 'a PRN does NOT refuse any more - it re-sites like a licensed business');
...
console.log('\n=== 3. the wiring ===');
{
  ...
  /* ...AND A PRN THE SAME WAY, because it is the same hazard: a write to a
     table that outlives the tile. Release first (so a failed release touches
     nothing), then site; if the site fails, stand it back on the old plot.
     The third state is the one worth asserting separately - release worked,
     site failed AND the rollback failed - because the licence is then safe
     but UN-SITED, which on the map is indistinguishable from a lost one. */
  ok(/const u = await prnUnsite\(prow\.id\);[\s\S]{0,200}r = await prnSite\(prow\.id, x, z, t\.rot \| 0\);[\s\S]{0,400}await prnSite\(prow\.id, ox, oz, t\.rot \| 0\);/.test(SRC), 'a PRN is released and re-sited first, and stood back on its old plot if the new site is refused');
  ok(/could not be stood there, and it could not be put back either[\s\S]{0,160}Foundation Reserve/.test(SRC), '...and if even the rollback fails the player is told the licence is un-sited in the Reserve, not lost');
