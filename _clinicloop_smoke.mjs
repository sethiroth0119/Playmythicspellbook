...
{
  /* 📌 2026-09-19 — THE RULE THIS PINS CHANGED, so the sentence it looks for
     had to. The check was written because "I managed to build two of the same
     PRNs" was reported as a bug when it was the design, and nothing on the
     screen said so. The owner then changed the design itself (feature board,
     Clarence Freycinet): buying is no longer capped at six at all — you may own
     any mix up to NODE_MAX_OWNED and PLACE six at a time, with the overflow
     waiting in storage.
     What is pinned is therefore the same property as before — the dialog states
     the rule the player is about to run into — expressed against the rule that
     now holds. All three clauses are required, because the confusing half is no
     longer "duplicates are allowed" but "you bought it and it is not on the
     map". */
  ok(/in any mix — duplicates included — up to/.test(SRC),
    'the licence dialog still says duplicates are allowed, and what the owning limit is');
  ok(/can be placed at a time/.test(SRC) && /waits in 📦 storage/.test(SRC),
    '…and that only some may be PLACED, and where the rest go — the new half of the rule');
  ok(/const NODE_MAX_PLACED\s+= NODE_MAX_PER_CORP;/.test(SRC) && /const NODE_MAX_PER_CORP\s+= 6;/.test(SRC),
    'the placed cap is still six, and still the one number (mirrors node_payout_config.max_placed, sql/167)');
}
