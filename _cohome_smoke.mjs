...
/* bug-mtw1n1bi: the same rule, preferring a row the viewer can site (`mine !== false`) first. */
ok(/const row = rows\.unsited\.find\(_canSite\) \|\| \(opType === 'construction' && !rows\.here\.length \? rows\.elsewhere\.find\(_canSite\) : null\)\s*\|\| rows\.unsited\[0\] \|\| \(opType === 'construction' && !rows\.here\.length \? rows\.elsewhere\[0\] : null\);/.test(NC),'tryPlace sites the elsewhere row for construction (never for another op, never when one already stands here)');
...
/* ── the parent's site writer ── */
{
  ...
  const mk = (row, ownerId, nodeId) => {
    const g = { App: { _cityOwnerId: ownerId, _cityNodeId: nodeId, _cityOwnerName: 'Owner' }, OP_LABELS: { construction: 'Construction Co.', mining: 'Mining Company' },
      _opRowById: () => row, _opRowIsMine: () => true /* bug-mtw1n1bi: the viewer's own rows */, _opSite: (o) => (o.meta && o.meta.site) || null, _opWriteMeta: async (o, m) => { o.meta = Object.assign({}, o.meta, m); g.wrote = m; }, showToast: (m) => { g.toast = m; }, window: {} };
