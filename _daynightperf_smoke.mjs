...
/* a top-level `const NAME = { … };` BLOCK — constLine only reaches one line */
function constBlock(name) {
  const i = BB.indexOf('const ' + name + ' = {');
  if (i < 0) throw new Error('cannot find const block ' + name);
  let d = 0;
  for (let k = BB.indexOf('{', i); k < BB.length; k++) {
    if (BB[k] === '{') d++;
    else if (BB[k] === '}') { d--; if (!d) return BB.slice(i, k + 1) + ';'; }
  }
  throw new Error('unterminated const block ' + name);
}
/* 🔴 THE MARKS TERM, AND WHY IT IS A STAND-IN RATHER THAN THE REAL markPlan.
   terrainKeyParts folds the arena markings into the terrain cache key as
   'mk' + (MARKS.on ? 1 : 0) + ':' + markPlan().sig. This suite is about the
   day/night lerp and the bake budget; it is not about markings, and lifting the
   real markPlan would drag in hexV, hexW, hexSize, the objective quincunx and
   the whole plan cache to answer a question it never asks.
   ⚠ BUT A HAND-WRITTEN STUB WOULD DRIFT, and a drifted stub is the exact
   failure this file has already been bitten by twice. So the sig EXPRESSION is
   lifted verbatim out of the real markPlan, and if it is ever rewritten this
   throws at load rather than quietly computing the old shape. MARKS itself is
   the real declaration, so MARKS.on is whatever the board actually ships. */
function marksStandIn() {
  const mp = fnText(BB, 'markPlan');
  const m = /const sig = ([^\n]*);/.exec(mp);
  if (!m) throw new Error("markPlan's sig expression changed shape — update marksStandIn, do not guess it");
  return [constBlock('MARKS'), 'function markPlan(){ return { sig: ' + m[1] + ' }; }'].join('\n');
}
...
/* 🟢 THE TRIPWIRE FOR THE THIRD TIME — AND IT ASKS RATHER THAN GUESSES.
   terrainKeyParts is sliced out of the board and dropped into two vm sandboxes,
   so every free variable it reads has to be taught to both. Twice now a new one
   landed — HF when the heightfield joined the cache key, MARKS when the arena
   markings did — and each time this suite DIED mid-run rather than failing: the
   remaining checks silently stopped and the summary reported nothing wrong.
   ⚠ THE FIRST VERSION OF THIS TRIPWIRE SCANNED THE TEXT FOR IDENTIFIERS and
   reported art, tg, hf and lerp — all of them locals or object keys. A scanner
   that cries wolf gets its list widened until it stops catching anything, which
   would have made the tripwire the fourth instance of the thing it exists to
   catch. So it does the real thing instead: build the context the harnesses
   build, run terrainKeyParts, and let the runtime answer. A ReferenceError here
   names the missing symbol exactly, with no list to keep in sync. */
{
  const probe = {
    window: { BBX: null },
    clamp: (v, a, b) => Math.max(a, Math.min(b, v)),
    W: 100, H: 80, DPR: 1, VIEW: { cx: 0, cy: 0, scale: 1 },
    MAP: { cols: 10, rows: 8 }, MAPSIG: 0,
    CONFIG: { tile: 64, camera: { pos: { x: 0, y: 1, z: 2 }, target: { x: 0, z: 0 } } },
    CAMERA: { yaw: 0, moving: false },
  ...
  try {
    vm.runInContext(['const TERR_MIX_Q = 16;', constLine('HF'), marksStandIn(),
      fnText(BB, 'lightLk'), TKP].join('\n'), probe);
    out = vm.runInContext('terrainKeyParts()', probe);
  } catch (e) { err = e; }
  ok(!err, 'terrainKeyParts runs in the harness context — it reads nothing the sandboxes were not taught',
     err ? (err.name + ': ' + err.message + '  — teach BOTH vm sandboxes above, or this whole suite dies mid-run and reports nothing wrong') : '');
  ok(!!out && typeof out.base === 'string' && out.base.length > 0 && out.lerp === false,
     '…and returns a real key, so the run above was not a no-op',
     JSON.stringify(out));
  /* the two terms that arrived late are IN the key, not merely defined — a
     sandbox could satisfy the reference and still be handed a key that had
     quietly stopped folding them in. */
  ok(!!out && /\|hf/.test(out.base), 'the heightfield term is in the key');
  ok(!!out && /\|mk\d:/.test(out.base), 'the arena-markings term is in the key');
}
   ...
   the whole-pair step (round 2), then the banded protocol (round 3) */
{
  ...
  vm.runInContext([
    ...
    /* ⚠ HF — same reason as the other sandbox below: terrainKeyParts reads it. */
    constLine('HF'),
    /* ⚠ MARKS / markPlan — the SECOND thing terrainKeyParts learned to read.
       When the arena markings landed, this suite started dying at 'MARKS is not
       defined' three assertions in: it THREW rather than failed, so every
       remaining check silently stopped running while the summary reported
       nothing wrong. Same shape as HF above, and there will be a third — when a
       suite slices a function out of a living file, every new free variable that
       function reads is a new way for the slice to die quietly. The tripwire
       below the terrainKeyParts assertions is what makes the next one loud. */
    marksStandIn(),
...
/* the terrain path, run for real with fake canvases (round 1's in-lerp path, unchanged) */
{
  ...
  const ctx = {
    ...
    clamp: (v, a, b) => Math.max(a, Math.min(b, v)),
    W: 100, H: 80, DPR: 1, VIEW: { cx: 0, cy: 0, scale: 1 }, MAP: { cols: 10, rows: 8 }, MAPSIG: 0,
    /* `tile` is read by markPlan's sig — see marksStandIn */
    CONFIG: { tile: 64, camera: { pos: { x: 0, y: 1, z: 2 }, target: { x: 0, z: 0 } } }, CAMERA: { yaw: 0, moving: false },
    GROUND: { img: null, key: '', fade: 1 }, tileArtLoaded: () => 0,
  ...
  const src = [
    ...
    'const TERR_MIX_Q = 16;',
    /* ⚠ HF — the sandbox must learn what the sliced functions READ. terrainKeyParts
       folds the heightfield into the terrain cache key, so without this the suite
       dies at 'HF is not defined' BEFORE asserting anything: it throws rather than
       failing, every check silently stops running, and the summary reports nothing
       wrong. Lifted with constLine so the real declaration cannot drift from a copy. */
    constLine('HF'),
    /* ⚠ MARKS / markPlan — the SECOND thing terrainKeyParts learned to read.
       When the arena markings landed, this suite started dying at 'MARKS is not
       defined' three assertions in: it THREW rather than failed, so every
