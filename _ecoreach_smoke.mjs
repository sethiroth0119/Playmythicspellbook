...
/* ── 6. THE 60× CINDER LIE ───────────────────────────────────────────────── */
{
  ...
  /* v173 (bug-mtrmtl5s, again): the per-minute genOf figure was printed "/min"
     to two decimals, so an L1 station read 0.00, and the inspect card labelled
     the same figure "/hr". Every Cinder row now goes through genShown (×60)
     and rateUnit ('/hr'); genShown is genOf underneath, so the rule above holds. */
  ok(/const genShown = \(def, r\) => genOf\(def, r\) \* \(r === 'cinder' \? 60 : 1\);/.test(NC), 'genShown shows Cinder per hour, from genOf');
  const tip = /if \(def\.gen\) h \+= 'Output: '[\s\S]{0,400}?resIco\(r\) \+ rateUnit\(r\)/.exec(NC);
  ok(!!tip, 'found the hover tooltip output line');
  ok(!!tip && /genShown\(def, r\)/.test(tip[0]), 'the hover tooltip goes through genShown — this line IS the reported 0.72', tip ? tip[0].slice(0, 160) : '');
  ok(!/\* acc\) \+ \(r === 'cinder' \? '\/hr' : '\/min'\)/.test(NC), 'the inspect chip no longer labels a per-minute figure "/hr"');
