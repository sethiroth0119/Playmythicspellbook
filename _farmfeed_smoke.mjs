...
/* ── 2. the unwind paths ── */
/* 7133f4570d moved the reward to the server (fr_contribute): the refund now
   sits on the unsaved-debit and "refused / missing / offline" branches, not
   before _frErr(e). The invariant is unchanged and is what is pinned: inside
   frDeposit every undo goes through the UNCLAMPED _refundRes and is persisted;
   the clamped addSalvage is only the fallback inside that same statement. */
{
  const i = SRC.indexOf('async function frDeposit(');
  const fd = i >= 0 ? SRC.slice(i, SRC.indexOf('\n}\n', i)) : '';
  const undo = /try \{ if \(typeof _refundRes === 'function'\) _refundRes\(resId, qty\); else addSalvage\(\{ \[resId\]: qty \}\); \} catch \(_\) \{\}\n\s*try \{ if \(typeof _persistResourcesSoon === 'function'\) _persistResourcesSoon\(\); \} catch \(_\) \{\}/g;
  const n = (fd.match(undo) || []).length;
  ok(fd.length > 0 && n >= 1 && !/addSalvage\(\{ \[resId\]: qty \}\)/.test(fd.replace(undo, '')),
     'a failed Reserve contribution refunds through the UNCLAMPED _refundRes and persists', 'undo sites: ' + n);
}
