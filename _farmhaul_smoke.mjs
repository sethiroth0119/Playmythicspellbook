...
/* ── 3. the Feed Operation, everywhere an op has to exist ────────────────── */
{
  ...
  // bug-mtw1n1bi: the duplicate test asks about the PLAYER's rows (_opMineOfType), still before any charge.
  ok(/if \(_opMineOfType\(a\.op\)\) \{ showToast\('You already operate that\.'/.test(B) && B.indexOf('_opMineOfType(a.op)') < B.indexOf('spendSovereigns(azaPrice'), '…and refuses a duplicate before charging');
...
/* ── 5. the modules and their bridges ────────────────────────────────────── */
{
  ...
  ok(/__farm__:\s+_farmUploadValue\(\),/.test(SRC) && /return Profile\.farm;/.test(SRC) && /f\.__farm__/.test(SRC), 'the farm save syncs through the cloud profile (via the null guard, see _farmpersist_smoke)');
