/* 🧾 THE FORGE SAVE PATH KEEPS WHAT IT WAS GIVEN (v121v76).
   ...
   derived, never transcribed:
     ...
     HALF A — every string-literal id and every CSS selector the save path reads
       ...
       ⚠ 388/468 → 398/478 in v121v167: TEN editor ids were deliberately added —
       ed-onplay-global, and targeted/targetcount/global for each of the three
       ability editors (ed-field, ed-grave, ed-hand). The +80 helper share is
       unchanged, which is the check that the ten arrived through the ordinary
       read forms and not through a new helper nobody enumerated.
       The literal-fallback count stayed at 144 through this, and that is the
       load-bearing part: it briefly read 153 while those nine reads were
       behind a spread of a helper, which ALSO stopped the instrumented
       copy of captureEditorIntoCard compiling at all — 424 wrapped sites
       checking nothing. The reads are inline now. A read this suite cannot
       see is a read it cannot audit.

       ⚠ 398/478 → 399/479 in v121 (turn effects): ONE editor id was added —
       ed-onplay-required, the ⛔ Required box, read by a literal
       getElementById. Its six extra-slot twins are read as
       'ed-onplayx-' + i + '-required', a built id, and so — like every other
       slot read — are not in this count. The +80 helper share is unchanged,
       which is the check that the one id arrived through an ordinary read form.

       ⚠ 399/479 → 400/480 (On-Grave "Fires on"): ONE editor id was added —
       ed-ongrave-when, read by an ordinary v( inside an inline IIFE so that
       'any' is saved as ABSENT (an existing card's bytes do not change). The
       +80 helper share is unchanged again, for the same reason as above.

       ⚠ 400/480 + 80 → 394/484 + 90 (Card Filter persistence): the on-play
       filter is now read through the shared _readCardFilterFrom (which also
       reads the Specific Cards picker's saved rule) instead of its own literal
       getElementById reads, so six literal ids left the named share and the
       helper share grew by ten — the rule inputs and the per-slot filter reads
       that were never saved before. Measured by this suite, not transcribed.

       ⚠ 394/484 + 90 → 395/485 + 90 (Purity Pact "keeps checking"): ONE
       editor id was added — ed-onplay-pactcont, a literal getElementById read.
       The helper share is unchanged.

       ⚠ 395/485 → 399/489 (Convergence, 37e3c0dc10): FOUR literal editor ids —
       ed-anchor and ed-cv-on (getElementById), ed-cv-value and ed-cv-element
       (v). The +90 helper share is unchanged.

       ⚠ 399/489 → 400/490 (Realm card type, 2026-09-18): ONE literal editor
       id — ed-realm-kind, the Realm-kind select, read by v() in the type
       capture and as the "only the kind on screen" guard on the Archon / Fusion
       / Convergence captures. The +90 helper share is unchanged. Owner: "Make
       Realm Deck a card type where Fusions, Convergence, Evo Units, Archons
       are all in that" — so those blocks moved from type 'summon' to type
       'realm', and the matrix below renders a Realm card of EACH kind.

       ⚠ 400/490 → 403/493 (Fetch round, 2026-09-19): THREE literal editor ids —
       ed-onplay-cond-name, ed-onplay-cond-elem and ed-onplay-cond-fac, the
       condition's "if you control a X unit" row, read by v() in the on-play
       condition capture. The Fetch box and the Card Filter's "…or element" are
       injected at runtime under every effect prefix and read by
       _fxCaptureFetchRound, so they add no literal id. The +90 helper share is
       unchanged.
       ...
       for 403 of the 493 ids; the other 90 arrive through helpers the save path
...
const HALF_A = ({ ids, sels, fb, instr, mut }) => {
  ...
  const rich = (t) => {
    const c = {
      id: 'probe_' + t, name: 'Probe ' + t, type: (t === 'bred' ? 'unit' : (t.indexOf('realm-') === 0 ? 'realm' : t)), icon: '✦', cost: 3, rarity: 'rare', desc: 'probe',
    ...
    if (t.indexOf('realm-') === 0) c.realmKind = t.slice(6);   // 🌌 one Realm card per kind
    if (t === 'hero') {
      c.class = 'vanguard';
  ...
  /* 🌌 2026-09-18 — the Fusion / Archon / Convergence (and Evo) editor blocks
     render only for type 'realm' now (owner: Realm is a card type; a Summon is a
     token), one variant per realmKind. Without these four rows every one of
     their ids read "NO card type renders it". 'summon' stays: it is still a
     type, the token. The kinds are taken from the editor's own kind select,
     not typed here — see realmKindsOf below. */
  const REALM_KINDS = (() => {
    try {
      Forge.customCards = [{ id: 'probe_rk', name: 'rk', type: 'realm', realmKind: 'fusion' }]; App.editingCardId = 'probe_rk';
      const d = document.createElement('div'); d.innerHTML = renderCardEditor();
      const sel = d.querySelector('#ed-realm-kind');
  ...
  const types = ['unit', 'hero', 'summon', 'spell', 'trap', 'weather', 'wall', 'location', 'counter', 'enchantment', 'bred']
    .concat(REALM_KINDS.map(k => 'realm-' + k));
...
async function reportA(A) {
  ...
  ok(rnr.length === 0, 'on all ' + Object.keys(A.perType).length + ' card types, every id the real capture READ on that type is rendered on that type, but for the ' + KNOWN_TYPE_HOLES.length + ' known per-type hole',
  ...
  ok(A.bindErrs.length === 0, 'bindCardEditor() ran clean on all ' + Object.keys(A.perType).length + ' card types', A.bindErrs.join(' | '));
