...
import { STATIONS, ROOM, HOT_Z, PLAN, DESK_MODEL, BAY_MODEL, deskFootprint, deskScale, deskFront, footprintOf, scaleOf, quarterTurned } from './public/src/hospital/floor.js';
import { queueSpot, DOOR, deskBypass, BED_MODEL, BED_SIZE, COT, bedScale, bedYaw, bedTop, lieAt, LIE_CLEARANCE, bedColliders } from './public/src/hospital/beds.js';
...
const PEN = [];   // every heroStaminaPenalty call the hospital makes
globalThis.window = { MythicPlagueBridge: {
  heroStaminaPenalty: (fraction, why) => { PEN.push({ fraction, why }); return { ok: true, heroes: 3, cut: 5 }; },
...
/* 🔴 The regression that found the walker bug. Pressed straight into the
   desk at its centre line, the player must STOP — not be pushed 1.49 m
   sideways round it by a rounding-error overlap in the x pass (player.js
   resolveAxis). It passed on the 5.2 m mock by luck and failed on the real
   1.64 × 0.79 m counter; a 0.01 m change in depth flipped it either way. */
ok(Math.abs(p.x) < 1e-9, "pressed straight into the desk, the player is stopped — not shoved sideways round it (x=" + p.x.toFixed(3) + ")");
{
  /* …and pressed DIAGONALLY into the front face, the player slides along it
     — never through it — until they clear the desk's span, and then walks
     on. Judged PER FRAME while within the span: a 1.64 m counter is cleared
     in a third of a second, so "where did they end up after two seconds" says
     nothing about the desk (first draft of this check assumed an endless
     face and failed on a player who had correctly walked past the end). */
  const q = makePlayer(); q.x = 0; q.z = -16.2;
  const diag = makeInput(); diag.keys.w = true; diag.keys.d = true;
  const front = desk.pos[1] - desk.size[1] / 2, span = desk.size[0] / 2 + 0.25;
  let deepest = -Infinity, slid = false;
  for (let i = 0; i < 120; i++) {
    step(q, diag, 16, 1, plan);
    if (Math.abs(q.x) <= span) deepest = Math.max(deepest, q.z);
    if (Math.abs(q.x) > 0.3) slid = true;
  }
  ok(deepest < front - 0.3, "pressed diagonally, the player never gets past the desk's front while within its span (deepest z=" + deepest.toFixed(2) + ")");
  ok(slid, 'and slides along the face instead of sticking to it (x=' + q.x.toFixed(2) + ')');
}
...
console.log('\n=== 1b. the front desk is the packed model, its collider is its footprint, and the queue is at its front ===');
{
  /* The sidecar is what tools/build-intake-desk.mjs measured on the file that
     actually ships. DESK_MODEL.raw is what the collider is sized from. If the
     two disagree, someone re-exported the desk without re-running the pack —
     and the walker would be blocking on a shape nobody can see. */
  const SIDE = './public/models/hospital/intake-desk.json';
  ok(existsSync(SIDE), 'the packed desk sidecar exists (run tools/build-intake-desk.mjs)');
  let side = null; try { side = JSON.parse(readFileSync(SIDE, 'utf8')); } catch (e) {}
  const near = (a, b, tol) => Math.abs(a - b) <= tol;
  ok(!!side && side.bytes < 25 * 1048576, "the packed desk is under Cloudflare's 25 MiB cap (" + (side ? (side.bytes / 1048576).toFixed(2) : '?') + " MB)");
  ok(!!side && side.triangles < 80000, 'and is a browser-sized mesh (' + (side ? side.triangles.toLocaleString() : '?') + ' triangles)');
  ok(!!side && near(side.dims.w, DESK_MODEL.raw.w, 0.02) && near(side.dims.h, DESK_MODEL.raw.h, 0.02) && near(side.dims.d, DESK_MODEL.raw.d, 0.02),
     "DESK_MODEL.raw matches the packed file's measured extents" + (side ? ' ' + JSON.stringify(side.dims) : ''));
  ok(existsSync('./public/models/hospital/intake-desk.glb'), 'the packed .glb is on disk');
  const sc = deskScale(), fp = deskFootprint();
  ok(near(DESK_MODEL.raw.h * sc, DESK_MODEL.height, 0.001), 'the model is scaled to a real counter height (' + DESK_MODEL.height + ' m)');
  ok(desk.size[0] === fp[0] && desk.size[1] === fp[1], "the desk station's size IS the model's footprint at that scale (" + fp.join(' × ') + ' m)');
  ok(fp[0] > fp[1], 'the counter is wider than deep, so its front is a z-face');
  ok(DESK_MODEL.yaw === 0 || DESK_MODEL.yaw === Math.PI, "yaw is 0 or π — a quarter turn would swap the footprint's axes");
  const box = colliders(STATIONS).find((c) => c.key === 'desk');
  ok(!!box && near(box.hx, fp[0] / 2 + 0.25, 0.001) && near(box.hz, fp[1] / 2 + 0.25, 0.001), "the walker's collider hugs that footprint");

  // the queue
  const f = deskFront();
  const q0 = queueSpot(0);
  ok(q0.z < f.z && near(q0.x, f.x, 0.001), 'the first patient stands at the FRONT of the desk, on the door side');
  ok(near(f.z - q0.z, DESK_MODEL.standOff, 0.001), 'at the stand-off distance');
  const inBox = (pt) => Math.abs(pt.x - box.x) < box.hx && Math.abs(pt.z - box.z) < box.hz;
  const line = [0, 1, 2, 3, 4, 5, 6, 7].map(queueSpot);
  ok(line.every((pt) => pt.z < f.z), 'every queue spot is on the door side of the desk');
  ok(line.every((pt) => !inBox(pt)), 'no queue spot is inside the desk');
  ok(line.every((pt) => pt.z > DOOR.z), 'the queue never reaches the doors');
  ok(line[1].z < line[0].z && line[2].z < line[1].z && line[3].z < line[2].z, 'the file runs back toward the doors');
  ok(line[4].x !== line[0].x && near(line[4].z, line[0].z, 0.001), 'the fifth patient starts a second file beside the first');

  // walking round it
  const bed = { x: -7, z: -5.5 };
  const by = deskBypass(q0.x, q0.z, bed.x, bed.z);
  ok(!!by && Math.abs(by.x - f.x) > f.hw && near(by.z, f.cz, 0.001), "an admitted patient walks round the desk's side, not through it");
  ok(!!by && by.x < f.x, 'on the side the ward is on');
  ok(!!by && deskBypass(by.x, by.z, bed.x, bed.z) === null, 'and from beside the desk the way to the bed is clear');
  ok(deskBypass(-7, -5.5, -12.2, -5.5) === null, "a walk that never crosses the desk's line is left alone");
  const leave = deskBypass(-1.8, -5.5, DOOR.x, DOOR.z);
  ok(!!leave && Math.abs(leave.x - f.x) > f.hw, 'a patient leaving from the aisle-end bed also goes round');
  ok(deskBypass(-9.6, -5.5, DOOR.x, DOOR.z) === null, 'a patient leaving from the far bed does not need to');
}

console.log('\n=== 1c. the Ward Bay counter and the bed are models, pinned to what was measured ===');
{
  const near = (a, b, tol) => Math.abs(a - b) <= tol;
  const sidecar = (f) => { try { return JSON.parse(readFileSync(f, 'utf8')); } catch (e) { return null; } };

  // every station that is a model derives its size from it — no hand-typed footprints
  for (const st of STATIONS) {
    if (!st.model) continue;
    const fp = footprintOf(st.model);
    ok(st.size[0] === fp[0] && st.size[1] === fp[1], st.key + ": the station's size IS its model's footprint (" + fp.join(' × ') + ' m)');
    const q = Math.abs(st.model.yaw || 0) / (Math.PI / 2);
    ok(Math.abs(q - Math.round(q)) < 1e-9, st.key + ': yaw is a multiple of π/2 (an axis-aligned box has no other meaningful turn)');
    const sc = scaleOf(st.model), rw = +(st.model.raw.w * sc).toFixed(2), rd = +(st.model.raw.d * sc).toFixed(2);
    ok(quarterTurned(st.model) ? (fp[0] === rd && fp[1] === rw) : (fp[0] === rw && fp[1] === rd),
       st.key + ': the footprint is ' + (quarterTurned(st.model) ? 'SWAPPED for its quarter turn' : 'unswapped') + ' — the collider turns with the model');
    ok(near(st.model.raw.h * scaleOf(st.model), st.model.height, 0.001), st.key + ': scaled to counter height ' + st.model.height + ' m');
  }

  /* EVERY station model is pinned to its packed file's sidecar — the same
     contract the intake desk and the bay have, applied by loop so a station
     cannot be given a model without also being given the check. */
  for (const st of STATIONS) {
    if (!st.model) continue;
    const glb = './public' + st.model.url, json = glb.slice(0, -4) + '.json';
    ok(existsSync(glb), st.key + ': the packed model is on disk (' + st.model.url + ')');
    const j = sidecar(json);
    ok(!!j, st.key + ': its sidecar exists (tools/pack-glb.mjs)');
    ok(!!j && j.bytes < 25 * 1048576 && j.triangles < 80000, st.key + ': under the cap and browser-sized (' + (j ? (j.bytes / 1048576).toFixed(2) + ' MB, ' + j.triangles.toLocaleString() + ' tris' : '?') + ')');
    ok(!!j && near(j.dims.w, st.model.raw.w, 0.02) && near(j.dims.h, st.model.raw.h, 0.02) && near(j.dims.d, st.model.raw.d, 0.02),
       st.key + ": raw matches the packed file's measured extents" + (j ? ' ' + JSON.stringify(j.dims) : ''));
    const box = colliders(STATIONS).find((c) => c.key === st.key);
    ok(!!box && near(box.hx, st.size[0] / 2 + 0.25, 0.001) && near(box.hz, st.size[1] / 2 + 0.25, 0.001), st.key + ": the walker's collider hugs its footprint");
  }

  // the Ward Bay desk
  const bayJ = sidecar('./public/models/hospital/ward-desk.json');
  ok(!!bayJ, 'the ward desk sidecar exists (tools/pack-glb.mjs)');
  ok(!!bayJ && bayJ.bytes < 25 * 1048576 && bayJ.triangles < 80000, 'the ward desk is under the cap and browser-sized (' + (bayJ ? (bayJ.bytes / 1048576).toFixed(2) + ' MB, ' + bayJ.triangles.toLocaleString() + ' tris' : '?') + ')');
  ok(!!bayJ && near(bayJ.dims.w, BAY_MODEL.raw.w, 0.02) && near(bayJ.dims.h, BAY_MODEL.raw.h, 0.02) && near(bayJ.dims.d, BAY_MODEL.raw.d, 0.02), "BAY_MODEL.raw matches the packed file's measured extents");
  const bay = STATIONS.find((x) => x.key === 'bay');
  const bayBox = colliders(STATIONS).find((c) => c.key === 'bay');
  ok(!!bayBox && near(bayBox.hx, bay.size[0] / 2 + 0.25, 0.001) && near(bayBox.hz, bay.size[1] / 2 + 0.25, 0.001), "the walker's collider hugs the ward desk's footprint");
  ok(existsSync('./public/models/hospital/ward-desk.glb'), 'the packed ward desk is on disk');

  // the bed
  const bedJ = sidecar('./public/models/hospital/ward-bed.json');
  ok(!!bedJ && !!bedJ.bed, 'the bed sidecar exists and carries the bed measurements (pack-glb --bed)');
  ok(!!bedJ && bedJ.bytes < 25 * 1048576 && bedJ.triangles < 80000, 'the bed is under the cap and browser-sized (' + (bedJ ? (bedJ.bytes / 1048576).toFixed(2) + ' MB, ' + bedJ.triangles.toLocaleString() + ' tris' : '?') + ')');
  ok(!!bedJ && near(bedJ.dims.w, BED_MODEL.raw.w, 0.02) && near(bedJ.dims.h, BED_MODEL.raw.h, 0.02) && near(bedJ.dims.d, BED_MODEL.raw.d, 0.02), "BED_MODEL.raw matches the packed file's measured extents");
  ok(!!bedJ && bedJ.bed && bedJ.bed.longAxis === 'z', "the bed's long axis is z — the slot's convention (a bed long in x would need the loader to turn it)");
  ok(!!bedJ && bedJ.bed && near(bedJ.bed.mattressTop, BED_MODEL.mattressTop, 0.005), 'BED_MODEL.mattressTop is the MEASURED mattress (' + (bedJ && bedJ.bed ? bedJ.bed.mattressTop : '?') + ')');
  ok(!!bedJ && bedJ.bed && bedJ.bed.headEnd === BED_MODEL.headEnd, 'BED_MODEL.headEnd is the MEASURED headboard end (' + (bedJ && bedJ.bed ? bedJ.bed.headEnd : '?') + ')');
  ok(bedYaw() === (BED_MODEL.headEnd === '-z' ? 0 : Math.PI), "bedYaw turns the model so its headboard lands at the cot's (local -z)");
  ok(near(BED_MODEL.raw.d * bedScale(), BED_SIZE.d, 0.001) && BED_SIZE.d === 2.2, 'the bed is scaled to fill the 2.2 m slot');
  ok(near(BED_SIZE.w, BED_MODEL.raw.w * bedScale(), 0.01), "BED_SIZE.w is the model's width at that scale (" + BED_SIZE.w + ' m) — the collider follows the mesh');
  ok(bedTop() > 0.4 && bedTop() < 0.8, 'the mattress is at bed height, not table height (' + bedTop() + ' m)');
  const l = lieAt(0);
  ok(!!l && near(l.y, bedTop() + LIE_CLEARANCE, 0.001), 'a patient lies ON the measured mattress, plus the clearance (y=' + (l ? l.y : '?') + ')');
  ok(COT.url === BED_MODEL.url, 'the built-in Ward Cot places the model, so every ward gets the bed');
  const bc = bedColliders([{ slot: 0 }])[0];
  ok(!!bc && near(bc.hx, BED_SIZE.w / 2 + 0.15, 0.001), "a placed bed's collider is as wide as the model");
  ok(existsSync('./public/models/hospital/ward-bed.glb'), 'the packed bed is on disk');
}

console.log('\n=== 1d. exposure past 70% costs the heroes a quarter of their stamina — once per crossing ===');
{
  const n0 = PEN.length;
  ok(HS.EXPOSURE_PENALTY.threshold === 0.7 && HS.EXPOSURE_PENALTY.fraction === 0.25, 'the rule is 70% and 25%, as asked');
  ok(HS.exposurePenaltyTick(0.60, 0.65) === null && PEN.length === n0, 'climbing but still under 70%: nothing');
  ok(HS.exposurePenaltyTick(0.70, 0.70) === null && PEN.length === n0, 'exactly 70% is not MORE than 70%');
  const hit = HS.exposurePenaltyTick(0.69, 0.71);
  ok(!!hit && hit.ok && PEN.length === n0 + 1, 'crossing 70% charges the heroes');
  ok(PEN[n0] && PEN[n0].fraction === 0.25 && PEN[n0].why === 'exposure', 'by a quarter, for exposure');
  ok(HS.exposurePenaltyTick(0.71, 0.90) === null && PEN.length === n0 + 1, 'staying above 70% does NOT charge again every frame');
  ok(HS.exposurePenaltyTick(0.90, 1.00) === null && PEN.length === n0 + 1, 'nor at 100%');
  ok(HS.exposurePenaltyTick(0.90, 0.495) === null && PEN.length === n0 + 1, 'a doff (x0.55) dropping it back is free');
  const again = HS.exposurePenaltyTick(0.495, 0.72);
  ok(!!again && again.ok && PEN.length === n0 + 2, 'a second breach that crosses again pays again');
  ok(HS.exposurePenaltyTick(undefined, 0.8) && PEN.length === n0 + 3, 'a missing "before" reads as 0 — the first frame over the line still counts');
  // the bridge missing the method is a refusal, not a throw
  const saved = globalThis.window.MythicPlagueBridge.heroStaminaPenalty;
  delete globalThis.window.MythicPlagueBridge.heroStaminaPenalty;
  const nob = HS.exposurePenaltyTick(0.1, 0.9);
  ok(nob && nob.ok === false && /bridge/.test(nob.why) && PEN.length === n0 + 3, 'an old game build without the bridge method: refused, not thrown');
  globalThis.window.MythicPlagueBridge.heroStaminaPenalty = saved;
}

console.log('\n=== 1e. the building is a hospital, not a bunker ===');
{
  const lum = (hex) => (0.2126 * ((hex >> 16) & 255) + 0.7152 * ((hex >> 8) & 255) + 0.0722 * (hex & 255)) / 255;
  const C = PLAN.colors;
  ok(lum(C.wall) > 0.9, 'the walls are white (luminance ' + lum(C.wall).toFixed(2) + ')');
  ok(lum(C.floorClean) > 0.7 && lum(C.floorClean) < lum(C.wall), 'the floor is pale, and darker than the walls');
  ok(lum(C.floorHot) > 0.7, 'the clean room floor is pale too');
  ok(lum(C.hazard) < 0.5 && lum(C.hazard) < lum(C.floorHot) - 0.2, 'the sterile line is dark enough to read on that floor');
  ok(lum(PLAN.bg) > 0.5, 'the background (and fog) is light, so the far walls fade to daylight, not a void');
}
...
for (const pid of PH.PRODUCT_IDS) for (const id of Object.keys(PH.PRODUCTS[pid].inputs))
  /* ⚠ A MIRROR of RESOURCES in index.html, and the reason this check exists:
     a recipe that names an id the game cannot hold is a product nobody can
     ever make, and nothing else in the codebase would say so. The core
     fourteen, plus the thirteen refinery cuts promoted in v121t5 — one of
     which (ethanol) the vaccine now needs. If you add a resource upstream and
     a recipe uses it, add it here too or this fails, which is the point. */
  ok(['food','ammo','water','medicine','energyDrink','supplies','metal','fuel','corruptedEssence','memoryShards','dna','wood','stone','cloth',
      'naphtha','kerosene','diesel','gasOil','heavyOil','slop','ethanol','reformate','alkylate','butane','catGasoline','hydrotreatedCut','reprocessedSlop'
     ].includes(id), pid + ' input ' + id + ' is a live resource id');
