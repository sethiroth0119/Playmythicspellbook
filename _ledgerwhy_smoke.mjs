...
/* ── phone ── */
/* The AZA ledger (_azaledger_smoke) added a currency filter beside the mode and
   merges AZA rows into the fallback; the mode still starts on the account record. */
...
ok(/if \(!r\.ok && r\.reason === 'missing'\) \{\s*paint\(ledgerMerge\(rows, /.test(HS), 'falls back to the device diary when the audit RPC is absent');
