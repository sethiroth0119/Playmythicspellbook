...
/* ── the phone app ── */
/* The player-facing title is "Node Manager" (owner rename, 2026-09-17). The
   app id stays 'mayor' on purpose: the open latch, the RPC names and the
   window.MythicMayorDash seam all key off it, and renaming an id is a data
   migration, not a wording change. */
ok(/\{ id: 'mayor',\s+name: 'Node Manager',/.test(HS), 'the phone has a Node Manager app (id still mayor)');
...
}
{
  ...
  const iNone = seg.indexOf('You are not a Node Manager anywhere yet');
...
}
{
  const w = world([]);
  await w.run();
  ok(/not a Node Manager anywhere yet/.test(w.html.box), 'a genuine empty list reads as empty once the fetch answered');
}
ok(/Node Manager since/.test(HS) && /Revenue split/.test(HS) && /Cards found on shift/.test(HS) && /Resources found on shift/.test(HS),
