...
/* 🔌 2026-09-17 owner's rule ("PRN claims: when they buy them from Foundation
   Reserve") replaced the v121v117 linked-node union — bug-mu2n0w7s: a Node
   Manager's own city rang a client's PRNs through it. The pins below moved with
   the rule; the count of checks did not drop. */
/* 📌 2026-09-19: this pin used to demand the filter's EXACT text, and the
   six-placed rule added a clause to it (a PRN the player owns but has not
   PLACED — status 'stored' — must not ring the city). The rule the check
   exists to protect is unchanged, so the text is now matched in the two parts
   that ARE the rule — owner_id compared against the viewer, and city_node_links
   not read at all — and the stored clause is pinned by BEHAVIOUR in the block
   below, where a stored anchor is put in the fixture and must not come back.
   A behavioural pin cannot be satisfied by re-typing the line differently. */
ok(/const boughtByMe = \(rows\) =>/.test(NC)
   && /n\.owner_id != null && String\(n\.owner_id\) === String\(me\)/.test(NC)
   && !/P\.cityLinkedNodes/.test(NC),
   'fetchNodes (own city) keeps ONLY the PRNs the viewer bought (owner_id = viewer) and no longer reads city_node_links');
{
  ...
  ok(JSON.stringify(await fn()) === '["a"]', 'run for real (older parent, reserve list): only a — the founder\'s b and the ownerless c are not this player\'s PRNs');
  g.P.cityLinkedNodes = async () => [{ id: 'b', owner_id: 'founder' }, { id: 'a', owner_id: 'me' }];
  g.P.cityMyNodes = async () => [{ id: 'a', owner_id: 'me' }, { id: 'd', owner_id: 'me', corp_id: null }, { id: 'b', owner_id: 'founder' }];
  ok(JSON.stringify(await fn()) === '["a","d"]', '…and with the owner read: a and d (a PRN whose corporation is gone), never b — a link or a manager contract is not a claim');
  /* 📦 THE SHELF IS NOT THE CITY (six-placed rule, 2026-09-19). An anchor the
     player owns but has not placed is theirs — it is still bought, still their
     owner_id — and it must not ring the city, because "placed" is precisely
     what ringing the city means. This is the fallback path that reads
     FoundationReserve.nodes raw, so it needs the rule of its own. */
  g.P.cityMyNodes = async () => [{ id: 'a', owner_id: 'me', status: 'active' }, { id: 's', owner_id: 'me', status: 'stored' }];
  ok(JSON.stringify(await fn()) === '["a"]', '…and a STORED anchor the player owns is not rung — owning it is not placing it');
  g.P.cityOwnerIdentity = () => ({ viewerId: null, isOwner: true });
  ok(JSON.stringify(await fn()) === '[]', 'NEGATIVE CONTROL: an unresolved viewer rings nothing (the old `!me ||` kept the whole corporation\'s reserve)');
}
