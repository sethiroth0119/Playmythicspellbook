...
ok(/const FREIGHT_MODEL = \{ url: '\/models\/trucks\/freight_semi\.glb', scale: 1, rotY: 90 \};/.test(RIGS), 'FREIGHT_MODEL is the semi, turned 90° (its long axis is X, cab at −X); a wrecked rig drives the same file, dimmed (v121v120)');
...
ok(/const raw = await haulLoadGLB\(THREE, RIG\.model\.url\);[^\n]*const fit = haulFit\(THREE, raw, \{ w: 2\.6, l: 15\.0 \}, haulAutoOrient\(THREE, raw, RIG\.model\.url\), true\); truck = fit\.node; size = fit;/.test(HAUL) && /proc\.forEach\(\(o\) => \{ o\.visible = false; \}\);/.test(HAUL), 'the semi is fitted to the traffic trucks\' width and the procedural rig is hidden, never removed (it is the fallback)');
