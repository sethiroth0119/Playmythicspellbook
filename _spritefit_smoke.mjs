...
/* ── 2. the stage: one fit rule ────────────────────────────────────────── */
{
  ...
  ok(/function unitWorldH\(def\)\{ return \(def && def\.h \|\| 1\) \* SPRITE_SCALE \* UNIT_DRAW_MUL \* sizeMul\(def\); \}/.test(BB), 'unitWorldH multiplies by the class table');
  /* 🔍 v174 (owner: "make unit sprites on board tiles bigger"): one draw
     multiplier for every figure. The camera fit must divide it back out, or
     fitCamera picks a steeper pitch and cancels the enlargement. */
  ok(/const UNIT_DRAW_MUL = \d+(\.\d+)?;/.test(BB), 'one UNIT_DRAW_MUL for every figure');
  ok(/unitWorldH\(\{ h:1\.05 \}\) \/ UNIT_DRAW_MUL/.test(fnText(BB, 'fitSpriteH')), 'fitSpriteH divides UNIT_DRAW_MUL out — the camera frames the board as before');
  ok(/\* SPRITE_SCALE \* UNIT_DRAW_MUL/.test(fnText(BB, 'unitScreenH')), 'unitScreenH draws the enlarged figure too');
  ...
  ok(/const maxW = hexW\(\) \* UNIT_DRAW_MUL \* \(k \/ \(unitWorldH\(def\) \|\| 1\)\);/.test(sb) && /if \(w \* vw > maxW\)/.test(sb), 'visible width capped at one tile (enlarged with the figure)');
  ...
  const code = [
    ...
    /* the geometry below is checked at the pre-v174 scale: every assertion is a
       RATIO (padded vs tight, large vs normal), and the width cap scales with
       the multiplier, so the rule is independent of its value */
    'const UNIT_DRAW_MUL = 1;',
