...
/* ── the three lists, which is how the last one of these shipped ── */
{
  ...
  /* cloudSyncProfile became a queue wrapper (2026-09-17); the upload body
     is _cloudSyncProfileOnce when that exists. */
  const up = seg(SRC.indexOf('function _cloudSyncProfileOnce(') > 0 ? '_cloudSyncProfileOnce' : 'cloudSyncProfile'), hy = seg('cloudFetchProfile'), lo = seg('loadForge');
  ...
  'the free floor is never clamped by the purchase ceiling, and warehouse stacks on top');
{
  /* VAULT_MAX_ROWS must keep `bought` from ever exceeding the clamp, or the
     clamp repossesses space someone paid for.
     ⚠ THE DOOR COUNT IS DERIVED NOW, NOT PINNED AT 13. This read
       `rows * 250 + 13000`, hardcoding the 13 Relic doors that happened to
       balance the 31,250 era. When the ceiling moved to 100,000 the check went
       red against a tree that was perfectly consistent — it was asserting one
       historical SOLUTION rather than the RULE the solution satisfied. The rule
       is: there exists a whole number of Relic doors, buyable within the row
       budget, whose rows and surplus land exactly on the unit cap. That holds
       for 73/13/31,250 and for 228/43/100,000, and would fail for a pair that
       genuinely did not line up. */
  const rows = /const VAULT_MAX_ROWS = (\d+);/.exec(SRC);
  const max = /const RES_STASH_MAX = (\d+);/.exec(SRC);
  const R = rows ? Number(rows[1]) : 0, M = max ? Number(max[1]) : 0;
  const ROW_UNITS = 250, DOOR_SURPLUS = 1000, DOOR_ROWS = 5;
  let doors = null;
  for (let d = 0; d * DOOR_ROWS <= R; d++) {
    if (R * ROW_UNITS + d * DOOR_SURPLUS === M) { doors = d; break; }
  }
  ok(rows && max && doors !== null,
    'the row cap and the unit cap still land on the same number — SOME whole number of Relic doors, within the row budget, lands exactly on RES_STASH_MAX',
