...
{
  /* ⚠ Found by NAME, not by its full signature. This read
     indexOf('async function _opSettle(o) {'), which went -1 when the settle gained
     a `pre` argument (76efa7fe00 — the collect pays what it measured before the
     claim) — and slice(-1, 8999) is not the settle, so both checks below failed
     against wage-pool code that had not moved. The presence check keeps a
     missing function from turning them into a vacuous read again. */
  const i = SRC.indexOf('async function _opSettle(');
  ok(i > 0, 'the settle is found');
