...
/* 📖 THE HANDBOOK'S GENERATED TABLES, REBUILT FROM THE GAME ON EVERY DEPLOY.
   ═══════════════════════════════════════════════════════════════════════════
   Owner: "update it every 24 hours when we add new rules and stuff to the game
   so it can always be updated."

   ⚠ A 24-HOUR TIMER IS THE WRONG TRIGGER AND THIS IS THE RIGHT ONE. The book's
     element / faction / status / rarity tables are generated from index.html's
     own constants, so they can only fall out of date when index.html changes —
     and an index.html change only reaches players through a deploy. A daily job
     would therefore do two useless things and one harmful one: run when nothing
     had changed, and leave the book wrong for up to 24 hours when something
     had. Regenerating here means the book is never stale for a single build,
     with no scheduler to babysit.
     (The book's PROSE is a separate matter and needs no timer either — admins
     edit it in the page and publish straight to Supabase.)

   ⚠ BEFORE minify(), AND IT MUST STAY THERE. tools/handbook-sync.mjs locates
     the constants with anchors like /^const ELEMENTS = \[/m against the SOURCE
     file. After minification those line starts do not exist and the tool would
     throw on every deploy. Same ordering constraint syncVersionTxt() documents.

   ⚠ A FAILURE WARNS, IT DOES NOT ABORT. The sync throws deliberately when a
     constant has been renamed rather than emitting a half-empty file. That is
     worth shouting about, but it is not worth blocking a deploy that may be
     carrying a fix players are waiting on — a stale rules table is a smaller
     harm than a held release. _handbook_smoke catches staleness in `npm run
     check`, which is where it should be caught; this is the belt to that. */
function syncHandbook() {
  try {
    execSync('node tools/handbook-sync.mjs', { stdio: 'inherit', shell: true });
  } catch (e) {
    console.error('\n⚠⚠ HANDBOOK SYNC FAILED — the rules book may now describe a game');
    console.error('    that no longer exists. Deploy continues. Run');
    console.error('    `node tools/handbook-sync.mjs` and read what it says.\n');
  }
}
