...
`sql/040_weaponsmith.sql` — idempotent, re-runnable, ships its RLS in the same file,
...
| ✅ 6 | `sql/040_weaponsmith.sql` — `crafted_weapons`, `ws_blueprints_owned`, `ws_shop`, `ws_blueprints` + `ws_mint` / `ws_grant_blueprint` / `ws_deliver` / `ws_state` | **Done and verified against a real Postgres 16** — see the commit. `server.js` routes the bench mint through `ws_mint`; offline still mints `local: true` (untradeable). ⚠ **Not yet applied** — run it by hand in the SQL editor |
...
| ✅ 9 | Order board + reputation + rep-gated blueprint tiers | **Done.** `sql/041_weaponsmith_board.sql` — contracts are SERVER-generated from templates. Verified against Postgres 16; caught a real bug (a new smith's first roll was throttled to an empty board). ⚠ **Not yet applied** |
| ✅ 10 | Forge bench (blades) | **Done.** `bench.forge.js` + `sql/042_weaponsmith_blades.sql` (knife 4 / sword 11 / greatsword 12, none for sale). Quality is the **worst** step, not the average; a ruined billet is gone |
...
| ✅ 3D-4 | The forge in 3D — anvil, quench barrel, billet that glows with the heat | **Done.** `scene.forge.js` — 6-stop smithing ramp, billet lights the room, heat chases and cools slower than it warms, **quench reads the bar inverted** |
...
⚠ **Three migrations to apply by hand**, in order: `sql/040_weaponsmith.sql`,
`sql/041_weaponsmith_board.sql`, `sql/042_weaponsmith_blades.sql`. Until they are, the client keeps its mirror, the board
