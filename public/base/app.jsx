...
function App() {
  ...
  /* 🕒 bug-mty87zhi — this was a FAKE clock (started at 02:14:32 on every open,
     sped up by the 1/2/4 keys) beside a hard-coded "DAY 047", while the Camp
     Console bar showed a different day and the player's local time, and the
     website a third. All three now read the website's world calendar:
     SOCIETY_EPOCH = 24 May 2026 UTC, day = floor((now - epoch) / 1 day) + 1,
     time in UTC. Same three lines as mythicWorldDay() in index.html. */
  const WORLD_EPOCH = Date.UTC(2026, 4, 24);
  const worldNow = () => {
    const d = new Date();
    return { h: d.getUTCHours(), m: d.getUTCMinutes(), s: d.getUTCSeconds(),
             day: Math.max(1, Math.floor((d.getTime() - WORLD_EPOCH) / 86400000) + 1) };
  };
  const [clock, setClock] = useState(worldNow);
  useEffect(() => {
    const id = setInterval(() => setClock(worldNow()), 1000);
  ...
  return (
    ...
    >
      <TopBar day={String(clock.day).padStart(3, "0")} clock={clockStr + " UTC"} threat={38} />
      <LeftColumn />
      <RightColumn />
