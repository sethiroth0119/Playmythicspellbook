...
// TOP BAR
// ───────────────────────────────────────────────────────────────
function TopBar({ day = "DAY ---", clock = "--:--:--", threat = 38 }) {
  ...
  return (
    <div className="hud-top">
      ...
      <div className="hud-clock">
        <div className="clock-block" title="World day — the same calendar as mythicspellbook.xyz and the Camp Console. A new day starts at 00:00 UTC.">
          <span className="clock-lbl">World Day</span>
          <span className="clock-val day">{day}</span>
        </div>
        <div className="clock-block" title="World time (UTC) — the same clock every player and the website use">
          <span className="clock-lbl">World Time</span>
...
function LeftColumn() {
  ...
  // Asks the game to abandon the run; the confirm renders in the parent above
  // this iframe, and the parent re-sends base:data (bag unlocked) afterwards.
  const abandonExpedition = () => {
    if (!bagLocked) return;
    try { window.parent.postMessage({ type: "base:action", action: "abandonExpedition" }, window.location.origin); } catch (e) {}
  };
      ...
      {BAG && BAG.has && (
        ...
        >
          ...
          {bagLocked ? (
            ...
            >
              🔒 On expedition{BAG.runName ? ` (${BAG.runName})` : ""} — bank loot at a Convoy
            </div>
            {/* 🧭 Safety valve (v180): a run the player cannot reach used to
                lock this bag forever. The game side (base:action
                'abandonExpedition') runs the same rlcAbandonRun as every run
                screen, with its own confirm and forfeit warning. */}
            <button
              onClick={abandonExpedition}
              title="End the current expedition. Loot already in the Field Bag stays; rewards that only pay out on completing or dying in the run are forfeited."
              style={{
                display: "block",
                margin: "6px auto 0",
                cursor: "pointer",
                ...
                padding: "3px 10px",
                borderRadius: 7,
                border: "1px solid rgba(255,119,85,0.45)",
                background: "rgba(255,119,85,0.08)",
                color: "#ffb3a0",
              }}
            >
              ✖ Abandon expedition
