    ...
    <div className="screen">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 12 }}>
        {rows.map(o => {
          ...
          /* 🔒 OWNER, NOT OFFICER (feature board, Gary Brooks). E.amOwner is
             also true for a member appointed CEO, and that is exactly how
             "people trying to help out" ended up controlling the businesses:
             one promotion handed them Found, the Labour Pool, the job offers
             and Collect. E.amTrueOwner is corporations.founder_id and nothing
             else, and sql/166 makes the server agree on all four.
             ⚠ THE FALLBACK IS THE OLD FIELD, on purpose: an older index.html
               that does not send the new one must not lock a real owner out
               of their own page. It costs nothing, because this line only
               decides what is DRAWN — the handler refuses either way, and
               the server refuses under that.
             ⚠ MEMBER-SIDE CONTROLS DO NOT HANG OFF THIS. Apply, and
               accepting an offer made to you, are gated on `isMe`. A member
               must still be able to take a job. */
          const isOwner = !!(E.amTrueOwner != null ? E.amTrueOwner : E.amOwner);
          ...
          <div key={o.id} className="card" style={{ padding: 14, display: 'flex', flexDirection: 'column' }}>
            ...
            {hasCorp && !owned && (() => {
              ...
              const label = !isOwner ? 'The owner funds this'
              ...
              return (
                <button className="btn primary" disabled={off}
                  title={!isOwner ? 'The corporation owner buys Operations — ask them' : tooPoor ? ('Treasury holds ' + fmtC(treasury) + ' 🔥 — this operation costs ' + fmtC(startup) + ' 🔥') : ''}
