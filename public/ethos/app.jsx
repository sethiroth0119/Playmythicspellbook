    ...
    function onMsg(e) {
      ...
      setAccount(function (prev) {
        ...
        var next = {
          ...
          // 🪙 the wallet's own AZA moves (wallet_ledger 'sovereigns'), for PageLedger
          azaWallet: Array.isArray(d.azaWallet) ? d.azaWallet : ((prev && prev.azaWallet) || []),
      ...
      <div className="stack-sm" style={{ marginBottom: 12 }}>
        ...
        {kind === "resource" && (
          <>
            ...
            <select value={itemId} onChange={(e) => setItemId(e.target.value)} style={{ background: "var(--bg-2)", border: "1px solid var(--hair)", color: "var(--ink)", padding: "8px 10px" }}>
              <option value="">— select —</option>
              {/* A-Z by name (bug-mtycn31u). The catalogue is the base list followed by
                  ids found in the ledgers, so its own order is only partly alphabetical. */}
              {cat.filter(r => (walletRes[r.id] | 0) > 0)
                  .slice().sort((x, y) => String(x.name || x.id).localeCompare(String(y.name || y.id), undefined, { sensitivity: "base", numeric: true }))
                  .map(r => (
...
function PageLedger({ account, transfers = [] }) {
  ...
  // 🪙 Asked for: "Add AZA coin to the … bank ledger, show when AZA coin is
  // added or spent." account.azaWallet is the player's own WALLET AZA moves
  // (wallet_ledger, resource 'sovereigns'), seeded by the parent and already
  // labelled by /src/azahistory's classify() — the same words the phone and
  // the AZA history use. Those rows are wallet moves, not bank moves, so they
  // are tagged "Wallet" and never folded into the bank's Cinder figures.
  // The chips used to be placeholders ("Live · 2026-05-19", "CDR flow +94,200")
  // and the filter buttons did nothing; both are real now.
  const [cur, setCur] = useState("all");
  const ledger = Array.isArray(account && account.ledger) ? account.ledger : [];
  const azaWallet = Array.isArray(account && account.azaWallet) ? account.azaWallet : [];
  const fmtT = (ms) => { try { return new Date(ms).toLocaleString(); } catch (e) { return ""; } };
  const bankRows = ledger.map(e => ({
    ts: Number(e.ts) || 0,
    type: e.kind || "—",
  ...
  const walletRows = azaWallet.filter(e => e && (e.d | 0)).map(e => ({
    ts: Number(e.t) || 0,
    type: (e.d | 0) > 0 ? "AZA added" : "AZA spent",
    actor: "Wallet",
    memo: e.r || "",
    cinder: 0,
    aza: e.d | 0,
    after: e.b == null ? null : (e.b | 0),
    src: "wallet",
  }));
  const all = bankRows.concat(walletRows).sort((a, b) => b.ts - a.ts);
  const shown = all.filter(e => cur === "all" ? true : cur === "cinder" ? !!e.cinder : !!e.aza);
  const cdrFlow = all.reduce((s, e) => s + e.cinder, 0);
  const azaIn = all.reduce((s, e) => s + (e.aza > 0 ? e.aza : 0), 0);
  const azaOut = all.reduce((s, e) => s + (e.aza < 0 ? -e.aza : 0), 0);
  const fb = (id, label) => (
    <button className={"btn sm " + (cur === id ? "" : "ghost")} data-ledger-filter={id} onClick={() => setCur(id)}>{label}</button>
  );
  return (
    <div>
      <div className="page-head">
      ...
      <div className="panel">
        <div className="panel-h" style={{ flexWrap: "wrap", gap: 8 }}>
          <div className="row" style={{ gap: 10, flexWrap: "wrap" }}>
            <span className="chip">{shown.length} {shown.length === 1 ? "entry" : "entries"}</span>
            <span className="chip cinder">CDR flow {(cdrFlow > 0 ? "+" : "") + fmt(cdrFlow)}</span>
            <span className="chip aza" data-aza-flow>AZA +{fmt(azaIn)} added · −{fmt(azaOut)} spent</span>
          </div>
          <div className="row" style={{ gap: 6 }}>
            {fb("all", "All")}
            {fb("cinder", <span className="row" style={{ display: "inline-flex", gap: 4, alignItems: "center" }}><CinderGlyph size={11}/>Cinder</span>)}
            {fb("aza", <span className="row" style={{ display: "inline-flex", gap: 4, alignItems: "center" }}><AzaGlyph size={11}/>AZA</span>)}
          </div>
        </div>
        <div style={{ overflowX: "auto" }}>
        <table className="t">
          <thead>
            <tr><th>Time</th><th>Type</th><th>Counterparty</th><th>Memo</th><th style={{ textAlign: "right" }}>Amount</th><th>Currency</th><th>After</th></tr>
          </thead>
          <tbody>
            {shown.map((e, i) => (
              <tr key={i} data-cur={e.aza && !e.cinder ? "aza" : "cinder"} data-src={e.src}>
                <td className="mono dim">{fmtT(e.ts)}</td>
                <td><span className={"chip" + (e.src === "wallet" ? " aza" : "")}>{e.type}</span></td>
                <td>{e.actor}</td>
                <td className="dim">{e.memo}</td>
                <td className="mono" style={{ textAlign: "right", color: e.cinder > 0 ? "var(--good)" : e.cinder < 0 ? "var(--bad)" : "var(--ink-dim)", fontWeight: 600, whiteSpace: "nowrap" }}>
                  {e.cinder && cur !== "aza" ? <span><span className="row" style={{ display: "inline-flex", gap: 4, alignItems: "center" }}><CinderGlyph size={12}/>{(e.cinder > 0 ? "+" : "") + fmt(e.cinder)}</span></span> : (e.aza ? null : "—")}
                  {e.aza && cur !== "cinder" ? <span style={{ marginLeft: e.cinder ? 8 : 0, color: e.aza > 0 ? "var(--good)" : "var(--bad)" }}><span className="row" style={{ display: "inline-flex", gap: 4, alignItems: "center" }}><AzaGlyph size={12}/>{(e.aza > 0 ? "+" : "") + fmt(e.aza) + " AZA"}</span></span> : null}
                </td>
                <td className="dim mono">{e.cinder && e.aza ? "CDR · AZA" : (e.cinder ? "CDR" : (e.aza ? "AZA" : "—"))}</td>
                <td className="dim mono" style={{ whiteSpace: "nowrap" }}>{e.after != null ? <span className="row" style={{ display: "inline-flex", gap: 4, alignItems: "center" }}><AzaGlyph size={11}/>{fmt(e.after)}</span> : ""}</td>
              </tr>
            ))}
            {shown.length === 0 && (
              <tr><td colSpan="7" className="dim" style={{ textAlign: "center", padding: 28, fontStyle: "italic" }}>{cur === "aza" ? "(none) — no AZA moves yet" : "(none) — no transactions yet"}</td></tr>
            )}
          </tbody>
        </table>
        </div>
