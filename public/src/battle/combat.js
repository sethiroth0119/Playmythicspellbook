...
(function (global) {
   ...
   * ================================================================== */
  /* 🔴 THE WHITE BACKGROUND ON A MOVE VFX. Owner, three times: "when move vfx
     happen like sword slice a white background happens remove that it should
     just show the VFX" — then "the white background have not been removed" —
     then "remove it completely where the opacity is 0% and you can see the
     battle board with the vfx".
     Three previous attempts missed it because all three were greps, and each
     found a DIFFERENT white layer that was not this one: the board canvas
     'flash' pass, G.flashEl, and .mvfx-flash. All three are genuinely white,
     all three are genuinely full-screen, and none of them was the one playing.
     MEASURED, not read. Built the real overlay and asked the Web Animations
     API what it paints:
       #febx-root    position:fixed  z-index:4200  cover=100%
       .febx-flash   cover=100%  background rgb(255,246,232)
       animation febxFlash  [0%:0  8%:0.9  100%:0]   PEAK = 0.90
     A full-viewport near-white sheet at 90% opacity on every hit frame. At that
     value it does not accent the slice, it COVERS it — the slash, the board and
     both figures go pale at exactly the moment the player is watching the hit.
     ⚠ OFF, NOT DELETED, and behind ONE switch. `e.flash` is touched from three
     places in the sequencer (the static poster frame sets .14, the live hit
     adds .on, settle clears it). Deleting the node would mean editing all three
     and leaving a reader wondering what used to be there; `display:none` makes
     every one of those a harmless no-op. Flip this const to restore it.
     The defender's portrait flash (.febx-fig.flash .febx-art, a brightness
     filter on the FIGURE) is deliberately untouched — that is the VFX the owner
     asked to keep seeing, not a background. */
  var FEBX_HIT_FLASH = false;
