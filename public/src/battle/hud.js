...
function relayout() {
  ...
  /* 🎯 THE DOCK GOES INSIDE `.bsx-frame`, NOT ON THE SCREEN ROOT.
     Asked for: "make this button fit directly in the right ui panel". It used
     to be `scr.appendChild(dock)` — a fixed box floating in the band the rail
     reserves beneath itself, so END TURN read as a slab dropped next to the
     panel rather than part of it. As the frame's last flow child it is inside
     the gold border, under the same braces, and `.bsx-body { flex: 1 }` absorbs
     the change: the body ends up 6px TALLER (the old seam between the rail's
     foot and the dock's crown), and §7b.6's cap only ever asks for LESS shrink
     when the body grows, so none of its fitted constants move.
     ⚠ The fallback matters and is not defensive noise: on the GAME-OVER frame
     there is no `.bp-wrap` at all (see the FROZEN note above — `hudx-norail`),
     and END TURN still has to be somewhere. It falls back to exactly the old
     home, which is the layout that shipped. */
  const etwrap = scr.querySelector('.etwrap');
  if (etwrap) {
    const dock = mk('div', 'hudx-endturn');
    dock.appendChild(etwrap);
    const frame = bp && bp.querySelector('.bsx-frame');
    if (frame) { dock.classList.add('hudx-et-inrail'); frame.appendChild(dock); }
    else scr.appendChild(dock);
  }

  /* ── 4b. DAY/NIGHT AND WEATHER GO TO THE HEAD OF THE LEFT COLUMN ────────
        "Move the day, and weather ui above — I want to clean the clutter."
        They rendered SIXTH in the rail, between the SCP control track and the
        ultimate button (measured y=688 and y=720 at 1920x1080), which put the
        two facts that never change during a turn in the middle of the four
        that change constantly. Hoisted to the top of `.bc-rail` they read as
        the match's standing conditions, which is what they are, and the
        tactical rows below close up.

        MOVE, never re-emit: `.envcard.wthr` and `.envcard.loc` are BUTTONS with
        listeners bound by the render that just ran, and `escapeHtml` has
        already been applied to the location name inside them — re-serialising
        would be a second chance to get that wrong for nothing. */
  /* 🔴 EVERY MOVED BLOCK TRAVELS INSIDE A `.bcp` CARRIER. The left rail's
     contents are styled by rules scoped `.bcp .envcard`, `.bcp .ultbtn`,
     `.bcp .consbox` … (index.html ~5307-5700). Re-parenting one of them bare
     drops that ancestor and every rule with it — measured, not feared: lifting
     `.envrow` alone took the location glyph and the weather orb to ~150px and
     the cards lost their grid completely. Same trick §3 already uses for the
     command bar (`mk('div', 'bchrome bcp hudx-cmd')`): give the moved node a
     carrier that re-establishes the scope. Each carrier is built once and then
     reused, so a re-render does not stack a new one every frame. */
  const carry = (cls, host, before) => {
    let c = scr.querySelector('.' + cls.split(' ').pop());
    if (!c) { c = mk('div', cls); }
    if (c.parentElement !== host) {
      if (before && before.parentElement === host) host.insertBefore(c, before);
      else host.appendChild(c);
    }
    return c;
  };

  /* ── 4b. LOCATION / WEATHER GO UP WITH THE HERO FRAMES ──────────────────
        "I want this next to the hero health up here with the hero frame."
        They rendered in the middle of the left rail, between the SCP track and
        the ultimate — the two facts that never change during a turn sitting
        among the four that do. They belong with the other standing conditions
        of the match, which is the top band.

        INSIDE `.hudx-banners`, as a flex sibling of `#fighters` — NOT dropped
        into the band's left gutter. The band spans the full width at z=62 and
        the rails sit UNDER it (z=61/60), so anything parked at the band's left
        edge lands on top of the foe cluster. As a sibling the pair is centred
        as one group, which keeps it clear of both rails at every width the band
        has: measured free gutter 597px at 1920, 503 at 1600, 430 at 1366, all
        of them wider than the row needs. */
  /* 🔴 THE BAND ONLY HAS ROOM FOR IT ON A WIDE SCREEN, AND THE ROOM IS
     MEASURED, NOT ASSUMED. The banners row is centred, so putting the
     conditions beside it makes the GROUP wider and pushes its left edge toward
     the left rail. The budget is
         2 x rail + envrow + gap + fighters  <=  window
     and the fighters' own width moves with the hero names, so no media query
     can stand in for it. Measured at true scale (see the `_uiAutoScale` note in
     drive-battle-hud — an earlier pass measured this through a 0.78 root zoom
     the real app never applies, and the overlap was invisible):
        1920  336x2 + 320 + 12 + 726 = 1730 of 1920   fits, 190 spare
        1600                          = 1598 of 1600   overlapped the rail by 25px
        1366  292x2 + 320 + 12 + 594 = 1510 of 1366   overlapped by 88px
     So below the crossing the row goes back to the head of the left rail, which
     is where the previous release put it and is still "above the clutter". The
     alternative — letting it ride over the foe cluster — is the exact defect
     this whole round of work is about. */
  {
    const envrow = scr.querySelector('.envrow');
    const bcRail0 = scr.querySelector('.bc-rail');
    if (envrow) {
      let carrier = envrow.closest('.hudx-envtop');
      if (!carrier) {
        carrier = mk('div', 'bchrome bcp hudx-envtop');
        carrier.appendChild(envrow);                     // MOVE — buttons and their listeners intact
      }
      let room = 0;
      if (banners && fighters) {
        const bandW = (band.getBoundingClientRect().width) || window.innerWidth;
        const fw = fighters.getBoundingClientRect().width || 0;
        const railW = parseFloat(getComputedStyle(document.documentElement)
                        .getPropertyValue('--btl-rail-w')) || 336;
        room = Math.floor((bandW - fw) / 2 - railW - 24);
      }
      const MIN_ENV = 210;      // below this the two cards stop being readable
      if (room >= MIN_ENV && banners) {
        carrier.style.setProperty('--hudx-env-w', Math.min(320, room) + 'px');
        if (carrier.parentElement !== banners) banners.insertBefore(carrier, banners.firstChild);
      } else if (bcRail0) {
        carrier.style.removeProperty('--hudx-env-w');
        if (carrier.parentElement !== bcRail0) bcRail0.insertBefore(carrier, bcRail0.firstChild);
      }
    }
  }

  /* ── 4c. ULTIMATE · CONSUMABLES · FIELD BAG GO TO THE RIGHT RAIL ────────
        "Move [them] on top of the right ui in the empty space, make it neat."
        The right rail carries a deliberate band of slack — §7b.6a computes
        `--hudx-surplus` and spends it as symmetric padding on `.bsx-inner`
        precisely BECAUSE there was nothing to put there. Measured 161px at
        1920x1080. Now there is something to put there.

        Placed before `.hudx-cmd` so the column reads
          head · piles · buffs · tools · tunneled · KIT · concede/turn · END TURN
        i.e. the player's own kit sits directly above the controls that end the
        turn, and the two rails stop being "everything on the left, nothing on
        the right".

        ⚠ ON A SHORT VIEWPORT THIS COSTS RIGHT-RAIL SIZE, NOT VISIBILITY.
        `_bpFit()` scales `.bsx-inner` to its body — measured at 1366x768 the
        inner is 580 layout px rendered into a 455px body, i.e. --fit ~0.78 with
        nothing clipped. Adding the kit lowers --fit further rather than cutting
        anything off, which is the right trade and is asserted. */
  const rightInner = bp && bp.querySelector('.bsx-inner');
  if (rightInner) {
    const kitParts = [scr.querySelector('.ultbtn'),
                      scr.querySelector('.consbox'),
                      scr.querySelector('#btn-bc-bag')].filter(Boolean);
    if (kitParts.length) {
      const cmd = rightInner.querySelector(':scope > .hudx-cmd');
      const kit = carry('bchrome bcp hudx-kit', rightInner, cmd);
      kitParts.forEach((p) => { if (p.parentElement !== kit) kit.appendChild(p); });
    }
  }
