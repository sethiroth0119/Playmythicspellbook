...
(function (global) {
  ...
  var LEY = {
    ...
    // 🌋 SEED POWER FOR A DEDICATED ELEMENTAL SURFACE (a generated 'lava' /
    // 'ice' / 'void' plate, seeded by identity — see surfSeedFor). 2, not 1,
    // and the reasoning is the EXISTING vocabulary rather than a new opinion:
    //   • EDITOR_SEED already splits its keys two ways. The paints that ARE an
    //     element — lava, water, snow, blight — seed power 2. The paints that
    //     merely LEAN one — grass, dirt, sand, stone, rubble, ash — seed 1.
    //   • A generated elemental plate is the first kind. It is a lava realm
    //     tile, not dirt that happens to be warm. Seeding it 1 would make the
    //     generator's flagship ground weaker than an admin's lava brush for no
    //     reason a player could ever discover.
    //   • SEED_CAP is 2, so this is already the ceiling for seeded ground.
    //     Power 3 stays unreachable by seeding and remains the reward for
    //     holding a hex through a tick — entrenchment still means something.
    //   • Consequence, stated out loud: BOON_POWER is 2, so a unit of the
    //     matching element standing on generated elemental ground draws its
    //     boon on turn zero. That is NOT new — an EDITOR_SEED lava hex has
    //     always done exactly this. The bug was that generated ground did not.
    //     SEED_SHARE_CAP still knocks any element that swallows the board back
    //     down to 1, which is the ceiling that keeps this honest.
    ELEM_SURF_POWER: 2,
    ...
    // ⛰ HIGH GROUND, SECOND HALF — whether the shot LANDS, not just what it
    // does when it does. Percentage POINTS of accuracy per rung of difference,
    // added to the single `accuracy` number calculateDamage already rolls
    // against. See accuracyMod.
    //
    // 🔴 RANGED ONLY, exactly like ELEV_PER_RUNG above — same `dist > 1` test,
    // same reasoning, and the owner ruled on 2026-09-14 that neither is ever
    // widened to melee. The two rules are deliberately built to be impossible
    // to drift apart: they read the same rungAt(), share ELEV_MAX_RUNGS as the
    // cliff cap, and take the difference in the same direction, so their signs
    // cannot disagree without someone editing both.
    //
    // Why 5 and not 0.08's shape: the two numbers are in DIFFERENT UNITS and
    // copying one into the other is the obvious mistake. ELEV_PER_RUNG is a
    // fraction of outgoing damage; this is raw points off a 0-100 roll, and
    // that channel is already crowded — smoke -50, mistveil -25, sandShroud
    // -30, -15 per negative speed level, panicked -25, cat-eyes +5. At 5/rung a
    // full 3-rung cliff is worth exactly one speed level. That is the
    // calibration: a real edge that a player can feel and plan around, never
    // the term that decides the exchange on its own. §5.3 asks for "an
    // advantage and not an auto-win" and this is where that is spent.
    ELEV_ACC_PER_RUNG: 5,
    // The elevation term clamps ITSELF, and that is not belt-and-braces.
    // 🔴 The LIVE accuracy path in calculateDamage never clamps its running
    // total — only the estimate branch does (`Math.max(0, Math.min(100, ...))`)
    // — so an uphill penalty is free to stack on smoke + mist + speed and drive
    // the roll deeply negative with nothing to stop it. Capping the term at its
    // source is the only place it CAN be capped without touching the shared
    // roll, which §5.3 forbids ("do not add a second, parallel hit-resolution").
    // At ELEV_ACC_PER_RUNG 5 x ELEV_MAX_RUNGS 3 this binds at exactly the value
    // the rung cap already produces, so today it is invisible; raise either
    // number and it starts doing independent work, which is precisely why it is
    // a separate knob and not `PER_RUNG * MAX_RUNGS` inlined.
    ELEV_ACC_CAP: 15,
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     ...
     hand-painted Athena map or the procedural default.
       ...
       SURF_SEED      the CITY surfaces _bbGenTerrain emits — asphalt, water,
                      grass, rubble, dirt. Coarser on purpose: the generator's
                      city vocabulary genuinely does not distinguish snow from
                      ash, so `grass` leans nature rather than BEING nature.
       surfSeedFor()  everything else, by IDENTITY. See the block below.

     `null` means NEUTRAL — unclaimed ground, and the point of the mode. A
     `null` ROW is not the same thing as a MISSING row: a null row says "this
     surface is deliberately neutral" and stops the identity fallback dead; a
     missing row says "not a city surface, ask the fallback".
     ══════════════════════════════════════════════════════════════════════════ */
  var EDITOR_SEED = {
  ...
  var SURF_SEED = {
    ...
    /* 🪨 GROUND APPENDED 2026-09-14 (board _SURF_ORDER 24, 25). Both rows are
       here because §2b of _ley_smoke asserts every generator surface is
       EXPLICITLY DECIDED — a `null` is a decision, a missing row is an
       accident — and the gate fired the moment the board page appended these
       two. That firing was the gate working, not a regression.

       sand SEEDS, and seeds exactly what an admin's sand brush seeds.
       EDITOR_SEED.sand is { earth, 1 } and dirt is { earth, 1 }; sand is
       ground that leans earth in precisely the way dirt does, and a player who
       has learned that an admin's sand is earth ground would be wrong-footed
       by generated sand that is not. Note that "neutral filler" in
       TABLETOP-BAR §4 is a statement about SHARE OF THE BOARD, not about
       bonuses — water and rubble sit in that same filler band and both seed.

       mud is NULL, and this is the one row that is a rule rather than a
       vocabulary choice. The owner's ask (§5.4) is a −1 speed and nothing
       else; mud is a hazard basic, not an element. Seeding it anything would
       hand a deck a damage bonus nobody asked for on the tile that is already
       costing them a step. If mud ever earns an element, it wants a decision
       recorded here, not an identity fallback picking one for it. */
    sand:    { elem: 'earth',  power: 1 },
    mud:     null,
  };
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     🔴 GENERATED ELEMENTAL GROUND — the seed resolver. FIXED 2026-09-14.

     THE BUG THIS CLOSES. `_bbGenTerrain` paints 24 surfaces (`_SURF_ORDER`,
     public/battle-board/index.html:1725): five city surfaces, then NINETEEN
     dedicated elemental plates — earth, wind, light, shadow, nature, storm,
     ice, metal, poison, psychic, arcane, void, blood, crystal, corruption,
     spirit, lava, sound, gravity. SURF_SEED listed only the five city keys, and
     seed() drops any surface it cannot match (`if (!s) continue`). So every one
     of those nineteen seeded NO tile.ley, damageMod returned a flat 1, and
     ELEM_BOON never paid. Measured on the real generator before the fix, over
     200 seeds: 19/19 elemental surfaces inert, describe() null on all of them.
     The absurd part: a `dirt` hex granted earth ley while an actual `earth`
     REALM hex next to it granted nothing, because dirt was in the table and
     earth was not. The elemental tiles were decoration until a unit walked over
     and converted the hex to its own element with tick().

     This PREDATES the tabletop pass — it is not a regression from it — and it
     broke Ask A ("tiles randomised every match, STILL CARRYING ELEMENT
     BONUSES") and TABLETOP-BAR §4 ("every element that has a bonus must still
     be reachable; the bonus system is a live function").

     🔴 WHY IDENTITY-WITH-A-BOON-ROW AND NOT NINETEEN MORE TABLE ROWS.
     A literal list would have to be re-edited every time the owner drops in a
     new tile PNG, and the §4.1 contract promises a new surface costs ONE table
     row in the board page. It would also rot silently: a missing row is not a
     crash, it is a tile that quietly grants nothing — which is the exact bug
     being fixed here, reintroduced. So the rule is structural instead:

         a surface whose key IS an element with an ELEM_BOON row seeds itself.

     That lands the nineteen automatically and lands the twentieth the day it is
     drawn, with no edit here at all.

     🔴 AND IT IS WHY MUD CANNOT ACCIDENTALLY BECOME ELEMENTAL. §5.4 is explicit
     that mud is a HAZARD BASIC and grants no elemental bonus, and the obvious
     lazy fix for this bug — bolting mud/sand/street into SURF_SEED alongside
     everything else — would have broken that outright. It cannot happen here:
     `mud`, `sand`, `asphalt` and `street` have no ELEM_BOON row, because they
     are not elements, so the fallback structurally refuses them. Anyone who
     wants mud to seed ley now has to invent a `mud` element first, which is a
     conversation, not a typo.

     ORDER MATTERS. The explicit table is consulted FIRST and its `null` rows
     are honoured as decisions, not as absences — otherwise a surface someone
     deliberately neutralised would be resurrected by the fallback behind their
     back. `water` is the live example: it is an ELEM_BOON element AND a city
     surface, and the explicit row (power 2) is what a reader expects to win.
     ══════════════════════════════════════════════════════════════════════════ */
  function surfSeedFor(surf) {
    if (!surf) return null;
    // 1. The city table decides, including when it decides NEUTRAL.
    if (Object.prototype.hasOwnProperty.call(SURF_SEED, surf)) return SURF_SEED[surf];
    // 2. A dedicated elemental plate seeds its own element. The ELEM_BOON row
    //    is the membership test — it is the one table that already contains
    //    exactly the elements and nothing but the elements.
    if (Object.prototype.hasOwnProperty.call(ELEM_BOON, surf)) {
      return { elem: surf, power: LEY.ELEM_SURF_POWER };
    }
    // 3. Anything else — a basic, a hazard, a surface from a future batch that
    //    is not an element — stays bare ground. Bare ground is a real answer.
    return null;
  }
  ...
  // ⛰ The same shape as clampMod, deliberately NOT clampMod itself: LEY.CAP is
  // a damage FRACTION (0.60) and this value is accuracy POINTS, so sharing the
  // clamp would silently cap every elevation accuracy term at 0.6 of a point —
  // a rule that runs, reports a number, and does nothing a player could ever
  // notice. Kept adjacent so the "INTENSITY is applied inside the clamp" rule
  // above is visibly true of BOTH exits and a third modifier cannot be added
  // later that quietly skips the dial.
  function clampAcc(p) {
    if (!isFinite(p)) return 0;
    var scaled = p * (isFinite(LEY.INTENSITY) ? LEY.INTENSITY : 1);
    return Math.max(-LEY.ELEV_ACC_CAP, Math.min(LEY.ELEV_ACC_CAP, scaled));
  }
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════════════════ */
  // 🪜 COPY 3 OF 4 OF THE LADDER SPACING, and the only one that is a RULE
  // rather than a picture: index.html's _BB_ELEV is the source, _harness.html
  // holds a byte-for-byte copy and .gauntlet/tabletop/shot.mjs a deliberate
  // one. This module cannot read _BB_ELEV — it is a top-level `const` in
  // index.html and therefore not on window (see the header's note about the
  // globals trap), which is why the number is written down again here rather
  // than imported. A drift shows up as a unit being told it is one rung above
  // an enemy it is level with: no error, no visual tell, just a wrong hit
  // chance. `node .gauntlet/tabletop/ladder-sync.mjs` checks all four.
  // Verified 2026-09-15: all four at 0.34. The rung-legibility round measured
  // whether the ladder could be deepened and concluded it could not — the cap
  // is the pick ray, not taste; the derivation is at _BB_ELEV.
  ...
  function seed(state, map) {
    ...
    for (var y = 0; y < H; y++) {
      for (var x = 0; x < Wd; x++) {
        ...
        var s = null, painted = false;
        if (ed && x < eCols && y < eRows) {
          var key = Array.isArray(ed[x]) ? ed[x][y] : ed[x * eRows + y];
          if (key && Object.prototype.hasOwnProperty.call(EDITOR_SEED, key)) {
            s = EDITOR_SEED[key];
            // 🔴 `painted` SEPARATES "the admin said NEUTRAL here" from "the
            // admin painted nothing here". Both leave `s` null, and before this
            // flag existed the generator surface then overrode the admin's
            // decision — EDITOR_SEED.road is the only null row, so today this
            // is a NO-OP (_bbMapFromEditor maps a `road` paint to the
            // 'asphalt' surf, which surfSeedFor also calls neutral, so the two
            // already agree and no live map changes). It is written down
            // because the identity fallback below now covers 19 surfaces
            // instead of zero, which makes "an explicit null is a decision, not
            // an absence" load-bearing in a way it was not before.
            painted = true;
          }
        }
        // 🔴 Generator surface. Goes through surfSeedFor, NOT through a raw
        // SURF_SEED lookup: the raw lookup is what silently dropped all 19
        // elemental surfaces on the floor. See the surfSeedFor block.
        if (!s && !painted && surfByKey) s = surfSeedFor(surfByKey[x + ',' + y]);
        if (!s) continue;
        tile.ley = {
          elem: s.elem,
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     ⛰ HIGH GROUND → ACCURACY — the companion to damageMod's elevation block.

     Returns { pts, rungs }. `pts` is added straight onto calculateDamage's one
     mutable `accuracy` number; `rungs` is the capped signed rung difference,
     reported so a readout can print "+2" without re-deriving it and without
     having to know ELEV_ACC_PER_RUNG. Positive means the ATTACKER is higher,
     the same direction damageMod.elevRungs and knockbackBonus already use — the
     three must never disagree in sign and this is the only reason all three
     take the difference in the same order.

     🔴 IT DOES NOT ROLL. It returns points; calculateDamage owns the single
     `Math.random() * 100 >= accuracy` roll and keeps owning it. §5.3: "it must
     go through the existing accuracy path. Do not add a second, parallel
     hit-resolution." A second roll here would also double-count against the AI,
     which reads hit chance through that very same function.

     RANGED ONLY (`dist > 1`), by owner decision 2026-09-14 and to match
     damageMod exactly. The melee argument is damageMod's, unchanged: two
     fighters in the same scrum are not decided by the rung they stand on.

     ⚠ NO FLYING TEST, also to match damageMod's high-ground block, which reads
     raw rungs for both parties. Flight is modelled as its own thing in
     index.html (the `highGround` passive pays an airborne attacker +20% damage
     against a grounded target); adding a flight branch HERE would make ley.js
     and that passive two opinions about the same situation.

     ⚠ INERT BEFORE THE FIRST TICK, and that is not a bug to paper over: rungAt
     reads `tile.elevRung`, which only seed() stamps, and seed() runs from
     tick(). Until then every tile answers rung 0, the difference is 0 and this
     returns a clean zero rather than a wrong number. Same for a board with no
     elevation at all.
     ══════════════════════════════════════════════════════════════════════════ */
  function accuracyMod(state, attacker, defender) {
    // Same early-out shape as damageMod, on purpose: a caller that is safe
    // against one is safe against the other.
    var out = { pts: 0, rungs: 0 };
    if (!LEY.ENABLED) return out;
    if (!state || !attacker || !attacker.pos) return out;
    if (!defender || !defender.pos) return out;
    if (dist(attacker.pos, defender.pos) <= 1) return out;

    var dr = rungAt(state, attacker.pos.x, attacker.pos.y)
           - rungAt(state, defender.pos.x, defender.pos.y);
    if (!dr) return out;

    // ELEV_MAX_RUNGS is SHARED with the damage rule rather than given its own
    // constant: a 4-rung cliff is not four times a 1-rung step, and if the two
    // rules ever capped at different heights a player would see damage stop
    // scaling at a different cliff than accuracy does.
    out.rungs = Math.max(-LEY.ELEV_MAX_RUNGS, Math.min(LEY.ELEV_MAX_RUNGS, dr));
    // ⚠ NOT ROUNDED. Math.round(2.5) is 3 but Math.round(-2.5) is -2, so
    // rounding here would break the symmetry §5.3 requires the moment INTENSITY
    // is anything but 1. The roll compares against a float perfectly well.
    out.pts = clampAcc(out.rungs * LEY.ELEV_ACC_PER_RUNG);
    return out;
  }
  ...
  global.MythicLey = {
    ...
    SURF_SEED: SURF_SEED,
    // 🔴 SURF_SEED ALONE NO LONGER ANSWERS "what does this surface seed?" —
    // the elemental plates are resolved by identity and are not in the table.
    // Anything auditing seeding (a smoke test, a debug overlay, a future
    // forecast readout) must call this, or it will report 19 surfaces as inert
    // that are not, which is how the original bug stayed invisible for so long.
    surfSeedFor: surfSeedFor,
    wire: wire,
    seed: seed,
    tick: tick,
    at: leyAt,
