...
/* ── ROCK SPRITES ─────────────────────────────────────────────────────────
   A boulder's stone body is the SAME picture every frame the light rig and
   the camera hold still: paintStone is a pure function of (pts, r, tone,
   seed) — fixed per item — and of a.sx / a.sy (pixels per world unit at that
   depth) and FRAME's light rig (direction + four graded tones), which change
   only when the camera or the sun moves. Yet a parked board repainted every
   boulder from scratch each frame: ~30 canvas ops per stone (two gradients,
   a clip, up to eight strokes), ~1,100-1,600 ops a frame across the field —
   round 5 measured that pass at roughly a third of a vsync of GPU raster
   (`abl-norock`). So each rock's body is now baked ONCE into its own small
   canvas (drawn by the same paintStone, into a context that carries the
   stage's DPR transform, so it is pixel-identical at 1:1) and re-used with a
   single drawImage per frame. The cache KEY is the quantised rig: a.sx / a.sy
   to a quarter pixel per unit, the light direction to 0.02, each tone to 6
   levels per channel — steps far under what the eye can pick out on a
   30-pixel stone, so a static frame never re-bakes and a light lerp re-bakes
   a handful of times over its whole run instead of every frame. A per-frame
   BUDGET (ROCKSPR.budget, reset in items()) bounds the re-bake burst when the
   key changes for every rock at once: the rest blit their previous sprite for
   a frame or two (a stale sprite is at most one quantum of light behind).
   The contact shadow and the foot occlusion stay live — they are two cheap
   radial fills and they carry the shadow throw, which is not in the key.
   Any throw, or no 2D context, latches ROCKSPR.failed and the live painter
   below runs exactly as it always has; window.__bbRockSprite({ off: true })
   is the one-flag counterfactual. */
const ROCKSPR = { off: false, failed: false, budget: 12, perFrame: 12, bakes: 0, blits: 0, live: 0, stale: 0, dpr: 1, pad: 4 };
window.__bbRockSprite = (opt) => {
  if (opt && 'off' in opt) ROCKSPR.off = !!opt.off;
  return { off: ROCKSPR.off, failed: ROCKSPR.failed, bakes: ROCKSPR.bakes, blits: ROCKSPR.blits, live: ROCKSPR.live, stale: ROCKSPR.stale, dpr: ROCKSPR.dpr, perFrame: ROCKSPR.perFrame };
};
function qStep(v, step){ return Math.round(v / step) * step; }
/* '#rrggbb' → the same hex with every channel rounded to a multiple of 6 */
function qHex(h){
  if (typeof h !== 'string' || h.length !== 7 || h[0] !== '#') return String(h);
  let out = '#';
  for (let i = 1; i < 7; i += 2){
    const v = Math.min(255, Math.round(parseInt(h.slice(i, i + 2), 16) / 6) * 6);
...
function rockKey(a){
  return qStep(a.sx, 0.25) + '|' + qStep(a.sy, 0.25) + '|' + ROCKSPR.dpr + '|' +
         qStep(FRAME.lx, 0.02) + '|' + qStep(FRAME.ly, 0.02) + '|' +
         qHex(FRAME.lit) + '|' + qHex(FRAME.mid) + '|' + qHex(FRAME.shd) + '|' + qHex(FRAME.rim);
}
/* local-unit bounds of every silhouette vertex the body paints */
function rockBounds(B){
  let x0 = Infinity, x1 = -Infinity, y0 = Infinity, y1 = -Infinity;
  const take = (pts) => { for (let i = 0; i < pts.length; i++){ const p = pts[i]; if (p.x < x0) x0 = p.x; if (p.x > x1) x1 = p.x; if (p.y < y0) y0 = p.y; if (p.y > y1) y1 = p.y; } };
  if (B.parts) for (let i = 0; i < B.parts.length; i++) take(B.parts[i].pts); else take(B.pts);
  if (!isFinite(x0) || !isFinite(y0)) return null;
  return { x0, x1, y0, y1 };
}
function paintRockBody(api, a, it){
  const B = it.body;
  if (B.parts){
...
function bakeRock(api, it, a, key){
  const bd = it._rb || (it._rb = rockBounds(it.body)); if (!bd) return null;
  const sx = qStep(a.sx, 0.25), sy = qStep(a.sy, 0.25), dpr = ROCKSPR.dpr, pad = ROCKSPR.pad;
  const w = Math.ceil((bd.x1 - bd.x0) * sx + pad * 2), h = Math.ceil((bd.y1 - bd.y0) * sy + pad * 2);
  const cw = Math.ceil(w * dpr), ch = Math.ceil(h * dpr);
  if (!(cw > 0 && ch > 0) || cw > 4096 || ch > 4096) return null;
  let cv = it._rs && it._rs.cv;
  ...
  /* the anchor inside the sprite: X = ox + pt.x·sx, Y = oy − pt.y·sy */
  const ox = pad - bd.x0 * sx, oy = pad + bd.y1 * sy;
  paintRockBody(Object.assign({}, api, { ctx: g }), { x: ox, y: oy, sx, sy }, it);
  ROCKSPR.bakes++;
  return (it._rs = { cv, key, w, h, ox, oy });
}
function drawRock(api, it){
  ...
  if (!ROCKSPR.off && !ROCKSPR.failed){
    const key = rockKey(a);
    let spr = it._rs;
    if (!spr || spr.key !== key){
      if (ROCKSPR.budget > 0){
        ROCKSPR.budget--;
        try { spr = bakeRock(api, it, a, key) || spr; } catch (e){ ROCKSPR.failed = true; spr = null; }
      } else if (spr) ROCKSPR.stale++;
    }
    if (spr){
      const d = ROCKSPR.dpr;
      api.ctx.drawImage(spr.cv, Math.round((a.x - spr.ox) * d) / d, Math.round((a.y - spr.oy) * d) / d, spr.w, spr.h);
      ROCKSPR.blits++;
      return;
    }
  }
...
const dressing = {
  ...
  items(api){
    ...
    /* the rock sprites: a fresh re-bake budget every frame, and the stage's
       DPR read off the frame context's transform (resize() does
       setTransform(DPR,…)) so a sprite is baked at device resolution */
    ROCKSPR.budget = ROCKSPR.perFrame;
    try {
      const t = api.ctx && api.ctx.getTransform ? api.ctx.getTransform() : null;
      const d = t && isFinite(t.a) && t.a >= 0.5 && t.a <= 4 ? t.a : 1;
      ROCKSPR.dpr = d;
    } catch (e){ ROCKSPR.dpr = 1; }
