...
function jitQuad(api, x, z, lift, amp) {
  ...
  /* ⚠ FALL BACK TO THE UNJITTERED CELL, NEVER TO NOTHING. If a jittered lattice
     point lands behind the camera at a shallow row, dropping the tile silently
     makes a LEGAL TILE UNPAINTABLE — the reachable region would have a hole in
     it exactly where the player is looking.

     🔷 HEX RE-DERIVATION. This used to fall back to api.tilePoly() and demand
     `tp.length !== 4`, because on the square board tilePoly WAS a quad. On the
     hex board tilePoly is hexCorners() and returns SIX vertices
     (battle-board/index.html:1061), so that test was true every single time and
     the guard returned null on every tile it was asked to rescue — the precise
     failure its own comment exists to forbid. Driven and confirmed before the
     change: forcing the fallback returned NULL (.gauntlet/hexdrive/d8-jitfallback.mjs).

     Rebuilding from tilePoly is the wrong repair anyway: a 6-gon cannot be
     expanded into the 8-point form whose indices boundaryEdges() addresses
     (corners at 0/2/4/6, edge midpoints at 1/3/5/7). Re-run the SAME lattice
     with zero jitter instead. It is the identical cell — jitter is a cosmetic
     displacement of at most `amp` — it keeps the index contract exactly, and it
     is strictly more likely to project than the jittered version, which is the
     only reason we are here. If even that fails the point is genuinely off the
     projection and there is nothing to draw. */
  if (!ok) {
    const q = [];
    for (let i = 0; i < 8; i++) {
      const bx = (i === 1 || i === 5) ? 0 : (i === 2 || i === 3 || i === 4) ? 0.5 : -0.5;
      const bz = (i === 3 || i === 7) ? 0 : (i === 4 || i === 5 || i === 6) ? 0.5 : -0.5;
      const w = api.gw(x + bx, z + bz, e);
      const p = api.project({ x: w.x, y: w.y, z: w.z });
      if (!p) { _polys.set(key, null); return null; }
      q.push(p);
    }
    _polys.set(key, q);
    return q;
...
/* The OUTER boundary of a merged region, as polylines in tile-edge units.
   ...
   outer boundary". Each entry is the three points of one segment (start, mid,
   end) so the outline inherits the same lattice jitter the mask does and cannot
   drift off it.

   🔷 HEX RE-DERIVATION — this used to make FOUR adjacency tests, one per side
   of the cell: (x,z−1) (x+1,z) (x,z+1) (x−1,z). All four are genuine hex
   neighbours on both row parities, so nothing it drew was in the wrong place —
   but a hex tile has SIX edge-neighbours, and the two that change with row
   parity were never tested at all, so their sides could never be traced.

   WHY the four-test version looks complete and is not. jitQuad does not build a
   hexagon: it builds the unit square (x±0.5, z±0.5) in TILE space and pushes it
   through gw(), which applies the odd-row +½ stagger. That is a BRICK lattice —
   bricks of hexW × hexV, offset half a brick on odd rows — and a brick has four
   sides but SIX edge-neighbours, because its N and S sides are each shared by
   TWO bricks, half each. Testing the N side against one neighbour therefore
   answers for half of it and guesses the other half.

   Measured on the shipped code, five real getValidMoves regions on the 14×12
   board (.gauntlet/hexdrive/d7-loop.mjs):

       region            exposed hex sides   traced   loose ends
       spd2, 19 tiles            30            20         16
       spd3, 37 tiles            42            28         24
       spd4, 61 tiles            54            36         32
       spd5 + walls, 47          90            60         64
       spd6, 120 tiles           76            50         44

   "Loose ends" is odd-degree vertices in the emitted segment set — a closed
   outline has none. So the mark the player saw was not one continuous contour
   around the reachable region; it was a broken outline missing a third of its
   sides, and it broke worse the more the region was shaped by terrain. That is
   the one thing the reachable highlight exists to say.

   The mapping below was DERIVED from the board's own gw(), not assumed: probing
   just outside each brick side at its quarter points and asking which brick owns
   that world point returns, for even rows, N-left→(x−1,z−1) N-right→(x,z−1)
   S-right→(x,z+1) S-left→(x−1,z+1); and for odd rows N-left→(x,z−1)
   N-right→(x+1,z−1) S-right→(x+1,z+1) S-left→(x,z+1). Every one of those is a
   true hex neighbour under the game's own hexNeighbors(). E and W are whole
   sides and unchanged. (.gauntlet/hexdrive/d6-brickadj.mjs)

   ⚠ The N and S entries are HALF sides, so they carry a synthesised midpoint to
   keep the three-point shape the rim/glint loop indexes as p[0..2]. `k` now runs
   0..5 rather than 0..3; it is only a hash salt, but it must stay distinct per
   segment or the two halves of one side would break identically. */
function boundaryEdges(g) {
  const out = [];
  const mid = (a, b) => ({ x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 });
  for (const it of g.list) {
    const q = it.q, x = it.x, z = it.z, odd = z & 1;   /* & never %, HEXSPEC §3 */
    const nL = odd ? [x,     z - 1] : [x - 1, z - 1];
    const nR = odd ? [x + 1, z - 1] : [x,     z - 1];
    const sR = odd ? [x + 1, z + 1] : [x,     z + 1];
    const sL = odd ? [x,     z + 1] : [x - 1, z + 1];
    if (!g.set.has(nL[0] + ',' + nL[1])) out.push({ p: [q[0], mid(q[0], q[1]), q[1]], k: 0, it });
    if (!g.set.has(nR[0] + ',' + nR[1])) out.push({ p: [q[1], mid(q[1], q[2]), q[2]], k: 1, it });
    if (!g.set.has((x + 1) + ',' + z))   out.push({ p: [q[2], q[3], q[4]],            k: 2, it });
    if (!g.set.has(sR[0] + ',' + sR[1])) out.push({ p: [q[4], mid(q[4], q[5]), q[5]], k: 3, it });
    if (!g.set.has(sL[0] + ',' + sL[1])) out.push({ p: [q[5], mid(q[5], q[6]), q[6]], k: 4, it });
    if (!g.set.has((x - 1) + ',' + z))   out.push({ p: [q[6], q[7], q[0]],            k: 5, it });
  }
  return out;
}
...
function scheduleGround(api, litSet) {
  ...
  _bgLit = sig;
  if (_bgPend) return;
  /* 🌗 NOT WHILE THE LIGHT LERPS OR THE BOARD IS STAGING A PRESS. The capture
     is a readback of the main canvas — a pipeline sync that waits for every
     frame and every staged bake still on the GPU (measured: a 52–63 ms task
     ~0.8 s into a day→night lerp on an RTX 5060 Ti, the one long task left in
     the transition). The bare-ground chroma it estimates moves with the light,
     but a 2.5 s-old estimate is a smaller error than a dropped frame in the
     one moment the player is watching the sky; the first frame after the lerp
     and its drain retakes it (the 2 s clock has long expired by then). Same
     rule as the vista's postMap. */
  if (api.lightLerp || api.staging) return;
...
function captureGround(api, src, lit, T) {
  ...
  const sig = _bgLit;
  /* 🧵 OFF THE MAIN THREAD when the vista's readback worker is up (see THE
     OFF-THREAD READBACK in vista.js): this drawImage + getImageData was the
     other steady-state pipeline sync on the board — 50–65 ms every 2 s while
     any tile is lit, on the thread the player's frames run on. The snapshot is
     the same graded frame (createImageBitmap of the canvas, now), the lit set
     and the clock are the ones of THIS frame (bound here, not read later),
     and _bgPend stays up until the pixels are back so nothing re-asks in the
     meantime. Without the worker the read below runs exactly as before. */
  const rb = window.BBX && window.BBX.readback;
  if (rb && typeof rb.ok === 'function' && rb.ok()) {
    const p = rb.read(src, [{ w: dw, h: dh }], 'ground');
    if (p) {
      _bgPend = 1;
      p.then((r) => { _bgPend = 0; try { applyGround(api, r.planes[0], dw, dh, src.width, lit, sig, T); } catch (e) {} },
             () => { _bgPend = 0; });
      return;
    }
  }
  if (!_bg || _bg.w !== dw || _bg.h !== dh || !_bg.ctx) {
    const cv = mkCv(dw, dh);
  ...
  applyGround(api, img.data, dw, dh, src.width, lit, sig, T);
}

/* the estimator itself, on pixels `d` of the frame at dw×dh (a dw/srcW scale
   of DEVICE px) — the second half of what captureGround used to be, so the
   worker path and the synchronous one feed the same loop. */
function applyGround(api, d, dw, dh, srcW, lit, sig, T) {
  if (!_bg || _bg.w !== dw || _bg.h !== dh) _bg = { cv: null, ctx: null, w: dw, h: dh, s: dw / srcW, t: -1e9 };
  _bg.t = T; _bg.sig = sig;
  const MAP = api.MAP || {};
