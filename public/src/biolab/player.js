...
/* 🔴 A KEY IS ONLY DOWN WHILE THE KEYBOARD KEEPS SAYING SO (v121v54).
   Reported: "the character gets stuck moving in one direction; they move
   normally, then when you interact with an object the controller breaks."
   A keyup can be lost for good reasons — a native confirm() (the hospital's
   fallback), a focused number field that stops propagation, a panel that
   opens on the key's own keydown — and a lost keyup used to mean walking
   into a wall until the next press of that key. attachInput now stamps each
   key with the time of its last keydown; the keyboard auto-repeats while a key
   is held, so once repeat has been seen in this session a stamp older than
   two seconds is a key that was released while nobody was listening. Tests
   and the stick may still write `true`, which never expires. */
const STALE_KEY_MS = 2000;
function keyLive(input, n) {
  const t = input.keys[n];
  if (!t) return false;
  if (t === true) return true;
  if (input._repeatSeen && (Date.now() - t) > STALE_KEY_MS) { delete input.keys[n]; return false; }
  return true;
}
export function axisOf(input) {
  let ax = 0, az = 0;
  const k = input.keys;
  const L = (n) => keyLive(input, n);
  if (L('w') || L('ArrowUp')) az += 1;      // up the screen
  if (L('s') || L('ArrowDown')) az -= 1;    // down the screen
  if (L('a') || L('ArrowLeft')) ax -= 1;    // left on the screen
  if (L('d') || L('ArrowRight')) ax += 1;   // right on the screen
  void k;
  ax += input.stickX;
  az += input.stickY;
...
/* ── collision ─────────────────────────────────────────────────────────────
   ...
   🔴 …AND "SHORTEST AXIS" IS ENFORCED, NOT ASSUMED. This used to push along
   whichever axis it was CALLED for, as long as the box overlapped on both.
   Walk straight into a bench's front face at its exact centre line (x = 0
   for the hospital's desk — where the doors put you): the z pass stops you
   correctly, and then one floating-point rounding later the x pass sees an
   overlap of 10⁻¹⁵ m and pushes you out along x — by the FULL x penetration,
   1.49 m — clean round the side of the desk, and you walk on. Measured: a
   1.64 × 0.79 m desk shoves the player at frame 20; 1.64 × 0.80 and 0.78 do
   not. The 5.2 m mock was passing the same test by the same luck.
   So each pass now pushes only when ITS axis is the shallower one. Pressed
   into a face you stop; pressed diagonally you slide along it. */
function resolveAxis(px, pz, boxes, axis) {
  for (const b of boxes) {
    ...
    if (axis === 'x') { if (ox <= oz) px += (dx >= 0 ? ox : -ox); }
    else { if (oz <= ox) pz += (dz >= 0 ? oz : -oz); }
  }
  return axis === 'x' ? px : pz;
}
...
/* ⚠ `plan` is optional and defaults to the LAB's room and station colliders.
   /src/hospital walks the same code over its own floor — one movement rule for
   both buildings, rather than a second walker that drifts out of step with
   this one. Passing neither field is exactly the old behaviour. */
...
export function attachInput(root, input, opts) {
  ...
  /* The physical key first (e.code: KeyW / ArrowUp), the printed key as the
     fallback — Shift, Caps Lock and non-Latin layouts change e.key mid-hold and
     a keyup that spells the key differently from its keydown never matched. */
  const CODE = { KeyW: 'w', KeyA: 'a', KeyS: 's', KeyD: 'd', ArrowUp: 'ArrowUp', ArrowDown: 'ArrowDown', ArrowLeft: 'ArrowLeft', ArrowRight: 'ArrowRight' };
  const nameOf = (e) => {
    if (e.code && CODE[e.code]) return CODE[e.code];
    const k = e.key || '';
    const n = k.length === 1 ? k.toLowerCase() : k;
    return (n === 'w' || n === 'a' || n === 's' || n === 'd' || (typeof k === 'string' && k.startsWith('Arrow'))) ? n : null;
  };
  const isInteract = (e) => { const k = e.key; return k === ' ' || k === 'Enter' || k === 'e' || k === 'E' || e.code === 'Space' || e.code === 'KeyE'; };
  /* Typing in a field is not walking. A focused number field (titration,
     quantity) or a text box owns every key it sees. */
  const typing = () => {
    try {
  ...
  const clearKeys = () => { input.keys = {}; input._interactEdge = false; };

  const kd = (e) => {
    const k = e.key;
    if (k === 'Escape') { onExit(); return; }
    if (typing()) { clearKeys(); return; }
    if (isInteract(e)) {
      if (!input._interactEdge) {
        input._interactEdge = true;
        /* An interaction opens a panel, a dialog or a modal — whatever it is,
           it may swallow the next keyup. Let go of the movement keys now. */
        input.keys = {};
        onInteract();
      }
      e.preventDefault();
    ...
    const n = nameOf(e);
    if (n) {
      if (e.repeat) input._repeatSeen = true;
      input.keys[n] = Date.now();
      e.preventDefault();
    }
  };
  const ku = (e) => {
    if (isInteract(e)) { input._interactEdge = false; return; }
    const n = nameOf(e);
    if (n) delete input.keys[n];
    // …and whatever the printed key spelled, in case it differs from the code.
    const k = e.key || '';
    const alt = k.length === 1 ? k.toLowerCase() : k;
    if (alt && alt !== n) delete input.keys[alt];
  };
  ...
  on(window, 'blur', blur);
  /* A hidden tab, a field taking focus, or a native dialog closing (which
     fires focus without a keyup) all mean the same thing: nothing is held. */
  on(document, 'visibilitychange', () => { if (document.hidden) blur(); });
  on(document, 'focusin', (e) => { try { const t = e.target; const tg = (t && t.tagName || '').toLowerCase(); if (tg === 'input' || tg === 'textarea' || tg === 'select' || (t && t.isContentEditable)) clearKeys(); } catch (err) {} });
  on(window, 'focus', clearKeys);
  ...
  const te = (e) => {
    ...
    /* A cancel that names a different finger (a system gesture took the
       touch) still means our finger is gone — the stick must not stay pushed. */
    if (e.type === 'touchcancel' && touchId !== null) end();
  };
