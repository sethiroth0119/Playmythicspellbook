...
/* ⚠ `hotZ` is optional and defaults to the LAB's line. /src/hospital has
   its own floor plan — its sterile line sits elsewhere — so it passes its own,
   and one rule serves both rooms instead of each keeping a copy that can
   drift. Same reasoning for the optional `list` on the two below. */
