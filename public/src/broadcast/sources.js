...
/* ══════════════════════════════════════════════════════════════════════════
   💼 A BUSINESS WITH SEATS TO FILL SAYS SO ITSELF.
   ══════════════════════════════════════════════════════════════════════════
   Asked for as: "when a business is hiring add that business on the phone
   posting a status that they are hiring".

   ⭐ THE SUBJECT AND THE WORDS ALREADY EXISTED. `hiring` is registered in
      subjects.js with a business voice and two phrasings — 'we are {tag} at
      {p}', '{p} has {n} seats to fill' — and NOTHING HAS EVER EMITTED ONE.
      It was written and then never wired, so the feed has been silent about
      the single thing a city with unemployed residents most needs to say.
      This is the missing half, not a new feature.

   🔴 THE SEATS ARE THE ECONOMY'S OWN COUNT. headcountFor() is what the
      production path uses to decide staffing, and jobs() reports the same
      figure to the Job Fair — so the phone, the paper and the simulation
      cannot disagree about how many people a shop is short. A second count
      here would drift the moment a firm levelled up.
   ⚠ HOUSING IS ALREADY EXCLUDED, upstream: jobs() drops `landlord` firms
     because a house is not a job. Reading that seam rather than the firm list
     is what keeps this post from advertising somebody's home as a vacancy.
   ⚠ ONE BUSINESS PER PASS. A city that opened six shops at once would
     otherwise bury every other kind of news under six identical adverts —
     the price story below takes the same `break` for the same reason.
   ⚠ AND IT IS LOCAL NEWS. `affected` is the seats it can fill, never the
     city population: a corner shop needing one waiter must not outrank a
     water shortage.
   ══════════════════════════════════════════════════════════════════════════ */
function fromHiring(ctx, pop) {
  const E = EC(); if (!E || typeof E.ready !== 'function' || !E.ready()) return [];
  let d = null;
  try { d = (typeof E.jobs === 'function') ? E.jobs() : null; } catch (e) { return []; }
  if (!d || !Array.isArray(d.firms) || !d.firms.length) return [];
  const out = [];
  /* Biggest shortfall first — the business that most needs somebody is the one
     worth a post, and it keeps the choice stable between passes. */
  const rows = d.firms.slice().sort((a, b) => (b.openings | 0) - (a.openings | 0));
  for (const f of rows) {
    const seats = f.openings | 0;
    if (seats <= 0) continue;
    const key = f.tileKey ? String(f.tileKey) : null;
    /* The name on the sign, when a tenant took the lot — the same name the
       opening post and the dossier use. Falls back to the firm's own. */
    const nm = (key && bizNameAt(key, f.name)) || f.name;
    if (!nm) continue;
    const where = (key && addrOf(key)) ? nm + ', ' + addrOf(key) : nm;
    out.push({
      src: 'hiring',
      /* Keyed on the SEAT COUNT, so a shop that goes 1 → 3 short says so again
         and one that stays 1 short does not repeat itself every pass. */
      key: 'hire|' + (key || nm) + '|' + seats,
      subject: 'hiring', pole: 'good',
      severity: Math.max(0.3, Math.min(1, seats / 6)),
      affected: Math.max(1, seats),
      facts: { p: where, n: String(seats) },
      posterKind: 'company',
      poster: { name: nm, sub: f.industry || null, ico: f.ico || '🏢' },
      why: seats + ' unfilled ' + (f.band || '') + ' seat' + (seats === 1 ? '' : 's') + ' at ' + nm,
    });
    break;
  }
  return out;
...
/* 🦠 WHO IS GETTING SICK. `ctx.plague()` is /src/plague's cityReport() for
   this city, handed over by node-city (the module reads the host's roster and
   coverage, so the feed cannot ask it directly). Two readings:
     · active cases — a complaint whose severity is the share of the roster
       that is sick (one in three is as bad as it gets), so the first few
       cases read as residents noticing and a real outbreak reads as the
       Health Department announcing it;
     · a cure that cleared a strain — the module logs `cured`, and that is the
       contented post. Keyed on the strain so it is said once.
   Absent module, absent reader, or a throw ⇒ nothing, which is the feed's
   contract for every subject. */
function fromVirus(ctx, pop) {
  if (typeof ctx.plague !== 'function') return [];
  let r = null; try { r = ctx.plague(); } catch (e) { return []; }
  if (!r || typeof r !== 'object') return [];
  const out = [], at = Date.now();
  const cases = r.cases | 0;
  if (cases > 0) {
    const w = r.worst, st = w && w.strain ? w.strain : null;
    const share = Math.max(0, Math.min(1, +r.share || 0));
    const sev = Math.max(0.12, Math.min(1, share * 3));
    out.push({ src: 'virus', at, key: 'virus|' + (st ? st.id : 'x') + '|' + Math.round(share * 20),
               subject: 'virus', pole: 'bad', severity: sev, affected: cases,
               facts: { n: num(cases), v: st && st.name ? String(st.name) : null },
               why: cases + ' active case' + (cases === 1 ? '' : 's') + (st && st.name ? ' of ' + st.name : '') +
                    ' — ' + Math.round(share * 100) + '% of the roster (plague cityReport)' });
  }
  for (const e of (Array.isArray(r.log) ? r.log : []).slice(0, 6)) {
    if (!e || e.kind !== 'cured') continue;
    const name = String(e.text || '').replace(/^\S+\s*/, '').replace(/\s*\(.*$/, '').replace(/\s*cleared\.?$/, '').trim();
    out.push({ src: 'virus', at, key: 'cured|' + (e.strainId || e.at),
               subject: 'virus', pole: 'good', severity: 0.8, affected: Math.round(pop),
               facts: { v: name || null },
               why: (e.text || 'a strain was cleared') + ' (plague log)' });
  }
  return out;
}
...
export function observe(ctx) {
  ...
  for (const fn of [fromCoverage, fromPower, fromWater, fromPollution,
                    ...
                    /* 💼 fromHiring joins the list the comment above is about: the
                       `hiring` subject and its business phrasings have existed
                       since subjects.js was written and nothing ever emitted one,
                       which is exactly the failure described — it parsed, it
                       exported, and the feed was simply silent about it. */
                    fromStreets, fromRoster, fromSchools, fromHiring,
                    /* 🦠 fromVirus: the illness report, see its header. */
                    fromVirus]) {
