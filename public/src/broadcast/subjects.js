...
export const SUBJECTS = {
  ...
  /* 🦠 ILLNESS — /src/plague's case count for this city (sources.js
     fromVirus). Kept separate from `health` for the same reason `water` and
     `water_draw` are: that is "the clinics cannot see everyone", which a
     building fixes; this is "people are getting sick with something", which a
     cure fixes. The Health Department speaks for the city; residents notice
     the coughing on their own street first. {v} is the strain's name. */
  virus:    { id: 'virus',    label: 'Illness',     tag: 'outbreak',    tagsBad: ['sick', 'virus'], tagsGood: ['recovered'], dept: 'health', scope: 'city', citizen: true, poles: ['bad', 'good'] },
