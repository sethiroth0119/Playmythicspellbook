...
export function activityOf(C, id) {
  ...
  if (a.state === 'inside') {
    label = 'Indoors';
    note = 'they are inside ' + placeName(C, a.inside || a.at || '');
  } else if (a.state === 'enter') {
    label = 'Going in';
    note = 'stepping through the door of ' + placeName(C, a.inside || '');
  } else if (a.state === 'exit') {
    label = 'Coming out';
    note = 'leaving ' + placeName(C, a.inside || '');
  } else if (a.dwell) {
    label = a.phase === 'home' ? 'Stopped, then home' : 'Stopped, then to work';
    note = 'stopped on the road for a moment, before picking the next street';
  } else {
    label = a.phase === 'home' ? 'Going home' : 'Going to work';
    note = 'out on the street, heading ' + (a.phase === 'home' ? 'home' : 'to work');
  }
...
export function factsOf(C, id) {
  ...
  out.mood = { v: mood, label: bandWord,
    src: mood == null
      ...
      : 'how content they are right now, sorted into the same five bands the citizen dialogue uses' };
  ...
  if (la && la.ok && clk && clk.ok) {
    ...
    citizenRows.push(row('Age', '≈ ' + la.whole + ' years · ' + la.bandIco + ' ' + la.bandLabel,
      'ESTIMATED, not recorded — nobody in this city has a birthday on file. Every named ' +
      'citizen is given an age from the city’s own age profile (children left out of it, ' +
      'because the roster is sized by the jobs there are to fill), taking whichever age band the ' +
      'city is shortest of, and the year inside that band is fixed by their name — so it is the ' +
      'same age in every session and on every machine. It advances with the city clock at ' +
      clk.daysPerYear.toFixed(1) + ' economic days to the year, which is the game’s own ageing ' +
      'rate read backwards (' + clk.src.agePerDay + ' of a year per day, across a ' +
      clk.workingLifeYears + '-year working life) — about ' + clk.hoursPerYear.toFixed(1) +
      ' hours of play per year of their life' +
      (la.born < 0 ? '. By that reckoning they were born before this city was founded, which is ' +
        'what you would expect of somebody who arrived already grown' : '') +
      ...
      '. ⚠ THIS MATCHED THE CITY’S REAL AGE PROFILE TO WITHIN ONE PERSON AT THE MOMENT IT WAS ' +
      'WORKED OUT, AND DRIFTS AFTERWARDS: ' +
      (mort && mort.ok
        ? mort.note + (mort.live
            ? '. The ages are only shared out again when somebody new arrives, so that match is ' +
              'a claim about the moment it was made and not about the hours between'
            ...
            : '. About three real hours of play is enough to take the largest age band further ' +
              'out than one person’s worth')
        : 'no named citizen dies, retires off the roster or leaves, so they all age together on ' +
          'one clock while the city’s own age profile does not follow them. About three real ' +
          'hours of play is enough to take the largest age band further out than one ' +
          'person’s worth') +
      (dist && dist.ok && dist.drift ? ', and it is outside it now — ' + dist.drift : '')));
  } else {
    citizenRows.push(unav('Age',
      'the city records a name, a job, a mood and an employer for each person — but no age. It ' +
      'does model an age split for each KIND of household, city-wide; hanging one of those on a ' +
      'named person would be a second, different claim about them' +
      (la && la.why ? '. The ageing model is loaded but cannot answer: ' + la.why : '') +
      (LP && clk && !clk.ok ? '. The ageing model cannot work out how long a year is: ' + clk.why : '')));
  }
  if (bi && bi.rung) {
    citizenRows.push(row('Education', bi.rung.ico + ' ' + bi.rung.label + ' or better',
      'A MINIMUM, not a record: nothing says what this person studied. They hold ' +
      bi.label.toLowerCase() + '-band work, and that band of work asks for ' + bi.requires +
      ' at least — so that is the floor the job demands, not necessarily what they have'));
  } else {
  ...
  let hhNote = null;
  if (!HH) {
    hhRows.push(unav('Household', 'the building records are not loaded, and they own the housing ' +
      'assignment this panel reads. Nothing here works it out a second time'));
    hhRows.push(unav('Residence', 'the same: where somebody lives comes from the building records, and those did not load'));
  } else if (!res.ok) {
    const why = res.why === 'homeless'
      ? 'the city’s people are sorted into the ' + (res.homes || 0) + ' homes it has, and it ran out ' +
        'of beds before it reached this person'
      : res.why === 'roster' ? 'the list of named people could not be read, so nobody could be housed'
    ...
    hhNote = 'The people sharing this address — ' + members.length +
    ...
    if (w && w.label) {
      hhRows.push(row('Household wealth', w.label,
        'the same wealth reading the building panel prints for this address. ' +
        (w.note || '')));
    } else {
    ...
    hhRows.push(row('Residence', placeName(C, res.key),
      'WORKED OUT, not stored: the city sorts its people into the homes it has by surname and ' +
      'keeps no address on file, so building or clearing housing re-sorts them. It is the same ' +
      'sorting the building panel prints',
  ...
  if (jobTile) {
    occRows.push(row('Occupation', titleOf(C, jobTile),
      'they hold one of the crew seats at this building',
      { kind: 'tile', key: jobTile, label: titleOf(C, jobTile) }));
  } else if (emp) {
    occRows.push(row('Occupation', (emp.indIco || '🏭') + ' ' + emp.indName,
      'they crew no building in the city, so this is the TRADE of the business the economy has ' +
      'them working for'));
  } else {
    occRows.push(row('Occupation', 'Not working',
      'they crew no building, and the economy has no seat for them either. This is a real state, not a gap: ' +
  ...
  if (lc && lc.ok && clk && clk.ok) {
    ...
    const siteWords = lc.siteFrom === 'tile'
      ? 'the building it occupies has stood ' + lc.siteYears.toFixed(1) + ' years'
      : lc.siteFrom === 'gone'
        ? 'the business names a site, but nothing standing there records when it went up — the ' +
          'building was cleared, or this city kept no clock on it — so the ceiling falls back ' +
    ...
    const firmWords = lc.firmYears != null
      ? 'the business itself has traded ' + lc.firmYears.toFixed(1) + ' years (founded on economic ' +
        'day ' + lc.foundedDay + ', and the city is now on day ' + lc.econDay + ')'
      : null;
    const ceilWords = lc.tenureFrom === 'worklife'
      ? 'and it is THEIR OWN AGE that caps it: the ' + lc.workedYears.toFixed(1) + ' years since ' +
        ...
        '. 🔴 So this row is a restatement of the ESTIMATED age above it — that is why it ' +
        'carries a "≈" and why this line says ESTIMATED and not WORKED OUT'
      : lc.tenureFrom === 'firm'
        ? 'and it is THE BUSINESS that caps it: ' + firmWords + ', shorter than the ' +
          ...
          (lc.siteFrom === 'tile' && lc.siteYears < lc.firmYears - 0.05
            ? '. ⚠ Note that ' + siteWords + ' — LESS than the business has traded. That is a ' +
              'rebuild: a building is dated from the moment it is placed, so re-raising a ' +
              'workplace resets the walls and not the business inside them. This ceiling ' +
              'deliberately does NOT read the building, because demolishing and re-raising a ' +
              'shop does not end anybody’s employment — and the economy agrees, keeping the ' +
              'same business when the rebuilt premises are the same business and founding a new ' +
        ...
        : 'and it is the SITE that caps it: ' + siteWords + ', shorter than the ' +
          ...
          (lc.siteFrom === 'tile'
            ? '. A building is dated from the moment it is placed, so while this fallback is in ' +
              'force, demolishing and re-raising the same workplace still takes this ceiling — ' +
              'and every grade under it — back to zero'
            : '');
    occRows.push(row('Job level',
      ...
      (lc.sampled
        ? 'ESTIMATED, because the number that caps this is the estimated age above. '
        : 'WORKED OUT from live readings and one limit. ') +
      'Their employer stands at level ' + lc.cap + ' of ' + lc.ladder + ' on the ladder this game ' +
      'uses for the SIZE OF A BUSINESS — ' + lc.firm.levelIco + ' ' +
      ...
      'off the game is the direction: a bigger business ' +
      'has more rungs than a smaller one. They can have worked there at most ' +
      lc.tenureYears.toFixed(1) + ' years, ' + ceilWords +
      '. One grade per ' + clk.gradeYears.toFixed(1) + ' years, taken from the rate at which this ' +
      'game moves a studying household up one education step — ⚠ also an ANALOGY: that rate moves an ' +
      ...
      'pair on the citizen roster, which nothing in this game writes down — and ' +
    ...
    occRows.push(unav('Job level',
      'this city keeps no wage, no skill and no rank for a PERSON. What it does model is the ' +
      'band of the WORK, shown below' +
      (lc && lc.why === 'nofirm'
        ? '. The one ladder the game does have measures the SIZE OF A BUSINESS — this person ' +
          'stands in none, so there is no ladder to be part-way up'
        : lc && lc.why ? '. The career model is loaded but cannot answer: ' + lc.why : '')));
  }
  ...
  if (bi) {
    ...
    occRows.push(row('Work band', bi.ico + ' ' + bi.label,
      'WORKED OUT: this employer’s trade draws its workforce from the ' + bi.band + ' band' +
      (bi.wage == null ? ''
        : paid != null
          ? ', paid ' + (Math.round(paid * 100) / 100) + ' 🔥 an economic day: the band’s base ' +
            'wage of ' + bi.wage + ', times the ×' + lvlDef.wageMul + ' a level-' +
            lc.firm.level + ' employer pays. That is the same sum the economy actually pays out'
          : ', paid ' + bi.wage + ' 🔥 an economic day at the BASE of the wage ladder — this ' +
            'employer’s size cannot be read here, so the extra a bigger business pays ' +
            'is not applied')));
  } else if (jobTile) {
    occRows.push(unav('Work band',
      'no business stands on this tile, so this is a civic seat — a real job, but not a business ' +
      '— and the economy prices no wage band for it'));
  } else {
  ...
  if (emp) {
    occRows.push(row('Employer', emp.name,
      'the business the city has them working for, read fresh every time this panel opens so it ' +
      'cannot go stale' + (emp.tile ? '' : '. It holds no premises of its own to open'),
    ...
    occRows.push(row('Employer', '🏛 A city seat, no business',
      'no business was found on this tile, so the economy names no employer. They crew ' +
  ...
  const dRows = [];
  if (!act.ok) {
    dRows.push(unav('Destination', act.why + '. The city only moves the people it has out on the street; ' +
    ...
    dRows.push(k && C.game.tiles[k]
      ...
      : unav('Destination', 'they are at a door the city no longer has'));
  } else if (act.dest) {
    const a = addrOf(act.dest);
    dRows.push(row('Destination', a ? a.text : 'tile ' + act.dest,
      'the last stop on the route they are walking, ' + (act.steps - 1 - act.step) +
      ' tiles ahead of them. ⚠ It is a STREET, not their own address: the walk home ends on a ' +
      'road beside any housing and the walk to work on a road beside any workplace — the door ' +
      'itself is picked when they arrive',
      { kind: 'tile', key: act.dest, label: a ? a.text : act.dest }));
  } else {
    dRows.push(unav('Destination', 'they have no onward route this instant — they are stopped, and ' +
      'will pick one in a moment'));
  }
