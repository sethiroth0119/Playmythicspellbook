...
import { activeWorldEvent, worldEventCityFx } from '../plague/worldstrains.js';

/* ═══ 🌐 THE WORLD EVENT IS A SEEDING VECTOR ═══════════════════════════════
   /src/plague/worldstrains.js names five viruses that are loose across EVERY
   player's city in the same six-hour window, derived from the clock so no
   server has to agree on anything. This engine treats a running event exactly
   the way it treats bad water: as PRESSURE on one virus in the catalog. It is
   not a second engine and it does not bypass tryCatch — the grace period, the
   concurrent cap, the re-catch cooldown and the single bounded roll all still
   stand. What the event changes is that a city which did nothing wrong can
   catch its mapped virus while the window runs, and it files that outbreak
   under the event's `strainId`, so every city on the server has the same
   entry in its register.

   ⚠ THE HANDOFF RULE IS UNCHANGED: with no event running, a healthy city
     measures 0.000 pressure on all six — see pressures() and the harness.

   THE MAP. Each world virus lands as the catalog virus whose vector it most
   resembles, so the card the player reads still names a cause:
     ashlung     → ashlung      the same disease by name
     vaultrot    → violetwither anomalous, out of a Foundation vault; same 🟣
     quickblight → cinderpox    the fastest spreader in each catalog
     greyfever   → ferricrot    a relapsing fever, tier 2, lethal but survivable
     hollowtide  → hollowsleep  anomalous, "every axis at once", the one that
                                stops a city working
   Grey Fever is deliberately NOT Grey Marrow despite the name: Grey Marrow is
   the OPPORTUNIST whose `catch` requires another outbreak to already be
   running, and a world event that seeded it would be one that starts a crisis
   the entry itself says it cannot start. Anything the map does not name lands
   as WORLD_EVENT_DEFAULT — the teaching virus, because an unmapped event is a
   catalog addition nobody tuned for and must not be lethal by accident. */
export const WORLD_EVENT_MAP = {
  ashlung: 'ashlung', vaultrot: 'violetwither', quickblight: 'cinderpox',
  greyfever: 'ferricrot', hollowtide: 'hollowsleep',
};
export const WORLD_EVENT_DEFAULT = 'ashlung';
export function cityVirusForWorldEvent(defId) {
  const id = WORLD_EVENT_MAP[defId] || WORLD_EVENT_DEFAULT;
  return PLAGUE_VIRUSES[id] ? id : WORLD_EVENT_DEFAULT;
}

/* How hard the event pushes: 0.40 + 1.5 × the definition's severityBias, so
   Ashlung (0.18) presses at 0.67 and Hollowtide (0.42) at the full 1.0. Through
   CATCH_ROLL and the five-minute check that is roughly a 19%–28% chance per
   check — a city inside a window catches it within a handful of checks, not
   on the tick the window opens, so the banner lands before the outbreak does. */
const WORLD_PRESSURE_BASE = 0.40, WORLD_PRESSURE_PER_BIAS = 1.5;
function worldPressure(ev) {
  return clamp01(WORLD_PRESSURE_BASE + WORLD_PRESSURE_PER_BIAS * (+(ev && ev.def && ev.def.severityBias) || 0));
}

/* Bound on the register of world strains this city has already filed. One
   window is six hours and three in five are calm, so this is weeks of history;
   it exists so a save cannot grow one key per window forever. */
const WORLD_SEEN_MAX = 16;

/* 🏭 THE OUTPUT FLOOR — what outputMul() may never go below, whatever is loose.
   labourMul() in plague.data.js floors at 0.05 for the per-card figure, and
   three tier-3 viruses at 100% plus a quarantine multiply to 0.24 with a world
   event's own labour multiplier on top. A city at a quarter of its labour
   cannot make the reagents a cure batch costs before the next batch is needed,
   and a city that cannot research its way out is an unwinnable state reachable
   by ordinary play. 0.40 is the lowest figure at which a Research Facility's
   reagent supply still outruns one batch per outbreak on the driven worst
   city (.gauntlet/drive-city-outbreak.mjs asserts it). */
const PLAGUE_OUTPUT_FLOOR = 0.40;
...
function freshState() {
  return {
    active: {},      // virusId -> { id, inf, imm, since, deaths, quarantine, strain }
    cured: {},       // virusId -> city-minute of cure, for the re-catch cooldown
    seen: {},        // world strainId -> city-minute it was filed here (one outing per city)
...
/* ⏱ THE CITY CLOCK, IN CITY-MINUTES. `game.cityAge` counts ticked SECONDS
   ("a tab left open overnight must not age a city that ran no ticks" — see
   node-city's calendar note), and every constant in plague.data.js is in
   city-minutes, the unit economyTick runs on. The first port of this module
   compared the two directly, which made the 90-minute grace period a
   90-second one and the 240-minute re-catch cooldown four minutes. Every read
   of the clock goes through here so the ÷60 exists in exactly one place. */
function cityAgeMin() { return (+(C && C.game && C.game.cityAge) || 0) / 60; }

/* 🌐 The wall clock the world event is read against. Handed over by the ctx
   only so the harness can pin it to a calm or a live window and MEASURE the
   pressure each one produces against the real activeWorldEvent(); node-city
   leaves it unset and the event is read at the real Date.now(). */
function now() {
  try { if (C && typeof C.now === 'function') { const t = +C.now(); if (Number.isFinite(t)) return t; } } catch (e) {}
  return Date.now();
}
function worldEvent() { try { return activeWorldEvent(now()) || null; } catch (e) { return null; } }
function worldFx() { try { return worldEventCityFx(now()) || null; } catch (e) { return null; } }
...
/* ═══ 2. CATCHING ══════════════════════════════════════════════════════════
   pressures() is the whole vector model in one pure read: for every virus in
   the catalog, how hard the city is being pushed toward it RIGHT NOW, 0..1,
   with every gate applied (active, cooldown, cap, grace) so a 0 here means
   "cannot be caught on this check" and nothing else. tryCatch rolls against
   exactly this map, and the diagnostics seam exposes exactly this map, which
   is what lets the harness prove the handoff rule instead of trusting it. */
export function pressures() {
  const out = {};
  for (const id of PLAGUE_IDS) out[id] = 0;
  if (!C || !PL) return out;
  /* 🚪 A city cannot be born mid-plague, and that includes being born into a
     world event — the first ninety minutes of a save are spent laying roads
     with no clinic, and a virus the whole server is fighting is still a virus
     this city has no building to answer. See PLAGUE_GRACE_CITY_MIN. */
  const age = cityAgeMin();
  if (age < PLAGUE_GRACE_CITY_MIN) return out;

  const snap = snapshot();
  const activeCount = Object.keys(PL.active).length;
  const ev = worldEvent();
  /* 🌐 One outing per city: a strain this city has already filed (caught and
     cured inside the same window) does not come back on the next check. A
     later window is a NEW strainId by construction (worldStrainId carries the
     window), so it is a new outbreak, which is what the register should say. */
  const evId = (ev && ev.strainId && !PL.seen[ev.strainId]) ? cityVirusForWorldEvent(ev.def && ev.def.id) : null;
  ...
  for (const id of PLAGUE_IDS) {
    ...
    /* ⏳ Re-catch cooldown — a cure the player can watch fail to hold is worse
       than no cure. It holds against the world event too: the cooldown is a
       promise the panel made when it said "contained". */
    ...
    /* 🔒 The concurrent cap, and Grey Marrow's exemption. Capping the
       ...
       on a city that is not yet in trouble — exactly backwards. The world
       event is NOT exempt: the cap exists because cure research is serial
       through one Research Facility, and a third outbreak is unwinnable
       whoever sent it. The window is six hours; it waits for a slot. */
    if (activeCount >= PLAGUE_MAX_ACTIVE && id !== 'greymarrow') continue;

    let p = 0;
    try { p = clamp01(v.catch(snap)); } catch (e) { p = 0; }
    if (id === evId) p = Math.max(p, worldPressure(ev));
    out[id] = p;
  }
  return out;
}
...
function tryCatch(dtMin) {
  ...
  for (const id of PLAGUE_IDS) {
    ...
    seed(id, id === evId ? ev : null);
  }
}

/* The one place an outbreak is written. `ev` non-null means the world event
   claimed this one: it is filed under the event's strainId — whether or not
   the city's own vector was also red, because every city on the server files
   the same event under the same id — and the strain goes into `seen` so the
   same window cannot seed this city twice. */
function seed(id, ev) {
  const v = PLAGUE_VIRUSES[id];
  const age = cityAgeMin();
  const a = { id, inf: PLAGUE_SEED_FRAC, imm: 0, since: age, deaths: 0, quarantine: false, strain: null };
  if (ev && ev.strainId) {
    a.strain = String(ev.strainId);
    PL.seen[a.strain] = age;
    const keys = Object.keys(PL.seen);
    if (keys.length > WORLD_SEEN_MAX) {
      keys.sort((x, y) => PL.seen[x] - PL.seen[y]);
      for (const k of keys.slice(0, keys.length - WORLD_SEEN_MAX)) delete PL.seen[k];
    }
  }
  PL.active[id] = a;
  const cure = cureForVirus(id);
  const cause = ev ? ((ev.def && ev.def.line) || 'A world event has reached this city.') : v.vector;
  C.logEvent('raid', v.ico + ' OUTBREAK — <b>' + esc(v.name) + '</b> detected in the city. ' +
    (ev ? '🌐 ' : '') + esc(cause) + ' Synthesise <b>' + esc(cure ? cure.name : 'a cure') +
...
/* 🌐 The event's own drag on the city, applied while the window runs whether
   or not this city has caught anything yet — worldstrains.js declares cityFx
   as "a modifier the city applies while the event runs, on top of the
   infections themselves". `health` and `unrest` are per-city-minute nudges on
   the same scale as a virus's own `effects.health`; `labour`, `food` and
   `research` are multipliers read where those numbers are made. Nothing here
   is a new resource and none of it is Cinder. */
function worldTick(dtMin) {
  const fx = worldFx();
  if (!fx) return;
  if (fx.health) C.nudgeVital('health', fx.health * dtMin);
  if (fx.unrest) C.nudgeMorale(-fx.unrest * dtMin);
}
...
function researchTick(dtMin) {
  ...
  if (labs <= 0) return;
  /* 🌐 A world event that slows research (Vault Rot, Grey Fever) slows it
     here, on the same clock, rather than through a second research model. */
  const fx = worldFx();
  R.left -= dtMin * labs * (fx ? fx.research : 1);
  if (R.left > 0) return;
...
export function outputMul() {
  ...
  const fx = worldFx();
  let m = act.length ? labourMul(act) : 1;
  for (const a of act) if (a.quarantine) m *= QUARANTINE_LABOUR;
  if (fx) m *= fx.labour;
  if (!Number.isFinite(m)) return 1;
  return Math.max(PLAGUE_OUTPUT_FLOOR, Math.min(1, m));
}
export function demandMul(need) {
  ...
  const fx = worldFx();
  if (fx && need === 'food') m *= fx.food;
  return Number.isFinite(m) && m > 0 ? m : 1;
}
/* 🌐 The running world event, if any, for the panel and the city's HUD. */
export function worldEventNow() { return worldEvent(); }
...
export function save() {
  ...
  for (const k in PL.active) {
    ...
    act[k] = { id: a.id, inf: +a.inf.toFixed(5), imm: +(a.imm || 0).toFixed(5),
               ...
               strain: a.strain ? String(a.strain) : null };
  }
  return { v: 1, active: act, cured: PL.cured, seen: PL.seen, research: PL.research,
  ...
  try {
    for (const k in (raw.active || {})) {
      ...
      PL.active[k] = { id: k, inf: clamp01(+a.inf), imm: clamp01(+a.imm),
                       ...
                       strain: (typeof a.strain === 'string' && a.strain) ? a.strain : null };
    }
    for (const k in (raw.cured || {})) if (PLAGUE_VIRUSES[k]) PL.cured[k] = +raw.cured[k] || 0;
    for (const k in (raw.seen || {})) if (typeof k === 'string' && k) PL.seen[k] = +raw.seen[k] || 0;
...
const CSS = `
/* 🩺 THE CHIP LIVES IN THE TOP BAR, IN THE FLOW — "move this above so nothing
   can overlap it and it is not overlapping anything else". It sat fixed in
   the bottom-right corner, where the toast rail, the right column and every
   panel that opens down there drew over it (and it over them). Appended to
   #topbar's flex row instead, pushed to the right edge with margin-left:auto:
   a laid-out element cannot be overlapped by its siblings and overlaps none.
   The body-child rule is the fallback for a page with no top bar. */
.pl-badge{position:static;margin-left:auto;pointer-events:auto;background:#241016;border:1px solid #7d2740;
 color:#ffb3c1;border-radius:10px;padding:5px 11px;font:600 12px/1.2 system-ui,sans-serif;cursor:pointer;
 white-space:nowrap;box-shadow:0 4px 18px rgba(0,0,0,.5)}
body>.pl-badge{position:fixed;right:12px;top:calc(var(--topbarh,60px) + 4px);z-index:60;margin:0}
...
function renderPanel() {
  ...
  /* — the world event, when one is running — */
  const ev = worldEvent();
  if (ev && ev.def) {
    const mins = Math.max(0, Math.round((+ev.endsInMs || 0) / 60000));
    h += '<div class="pl-warnbox">🌐 <b>World event — ' + esc(ev.def.name) + '</b>: ' + esc(ev.def.line || '') +
      ' Every city on the server is inside it for the next ' + (mins >= 60 ? Math.floor(mins / 60) + 'h ' + (mins % 60) + 'm' : mins + 'm') +
      '; here it lands as <b>' + esc(PLAGUE_VIRUSES[cityVirusForWorldEvent(ev.def.id)].name) + '</b>.</div>';
  }
  ...
  if (!act.length) {
    h += '<div class="pl-card"><div class="pl-h">✅ No active outbreak</div>' +
      ...
      'Keep Health, Water and Food coverage up, housing under 90% of cap, Hope above 30, and any Anomaly Lab above 55% containment, and nothing here can fire on its own. ' +
      'The one exception is a 🌐 world event, which is announced above while it runs.</div></div>';
  }
  for (const a of act) {
    ...
    h += '<div class="pl-card t' + v.tier + '">' +
      ...
      '<div class="pl-vec">🧭 ' + esc(a.strain ? 'World event strain ' + a.strain + ' — the same outbreak every city on the server is fighting.' : v.vector) + '</div>' +
...
export function mount(ctx) {
  ...
  const api = {
    ...
    pressures, worldEventNow,
    _state: () => PL,
    _snapshot: () => snapshot(),
    _seed: (id, ev) => { if (PLAGUE_VIRUSES[id] && !PL.active[id]) seed(id, ev || null); },
