/* ═══════════════════════════════════════════════════════════════════════════
   ...
   node-city's section 12b header (OPERATIONS ARE BUILDINGS) said out loud
   were NOT built:
   ...
   both that header and the Containment Lab note in section 8 now point here
   rather than being left to lie.

   🌐 WORLD EVENTS are not a second catalog. /src/plague/worldstrains.js names
   the five that are loose across every city at once; plague.city.js maps each
   onto one virus below (WORLD_EVENT_MAP) and applies it as catch PRESSURE,
   through the same tryCatch every vector here goes through.
...
/* ⏱ THE CLOCK. Everything below is per CITY-MINUTE, the unit economyTick and
   ...
   day is not a style choice: the ops layer learned this the hard way (see the
   "💱 SCALE" ÷50 note above OP_BP in node-city's section 12b) and mixing the
   two scales in one file is how that bug happened.
   ⚠ And `game.cityAge` is ticked SECONDS, not minutes — plague.city.js reads
     it through cityAgeMin() and nothing here ever compares to it raw. */
...
/* 🚪 THE GRACE PERIOD. City-minutes of life a NEW city gets before any vector
   ...
   the SIMULATED clock (ticked seconds, ÷60 — see cityAgeMin() in the engine),
   so a tab left open overnight does not burn it. A world event does not skip
   it either: a city with no clinic yet has no answer to the server's virus. */
...
export const PLAGUE_VIRUSES = {
  ...
  /* ── 4 ─────────────────────────────────────────────────────────────────
     ...
     Node-city's section 8 (THE CONTAINMENT LAB) promised that containment "is
     not allowed to be a number with no consequence" and then listed only two consequences, both
...
/* 🧮 The city's total labour multiplier under every active outbreak.
   ...
   Multiplicative approaches zero and never arrives. This is the per-card
   figure; the number the city's economy actually multiplies by is
   outputMul() in plague.city.js, which floors at PLAGUE_OUTPUT_FLOOR. */
