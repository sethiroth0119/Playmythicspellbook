...
export const CITY_PRODUCTION = [
  ...
  },
  {
    ...
    maxLevel: 3,
    /* 🧱 …AND INGOTS, WHICH THIS ROW HAS PROMISED SINCE IT WAS WRITTEN.
       The description above says "Scrap in, ingots out" and the data yielded
       metal alone, so when ingots was promoted into RESOURCES (v121v70) the
       gate correctly refused it: "no producer for: ingots — else the pile is
       real and inert". It is THIS building, in the city builder too (the
       Smelting Works there runs gen ingots / use metal).
       ⚠ metal STAYS. This is the only metal producer in the catalogue, so
         swapping the yield rather than adding to it would strand metal in
         exactly the way the gate is there to catch.
       Tier 1 against metal's tier 0 (economy/tuning.js resTier), so the
       smaller number is the refined half of the same heat, not a nerf. */
    yields: { metal: 40, ingots: 12 }, inputs: { fuel: 20 },
    draw: { power: 35, water: 0, workers: 8, pollution: 18 },
    footprint: { w: 3, h: 3 },
    // Produces metal and ingots, priced mostly in fuel + supplies. Rule 1 of §3.
    cost: [
  ...
  /* ══ THE FREIGHT DEPOT (transport r1, re-landed) ═══════════════════════════
     ...
     before a farm. It is the LAST hand-written row; the generated chain
     producers are push()ed after the literal closes, so they still follow it.

     🔴 WHY IT IS BEING LANDED A SECOND TIME. This entry shipped once, in commit
     2535e5e57d on the transport feature branch, and never reached this branch:
     the transport module (src/transport/depot.js) was merged with its
     DEPOT_DEF_ID = 'freightdepot' and this def was not. Measured on a clean
     bundle: MythicCityProduction.cityProdDef('freightdepot') → null, so
     depotDef() → null and depotReady() refused every chartered haulier with
     'catalog-missing' — the one code that tells a player to stop touching the
     game — before it ever looked at their placed rows. Grepping this file for
     the id is the whole test; nothing about the def had to change to fix it.
     ...
     `freightdepot`, and NEITHER may be renamed to tidy the pair up. (The
     node-city Truck Depot is a third thing again — a node-city BUILDINGS tile
     typed `truckdepot`, which sql/075-076 index; it is not in this catalog.)
     ...
     be a second authority for a value the server (transport_caps() in sql/073)
     also derives. This function is the SINGLE client definition of the level
     table. Copy it nowhere. */
  {
    ...
    /* ⛽ `inputs` IS OMITTED, AND THE DESIGN DOC'S `inputs: { fuel: 30 }` IS A
       REJECTED DESIGN. Three verified facts, in the order that decides it:
         ...
         3. The fuel sink the doc wants is real, and it is somebody else's line.
            CLAUDE.md: all operation pricing goes through _opEcon(). A per-run
            burn priced HERE would be a second price for one thing, which is
            the failure this catalog's cost dial exists to prevent.
    ...
    /* 🚛 THE THREE THINGS THE BUILDING DECIDES, and the reason freight is a
       ...
       🔴 THE SERVER HAS ITS OWN COPY OF ALL THREE AND THE SERVER'S COPY BINDS.
       transport_caps() in sql/073 publishes `'reach', 3 + c.depot_level`,
       `'bays', least(2 * c.depot_level, f.max_bays)` and `'fleet_cap',
       least(4 * c.depot_level, f.max_fleet_rigs)` as plpgsql literals — a
       migration cannot import this catalog — and transport_quote / dispatch
       read those, not this. So changing the multipliers below WITHOUT a new
       migration that redefines transport_caps() makes the panel promise bays
       and reach that dispatch then refuses — this repo's worst bug class: shown
       one number, billed by another. The two copies must move together.
    ...
    footprint: { w: 3, h: 3 },
    /* 💰 AUTHORED PRE-DIAL, like every hand-written row above it. cost.js's
       buildingCostAt() applies RESOURCE_COST_MULT = 2.5 to every non-cinder leg
       and CINDER_COST_DIV = 3 to the cinder leg AT READ TIME, so these numbers
       are NOT the price: L1 is charged as 35,000 cinder + 238 metal + 175
       supplies + 125 stone + 75 fuel. Never hand-multiply a row to "fix" a
       price and never touch the two constants for one building — they are one
       knob for the whole catalog, and that is what keeps the UI and the spend
...
/* ══ 🛢 PROMOTED WITH A PRODUCER THAT IS NOT A BUILDING ═════════════════════
   🔴 THE DERIVATION BELOW ASSUMES A CITY TILE IS THE ONLY THING THAT CAN MAKE
      AN ID, and that stopped being true when Hidn Petro — The Cracking Yard
      started drumming its cuts into the camp stores
      (/src/refinery/state.js drawToStash). These thirteen are in index.html's
      RESOURCES and are NOT derivable from ECO_BUILDING_MAP, because no tile
      yields them.

   They are listed HERE rather than quietly appended to PROMOTED_CHAIN_IDS
   because that array's whole value is that it IS the derivation — putting a
   hand-picked id in it would make the drift gate meaningless for everything
   else. Keeping them separate says the true thing: "promoted, with a producer,
   just not a building one."

   ⚠ THE RULE THEY STILL OBEY is the one that matters: every id has exactly one
     producer standing behind it today. An id with none is "worse than missing —
     the player's pile of it is real and inert", and none of these is that.
   ⚠ WHAT THIS FIXES: terroir's FALLBACK_IDS is `legacy 14 + promoted`, and
     profileFor() compares that length against the slot bag — a mismatch
     degrades the ENTIRE terroir feature to all-COMMON behind one console.warn.
     Promoting fifteen ids into RESOURCES without extending this list left it
     at 124 against a 139-id ledger, which is exactly that silent failure. */
/* ⚠ ORDER IS LOAD-BEARING: this list is concatenated after the derivation to
   reproduce index.html's RESOURCES array id-for-id, so it is in the REFINERY's
   own cut order (light ends, middle distillates, bottoms, then the recovery
   cuts) exactly as the ledger lists them — NOT alphabetical. Sorting it breaks
   the RESOURCES equality check and terroir's FALLBACK_IDS alignment. */
...
/* 🧱 THE FOURTH SET: ids promoted out of the CITY'S OWN STOCK.
   ingots was never a chain id and never lootable — it lived in node-city's
   CITY_STOCK, the city's internal refined goods, which the player ledger
   could not see. Players produced it, saw it in the city panel and found
   nothing in any vault, which is the reported bug (v121v70).
   It is listed separately from PROMOTED_CHAIN_IDS on purpose: it is NOT
   part of the derivation (it is in no chain catalogue), so folding it in
   there would make that list stop meaning "the derivation" and quietly
   disarm the check that nothing rides in beside it.
   ⚠ THE SAME PRICE OF ADMISSION APPLIES: a producer (the Smelting Foundry
     now yields it, as its own description always claimed), an explicit
     RESOURCE_CINDER_VALUE, and a classification. The gate enforces all
     three — it caught this promotion mid-flight and refused it. */
export const PROMOTED_STOCK_IDS = ['ingots'];

export const PROMOTED_CHAIN_IDS = [
  'acids', 'adhesives', 'aluminum', 'aluminumOre', 'anomalousEnergy', 'anomalousMatter',
  /* 🛣 ✈️ asphalt and aviationFuel joined the derivation when the Asphalt Plant
     and the Jet Fuel Terminal were added to ECO_BUILDING_MAP — they ARE tile
     outputs, so unlike MINIGAME_IDS above they belong in this list, in the
  ...
  'gravel', 'herbs', 'householdGoods', 'industrialChemicals', 'industrialFuel', 'industrialGas',
  /* 🐄 livestock joined the derivation when the Livestock Farm was added to
     ECO_BUILDING_MAP — it is that building's promoted output, and the handle
     the coming slaughter business will pull. Alphabetical, because that is the
     order the derivation emits. */
  'industrialWater', 'limestone', 'lithium', 'livestock', 'lumber', 'machineParts', 'maintenanceParts',
  'medicalChemicals', 'metalAlloys', 'metalComponents', 'microchips', 'mythicResidue', 'naturalGas',
  'nickelOre', 'packagingMaterial', 'paint', 'petrochemicals', 'pigIron', 'plantFiber',
  'plastic', 'plasticFeedstock', 'platinumOre', 'potatoes', 'preparedMeals', 'printedCards',
  /* ♻️ recycledGlass and recycledPlastic joined the derivation when the Trash
     Crusher was added to OP_ECO_MAP — they are its two outputs. In the
     alphabetical order the derivation produces, which is why glass sits BEFORE
     metal here and plastic after it. */
  'quartz', 'rareEarthMinerals', 'rawWater', 'realityFragments', 'realityMatter',
  'recycledGlass', 'recycledMetal', 'recycledPlastic',
  'researchChemicals', 'researchEquipment', 'rice', 'rubber', 'sand', 'seafood',
  'seaweed', 'semiconductorChemicals', 'semiconductorMaterials', 'sheetMetal', 'shellfish', 'silica',
  'siliconWafers', 'silverOre', 'solvents', 'soybeans', 'specialtyPolymers', 'sportingGoods',
  'steel', 'sugar', 'sugarCrops', 'syntheticFiber', 'timber', 'tires',
  'titanium', 'toys', 'tungsten', 'vegetables', 'vehicleParts', 'wheat',
  'woodPanels', 'zincOre',
];
...
export const CITY_PREREQ = {
  ...
  /* 🚛 THE FREIGHT DEPOT — GATED, AND NOT LEFT ABSENT. An absent id means
     ...
     put a charter building on the bootstrap tier next to warehouse, wellhead
     and hydroponics — the three that are ungated so a NEW city can start — and
     a yard on an empty grid has no Power Plant to draw its 18⚡ from.
     ...
     warehouse, and nothing needs the depot. depotReady()'s 'no-depot' fix text
     ("It needs a Power Plant first") is this line, read from the other side. */
...
export function auditCatalog(resourceIds) {
  ...
  /* 🛢 A MINI-GAME OUTPUT HAS A PRODUCER, IT IS JUST NOT A TILE. The refinery
     cuts are made in the Cracking Yard, so demanding a CITY_PRODUCTION row for
     them would report fifteen false problems and train the reader to ignore
     this list — which is the only thing that makes rule 1 worth anything. The
     exemption is the curated MINIGAME_IDS above, so it still cannot wave
     through an id that nothing at all makes. */
  ids.forEach(r => { if (!produced.has(r) && MINIGAME_IDS.indexOf(r) < 0) problems.push(`no building yields "${r}"`); });
