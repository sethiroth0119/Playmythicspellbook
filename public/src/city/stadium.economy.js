...
export function eventReadiness(event, node, world, now, E = STADIUM_ECON) {
  ...
  /* 🏘 SAID IN THE CITY'S OWN NUMBERS — bug-mu2pgkdt, "the Housing Headroom
     figure is set to 8 million+ people … should be something realistic".
     The arithmetic was never wrong: this module works at world scale, where one
     citizen node-city tracks stands for residentsPerCitizen (6,000) people, so a
     city with room for 1,400 more population printed "Headroom 8,400,000". True
     at that scale, and meaningless next to the population figure the player can
     see. So the line is written in THAT figure — room left in the city's
     population, and how much of it this event would fill — while the status
     test above and the build advice below keep comparing people with people,
     which is the same comparison in different units. */
  const roomCity = Math.max(0, num(node && node.housingHeadroom));
  const fillCity = num(projMig.arrivals) / E.migration.residentsPerCitizen;
  /* "~0" beside a failing check reads as a contradiction; a small crowd still
     needs somewhere to go, so a fraction below a tenth says so. */
  const fillTxt = fillCity >= 10 ? fmt(fillCity)
    : fillCity >= 0.1 ? (Math.round(fillCity * 10) / 10).toLocaleString('en-US')
    : fillCity > 0 ? 'under 0.1' : '0';
  push('housing', '🏘', 'Housing',
    headroomPeople >= projMig.arrivals ? 'ok' : headroomPeople > 0 ? 'warn' : 'fail',
    'Room for ' + fmt(roomCity) + ' more population · this event would settle ' + (fillCity >= 0.1 ? '~' : '') + fillTxt
      + ' (from ~' + fmt(projMig.arrivals) + ' arriving visitors)',
