...
/* cycleDueAt is re-exported on the API, not just used internally, because
   index.html needs the SAME answer when it lists a standing deal in the corp
   Logistics panel. Re-deriving "when is the next cycle due" over there would
   put the rule in two places, and the two copies would drift the first time
   the cycle term changes — the exact shape of failure this codebase keeps
   recording. plan.js is the definition; this is the door to it. */
const API = { open, close, sweep, CYCLE_HOURS, cycleDueAt, dueCycles };
