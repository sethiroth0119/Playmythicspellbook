...
/* 🚶 THE SAME MODELS, STANDING (v121v55). The walking agents wear the four
   Meshy characters now, and a footway of mock-ups beside them would be the
   loudest possible "two kinds of people". A skinned mesh cannot go into the
   merged buckets as it is — its vertices live in bind space and only the
   skinning shader puts them where the pose says — so the pose is baked on the
   CPU once: the walking clip is sampled at a phase of the figure's own seed,
   the skeleton updated, and every vertex pushed through
   SkinnedMesh.applyBoneTransform into mesh space, then into world space like
   any other part. Normals are recomputed from the posed triangles. The
   result is a plain, textured, static geometry — the same cost as before.
   Falls back to the mock-up until the host has handed the loaded rigs over
   (CTX.civRigs), and the host refreshes the crowd when they land. */
function bakeRig(wx, wz, yaw, seed, out, dayOnly) {
  const { THREE, civRigs, civHeight, SkeletonUtils } = CTX;
  const gltf = civRigs[((seed % civRigs.length) + civRigs.length) % civRigs.length];
  ...
  const s = (civHeight > 0 ? civHeight : 0.36) / h;
  inst.scale.setScalar(s);
  inst.position.y = -b.min.y * s;
  ...
  if (clip) {
    ...
    mixer.update(((seed * 0.37) % 1) * clip.duration);   // a phase per figure — no rank of clones
  }
  ...
  inst.traverse(o => {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      if (o.isSkinnedMesh) o.applyBoneTransform(i, v);   // bind space → posed mesh space
      v.applyMatrix4(o.matrixWorld);                     // → world
...
function bake(wx, wz, yaw, seed, colour, out, dayOnly) {
  if (CTX.civRigs && CTX.civRigs.length && CTX.SkeletonUtils) { try { return bakeRig(wx, wz, yaw, seed, out, dayOnly); } catch (e) { try { console.warn('[Crowd] rig bake failed, mock-up instead', e); } catch (e2) {} } }
...
export function mount(ctx) {
  ...
  const api = {
    ...
    spots: () => spots.slice(),
    group: () => group,
    /* the host mutates ctx (civRigs land later) and asks for a rebuild */
    ctx: () => CTX,
    rebuild() { sig = ''; return api.refresh(); },
  };
  api.refresh();
  return api;
}
