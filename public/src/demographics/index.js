...
function tick(dtMin, host) {
  ...
  try {
    ...
    LAST.budget = budget;
    /* 👥 THE HOST CITY'S OWN HEADCOUNT, KEPT SO THE PANEL CAN SHOW IT.
       Asked for: "in the living economy tab show the player's city population."
       `population` below is this module's modelled residency, which is the
       right number for everything the panel derives; `hostPop` is the exact
       figure the city's own Vital Signs prints. Publishing both is what lets a
       player see the two agree instead of having to trust that they do — and
       when they diverge, that divergence is itself the thing worth seeing. */
    LAST.hostPop = budget;
...
function report() {
  ...
  try {
    ...
    return {
      ...
      /* The city's own headcount and housing ceiling — the same pair Vital
         Signs shows as "NPC Population n / cap". See LAST.hostPop in tick(). */
      hostPop: Math.round(LAST.hostPop || 0),
      hostCap: sv ? Math.round(sv.totalCapacity) : 0,
