...
export function step(days, ctx) {
  ...
  /* 🔴 "BUILD HOUSING" PRINTED DIRECTLY UNDER "23 DWELLINGS ARE STANDING
     EMPTY". The cap here is the HOST's headcount (budget = cityPop()), not
     this city's housing — so when homes are free the honest sentence is that
     the city is not ATTRACTING anyone, which is a different fix entirely.
     index.js already says of this very line that it 'names the block and not
     the fix'; the growth-gate work fixed the coverage half and left this one.
     Housing genuinely full still says build. */
  if (capped) {
