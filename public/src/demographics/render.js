...
export function renderPanel(r) {
  ...
  h.push('<div class="eco-grid">' +
    /* 👥 THE CITY'S POPULATION, FIRST AND NAMED AS SUCH.
       Asked for explicitly. It is the same pair the city's Vital Signs panel
       prints, so a player can hold the two side by side and see them agree —
       the point of the request was that the population was legible in one
       place and not the other. `Residents` beside it is this module's own
       modelled residency, which is what every figure below it derives from. */
    stat(n0(r.hostPop || r.population) + (r.hostCap ? ' / ' + n0(r.hostCap) : ''), 'City population') +
