...
export const DILEMMA_ECON = Object.freeze({
  ...
  /* 🎲 WHERE THE HEIGHTS INTERRUPTS YOU, and how often. The frozen contract
     said there was no auto-fire at all, and the reason given was real: an
     unsolicited modal can land over a battle, a deck edit or one of four
     full-viewport sub-app iframes, none of which will arbitrate z-index with
     it. THESE TWO MOMENTS ARE NOT THAT. Both are on the mission map — one
     screen, no iframe, nothing running — and both are moments the player has
     just chosen to act. The objection was to firing ANYWHERE; this fires in
     the two places the city can plausibly stop you.
     ⚠ THE ODDS ARE A CEILING, NOT A RATE. `offerCooldownMs` above still
       gates every offer, so these cannot compound: a player who moves ten
       times in a minute gets at most one dilemma, not ten. That is why the
       numbers can be generous enough to actually be met.
     ⚠ Deploy is the lower of the two on purpose. A move is a beat between
       things; a deploy is the player committing to a run, and an interruption
       there costs more attention than one on the rails. */
  fireOnMove:       0.34,            // a crossing turns something up about a third of the time
  fireOnDeploy:     0.22,            // …and the city asks less often when you are already committed
