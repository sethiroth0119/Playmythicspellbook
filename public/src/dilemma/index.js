...
import { DILEMMAS, DILEMMA_SCHEMA_VERSION, validateCorpus, DILEMMA_ECON } from './data.js';
import {
  seedFrom, makeRng,
...
function makeHost() {
  ...
  return {
    ...
    /* 🎴 The card art URL, or null. DECLARED HERE ON PURPOSE — makeHost is a
       WHITELIST, not a pass-through, so a capability the bridge offers and this
       object does not name simply does not exist to the module. Adding
       cardArt to index.html alone left engine reading host.cardArt as undefined
       and every roster row silently fell back to its glyph: no error, no
       warning, just the feature quietly not happening. That is the shape of
       this seam and it is worth the four lines to say so.
       ⚠ NULL IS ORDINARY. A signed-out player holds no catalogue art at all,
         so null here is the common answer and faceFor treats it as one. */
    cardArt: (id) => { try { return B.cardArt ? (B.cardArt(id) || null) : null; } catch (e) { return null; } },
...
const MythicDilemmas = {
  ...
  /* 🎲 SHOULD THE HEIGHTS INTERRUPT, HERE, NOW? The odds live in
     DILEMMA_ECON with every other number; the caller supplies only WHERE.
     Answering here rather than in /src/missions is what keeps the one-table
     rule true across a module boundary — the map asks, it does not decide.
     ⚠ available() FIRST, AND THE ORDER MATTERS. Rolling first would burn a
       random number on a cooldown that was never going to let anything
       through, which is harmless — but it also reads as though the odds are
       the whole gate, and they are not. The cooldown is.
     ⚠ An unknown `where` returns false rather than a default rate: a typo
       should cost the interrupt, not silently invent one. */
  rollFor(where) {
    try {
      if (!host()) return false;
      if (!MythicDilemmas.available()) return false;
      const p = where === 'move'   ? Number(DILEMMA_ECON.fireOnMove)
              : where === 'deploy' ? Number(DILEMMA_ECON.fireOnDeploy)
              : 0;
      if (!(p > 0)) return false;
  ...
  open(opts) {
    try {
      ...
      _instance = inst;
      /* 🎬 `opts.after` FIRES ONCE, WHEN THE MODAL IS GONE — resolved,
         dismissed or closed on the X, all three. The mission map needs it for
         the pre-deploy interrupt: the run must start AFTER the player has
         answered, not behind the modal they are still reading.
         ⚠ WRAPPED AROUND THE REAL onClose RATHER THAN REPLACING IT. That
           handler is what clears `_instance`; a caller-supplied function that
           took its place would leave the module believing a modal is still
           open and `open()` would return true forever after. */
      const hs = handlersFor(inst);
      const after = (opts && typeof opts.after === 'function') ? opts.after : null;
      if (after) {
        const inner = hs.onClose;
        let fired = false;
        hs.onClose = () => {
          try { inner(); } catch (e) {}
          if (fired) return; fired = true;      // once, whatever the close path
          try { after(); } catch (e) {}
        };
      }
      const ok = openModal(inst, inst.roster, hs);
