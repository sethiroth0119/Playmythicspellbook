...
/* 🎴 THE FIRST TRACK WAS 26px BECAUSE IT HELD ONE CHARACTER. It now holds a
   face, and 26px of a portrait is a thumbnail of a chin. 38px is what lets a
   painted head read at a glance without pushing the name and the bond pill
   into a second line at 390px — measured against the longest shipped unit name
   the roster can carry. */
#${OV} .md-urow{display:grid;grid-template-columns:38px minmax(0,1fr) auto;gap:9px;align-items:center;
...
/* 🎴 …AND WHEN IT HOLDS A FACE INSTEAD OF A GLYPH. The clamp above is sized
   for one character; a portrait needs a box. Both states live in the same
   26px grid track, so the row height does not jump between a unit that has
   art and one that does not — which, in a list where most rows have art and
   a few do not, is the difference between a roster and a ransom note.
   The max-height clamp is lifted only when an image is actually inside.
   NO BACKTICKS IN THIS COMMENT, and that is not a style note: this stylesheet
   is ONE template literal, so a backtick used as quoting punctuation ends the
   CSS string early. Two of them balance out, which is why node --check passes
   while the sheet is silently cut to pieces. That is exactly what this block
   did on its first draft. */
#${OV} .md-uicon:has(.md-uart){position:relative;max-height:none;height:48px;border-radius:5px;
  border:1px solid rgba(212,175,55,.28);background:#0b0d14;overflow:hidden}
#${OV} .md-uart{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;
  object-position:50% 22%;display:block;image-rendering:auto}
/* the glyph sits under the art, centred in the same box, and is what shows
   when the art is absent or fails to load */
#${OV} .md-uglyph{display:block;line-height:1.4em}
#${OV} .md-uicon:has(.md-uart) .md-uglyph{position:absolute;inset:0;display:flex;
  align-items:center;justify-content:center;line-height:1;opacity:.85}
/* The :has() selector is not universal. Where it is not supported the image
   still needs a ceiling, so it is clamped on its own terms too — the row
   degrades to a small square face rather than to a 55px image painting across
   the name, which is the exact failure the header above this block describes. */
@supports not selector(:has(*)) {
  #${OV} .md-uart{height:1.4em;width:1.4em;border-radius:3px;margin:0 auto}
}
...
/* 🎴 THE ART GATE. A card's art URL can be player-authored (Forge upload,
   admin artUrl), and this file's own header explains what an unescaped
   author field did to the icon cell — so a URL is not trusted because it
   arrived through the bridge. Only three shapes reach an <img>:
     • https:  — the cloud-hosted art every published card uses
     • data:image/  — a local upload, still held as base64
     • a bare relative path with no scheme and no protocol-relative //
   Anything else (javascript:, a scheme-relative //evil, a quote, an angle
   bracket) falls through to the glyph, which is exactly what shipped before.
   ⚠ THE QUOTE TEST IS NOT REDUNDANT WITH esc(). It is the second lock: esc
     is what makes the attribute safe, and this is what keeps a URL that
     needed escaping from being treated as a picture at all. */
const ART_OK = /^(https:\/\/|data:image\/)/i;
const ART_BAD = /[<>"'`\s\\]|^\/\//;
function artUrlOf(unit) {
  try {
    const u = String((unit && unit.art) || '');
    if (!u || ART_BAD.test(u)) return null;
    if (ART_OK.test(u)) return u;
    return /^[A-Za-z][A-Za-z0-9+.-]*:/.test(u) ? null : u;   // a scheme we do not know is not a picture
  } catch (e) { return null; }
}

/* The cell's contents: the card's face when there is one, the safe glyph when
   there is not. Card art is PORTRAIT — the deck picker learned this the hard
   way, cropping a tall painting into a landscape box and taking a slice out
   of the middle of every one. Same crop as there: fill the cell, anchor high,
   so a face lands on the face. */
function faceFor(unit) {
  const glyph = '<span class="md-uglyph">' + esc(iconFor(unit)) + '</span>';
  const u = artUrlOf(unit);
  if (!u) return glyph;
  /* 🔴 THE GLYPH IS ALWAYS IN THE DOM, UNDER THE ART, AND THAT IS THE WHOLE
     FALLBACK. A card art URL can be dead — a withdrawn upload, an expired
     bucket, a player offline — and an <img> that fails paints NOTHING. Drawn
     alone, that turns a companion into an empty bordered box, which reads as a
     unit the game has lost rather than as a picture that did not arrive. So the
     art is layered OVER the glyph and hides itself on error.
     ⚠ THE onerror IS A FIXED LITERAL with nothing interpolated into it. This
       file is careful about author-supplied fields reaching markup for good
       reason (see the icon-cell header); an inline handler that took any part
       of its text from data would be exactly that mistake. This one is a
       constant, and it is inline rather than bound after paint because an image
       can fail BEFORE any binding pass runs. */
  return glyph + '<img class="md-uart" src="' + esc(u) + '" alt="" loading="lazy" decoding="async" onerror="this.style.display=&quot;none&quot;">';
}
...
function rosterRowHtml(unit, choice) {
  ...
  return `<div class="md-urow ${meta.cls}${quiet ? ' quiet' : ''}">
    <span class="md-uicon" aria-hidden="true">${faceFor(unit)}</span>
    <span class="md-ucell">
