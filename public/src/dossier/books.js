  ...
  if (t.type === 'lot') {
    ...
    if (!DG) {
      ...
      rows.push({ label: 'Rent', value: '—', cls: 'fl', un: true,
        sub: 'this city is not counting its housing, so what a dwelling here lets for cannot be read — the reading is missing, not the model' });
    } else if (!R || !R.ok) {
      rows.push({ label: 'Rent', value: '—', cls: 'fl', un: true,
...
function residentIncomeRow(DG, R, rep, incomeCycle, let_, cyc, C, snap, days) {
  if (!DG) {
    return { label: 'Income', value: '—', cls: 'fl', un: true,
      sub: 'this city is not counting its housing, so what the people here earn cannot be read — the reading is missing, not the model' };
  }
  if (!R || !R.ok) {
    return { label: 'Income', value: '—', cls: 'fl', un: true,
      sub: (R && R.why) ? String(R.why) : 'the housing layer did not answer for this tile' };
