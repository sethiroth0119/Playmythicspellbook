...
export const BASKET = [
  ...
  { key: 'entertainment', name: 'Entertainment', ico: '🎭', share: 0.08, ind: 'venue',
    res: ['sportingGoods', 'toys', 'books', 'beverages'] },
  /* 🚗 VEHICLES. The demand side of the automotive chain — see `carDealer` in
     recipes.js for why it had none. A resident buying a car here is paying the
     dealership, which is paying the assembly plant, which is paying the engine
     shop, the tyre works and the glassworks.

     ⚠ THE SHARE IS TAKEN, NOT ADDED. luxury 0.08 → 0.05 and entertainment
       0.10 → 0.08 fund this 0.05 exactly, so the basket still totals 1.00.
       demand() normalises against spendable savings, so an ADDED share would
       not mint anything — but it would raise total consumption, and ECONOMY.md
       records that households are already the thin side of this economy (14 🔥
       of savings against 690,768 🔥 of firm cash on the 705-day board). Taking
       the share redirects spending; adding it would have drained the side that
       is already empty. Both discretionary categories, which is the honest
       place for a big-ticket purchase to compete.

     `tiered: true` — car ownership tracks wealth far more steeply than bread
     does, and TIER_LUX is the existing expression of that. */
  { key: 'vehicles',      name: 'Vehicles',      ico: '🚗', share: 0.05, ind: 'carDealer',
    res: ['cars', 'electricVehicles'], tiered: true },
  ...
  { key: 'luxury',        name: 'Luxury',        ico: '💎', share: 0.05, ind: 'luxuryStore',
