...
function tradePublishPayload() {
  ...
  /* 🏕 WHO IN THIS CITY IS LOOKING FOR WORK, by the trade a camp would ask
     for. Read off the host's named roster through the window seam, because
     this module models employment in AGGREGATE BANDS and has no idea that any
     individual exists — it could tell you 12 unskilled seats are unfilled but
     never that Ilva Vantree is a farmhand with nothing to do.
     ⚠ null, NOT {}, WHEN THERE IS NO SEAM. An empty object is a city saying
       'nobody here wants work'; a null is 'this host cannot answer', and the
       publisher must not wipe a city's advertised labour on a build where the
       accessor is missing. */
  let labour = null;
  try {
    if (typeof window !== 'undefined' && typeof window.cityLabourByRole === 'function') {
      const raw = window.cityLabourByRole();
      if (raw && typeof raw === 'object') labour = raw;
    }
  } catch (e) { labour = null; }
  return {
...
const api = {
  ...
  ready: () => mounted,
  /* 📰 What the last hiring pass did, for the Job Fair's report. Guarded: an
     older sim without the tracker returns an empty shape rather than throwing
     inside the bulletin's render. */
  labourReport: () => {
    try { return Sim.labourReport ? Sim.labourReport() : null; } catch (e) { return null; }
  },
  ...
  /* 📰 THE JOB FAIR SEAM. One call, because the bulletin needs the city-wide
     labour market AND per-business openings together — asking for them
     separately would let the two answers come from different ticks and read as
     a contradiction on screen ("3 openings" beside "0 vacancies").

     ⚠ OPENINGS ARE DERIVED, NOT STORED. headcountFor() is what the production
       path itself uses to decide staffing, so this reports the same number the
       simulation acts on. A parallel "openings" field would be a second source
       of truth that drifts the moment a firm levels up. */
  jobs: () => {
    const rows = [];
    /* 🏠 HOUSING IS NOT A JOB, AND IS NOT SILENTLY DELETED EITHER.
       Every housing tile is founded as a `landlord` firm (node-city's
       ECO_BUILDING_MAP: housing → out constructionComponents, ind landlord),
       so the bulletin listed one 'Housing · Property Company' advert per house
       — pages of them — and a city with twelve homes read as twelve businesses
       hiring. Nobody looks for work at a house, so they are out of the paper.

       🔴 BUT THOSE POSTS ARE REAL AND THEY COMPETE. They are the maintenance
          crew: housing is the ONLY producer of constructionComponents in the
          city, and the housing line of the household basket buys exactly that
          to keep the homes standing. Zeroing the crew would have been the
          tidier-looking fix and it would have cut the city's only supply of
          its own upkeep, pushing that spend out of the city as a leak.
       ⚠ SO THEY ARE REPORTED, NOT HIDDEN. Dropping them from `rows` alone
         would have made the bulletin lie by omission: the paper would say
         'Level 2 — 1 qualified, short 3' while four more skilled residents
         were being taken by houses the reader was never shown. `upkeep` below
         carries that count so the screen can say so out loud. */
    const upkeep = { posts: 0, byBand: {} };
    try {
      for (const f of Firms.alive()) {
        const hc = Firms.headcountFor(f);
        if (!hc || !hc.band) continue;
        const want = Math.max(0, Math.round(hc.n || 0));
        const have = Math.max(0, Math.round((f.workers && f.workers[hc.band]) || 0));
        const open = Math.max(0, want - have);
        if (open <= 0) continue;
        if (f.ind === 'landlord') {
          upkeep.posts += open;
          upkeep.byBand[hc.band] = (upkeep.byBand[hc.band] || 0) + open;
          continue;
        }
        const meta = (Recipes.INDUSTRIES && Recipes.INDUSTRIES[f.ind]) || null;
        rows.push({
          id: f.id, name: f.name || (meta && meta.name) || f.out,
          ind: f.ind, industry: (meta && meta.name) || f.ind,
          ico: (meta && meta.ico) || '🏢',
          band: hc.band, openings: open, staffed: have, seats: want,
          /* 🏷 THE TILE IT STANDS ON. Without it a reader can name the firm but
             not the SIGN — /src/naming gives a tenanted lot a company name and
             an address, and both are keyed by tile. The broadcast's hiring post
             needs it to say "Moreau's Dining Rooms, 40 Bellows Street" rather
             than "Restaurant". Null for a firm with no tile, which is what a
             bootstrap-seeded one is. */
          tileKey: f.tileKey ? String(f.tileKey) : null,
          wage: Math.round((f.lastWageBill || 0) / Math.max(1, have)) || 0,
        });
      }
    } catch (e) {}
    return {
  ...
  /* 🏗 WHAT A BUILDING WOULD MEAN, BEFORE IT IS BUILT.
     ═══════════════════════════════════════════════════════════════════════
     The build shop's detail modal has to answer "what will this do for my
     city" about a tile that does not exist yet, so firmAt() — which reads a
     firm — cannot answer it. This works forward from the same tables the
     simulation will use once the thing is standing.

     🔴 EVERY FIGURE IS DERIVED FROM THE MODEL, AND THE ONES THAT CANNOT BE
        ARE ABSENT RATHER THAN GUESSED. `wagesPerDay` is real: seats ×
        ECON.labor.bands[band].wage is exactly what runPayroll will bill.
        REVENUE IS NOT RETURNED AT ALL, and that is deliberate — a firm's
        takings depend on the ground it lands on, the city's demand, the
        prices on the day and whether anything local buys the output. A
        number here would be a forecast wearing the clothes of a measurement,
        and this file's own rule is that a plausible number is worse than no
        number.
     ⚠ THE BAND IS THE HIRING GATE, and it is the useful half: it is what
       decides whether this city can staff the thing at all. A player about to
       spend 11,000 Cinder on a Clinic deserves to know it needs technicians
       before they build it, not after it stands empty.
     ⚠ `out` IS RESOLVED AGAINST THIS GROUND. pickAvailable() is what
       syncBuildings will use, so the band quoted here is the band the tile
       will really get — not the first entry of a wish list. */
  previewBuilding: (outs, ind) => {
    try {
      const list = Array.isArray(outs) ? outs : [outs];
      const out = pickAvailable(list);
      if (!out) return null;                      // this ground supports none of them
      const band = Recipes.bandOf(out);
      const lvl = Firms.levelDef(1);
      const wage = ((ECON.labor.bands[band] || ECON.labor.bands.unskilled).wage) * (lvl ? lvl.wageMul : 1);
      const meta = (Recipes.INDUSTRIES && Recipes.INDUSTRIES[ind]) || null;
      return {
        out,
        outName: out,
        band,
        industry: (meta && meta.name) || ind || null,
        kind: (meta && meta.kind) || null,
        wagePerWorkerPerDay: Math.round(wage * 100) / 100,
        /* The rung a resident must have finished to hold one of these seats —
           the same table citQualifies reads, so the shop and the roster cannot
           disagree about who can work here. */
        /* ⚠ ECON.demographics.education, NOT ECON.edu. The ladder is nested
           under demographics (tuning.js:181) and the short path is undefined —
           so this returned null for every building and the brief printed "no
           schooling recorded for this trade" about a Clinic that plainly needs
           technicians. It failed SOFT, which is why it needed a test to see. */
        needsRung: (ECON.demographics && ECON.demographics.education
          && ECON.demographics.education.requires) ? ECON.demographics.education.requires[band] : null,
      };
    } catch (e) { return null; }
  },

  /* 🧾 THE BOOKS OF THE BUSINESS STANDING ON ONE TILE.
     ═══════════════════════════════════════════════════════════════════════
     The building panel prints 'this game keeps no per-building history, so
     there is nothing here to chart' and leaves Income, Upkeep and Fees blank.
     That was TRUE OF index.html ALONE and has not been true since this module
     shipped: every firm already carries its own revenue, cost, profit,
     customer count, wage bill, ground rent and lifetime totals, because the
     level gates and the distress ladder are computed from them. The figures
     existed; nothing was handing them to the tile.

     🔴 EVERY FIELD HERE IS READ, NOTHING IS DERIVED FOR DISPLAY. `revenueDay`
        is what customers actually paid this firm today — it is incremented in
        firms.js earn() by the same call that moves the Cinder — and
        `profitDay` is the last CLOSED day, because closeDay zeroes the
        running totals. Both are handed over as they are, with the difference
        named in the field names rather than blended into one 'profit' that
        would mean different things on either side of a day boundary.
     ⚠ A COPY, NEVER THE FIRM. This codebase has already paid for a host
       object handed out BY REFERENCE (see labourMarket below), and a panel
       that could write to a firm's cash is a money leak wearing a readout.
     ⚠ null MEANS NO BUSINESS HERE, and the panel must say that rather than
       printing zeroes — a school and a bankrupt shop are not the same thing. */
  firmAt: (tileKey) => {
    try {
      const k = String(tileKey);
      const f = Firms.alive().find(x => String(x.tileKey) === k);
      if (!f) return null;
      const hc = Firms.headcountFor(f);
      const seats = Math.max(0, Math.round((hc && hc.n) || 0));
      const band = (hc && hc.band) || null;
      const staffed = Math.max(0, Math.round((band && f.workers && f.workers[band]) || 0));
      return {
        id: f.id, name: f.name, ind: f.ind, out: f.out, level: f.level, rung: f.rung,
        cash: f.cash || 0, debt: f.debt || 0,
        // today, so far — zeroed at each day close
        revenueToday: f.revenueDay || 0,
        costToday: f.costDay || 0,
        customersToday: f.customersDay || 0,
        rentToday: f.rentDay || 0,
        // the last COMPLETED day, which is the one that can be called a result
        profitLastDay: f.profitDay || 0,
        profitableDays: f.profitStreak || 0,
        badDays: f.badDays || 0,
        customersAvg: f.customersAvg || 0,
        // since it was founded
        lifetimeRevenue: f.lifetimeRevenue || 0,
        lifetimeProfit: f.lifetimeProfit || 0,
        rentLife: f.rentLife || 0,
        // what it costs to run, and who runs it
        wageBill: f.lastWageBill || 0,
        dailyCost: (() => { try { return Firms.dailyOperatingCost(f); } catch (e) { return 0; } })(),
        band, seats, staffed,
        inventory: f.inventory || 0,
        throttle: f.throttle == null ? 1 : f.throttle,
        bottleneck: f.lastBottleneck || null,
        foundedDay: f.foundedDay == null ? null : f.foundedDay,
      };
    } catch (e) { return null; }
  },

  /* 🎓 WHAT SCHOOLING EACH BUILDING'S SEATS DEMAND, keyed by tile.
     The named roster in node-city seats people at TILES; this file bands jobs
     by the firm's OUTPUT (bandOf). Without this map the two cannot be talked
     about together, and the city was seating self-taught residents into
     technical posts while the bulletin printed 'nobody has this level of
     schooling' about the very same seats — two systems, two answers, one
     screen. Derived from headcountFor so it is the same band the hiring pass
     and the job adverts use; a second rule here would drift from both. */
  tileBands: () => {
    const out = {};
    try {
      for (const f of Firms.alive()) {
        ...
        const hc = Firms.headcountFor(f);
        if (hc && hc.band) out[String(f.tileKey)] = hc.band;
      }
    } catch (e) {}
    return out;
  },
