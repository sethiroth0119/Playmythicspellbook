...
export const INDUSTRIES = {
  ...
  /* 🚗 THE CAR DEALER — added for the opposite reason to the Arms Dealer above.
     That one had a building with no resource; this one had a RESOURCE WITH NO
     BUYER.

     `cars`, `electricVehicles` and `buses` each have a full recipe and a real
     chain behind them (engines + vehicleParts + tires + constructionGlass +
     electronicComponents, five industries deep), and node-city already ships a
     "Car Dealership" operation that manufactures them. But grep the graph and
     those three ids appear ONLY as definitions — never once as an input to
     anything, and no basket category bought them. The city could build the
     entire automotive chain and the cars would pile up unsold for ever, which
     is a firm that looks healthy on its balance sheet and earns nothing.

     ⚠ IT IS `retail`, AND THAT IS THE WHOLE FIX. demand.js routes household
       spending to retail/service only; `autoPlant` is `manufacturing`, which
       sells B2B — to a downstream buyer that does not exist. Re-pointing the
       dealership at a retail industry is what puts an NPC customer at the end
       of the chain, and it does it through the audited circular flow rather
       than by crediting anyone.
     `skilled`, like the Arms Dealer: it is a licensed, financed counter. */
  carDealer:    { name: 'Car Dealership',        ico: '🚗', band: 'skilled',   kind: 'retail' },
...
export const RECIPES = {
  ...
  woodPanels:          { in: { lumber: 1.0, adhesives: 0.05 },               labor: 0.07, power: 0.14, ind: 'sawmill' },
  ...
  steel:                   { in: { pigIron: 1.05, coal: 0.22 },                        labor: 0.11, power: 0.55, ind: 'steelMill' },
...
export const ALT_FEEDSTOCK = {
  electricity: [
    ...
    /* 🛢 A generator running on the yard's diesel. Appended to an array that
       already existed, so unlike the four new keys below this one cannot have
       deleted anything. Priced between gas and biomass: refined fuel burns
       cleaner than coal and costs more than digging it up. */
    { in: { diesel: 0.10 },                labor: 0.005, tag: 'diesel'  },
  ],
  steel:          [{ in: { pigIron: 1.05, coal: 0.22 }, labor: 0.11, tag: 'blast' },
  ...
  /* ══ 🛢 THE REFINERY CUTS AS FEEDSTOCK ═══════════════════════════════════
     Hidn Petro's thirteen cuts are real resources now, and these are the legs
     that let a city BURN them. Every one is a second route to something the
     city could already make from crude — a player who has never seen the
     Cracking Yard is unaffected, and a player who has one can feed its output
     straight into their own city instead of selling it at the rack.

     🔴 THE FIRST LEG OF EACH IS THE ORIGINAL RECIPE, COPIED EXACTLY, AND THAT
        IS NOT DECORATION. legsOf() returns ALT_FEEDSTOCK entries INSTEAD of
        RECIPES[id] when the key exists — so adding a key with only the new
        route would DELETE the crude-oil path from the game silently, and every
        city already running on it would stop. Check the numbers against
        RECIPES above before touching these.

     ⚠ The cut routes are deliberately CHEAPER IN LABOUR than the crude ones.
       A refinery already did the hard part; asking a city to spend the same
       effort cracking a barrel it has been handed as finished product would
       make owning the yard worth nothing. */
  asphalt:        [{ in: { crudeOil: 0.35, gravel: 1.1, sand: 0.4 },       labor: 0.05, power: 0.14, tag: 'crude' },
                   /* Bitumen IS the bottom of the barrel — heavy residue is
                      what a real asphalt plant runs on, and the yard's Heavy
                      Oil is exactly that cut. */
                   { in: { heavyOil: 0.4, gravel: 1.1 },                   labor: 0.03, power: 0.11, tag: 'residue' }],
  aviationFuel:   [{ in: { crudeOil: 1.9, industrialChemicals: 0.05 },     labor: 0.08, power: 0.29, tag: 'crude' },
                   /* Jet fuel IS treated kerosene. This is the shortest and
                      truest of the five routes. */
                   { in: { kerosene: 1.05 },                               labor: 0.04, power: 0.12, tag: 'kerosene' }],
  industrialFuel: [{ in: { crudeOil: 1.6 },                               labor: 0.04, power: 0.17, tag: 'crude' },
                   { in: { heavyOil: 1.1 },                                labor: 0.02, power: 0.07, tag: 'residue' }],
  solvents:       [{ in: { petrochemicals: 0.6, industrialChemicals: 0.3 }, labor: 0.09, power: 0.26, tag: 'petrochem' },
                   /* Naphtha is the classic solvent feedstock. */
                   { in: { naphtha: 0.9 },                                 labor: 0.05, power: 0.14, tag: 'naphtha' }],
