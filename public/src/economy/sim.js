...
/* 📰 LABOUR REPORT — what the last hiring pass actually did, and the running
   totals since the city was opened. The Job Fair reads these; nothing else
   depends on them, so a failure to populate costs a report and not a city. */
let LABOUR_REPORT = { at: 0, hired: 0, fired: 0, unfilled: 0, firms: [] };
const LABOUR_TOTALS = { hired: 0, fired: 0, runs: 0 };
export function labourReport() {
  return { last: LABOUR_REPORT, totals: { ...LABOUR_TOTALS } };
}
...
function runProduction(days, host) {
  ...
  const fill = HH.hire();
  /* 📰 DISTRIBUTE THE HIRED HEADCOUNT — PRIORITISED FIRMS FIRST.
     This was a flat proportional split: every firm in a band received the same
     fraction of that band's hires, so the Job Fair's "Prioritise ticked
     businesses" button could not possibly do anything — the allocation had no
     notion of order, and the button only ever printed a sentence.
     Now: firms the player ticked are served FIRST, each taking its full crew
     until that band's hires run out. Whatever is left is split among the rest
     exactly as before, so unticked firms keep the old behaviour and an empty
     tick list reproduces the previous allocation almost exactly.
     Also records what actually happened, per firm, so the fair can report who
     was hired and who was let go instead of the player guessing. */
  const _before = new Map();
  for (const f of Firms.alive()) {
    const hc = Firms.headcountFor(f);
    _before.set(f.id, (f.workers && f.workers[hc.band]) || 0);
  }
  let _picked = new Set();
  try {
    const raw = (typeof globalThis !== 'undefined' && globalThis.__jfPriority) || [];
    (Array.isArray(raw) ? raw : []).forEach((id) => _picked.add(String(id)));
  } catch (e) {}

  const _pool = {};
  const _emp = HH.state().employed || {};
  for (const b in _emp) _pool[b] = _emp[b] || 0;

  const _alive = Firms.alive();
  const _prio = _alive.filter((f) => _picked.has(String(f.id)));
  const _rest = _alive.filter((f) => !_picked.has(String(f.id)));

  for (const f of _alive) { for (const b in f.workers) f.workers[b] = 0; }

  // Ticked firms take full crews, in the order the fair shows them.
  for (const f of _prio) {
    const hc = Firms.headcountFor(f);
    const take = Math.max(0, Math.min(hc.n, _pool[hc.band] || 0));
    f.workers[hc.band] = take;
    _pool[hc.band] = (_pool[hc.band] || 0) - take;
  }
  // Everyone else shares what remains, proportionally, as before.
  const _restBand = {};
  for (const f of _rest) {
    const hc = Firms.headcountFor(f);
    _restBand[hc.band] = (_restBand[hc.band] || 0) + hc.n;
  }
  for (const f of _rest) {
    const hc = Firms.headcountFor(f);
    const share = _restBand[hc.band] > 0 ? hc.n / _restBand[hc.band] : 0;
    f.workers[hc.band] = Math.floor((_pool[hc.band] || 0) * share);
  }

  // ── What changed, for the Job Fair's report ──
  try {
    let hired = 0, fired = 0;
    const rows = [];
    for (const f of _alive) {
      const hc = Firms.headcountFor(f);
      const got = (f.workers && f.workers[hc.band]) || 0;
      const was = _before.get(f.id) || 0;
      if (got > was) hired += got - was;
      else if (got < was) fired += was - got;
      if (got !== was || got < hc.n) {
        rows.push({ id: f.id, name: f.name, band: hc.band, want: hc.n, got, was,
                    delta: got - was, prioritised: _picked.has(String(f.id)) });
      }
    }
    LABOUR_REPORT = { at: Date.now(), hired, fired, unfilled: rows.reduce((n, r) => n + Math.max(0, r.want - r.got), 0), firms: rows };
    LABOUR_TOTALS.hired += hired;
    LABOUR_TOTALS.fired += fired;
    LABOUR_TOTALS.runs += 1;
  } catch (e) {}
