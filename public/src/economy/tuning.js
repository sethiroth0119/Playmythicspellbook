...
export const ECON = {
  ...
  clock: {
    ...
    maxCatchUpDays: 7,       // offline catch-up ceiling. See sim.advance(). v121v49: 3 → 7 — "do not stop because they left the city builder"; a week away is caught up and paid on return.
  ...
  construction: {
    ...
    /* 🏗 THE CREW LADDER — 2 free, up to 25 with a fully staffed Construction
       Company. Asked for as: "upgrade their construction company can hire more
       Crew workers to build instead of 3/3 … make it where the max is 25".

       ⚠ `municipal.slots` IS DELIBERATELY STILL 2, and the note under zoned
         development above is why: the free crew limit is what gates hand
         placement and is a real cost the player plays against. Raising THAT
         would re-tune the whole construction feature for every city including
         ones that never invest in it. What changed is the CEILING and the
         conversion rate — the entire climb from 2 to 25 is bought, one
         worker at a time, at a business the player owns and pays wages on.
       📈 perWorkerStep 6 → 1: one worker, one crew. At the old step a maxed
         12-worker Company bought 2 extra crews and the whole city topped out
         at 5 against a cap of 6 — so the last slot was unreachable and the
         upgrade path stopped mattering after six hires. Now every hire moves
         the number, which is what makes staffing the Company a decision.
       🧮 The arithmetic, so the ceiling is reachable and not a tease:
            2 municipal + 1 per Co + 1 per worker
            = 2 + 1 + 22 workers → 25, the cap.
         OPS.construction.maxWorkers is 25 to match, so ONE Company can reach
         it; a second Company adds its own +1 and its own workers under the
         same ceiling.
       ⚠ SPEED IS UNTOUCHED AND STILL CAPS AT 2×. 25 workers × 0.025 + 0.20
         = 1.83, under maxMul — so more crews build MORE THINGS AT ONCE, not
         each one faster. Those are different levers and conflating them
         would make a staffed Company trivialise the build timers as well. */
    slots:     { perCo: 1, perWorkerStep: 1, max: 25 },
