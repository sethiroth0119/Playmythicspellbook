...
/* ♻️ bug-mtzmjvgz: FOLLOW THE LIVE OBJECT. `_st` was taken once and kept, but
   Profile.foundry is REPLACED by the cloud merge, a sign-out/account switch and
   a force-restore — after which every build and upgrade spent Cinder into the
   orphaned copy while save() wrote the new one, and the work was gone on the
   next login. ensureState normalises and returns the host's own object, so
   identity tells us when it changed. */
function state() {
  const h = host(); if (!h) return null;
  let cur = null;
  try { cur = h.foundryState(); } catch (e) { cur = null; }
  if (!_st || (cur && cur !== _st)) _st = ensureState(h);
  return _st;
}
