...
import { mountStationModels } from './scene.desk.js';
...
/* 🔴 THE WARD IS LOADED ON DEMAND, NOT ASSUMED TO BE THERE. This read
   window.MythicWard and gave up if it was absent — but nothing registers that
   global at boot: /src/ward is a lazy import, pulled in the first time
   MythicContainment.openWard() is called. A player who walked into the Crate
   Intake room without having opened the ward from the Research Facility first
   therefore got "The ward did not load — reload the game", and reloading did
   not help, because the reload did not import it either.
   Importing it here is the fix and costs nothing on the paths that never open
   the room — the module is still absent until the door is used. */
async function openWard(run) {
  let W = (typeof window !== 'undefined') && window.MythicWard;
  if (!W || typeof W.open !== 'function') {
    try {
      await import('../ward/index.js');
      W = (typeof window !== 'undefined') && window.MythicWard;
    } catch (e) {
      try { console.warn('[hospital] ward import failed', e); } catch (e2) {}
    }
  }
  /* Last resort: the containment module owns its own loader for the same
     file, so if a build ever changes how /src/ward registers, this still
     reaches it rather than dead-ending. */
  if (!W || typeof W.open !== 'function') {
    try {
      const C = window.MythicContainment;
      if (C && typeof C.openWard === 'function') {
        run.done.ward = true;
        run.wardOpen = true;
        const r = await C.openWard({});
        if (r && r.ok === false) { run.wardOpen = false; HUD.toast(run.nodes, '⚠ ' + (r.error || 'The ward would not open.'), 'bad'); }
        return;
      }
    } catch (e) {}
    HUD.toast(run.nodes, 'The ward module could not be loaded — try a hard refresh.', 'bad');
    return;
  }
  run.done.ward = true;
  run.wardOpen = true;
...
function frame(run, now, wall) {
  ...
  /* ☣️ Exposure before and after this tick — HS.exposurePenaltyTick charges
     the heroes exactly once, on the frame it crosses 70%. See state.js. */
  const expBefore = +run.suit.exposure || 0;
  const evs = HZ.tick(run.suit, dt, { now: wallNow, atAirlock, inHot: hot });
  try {
    const pen = HS.exposurePenaltyTick(expBefore, run.suit.exposure);
    if (pen && pen.ok) {
      const n = pen.heroes | 0;
      HUD.toast(run.nodes, '☣️ Exposure past 70% — ' + (n ? n + ' hero' + (n === 1 ? '' : 'es') + ' lost' : 'your heroes lose') + ' a quarter of their battle stamina.', 'bad');
    }
  } catch (e) {}
  for (const ev of evs) {
...
export async function open(opts) {
  ...
  // The counters (Front Desk, Ward Bay) as models over the builder's boxes — scene.desk.js.
  if (run.scene) { try { run.desk = mountStationModels(run.scene); } catch (e) { run.desk = null; } }
...
export function close() {
  ...
  try { if (run.desk) run.desk.dispose(); } catch (e) {}
