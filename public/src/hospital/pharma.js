...
export const PRODUCTS = {
  ...
  vaccine: {
    ...
    /* 🛢 THE REFINERY FEEDS THIS ONE. A vaccine needs a preservative and a
       solvent, and ethanol is where both come from — so the Cracking Yard's
       blend component is a real input to the Medical Corporation's best
       product, and the two businesses are joined by material rather than by a
       menu link.
       ⚠ ONE PRODUCT, AND DELIBERATELY THE TOP ONE. Gating the salve or the
         tablets would wall a new hospital off from medicine entirely; gating
         the vaccine makes the refinery a step UP the ladder, not a toll on the
         bottom rung. Ethanol is also market-postable and lootable-adjacent, so
         a hospital owner with no refinery can still buy it from one who has. */
    priceMul: 0.140, perSample: 3, inputs: { medicine: 2, water: 2, supplies: 1, ethanol: 1 },
