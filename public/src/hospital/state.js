...
/* ☣️ EXPOSURE PAST 70% COSTS EVERY HERO A QUARTER OF THEIR BATTLE STAMINA.
   ──────────────────────────────────────────────────────────────────────────
   Asked for as: "when player EXPOSURE reach more than 70% decrease the Hero
   Battle Stamina by 25%". The suit's exposure is hazmat.js's 0..1 number,
   so "more than 70%" is > 0.70, and the heroes live in the main game, so the
   cut goes through the bridge (heroStaminaPenalty in index.html).

   🔴 IT FIRES ON THE CROSSING, NOT ON THE STATE. Exposure ticks every frame;
      a rule that charged "while above 70%" would empty every hero in a second.
      So the caller hands over the value BEFORE and AFTER the tick, and the
      cut lands exactly once, on the frame it goes over. A doff multiplies
      exposure by 0.55, so a player who scrubs down and then breaches again
      crosses again — and pays again. That is the rule working, not a bug.
   ⚠ PURE apart from the bridge call, so the smoke can drive every edge. */
export const EXPOSURE_PENALTY = { threshold: 0.70, fraction: 0.25 };
export function exposurePenaltyTick(before, after) {
  const t = EXPOSURE_PENALTY.threshold;
  if (!((+before || 0) <= t && (+after || 0) > t)) return null;
  const b = bridge();
  if (!b || typeof b.heroStaminaPenalty !== 'function') return { ok: false, why: 'no bridge' };
  try { return b.heroStaminaPenalty(EXPOSURE_PENALTY.fraction, 'exposure') || { ok: true }; }
