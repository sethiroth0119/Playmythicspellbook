  ...
  const why = (d.causes || []).length
    ? '<div class="dwhy">' + d.causes.map((c) => {
        ...
        return '<div class="drow"><div class="dtop ' + (up ? 'up' : 'dn') + '">'
          ...
          + '<div class="dsrc">Where this comes from: ' + esc(c.src) + '</div></div>';
      }).join('') + '</div>'
    : '';
...
export function render(force) {
  ...
  const fh = 'Every line above is measured from the city as it stands right now, and says where its number came from. '
