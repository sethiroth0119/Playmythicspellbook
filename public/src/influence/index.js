...
/* 🖼 Card art is loaded lazily: the first paint can ask for art that is still on
   disk (the resolver starts that read). Repaint a few times while the modal is
   open and the card still has no art, so the picture replaces the emoji. */
let _artRetry = 0, _artRetryId = null;
function artPending(v) {
  const c = v && v.enc && v.enc.card;
  if (!c || !c.id) return null;
  try { return v.cardArt(c.id) ? null : c.id; } catch (e) { return null; }
}
function draw() {
  const v = view();
  R.mount(v, handlers);
  const pend = artPending(v);
  if (pend !== _artRetryId) { _artRetryId = pend; _artRetry = 0; }
  if (pend && _artRetry < 4) {
    _artRetry++;
    setTimeout(() => { try { if (document.querySelector('.mif-card') && _artRetryId === pend) draw(); } catch (e) {} }, 700 * _artRetry);
  }
}
