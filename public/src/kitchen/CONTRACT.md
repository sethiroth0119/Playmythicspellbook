...
export function passPlan(now)              // → { [dishId]: ticketId }   read-only, for render.
                                           //   WHERE THE MATCHER IS SENDING EVERY PLATE,
                                           //   pinned OR planned, off the SAME planPass()
                                           //   the chip and the till read — so the badge on
                                           //   the plate, the fill on the ticket and the
                                           //   money at the till can never name three
                                           //   different customers. `assignmentOf` is the
                                           //   pin only (the override, the rare case).
                                           //   One planPass() per call, not one per plate.
                                           //   Listed 2026-09-04; shipped with the
                                           //   window-car rank, unlisted.
...
Re-verified 2026-09-04 against the live index.html: 24 keys built, 24 rows here, 0 drift.
(The self-test's 4f had been reporting 65 keys of drift — `state`, `grantCard`, `return`,
`catch`… — none of them on this bridge: three unpaired apostrophes in the bridge's own
comments had walked its brace matcher past `};` into MythicDilemmaBridge. It now strips
comments first. A key really added to the bridge still lands here as drift.)
