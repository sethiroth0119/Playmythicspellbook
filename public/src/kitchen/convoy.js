...
function isShippable(recipeId) {
  if (isCrateId(recipeId)) return !!crateSpec(recipeId);   // a supply crate always rides
...
function feeFor(tierId, items, owned) {
  ...
  // A crate pays freight on what the Supplies sheet charges for it.
  for (const id in (items || {})) {
    if (!isCrateId(id)) continue;
    const sp = crateSpec(id);
    if (sp) value += Math.max(0, _int(items[id])) * sp.cinder;
  }
...
function estimate(tierId, items, owned) {
  ...
  const parts = splitBoxes(picked);
  const dishes = parts.boxes;           // ⚠ for the panel: every box on the truck
  return {
    tierId,
    items: picked,
    dishes,
    ...
    // 🔴 COOKED boxes only. A crate is pantry, not food, and quoting food for it
    //    would advertise a payout the claim never makes.
    food: Math.max(0, Math.floor(parts.dishes * EC('CONVOY_FOOD_PER_DISH', 1))),
  };
}
...
/* ═══════════════════════════════════════════════════════════════════════════
   🥡 CRATES — PANTRY SUPPLIES AS FREIGHT
   ═══════════════════════════════════════════════════════════════════════════
   ...
   the supplies in their restaurant."

   A crate is one line of the Supplies sheet — the same qty that sheet sells
   (ten mushrooms, twelve rolls) — and it ships WHOLE: a truck takes crates,
   never loose units, so the pantry can never be shipped down to a fraction of
   a recipe. Crates ride in `items` under `ing:<ingredient>` keys next to the
   dishes, take a box each against the truck's capacity, and pay freight on
   the crate's Cinder price the way a dish pays on its menu price.

   🔴 A CRATE IS NEVER PAID AS FOOD. `dishes` — on the row, on the server, in
      the claim — stays the count of COOKED boxes, which is what
      kitchen_convoy_claim pays food for. Crates land in the RECIPIENT'S PANTRY,
      credited by the client exactly once per row (`cratesLanded`), on the
      server's first claim for an inbound truck and on the local claim for a
      practice run or a turned-back one. sql/110 lets the launch RPC store a
      crates-only load with dishes = 0 instead of rounding it up to a phantom
      dish.
   ⚠ THE SENDER'S PANTRY IS THE ONLY EVIDENCE OF WHAT WAS LOADED, and this is
     the same trust the dishes already run on: a pass is client state too.
     Crates are bought with food and Cinder on the Supplies sheet, so the worst
     a tampered client can do is give a friend something it could have bought.
   ═══════════════════════════════════════════════════════════════════════════ */
const ING_PREFIX = 'ing:';
function isCrateId(id) { return typeof id === 'string' && id.indexOf(ING_PREFIX) === 0; }
/** → { ing, sup, perCrate, name, icon, cinder } or null for an id that is not a crate the data knows. */
function crateSpec(id) {
  try {
    if (!isCrateId(id)) return null;
    const ing = id.slice(ING_PREFIX.length);
    const fi = DF('ingredient'), fsu = DF('supply');
    const meta = fi ? fi(ing) : null;
    if (!meta || !meta.supply) return null;
    const sup = fsu ? fsu(meta.supply) : null;
    const per = Math.max(1, _int(sup && sup.out && sup.out.qty) || 10);
    return { ing, sup: meta.supply, perCrate: per,
             name: (meta.name || ing) + ' crate', icon: meta.icon || '📦',
             cinder: Math.max(0, _int(sup && sup.cost && sup.cost.cinder)) };
  } catch (e) { return null; }
}
/** Whole crates on hand, per ingredient: { 'ing:mushroom': 2, … }. */
function pantryCrates(K) {
  const have = {};
  try {
    const pantry = (K && K.pantry && typeof K.pantry === 'object') ? K.pantry : {};
    const rows = Array.isArray(DATA.INGREDIENTS) ? DATA.INGREDIENTS : [];
    for (const m of rows) {
      if (!m || !m.id) continue;
      const sp = crateSpec(ING_PREFIX + m.id);
      if (!sp) continue;
      const n = Math.floor(Math.max(0, _int(pantry[m.id])) / sp.perCrate);
      if (n > 0) have[ING_PREFIX + m.id] = n;
    }
  } catch (e) {}
  return have;
}
/** Everything a truck could take right now — dishes on the pass plus crates in the pantry. */
function loadCounts(K) { return Object.assign({}, passCounts(K), pantryCrates(K)); }
/** One load, three counts. `boxes` is what the truck carries; `dishes` is what the server pays food for. */
function splitBoxes(items) {
  let dishes = 0, crates = 0;
  for (const id in (items || {})) {
    const n = Math.max(0, _int(items[id]));
    if (isCrateId(id)) crates += n; else dishes += n;
  }
  return { dishes, crates, boxes: dishes + crates };
}
function crateCount(items) { return splitBoxes(items).crates; }
/** Put a landed load's crates into THIS kitchen's pantry. → { crates, units, names } */
function creditCrates(K, items) {
  const out = { crates: 0, units: 0, names: [] };
  try {
    for (const id in (items || {})) {
      if (!isCrateId(id)) continue;
      const sp = crateSpec(id);
      const n = Math.max(0, _int(items[id]));
      if (!sp || !n) continue;
      if (!K.pantry || typeof K.pantry !== 'object') K.pantry = {};
      K.pantry[sp.ing] = _int(K.pantry[sp.ing]) + n * sp.perCrate;
      out.crates += n; out.units += n * sp.perCrate;
      out.names.push(n + '× ' + sp.name);
    }
  } catch (e) {}
  return out;
}
...
function shippablePass(K) {
  ...
  try {
    ...
    // 🥡 …and the crates. After the dishes so the pass keeps the top of the sheet.
    const crates = pantryCrates(K);
    for (const id in crates) {
      const sp = crateSpec(id);
      if (!sp) continue;
      out.push({ recipeId: id, kind: 'crate', name: sp.name, icon: sp.icon, have: crates[id],
                 perCrate: sp.perCrate, foodCost: 0, value: sp.cinder, ship: true });
    }
  } catch (e) {}
  return out;
}
...
export function manifest(K, tierId, wanted, forNetwork) {
  ...
  try {
    ...
    } else if (!bins.length) { code = 'NOT_READY'; why = 'Nothing on the pass or in the pantry can ride a convoy — fries and shakes do not travel, and a supply ships by the full crate.'; }
...
export function compose(K, tierId, items, forNetwork) {
  try {
    ...
    const parts = splitBoxes(picked);
    const dishes = parts.dishes, boxes = parts.boxes;

    if (boxes <= 0) {
      return no('NOT_READY', 'Nothing on the pass or in the pantry can ride a convoy — fries and shakes do not travel, and a supply ships by the full crate.');
    }
    const minDishes = Math.max(1, _int(EC('CONVOY_MIN_DISHES', 4)));
    if (boxes < minDishes) {
      return no('NOT_READY', 'A convoy needs at least ' + minDishes + ' boxes. You have ' + boxes + '.');
    }
    ...
    return ok({
      convoy: {
        tierId: t.id,
        items: picked,
        dishes,                       // cooked boxes — what the server pays food for
        crates: parts.crates,
        boxes,
        capacity: cap,
        feeCinder,
        ...
        food: Math.max(0, Math.floor(dishes * EC('CONVOY_FOOD_PER_DISH', 1))),   // dishes, never crates
      },
    });
  } catch (e) {
...
export async function launch(K, convoy, toUserId, now) {
  try {
    ...
    // 5b · TAKE THE CRATES. Whole crates out of the pantry; if the pantry moved
    //      under us (a cook fired between the quote and the click), put EVERY
    //      box back — dishes on the pass, crates in the pantry, fee in the purse.
    const crated = {};
    for (const id in load.items) {
      if (!isCrateId(id)) continue;
      const sp = crateSpec(id);
      const n = Math.max(0, _int(load.items[id]));
      if (!sp || !n) continue;
      const units = n * sp.perCrate;
      if (!K.pantry || typeof K.pantry !== 'object') K.pantry = {};
      if (_int(K.pantry[sp.ing]) < units) {
        for (const d of loaded) K.pass.push(d);
        for (const ing in crated) K.pantry[ing] = _int(K.pantry[ing]) + crated[ing];
        if (paid > 0) { try { b.addGems && b.addGems(paid); } catch (e) {} }
        K.rev++;
        return no('NOT_READY', 'The pantry changed while you were loading. Nothing was sent.');
      }
      K.pantry[sp.ing] = _int(K.pantry[sp.ing]) - units;
      if (K.pantry[sp.ing] <= 0) delete K.pantry[sp.ing];
      crated[sp.ing] = (crated[sp.ing] || 0) + units;
    }
    ...
    const row = {
      ...
      dishes: load.dishes,
      crates: Math.max(0, _int(load.crates)),
      cratesLanded: false,
      launchedAt: t,
...
export async function claim(K, convoyId, now) {
  try {
    ...
    let landedItems = c.items;    // the manifest the crates are credited from; the server's for an inbound truck
    if (found.where === 'in') {
      ...
      if (res.row && res.row.items && typeof res.row.items === 'object') landedItems = res.row.items;
      ...
      // 🥡 A crates-only truck has 0 dishes and is still a first delivery.
      if (res.firstClaim === false || (ceilingDishes <= 0 && crateCount(landedItems) <= 0)) {
    ...
    /* 🥡 CRATES LAND IN THE PANTRY — ONCE PER ROW. Inbound: only past the
       server's first-claim wall above, so a replayed claim credits nothing.
       Local (a practice run, a turned-back truck): the sender takes their own
       crates back. `cratesLanded` rides the row, and the row rides the save. */
    let landedCrates = { crates: 0, units: 0, names: [] };
    if (!c.cratesLanded && crateCount(landedItems) > 0) {
      landedCrates = creditCrates(K, landedItems);
      c.cratesLanded = true;
      K.rev++;
    }
    ...
    if (owed <= 0) {
      retire(K, c, t, 0, null);
      return ok({ granted: 0, spoil, crates: landedCrates.crates, crateUnits: landedCrates.units, crateNames: landedCrates.names });
    }
    ...
    if (noStash()) {
      ...
      retire(K, c, t, 0, null);
      return ok({ granted: 0, crates: landedCrates.crates, crateUnits: landedCrates.units, crateNames: landedCrates.names });
    }
    let before = 0;
    try { before = _int(b.getRes ? b.getRes('food') : 0); } catch (e) { before = 0; }
    let called = false;
    ...
    if (landed < owed) {
      ...
      // 🥡 The crates are already in the pantry (credited above); say so even
      //    though the food is on the dock, or the toast is silent about them.
      return no('CAP', shortLandingWhy(landed, left), { granted: landed, held: left,
        crates: landedCrates.crates, crateUnits: landedCrates.units, crateNames: landedCrates.names });
    }

    retire(K, c, t, landed, null);
    return ok({ granted: landed, spoil, crates: landedCrates.crates, crateUnits: landedCrates.units, crateNames: landedCrates.names });
  } catch (e) {
