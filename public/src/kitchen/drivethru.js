...
const VOICE = {
  ...
  mayor: {
    ...
    testy:   ['The Node Manager doesn’t wait. Which means I don’t wait.',
 ...
 *     say, line, items:[{recipeId, name, icon, qty, filled, heldBy, mods:[{label,
 *     ing, ingIcon, result, cinder}]}], honoured, broken,
 *     menu, worth, served, servedPaid, servedTip, servedLine,
 *     held, heldTo, heldLine, atWindow, canServe, canWave }
 *
 * 📌 `held` / `heldTo` / `heldLine` (and `items[].heldBy`) say who has a plate
 * this order is missing while it sits on the pass — see `heldElsewhere()`.
 * The window card prints `heldLine` instead of a bare "Not ready".
 *
...
/**
 * 📌 WHO HAS THE PLATE THIS ORDER IS MISSING — the reason behind "Not ready"
 * when the food is physically on the pass.
 *
 * → [{ dishId, recipeId, recipe, ticketId, name }], one entry per plate of a
 *   recipe this ticket still needs that `State.passPlan()` is sending to some
 *   OTHER open order — a pin the player made, or the matcher's own choice.
 *   Empty when nothing of theirs is on the shelf at all, which is the honest
 *   "Not ready": there is nothing to reassign.
 *
 * 🔴 THE BARE "NOT READY" WAS A LIE OF OMISSION. Measured in the mkserve
 * harness (out2.txt): the car at the window ordered a burger and a pizza, both
 * were on the pass, the pizza read 1/1 and the burger 0/1 — and the button read
 * "Not ready" while the burger sat in plain view on the shelf, spoken for by
 * somebody else. The player's only path out was to guess that the plate was
 * the problem, open the picker, and read the chips. The rank the state layer
 * asks the player to steer by ("window car first, then nearest due, a pin
 * outranks both" — THE PASS block in kitchen.state.js) is unsteerable when the
 * card cannot say WHO the plate went to. So the card names them.
 *
 * ⚠ `typeof`-GUARDED ACROSS THE IMPORT CYCLE like every State read here; a
 * State without `passPlan()` (the client can ship ahead of it) degrades to the
 * old caption, never to a throw on the paint path.
 */
function heldElsewhere(K, ticket, now) {
  const out = [];
  try {
    if (!ticket || typeof State.passPlan !== 'function') return out;
    const plan = State.passPlan(now);
    for (const it of (ticket.items || [])) {
      if (!it || _int(it.filled) >= _int(it.qty)) continue;
      for (const d of (K.pass || [])) {
        if (!d || d.recipeId !== it.recipeId) continue;
        const to = plan[d.id];
        if (!to || to === ticket.id) continue;
        const other = (K.tickets || []).find((x) => x && x.id === to);
        const rec = recipeOf(d.recipeId) || {};
        out.push({
          dishId: d.id, recipeId: d.recipeId, recipe: rec.name || d.recipeId,
          ticketId: to, name: (other && other.name) || 'somebody else',
        });
      }
    }
  } catch (e) { /* a caption is not worth a broken paint */ }
  return out;
}

/** "Corp Suit" · "Corp Suit and Raider" — the unique names, in pass order. */
function namesOf(held) {
  const names = [];
  for (const h of held) if (names.indexOf(h.name) === -1) names.push(h.name);
...
function cardFor(K, car, now) {
  ...
  /* 📌 THE REASON BEHIND "NOT READY", when there is one. Only a live, unfilled
     order can have its plate held by somebody else; a sold card is frozen and a
     ready one has everything. Each held line also learns who has it, so the
     card can print the sentence ON the line it is about. */
  const held = (!sold && ticket && !ready) ? heldElsewhere(K, ticket, now) : [];
  if (!sold) {                       // never write into the frozen sale record
    for (const it of items) {
      const mine = held.filter((h) => h.recipeId === it.recipeId);
      it.heldBy = mine.length ? namesOf(mine) : '';
    }
  }
  const heldTo = namesOf(held);
  ...
  return {
    ...
    /* 📌 WHO HAS THE MISSING PLATE — see heldElsewhere(). `heldLine` is the
       whole sentence for the card; `heldTo` is the names alone, for a button
       that has room for three words and not twelve. Both empty when the honest
       caption really is "Not ready". */
    held,
    heldTo,
    heldLine: heldTo ? 'given to ' + heldTo + ' — tap the plate to reassign' : '',
