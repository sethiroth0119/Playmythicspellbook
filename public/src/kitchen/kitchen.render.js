 ...
 */
function passHtml(k) {
  /* 📌 WHERE EVERY PLATE IS GOING, asked ONCE per paint and handed down —
     `State.passPlan()` runs the matcher, and running it per plate would run it
     22 times for one answer. Absent on a client that shipped ahead of the sim:
     every plate then falls back to its pin alone, exactly the old shelf. */
  const plan = safe(() => (typeof State.passPlan === 'function' ? State.passPlan(k.now) : null), null) || {};
  const dishes = (k.pass || []).map((d) => dishHtml(d, k, plan)).join('');
...
function pickerHtml(k, plan) {
  ...
  const chips = rows.map((t) => {
    ...
    /* The matcher's own answer, so the sheet says where the plate is going
       BEFORE the player overrules it — "tap the plate to reassign" on the
       window card lands here, and the row it points at has to be findable. */
    const auto = (!on && plan && plan[dish.id] === t.id) ? 1 : 0;
    return `<button class="mk-pick" data-act="assign" data-dish="${esc(dish.id)}" data-tk="${esc(on ? '' : t.id)}" data-on="${on}" data-auto="${auto}">
        <span class="mk-pick-ic" aria-hidden="true">${t.icon || '🧑'}</span>
        <span class="mk-pick-main"><b>${esc(t.name || 'Customer')}</b>
          <small>${item.filled | 0}/${item.qty | 0} ${esc(r.name || dish.recipeId)}${on ? ' · pinned' : (auto ? ' · going here now' : '')}</small></span>
        ${say}
      </button>`;
  }).join('');
...
function dishHtml(d, k, plan) {
  ...
  const strip = built.length
    ...
    : '';
  /* 📌 WHO THIS PLATE IS GOING TO — the pin if there is one, else the
     matcher's own choice off `State.passPlan()` (handed down by passHtml).
     🔴 IT USED TO DRAW THE PIN ALONE, and the pin is the rare case. Measured in
     the mkserve harness: a burger on the pass, spoken for by a walk-in the
     matcher ranked ahead of the car at the window, rendered `data-for="0"` with
     no badge — the shelf showed an unclaimed plate while the window card said
     "Not ready" for want of exactly that plate. The assignment the sim makes
     for the player has to be legible from the shelf, or "tap the plate to
     reassign" points at nothing. `data-pin` keeps the two cases apart for the
     stylesheet; the badge names the customer in its title/aria-label because a
     15px pill has room for a face and not a name.
     `assignmentOf()` is a pure read and the pin dies with the order it names
     (kitchen.state.js clears a pin whose ticket has gone), so a badge here can
     never outlive its customer. The whole plate is the picker's hit target — a
     56px disc is a comfortable thumb and a separate 15px pin button next to the
     15px bin button is not. */
  const canPick = typeof State.assignDish === 'function';
  const pinTo = canPick ? safe(() => State.assignmentOf(d.id), null) : null;
  const goingTo = pinTo || (plan && plan[d.id]) || null;
  const who = goingTo ? (k.tickets || []).find((t) => t && t.id === goingTo) : null;
  const whoSay = who ? `${pinTo ? 'Reserved for' : 'Going to'} ${who.name || 'them'}${canPick ? ' — tap to reassign' : ''}` : '';
  const badge = who
    ? `<span class="mk-dish-for" data-pin="${pinTo ? 1 : 0}" data-name="${esc(who.name || '')}"
        title="${esc(whoSay)}" aria-label="${esc(whoSay)}" role="img">${who.icon || '🧑'}</span>` : '';
  return `<div class="mk-dish" data-dish="${esc(d.id)}" data-q="${esc(d.quality || 'good')}"
      data-for="${who ? 1 : 0}" data-pin="${pinTo ? 1 : 0}" data-picking="${_pick === d.id ? 1 : 0}"
  ...
  const key = live.map(([slot, c]) => [
    ...
    slot === 'window'
      ? (c.items || []).map((i) => i.recipeId + ':' + i.filled + '/' + i.qty
          ...
          /* who holds the missing plate is drawn on this card, and it can change
             with `filled` standing still — the plate served away to its holder
             leaves 0/1 as 0/1 and the "given to" row with nobody behind it. */
          + '~' + (c.heldLine || '')
...
function pinCardHtml(slot, c) {
  ...
  const items = (c.items || []).map((it) => {
    ...
    /* 📌 WHO HAS IT, ON THE LINE THAT IS MISSING IT. `heldBy` is set by
       drivethru's heldElsewhere() only while a plate of this recipe is on the
       pass and going to somebody else — so the row appears exactly when there
       is something to reassign, and under the line it is about. Its own row
       rather than a span on the item: at ≤819px the item row is nowrap. */
    const held = it.heldBy
      ? `<div class="mk-pin-it mk-pin-held" data-held="1">
        <span aria-hidden="true">↳</span>
        <span class="mk-nm grow">given to ${esc(it.heldBy)} — tap the plate to reassign</span>
      </div>` : '';
  ...
  return `<div class="mk-pin-card" data-slot="${slot}" data-car="${esc(c.carId)}"
      ...
      ${head ? (c.served ? soldBandHtml(c) : `<div class="mk-pin-acts">
        <button class="mk-btn go" data-act="serve-car" data-car="${esc(c.carId)}" ${c.canServe ? '' : 'disabled'}
          ${c.heldLine ? `title="${esc(c.heldLine)}"` : ''}
          >${c.ready ? ('Serve' + (c.worth != null ? ' · ' + esc(fmtCinder(c.worth)) : ''))
            : (c.heldTo ? 'given to ' + esc(c.heldTo) : 'Not ready')}</button>
...
async function doConvoyClaim(id, now) {
  ...
  try {
    ...
    toast(out.crates
      ? `📦 ${out.granted ? fmtNum(out.granted) + ' food unloaded into the stash, and ' : ''}${out.crates} ${plural(out.crates, 'crate', 'crates')} put in your pantry${out.crateNames && out.crateNames.length ? ' — ' + out.crateNames.join(', ') : ''}.`
      : `📦 ${fmtNum(out.granted || 0)} food unloaded into the stash.`);
  } finally {
    _claiming.delete(id);
    paintSheetNow();
    paint();
...
function convoySheet(k) {
  ...
  const lines = ((man && man.lines) || []).map((L) => `
      <div class="mk-man-line">
        ...
        <span class="mk-card-sub nowrap">${L.have} ${L.kind === 'crate' ? (L.have === 1 ? 'crate' : 'crates') + ' in the pantry' : 'on pass'}</span>
  ...
  const manifest = `<div class="mk-man">
      ...
      ${lines || `<div class="mk-empty">Nothing on the pass or in the pantry can ride a convoy. Fries and shakes do not travel, and a supply ships by the full crate — buy or make one on the Supplies sheet.</div>`}
...
function convoyRow(k, c) {
  ...
  return `<div class="mk-row mk-convoy" data-cv="${esc(c.id)}" data-phase="${esc((r && r.phase) || 'transit')}">
      ...
      <div class="mk-row-main">
        ...
        <span class="mk-card-sub" data-eta="1">${(c.dishes | 0) + (c.crates | 0)} ${plural((c.dishes | 0) + (c.crates | 0), 'box', 'boxes')} ·
...
function updateRoutes(t) {
  ...
  for (const el of _root.querySelectorAll('.mk-convoy[data-cv]')) {
    ...
    if (!r) {
      if (eta && c.state === 'transit') setText(eta, `${(c.dishes | 0) + (c.crates | 0)} ${plural((c.dishes | 0) + (c.crates | 0), 'box', 'boxes')} · ${fmtEta((c.arrivesAt || 0) - t)} out`);
      continue;
    }
    if (eta) {
      setText(eta, r.caption
        ? `${(c.dishes | 0) + (c.crates | 0)} ${plural((c.dishes | 0) + (c.crates | 0), 'box', 'boxes')} · ${r.caption}`
        : `${(c.dishes | 0) + (c.crates | 0)} ${plural((c.dishes | 0) + (c.crates | 0), 'box', 'boxes')} · ${fmtEta((c.arrivesAt || 0) - t)} out`);
    }
