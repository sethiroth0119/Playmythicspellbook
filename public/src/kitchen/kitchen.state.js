...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   THE FIX, and note what it deliberately does NOT do:
     • `refreshReady()` runs every tick and only LOOKS. It assigns the plates on
       the pass to the tickets that want them — the car at the window first,
       then the walk-ins, then the queued cars, nearest due date within each
       (the rank is spelled out and measured below) — writes `item.filled` so
       the ticket's pips light up as food lands, and flips a fully-covered
       ticket to 'ready'. Nothing moves.
   ...
   🔴 THE CAR AT THE WINDOW HAS FIRST CLAIM. THEN THE WALK-INS. THEN THE QUEUE.
   Within a rank, nearest due date — even on a ticket it does not complete.
   The board planPass() walks is ordered by that rank and then by `dueAt`:
       0  a drive ticket whose car (K.lane, by carId) has station 'window'
       1  a walk-in — served from the board, it has no geography to wait on
       2  a drive ticket whose car is in the lane but NOT at the window
   (a drive ticket whose car is not in the lane at all is ranked with the queue:
   nobody can be handed its plates either).

   WHY DUE DATE ALONE WAS A DEADLOCK, MEASURED. The board used to be sorted by
   `dueAt` and nothing else, and drivethru.js's serveCar() refuses any car that
   is not at the window — correctly, that gate is the whole §GEOGRAPHY lesson.
   Put the two together: the car at the hatch orders a hot dog, the car BEHIND
   it orders a hot dog and (having queued less) is due sooner, the cook plates
   ONE hot dog for the car in front — and the matcher hands it to the car behind.
   The window car reads "not complete yet", the queued car is refused for not
   being at the window, and the plate sits under the lamp until it rots. Seeds
   7/42/99/1337 through .gauntlet/drive-kitchen-window.mjs (the harness that
   keeps this rule measured), a bot cooking exactly the window car's order:
   starvedFrames 929/896/696/434 (with the counter closed 553/455/140/31),
   first starvation at 129.6s/84.9s/183.1s/176.9s, and on 5 of the 8 runs
   serveCar() refused BOTH cars on the same frame. The only way out
   was the player pinning every plate by hand with assignDish() — an escape
   hatch built for "not that burger, THAT one", pressed into service as the
   only way to feed the customer physically at the window.

   ⚠ THIS IS NOT THE REJECTED "whichever ticket it finishes" SOLVER. That was
   rejected because the board's claim order stopped matching anything the
   player could see. The window car is the one thing the player can ALWAYS
   see: it is drawn at the hatch on the lane card, its ticket is the pinned
   card, and SERVE for it lives on that card. "The car at the window first,
   then nearest due" is a rule the player can hold in their head; it is also
   the rule every real drive-thru runs on. Walk-ins sit above queued cars
   because a walk-in can be served the instant its plates exist, while a
   queued car cannot take a plate until it reaches the hatch — giving it the
   plate first is giving it to nobody.

   Oldest dish first within a line, because food does not improve on the pass. */
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   "The car at the window picks first, then nearest due date, every time" is a
   rule a player can hold in their head and steer with (the rank is in THE PASS
   block above). "The solver rebalanced" is not a rule, it is a
...
function planPass(now) {
  /* 🔴 THE CAR AT THE WINDOW IS PLANNED FIRST. `dueAt` alone starved it — the
     car behind it, due sooner, was handed the plate it could not collect (the
     numbers are in THE PASS block). Rank 0 window car · 1 walk-in · 2 queued
     car, then nearest due within the rank. K.lane belongs to drivethru.js and
     is read here the way serveTicket() already reads it: by carId, and a
     missing lane or car is a queued car, never a throw on the tick path. */
  const lane = Array.isArray(K.lane) ? K.lane : [];
  const rankOf = (tk) => {
    if (!tk.carId) return 1;
    const car = lane.find((c) => c && c.carId === tk.carId);
    return (car && car.station === 'window') ? 0 : 2;
  };
  const rank = Object.create(null);
  const board = K.tickets
    .filter((x) => x.state === 'open' || x.state === 'ready')
    .map((x) => { rank[x.id] = rankOf(x); return x; })
    .sort((a, b) => (rank[a.id] - rank[b.id]) || (_num(a.dueAt) - _num(b.dueAt)));
...
/** Which order a plate is PINNED to, or null. Read-only, for the renderer.
    ⚠ The pin only — the rare case. Where the matcher is sending an unpinned
    plate is `passPlan()` below, and a renderer that draws only this one leaves
    nine plates in ten looking unclaimed. */
...
/**
 * 📌 WHO EVERY PLATE ON THE PASS IS GOING TO — pinned OR planned. Read-only,
 * for the renderer. → { [dishId]: ticketId } for each plate some open order
 * would take right now, off the SAME planPass() the chip and the till read.
 *
 * ⚠ NOT IN CONTRACT §1's EXPORT LIST. Same "say so" as assignDish(): it wants
 * adding to §1 beside `assignmentOf`.
 *
 * 🔴 WHY `assignmentOf()` WAS NOT ENOUGH. It answers only the pin, and a pin is
 * the override — the matcher decides where the plates go the rest of the time,
 * and that decision was drawn nowhere. Measured in the mkserve harness (a car
 * at the window ordering a burger, one burger on the pass, the plate spoken for
 * by somebody else): the plate rendered `data-for="0"` with no badge, and the
 * window card read a bare "Not ready" over food the player could see on the
 * shelf. The one rule the player is told they can steer — "the car at the
 * window picks first, then nearest due, and a pin outranks both" — is only
 * steerable if its answer is visible BEFORE the plate leaves. This is that
 * answer, and it is deliberately the same planPass() result, so the badge on
 * the plate, the fill count on the ticket and the money at the till can never
 * name three different customers.
 *
 * ⚠ ONE planPass() PER CALL, NOT ONE PER PLATE. The pass holds up to 22 plates
 * and the renderer draws them all in one paint; a per-plate read would run the
 * matcher 22 times for one answer. Same caveat as quoteTicket(): planPass()
 * resolves dead pins as it goes (livePin), so this can bump K.rev once when a
 * reserved plate's customer has just left — one repaint, never a loop, because
 * the pin is already cleared by the time the bump lands.
 */
export function passPlan(now) {
  const byDish = Object.create(null);
  const plan = planPass(_num(now, K.now));
  for (const id of Object.keys(plan.byTicket)) {
    for (const line of plan.byTicket[id].lines) for (const d of line.dishes) byDish[d.id] = id;
  }
  return byDish;
}
 ...
 * ⚠ EXCEPT FOR THE CAR AT THE WINDOW. Its ticket picks first (THE PASS block,
 * the rank in planPass), so no later order — not a sooner-due walk-in, not the
 * car behind it ordering the same dish — can take a plate out from under it.
 * While the pass physically holds its order it stays 'ready', and the only
 * thing that can un-ready it is the player pinning one of its plates to
 * somebody else with assignDish(), which is an instruction and outranks it.
 *
...
export function serveTicket(ticketId, now) {
  ...
  /* 🍔 A CORRECT ORDER PAYS IN BOTH CURRENCIES. Cinder went out above; this
     is the food that goes back into the player's own stash, which is what
     makes the restaurant a producer rather than a money press.
     🔴 GATED ON GETTING IT RIGHT. `raws` counts units that went out raw, so
     a ticket with any raw unit earns Cinder (the customer still paid) and no
     food. Rewarding a raw-spam ticket with resources would make tapping
     strictly better than cooking, which is the failure the grade system
     already exists to prevent.
     ⚠ addRes, NOT refundRes: this is CREATED for the player, so it must
       respect the stash cap. A full vault silently keeping nothing is the
       correct behaviour here — refundRes is uncapped and is only ever for
       undoing a deduction this call stack just made. */
  try {
    if (raws === 0 && units > 0) {
      const _food = Math.max(1, Math.round(units));
      if (bridge().addRes) bridge().addRes('food', _food);
    }
  } catch (e) {}
  ticket.state = 'served';
  ticket.paid = payout;
