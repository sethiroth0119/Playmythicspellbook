...
export function pyramid() {
  const D = DEMOG();
  if (!D) return { ok: false, why: 'the city has not counted its people yet, so there is no age profile to draw from' };
  let rep = null;
  try { rep = D.report(); } catch (e) { return { ok: false, why: 'the population count could not be read' }; }
  if (!rep || !rep.ok || !Array.isArray(rep.ages)) {
    return { ok: false, why: (rep && rep.why) || 'nobody has been counted in this city yet' };
  }
...
function targetShares(clk) {
  ...
  if (!(sum > 0)) {
    return { ok: false, why: 'the city has no working-age residents yet — everybody it counts is a child, so there is no age profile to give a named person an age from' };
  }
...
export function seed() {
  const M = CITS();
  const clk = clock();
  if (!M || !clk.ok) return { ok: false, stamped: 0, why: clk.ok ? 'this city keeps no roster of named people' : clk.why };
  const t = now();
  if (!isFinite(t)) return { ok: false, stamped: 0, why: 'this city has no clock the ageing model can read' };

  let roster = null;
  try { roster = M.list(); } catch (e) { roster = null; }
  if (!Array.isArray(roster)) return { ok: false, stamped: 0, why: 'the roster of named people did not answer' };
...
export function ageOf(id) {
  ...
  const t = now();
  if (!isFinite(t)) return { ok: false, why: 'this city has no clock the ageing model can read' };
  const key = String(id);
  if (STAMPS[key] == null) {
    const s = seed();
    if (STAMPS[key] == null) {
      return { ok: false, why: s.why || 'this person is not on the list the ages were shared out across' };
    }
  }
  ...
  if (years < clk.workAge) {
    return { ok: false, why: 'their birth stamp reads an age of ' + years.toFixed(1) +
      ...
      'so it was not written by this city (an edited save, or an older version of the game)' };
  }
...
export function careerOf(id) {
  ...
  const M = CITS();
  if (!M || typeof M.employer !== 'function') return { ok: false, why: 'this city does not record who employs anybody' };
  let emp = null;
  ...
  if (!levels.length) return { ok: false, why: 'the business-size ladder is empty, so there is no ladder to stand on' };
...
export function mortality() {
  ...
  const why = canRetire
    ? (live ? null
            : (mo ? 'the mortality model is loaded but not running, so nothing is calling on it'
                  : 'people can be taken off the roster, but no mortality model is running to do it'))
    : 'this city has no way to take anybody off the roster at all';
  ...
  const note = live
    ? 'named citizens now die: the city retires its OLDEST resident — a death, not somebody ' +
      'moving away — as its own death rate falls due' +
...
export function distribution() {
  ...
  if (!clk.ok) return { ok: false, why: clk.why };
  const M = CITS();
  if (!M) return { ok: false, why: 'this city keeps no roster of named people' };
  seed();
  const t = now();
  let roster = [];
  ...
  const mo = mortality();
  const cause = mo.live
    ? 'the oldest residents are now dying off the roster' +
      (mo.deaths != null && mo.deaths >= 1 ? ' (' + mo.deaths.toFixed(0) + ' deaths so far)' : '') +
      ', so this is lag rather than a one-way slide: the ages are only shared out again when ' +
      'somebody new arrives, and the city’s age profile moves between those moments'
    : 'no named citizen dies, retires off the roster or leaves, so they simply age together and ' +
      'the city’s age profile moves without them — ' + mo.why;
  const drift = withinBound === false
    ? 'the named people no longer match the city’s own age profile: the largest band is out by ' +
      (dev * 100).toFixed(2) + '% against the ' + (bound * 100).toFixed(2) + '% that one person ' +
      'is worth. The share-out met that mark when these people were given their ages; it ' +
      'promises nothing about afterwards, and ' + cause +
