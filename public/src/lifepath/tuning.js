...
export function clockOf(hostDayMin) {
  ...
  if (!D || !D.lifecycle || !D.education) {
    out.why = 'this city’s population rates cannot be read, so there is no way to work out how long a year is';
    return out;
  }
  ...
  if (!(agePerDay > 0) || !(gradPerDay > 0) || !(retLeave > 0) || !(rungs > 0) ||
      !(dayMin > 0) || !(workLife > 0)) {
    out.why = 'one of the rates a year is worked out from is missing or zero ' +
      '(ageing ' + agePerDay + ' a day, graduating ' + gradPerDay + ' a day, retiring ' +
      retLeave + ' a day, ' + rungs + ' education steps, ' + dayMin + ' minutes to the day)';
    return out;
  }
  ...
  for (const g in (D.ages || {})) {
    if (!bands[g]) { out.why = 'this city has an age group the ageing model has no year range for: ' + g; return out; }
  }
  if (!(youngTo > childTo) || !(adultTo > youngTo) || !(seniorTo > adultTo)) {
    out.why = 'the age bands this works out do not run in order, so no age can be read off them';
    return out;
  }
