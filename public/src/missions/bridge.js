...
const NULL_BRIDGE = {
  ...
  cardKeys: () => [],
  isAdmin: () => false,
  /* ✎ no host, no authoring — the map still renders and still plays */
  authorAt: () => false,
  editMission: () => false,
  /* ☁ no shared city without a host — the module then runs its own local
     board, which is exactly what a signed-out or offline player gets. */
  cloud: () => null,
  _null: true,
};
