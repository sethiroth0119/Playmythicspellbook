...
import { screen, select, travelling, openInfluence } from './render.js';
...
const api = {
  ...
  /* 🧍 The Influence tab, for a home outside this module (the Camp's command
     bar): html() renders it, bind(el, onPick) wires its rows, css() ships the
     styles, showOnMap() makes the next map render open on that tab. */
  influence: { html: influenceHtml, bind: bindInfluence, css: influenceCss, showOnMap: openInfluence },
  /* 🚂 is the train mid-journey? Exposed so the map's own animation can be
     asserted from outside rather than inferred from a screenshot. */
  travelling,
  ...
  parse,
  state: S,
  /* 🚂 The train is the player's own — where it is parked gates which
     districts they can deploy to. Exposed because index.html's camp tile and
     any future surface need to ask where the base is without importing an ES
     module they cannot see. */
  train: T,
  /* 🎲 the day-on-the-rails table, exposed for the same reason the train is:
     a surface outside this module may need to ask what a travel day did. */
  encounters: ENC,
  sites: SITES,
  ...
  _install() {
    try {
      ...
      mg.msnPark  = (site) => { const r = T.debugPark(site); try { bridge().render(); } catch (e) {} return r; };
      mg.msnFuel  = (n) => { const r = T.debugFuel(n); try { bridge().render(); } catch (e) {} return r; };
