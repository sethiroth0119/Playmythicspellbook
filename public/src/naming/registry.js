...
export class NameRegister {
  ...
  /* 🚚 A building moved: its name — and whether the player typed it — goes
     with it. Nothing is generated and nothing is saved here; the caller's
     saveSoon() writes the register under the new key on the next save. */
  moveKey(from, to) {
    if (!from || !to || from === to) return false;
    const v = this.names.get(from);
    if (v == null) return false;
    this.names.set(to, v); this.names.delete(from);
    if (this.custom.has(from)) { this.custom.delete(from); this.custom.add(to); }
    return true;
  }
