...
(function () {
  ...
  const APPS = [
    ...
    { id: 'ledger', name: 'Ledger',        ico: '📒', hue: '#3fa46b', sub: 'Every Cinder and AZA in and out of your wallet' },
    ...
    { id: 'mayor',  name: 'Node Manager',  ico: '🏛', hue: '#c9a227', sub: 'Your client cities · terms · what each one pays' },
  ...
  /* ── 📒 Ledger ── every Cinder AND every AZA in and out of the wallet, newest first.
  ...
  /* 🪙 AZA (asked for: "Add AZA coin to the phone ledger … show when AZA coin is
     added or spent"). AZA rows come from window.MythicAzaHistory.walletLedger()
     — the player's own wallet_ledger rows for resource 'sovereigns', labelled
     by the SAME classify() the AZA history view uses, so the words cannot drift
     between the two. They are NOT taken from serverLedger(): get_my_ledger() is
     every resource newest-first and Cinder is ~99.9% of it, so the AZA rows
     fall off its 200-row page (see azahistory walletLedger).
     ⚠ AZA is never added into the Cinder sums — one AZA is not one Cinder. The
     header carries two separate lines.
     ⚠ "This device" has no AZA: there is no local AZA diary. Every AZA move is
     a server RPC (sov_charge / _sov_apply / …) that writes its own ledger row,
     so the account record IS the complete AZA list; the tab says so. */
  let ledgerMode = 'server', ledgerFilter = 'all', ledgerSeq = 0;
  const azaSeam = () => { try { const A = window.MythicAzaHistory; return (A && typeof A.walletLedger === 'function') ? A : null; } catch (e) { return null; } };
  /* A refresh repaints on every Cinder diary write; the AZA read is reused for
     15 s so a burst of +1 Cinder rows does not become a burst of selects. */
  let azaCache = null, azaCacheAt = 0;
  function loadAza() {
    const A = azaSeam();
    if (!A) return Promise.resolve({ ok: false, reason: 'missing', rows: [] });
    if (azaCache && Date.now() - azaCacheAt < 15000) return Promise.resolve(azaCache);
    return Promise.resolve().then(() => A.walletLedger()).then((r) => {
      const v = r || { ok: false, reason: 'failed', rows: [] };
      if (v.ok) { azaCache = v; azaCacheAt = Date.now(); }
      return v;
    }, () => ({ ok: false, reason: 'failed', rows: [] }));
  }
  function azaSumsHtml(rows, loaded) {
    if (!loaded) return '<span class="lbl">🪙</span><span class="dim">AZA loading…</span>';
    let inA = 0, outA = 0; (rows || []).forEach((e) => { if (e && e.cur === 'aza') { if (e.d > 0) inA += e.d; else outA -= e.d; } });
    return '<span class="lbl">🪙</span><span class="in">+' + fmt(inA) + ' AZA received</span><span class="out">−' + fmt(outA) + ' AZA spent</span>';
  }
  function ledgerRowHtml(e) {
    if (e.cur === 'aza') {
      return '<div class="mgp-ldr aza ' + (e.d > 0 ? 'in' : 'out') + '" data-cur="aza"><span class="ic">🪙</span>' +
        '<div class="mgp-ldb"><b>' + esc(e.r) + '</b><small><i class="mgp-cur">AZA</i> ' + esc(when(e.t)) + (e.b != null ? ' · after 🪙 ' + fmt(e.b) : '') + '</small></div><span class="d">' + signed(e.d) + ' AZA</span></div>';
    }
    return '<div class="mgp-ldr ' + (e.d > 0 ? 'in' : 'out') + (e.k === 'sync' ? ' sync' : '') + '" data-cur="cinder"><span class="ic">' + (e.k === 'bank' ? '🏦' : e.k === 'sync' ? '☁' : e.d > 0 ? '💰' : '💸') + '</span>' +
      '<div class="mgp-ldb"><b>' + esc(e.r) + (e.n > 1 ? ' <i class="mgp-n">×' + e.n + '</i>' : '') + '</b><small>' + esc(when(e.t)) + (e.b != null ? ' · after 🔥 ' + fmt(e.b) : '') + '</small></div><span class="d">' + signed(e.d) + '</span></div>';
  }
  /* The merge is published (MythicHandset.ledgerMerge) so _azaledger_smoke.mjs
     can prove the filter and the no-leak rule without a DOM. */
  function ledgerMerge(cinderRows, azaRows, filter) {
    const c = filter === 'aza' ? [] : (cinderRows || []).filter((e) => e && e.cur !== 'aza');
    const a = filter === 'cinder' ? [] : (azaRows || []).filter((e) => e && e.cur === 'aza');
    return c.concat(a).sort((x, y) => (+y.t || 0) - (+x.t || 0));
  }
  function renderLedger(pane) {
    ...
    const fbtn = (id, lbl) => '<button type="button" data-lf="' + id + '" class="' + (ledgerFilter === id ? 'on' : '') + '">' + lbl + '</button>';
    pane.innerHTML =
      '<div class="mgp-ldg-head"><div class="mgp-balrow"><div class="mgp-bal"><small>Wallet</small><b>🔥 ' + fmt(st.walletCinder) + '</b></div>' +
        '<div class="mgp-bal aza"><small>AZA</small><b>🪙 ' + fmt(st.walletAza) + ' AZA</b></div></div>' +
        '<div class="mgp-sums"><span class="lbl">🔥</span><span class="in">+' + fmt(inSum) + ' received</span><span class="out">−' + fmt(outSum) + ' spent</span></div>' +
        '<div class="mgp-sums aza" id="mgp-aza-sums">' + azaSumsHtml(azaCache && azaCache.rows, !!azaCache) + '</div></div>' +
      ...
      '<div class="mgp-seg sm">' + fbtn('all', 'All') + fbtn('cinder', '🔥 Cinder') + fbtn('aza', '🪙 AZA') + '</div>' +
    ...
    pane.querySelectorAll('[data-lf]').forEach((b) => { b.onclick = () => { ledgerFilter = b.dataset.lf; renderLedger(pane); }; });
    const full = $('mgp-ldg-full'); if (full) full.onclick = () => openApp('bank');
    const list = $('mgp-ldg'), note = $('mgp-ldg-note');
    const live = () => seq === ledgerSeq && app === 'ledger';
    const paint = (list0, src) => {
      if (!list) return;
      if (!list0.length) {
        const why = ledgerFilter === 'aza'
          ? (src === 'server' ? 'Every AZA you buy, receive or spend shows here.' : 'AZA is not kept on this device — every AZA move is written by the server. Switch to Account record.')
          : (src === 'server' ? 'Server-recorded Cinder moves show here.' : 'Every Cinder you earn or spend from now on is written here.');
        list.innerHTML = '<div class="mgp-empty">📒<br>Nothing yet.<br><small>' + why + '</small></div>'; return;
      }
      list.innerHTML = list0.slice(0, 250).map(ledgerRowHtml).join('');
    };
    const noAza = (a) => a.reason === 'signin' ? 'Sign in to see your AZA moves.' : a.reason === 'missing' ? 'The AZA record is not available on this build.' : 'Could not load your AZA moves.';
    /* the AZA sums line is filled whichever tab is showing */
    const azaP = loadAza();
    azaP.then((a) => { if (!live()) return; const s = $('mgp-aza-sums'); if (s) s.innerHTML = a.ok ? azaSumsHtml(a.rows, true) : '<span class="lbl">🪙</span><span class="dim">' + esc(noAza(a)) + '</span>'; });
    if (ledgerMode === 'wallet') {
      paint(ledgerFilter === 'aza' ? [] : rows, 'wallet');
      if (note) note.textContent = (ledgerFilter === 'aza' ? 'No AZA diary on this device' : rows.length + ' Cinder entries on this device · newest first') + (ledgerFilter === 'all' ? ' · AZA: Account record' : '');
    } else {
      if (list) list.innerHTML = '<div class="mgp-empty">Loading the server audit…</div>';
      const cP = ledgerFilter === 'aza' ? Promise.resolve({ ok: true, rows: [] }) : B.serverLedger();
      Promise.all([cP, azaP]).then(([r, a]) => {
        if (!live() || ledgerMode !== 'server') return;
        const aRows = a.ok ? a.rows : [];
        const azaWord = a.ok ? '' : ' · ' + noAza(a).replace(/\.$/, '');
        if (ledgerFilter === 'aza') {
          if (!a.ok) { if (list) list.innerHTML = '<div class="mgp-empty">🪙<br>' + esc(noAza(a)) + '</div>'; return; }
          paint(ledgerMerge([], aRows, 'aza'), 'server'); if (note) note.textContent = aRows.length + ' AZA server rows · tamper-proof'; return;
        }
        if (!r.ok && r.reason === 'missing') {
          paint(ledgerMerge(rows, ledgerFilter === 'all' ? aRows : [], ledgerFilter), 'wallet');
          if (note) note.textContent = rows.length + ' entries on this device · the account record is not installed yet' + (ledgerFilter === 'all' ? azaWord : ''); return;
        }
        if (!r.ok && (ledgerFilter === 'cinder' || !aRows.length)) { if (list) list.innerHTML = '<div class="mgp-empty">' + (r.reason === 'signin' ? 'Sign in to read the server audit.' : 'Could not load the server audit.') + '</div>'; return; }
        const cRows = r.ok ? r.rows : [];
        paint(ledgerMerge(cRows, aRows, ledgerFilter), 'server');
        if (note) note.textContent = (ledgerFilter === 'all' ? cRows.length + ' Cinder + ' + aRows.length + ' AZA' : String(cRows.length)) + ' server rows · tamper-proof' + (ledgerFilter === 'all' ? azaWord : '') + (!r.ok ? ' · Cinder audit unavailable' : '');
      });
    }
  }
  ...
  function renderMayor(pane) {
    const D = mdSeam();
    if (!D) { pane.innerHTML = '<div class="mgp-empty">🏛<br>The Node Manager dashboard is not available on this build.</div>'; return; }
    if (!D.signedIn()) { pane.innerHTML = '<div class="mgp-empty">🏛<br>Sign in to see your contracts.<br><small>Node Manager contracts live on your account.</small></div>'; return; }
    mdEnsure(pane, D);
    ...
    pane.innerHTML =
      '<div class="mgp-ldg-head"><div class="mgp-bal"><small>Earned as Node Manager</small><b>🔥 ' + D.fmt(total) + '</b>' +
    ...
    if (!rows.length) {
      box.innerHTML = '<div class="mgp-empty">🏛<br>' + (
        ...
        : 'You are not a Node Manager anywhere yet.<br><small>Apply in the Node Manager Hall — owners hire from there.</small>'
      ) + '</div>';
    ...
    pane.innerHTML =
      '<div class="mgp-cx-tk">' +
        ...
        '<div class="mgp-ldg" style="margin-top:10px">' +
          ...
          '<div class="mgp-ldr"><span class="ic">📅</span><div class="mgp-ldb"><b>Node Manager since</b><small>' + esc(mdAgo(r.started_at)) + '</small></div></div>' +
        '</div>' +
  ...
  const CSS = [
    ...
    /* 🪙 AZA beside Cinder: its own balance, its own sums line, its own row tint */
    '.mgp-balrow{display:flex;justify-content:space-between;align-items:flex-end;gap:10px;}.mgp-bal.aza{text-align:right;}.mgp-bal.aza b{color:#f2d98a;font-size:17px;}',
    '.mgp-sums .lbl{flex:none;width:16px;}.mgp-sums .dim{color:#9d907a;}.mgp-sums.aza{margin-top:-3px;}',
    '.mgp-seg.sm{margin-top:2px;}.mgp-seg.sm button{padding:4px 4px;font-size:10.5px;}',
    '.mgp-ldr.aza{background:linear-gradient(90deg,rgba(242,217,138,.07),transparent 60%);}.mgp-ldr.aza .d{font-size:12px;}.mgp-cur{font-style:normal;font-weight:700;font-size:9px;letter-spacing:.08em;color:#1a1208;background:#f2d98a;border-radius:3px;padding:0 3px;margin-right:2px;}',
