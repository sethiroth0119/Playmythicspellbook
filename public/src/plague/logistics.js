  ...
  return (list || [])
    ...
    .map((c) => {
      ...
      return {
        ...
        level: c.level | 0 || 1,
        mine: !!c.mine,
        /* Marked so the panel can say what it is; the ranking itself does not
           special-case it — a contract haulier competes on integrity like
           anything else, and loses to any staffed player operation. */
        npc: String(c.id || '').indexOf('npc:') === 0,
