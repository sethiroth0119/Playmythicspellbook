...
export function pressureOf(host) {
  try {
    ...
    /* 💊 PROPHYLAXIS. Medicine the city's clinics actually SOLD to NPCs
       ...
       arrives through the host, is optional, and is 0 when no hospital exists —
       so a city with no Medical Corporation behaves exactly as it did.
       It is a MULTIPLIER, which is what keeps it honest at both ends: it
       cannot lift a clean city off zero, and it cannot by itself take a filthy
       one to zero (the share is capped inside pharma.js). */
    let proph = 0;
...
/* 🔴 A SAMPLE IS NOT AN OUTBREAK. `sample` strains are reference isolates the
   player pulled off the lab's register to work on; nobody in the city has
   them. They live in st.strains because that is where the bench, the batch
   records and the shipping manifest all look a strain up by id — but every
   consumer of activeStrains() is asking "what is loose in this city", and the
   honest answer excludes them. Without this filter the city's Vital Signs
   would report an outbreak of a virus that has infected nobody. */
export function activeStrains(st) {
  return st.strains.filter((s) => s && !s.curedAt && !s.sample);
}
/* The register's own list: what is on the shelf to study, as opposed to loose. */
export function samples(st) {
  return st.strains.filter((s) => s && !s.curedAt && s.sample);
}
...
export function introduce(host, st, strain, seedCount, why) {
  ...
  const known = strainById(st, strain.id);
  if (!known) st.strains.push(strain);
  /* 🔴 A SAMPLE THAT ACTUALLY ARRIVES STOPS BEING A SAMPLE. The player can
     pull Ashlung off the register to study it; if Ashlung then genuinely
     reaches the city, introduce() finds the id already present and does NOT
     push a second copy — so without this line the real outbreak would inherit
     the sample flag, be filtered out of activeStrains(), and infect people
     invisibly while every panel insisted the city was clean. */
  else if (known.sample) { delete known.sample; delete known.sampledAt; }
