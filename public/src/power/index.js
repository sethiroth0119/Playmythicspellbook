...
const API = {
  ...
  mount(h) {
    ...
    try {
      ...
      /* 🗼 …and TELL the player the connector is there. The pole has stood on
         the verge since lines.js shipped, but the only text describing it lives
         in the line tool's hint bar, which appears once the tool is already
         armed — the explanation sat behind the action it exists to explain.
         ⚠ AFTER Lines.mount, never before: the notice's button arms that tool,
           and offering it a beat earlier would hand the player a control that
           does nothing. Its own try/catch as well, because a welcome card is
           the last thing that may cost anyone their city. */
      try {
        Welcome.maybeShow(h, () => { try { Lines.setArmed(true); } catch (e) {} });
      } catch (e) { warnOnce('power intro failed: ' + (e && e.message)); }
      mounted = true;
      return true;
  ...
  solve(snapshot) {
    ...
    try {
      ...
      const s = Grid.solve({ grid: snapshot.grid, tiles: snapshot.tiles, plants: snapshot.plants,
                             ...
                             // 🕕 The clock, for the household demand peak. Passed
                             //    through untouched; grid.js treats a missing or
                             //    non-finite hour as "no peak", so a host that
                             //    predates this hands over nothing and changes
                             //    nothing.
                             hour: snapshot.hour,
                             metered, link,
  ...
  lines: {
    ...
    verify: () => Lines.verify(),
    /* The first-entry notice, exposed so a driver can ask "would a new player
       see this?" without clearing a real player's storage by hand. */
    intro: {
      show: (h, cb) => Welcome.maybeShow(h || host, cb || (() => Lines.setArmed(true))),
      seen: (h) => Welcome._seen(h || host),
      reset: (h) => Welcome._reset(h || host),
...
function refresh() {
  ...
  /* ⚡ THE CABLES SHOW WHILE THE LINE TOOL IS ARMED — the water rule, verbatim.
     ----------------------------------------------------------------------------
     🔴 THE REPORT, twice: "make it where it connects just like the water pipes
        do the exact same thing — when lines connect they surge; right now it is
        not working correctly." The surge was built, and it was invisible: it
        paints on the electricity-map overlay, which only appears when the
        player opens that panel and ticks a layer. /src/water/netui.js shows its
        mains plane on `armed || (layerOn && count > 0)` — arm the MAINS tool and
        the network lights up in the world, which is the picture in their
        screenshot.
     So arming the POWER LINES tool turns the LV and HV layers on for as long as
     it is armed. A player drawing a line now sees what is carrying current
     while they draw it, which is the whole diagnostic: a run that reaches the
     grid surges, and one that does not stays dark.
     ⚠ It does not TOUCH Panel.layers — the panel's own checkboxes are the
       player's setting and must survive putting the tool away. This merges a
       view on top for the duration, which is why it is a spread and not an
       assignment. */
  let _layers = Panel.layers;
  try {
    if (Lines.isArmed && Lines.isArmed()) _layers = { ...Panel.layers, lv: true, hv: true };
  } catch (e) {}
  if (host) Overlay.sync(state && state.ok ? state : null, _layers, { ...host, ...t });
}
...
try {
  if (typeof window !== 'undefined') {
    ...
    /* 🔬 DRIVER SEAM — the painter and the tuning table, for
       .gauntlet/drive-power-surge.mjs. node-city's own `window.__nc` block
       states the reason and it is the same one here: this is a MODULE, so
       nothing in it is on window and a driver cannot reach the canvas that
       would prove the surge animates. Two handles, both read-only in practice:
       Overlay is already a live singleton the host drives every tick, and
       POWER is the tuning table a driver has to be able to switch the feature
       OFF in to have a control at all. */
    window.__pwOverlay = Overlay;
    window.__pwTuning = POWER;
  }
} catch (e) {}
