...
/* Occupancy, asked of the HOST and never cached. `tileAt` closes over
   ...
   🔴 THIS IS NO LONGER A PLACEMENT REFUSAL — IT IS A POLE RULE.
      It used to be both, and that was the defect: `quote()` pushed every
      occupied cell into `blocked` and `stateOf()` painted it red, so a cable
      could not cross a ROAD. Not "should not" — could not. A city is a grid of
      roads, so any building more than one block from the connector was
      unreachable by construction, and the network the player was told to draw
      could not be drawn. The report was "make it where the lines connect even
      on the roads and buildings just like the pipes do", and pipes are the
      precedent in this repo: /src/water/network.js lays over ANY tile and never
      asks tileAt at all.
      What survives is the part that was always true — you cannot stand a POLE
      inside a building. So an occupied cell now CONDUCTS and CARRIES WIRE, and
      only its pole is skipped; the wire spans from the poles either side, which
      is what a real line does over a road. The wire builder already spans
      pole-less cells (`L.pole.everyOther` has always made poles sparse), so
      this costs no new geometry.
   ⚠ `inDomain` is untouched. Off the plate and past the one-cell apron is still
     refused — that is the earthworks, not a building. */
...
export function quote(run, erase) {
  ...
  for (const c of run) {
    ...
    /* 🚫 THE OCCUPANCY REFUSAL USED TO BE HERE and it is deliberately gone —
       see occupied()'s header. A cell with a road or a building on it is a
       legal, chargeable span now; `blocked` is left in the return shape because
       an OFF-DOMAIN cell still fills it and every caller reads it. */
    add.push(c);
  }
...
function changed() {
  ...
  /* 🙂 PLOT MOOD. O(1) — the module only records that a recompute is owed;
     see its header. Guarded: absent means the glyph layer is not in this
     build, never that this edit did not happen. */
  try { if (typeof window !== 'undefined' && window.MythicPlotMood) window.MythicPlotMood.invalidate('power-lines'); } catch (e) {}
}
...
function rebuild() {
  ...
  for (const c of list) {
    ...
    /* 🏗 NO POLE INSIDE A BUILDING OR ON A CARRIAGEWAY. Now that a cable may
       cross an occupied cell (see occupied()), this is the half of the old rule
       that was always right: a cylinder planted in the middle of a house reads
       as a bug, and one in the road reads as an obstruction. The WIRE still
       crosses — it is built from spans between the cells that do carry poles,
       which is exactly how a line crosses a street in the world. */
    if (occupied(c.x, c.z)) continue;
...
function stateOf(c, erase, room) {
  ...
  // Occupied is no longer blocked — the preview must agree with quote(), or the
  // player is shown a red cell that then gets built and charged for.
  if (!inDomain(c.x, c.z)) return 'blocked';
...
async function mountTool() {
  ...
  if (!doc.getElementById('npl-style')) {
    ...
    st.id = 'npl-style';
    st.textContent =
      '#npl-btn{border:1px solid var(--edge,rgba(212,175,55,.3));background:#1c1813;color:var(--bone,#e9e0cc);' +
      ...
      'background:var(--panel,rgba(22,19,15,.92));border:1px solid var(--edge,rgba(198,160,74,.34));' +
      'border-radius:6px;padding:6px 11px;color:var(--bone,#e9e0cc);font-size:11.5px;line-height:1.45;' +
