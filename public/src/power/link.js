...
export function read() {
  ...
  if (!O || typeof O.state !== 'function') {
    ...
    out.fix = 'Nothing you can build fixes this — part of the game did not load. Reload the city; if it persists, the highway, the interchange and the trade gate are all missing too, not just this meter.';
    return out;
  }
  ...
  if (!E || typeof E.utilityTrade !== 'function' || !E.ready || !E.ready()) {
    ...
    out.fix = 'Nothing to build: this city’s economy has not started yet. The link comes back on its own the moment it does.';
    return out;
  }
  out.priced = true;
