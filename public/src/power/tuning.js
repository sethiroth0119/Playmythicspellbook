...
export const POWER = {
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════════════════ */
  demand: {
    /* 🕕 THE DAILY PEAK — a multiplier on the HOUSEHOLD term, by hour.
       A grid sized to the average fails at the peak, and that failure is the
       reason to build a second plant or a bigger one. Demand here used to be
       flat around the clock, so "have I got enough power" was a question a
       player answered once and never revisited.
       THE SHAPE IS THE ORDINARY ONE, so it can be learned rather than merely
       suffered: a deep trough while the city sleeps, a plateau through the
       working day, and a real evening peak from 18:00 when people are home
       with the lights on. 1.0 is "exactly what this city drew before this
       table existed", so the working day is unchanged and the peak is a
       genuine addition rather than a re-scaling of everything.
       🔴 IT PEAKS WHERE SOLAR DIES, deliberately: sunset is 18:00 in
          POWER.plants.solar and the peak starts at 18:00 here. A solar city
          therefore meets its worst hour with its worst output — which is the
          most legible possible argument for storage, or for a second kind of
          plant, and it is made by the clock rather than by a warning.
       Set `on: false` to go back to flat demand. */
    peak: {
      on: true,
      byHour: {
        0: 0.72, 1: 0.68, 2: 0.66, 3: 0.66, 4: 0.68, 5: 0.75,
        6: 0.90, 7: 1.00, 8: 1.00, 9: 1.00, 10: 1.00, 11: 1.00,
        12: 1.05, 13: 1.00, 14: 1.00, 15: 1.00, 16: 1.05, 17: 1.15,
        18: 1.35, 19: 1.40, 20: 1.38, 21: 1.28, 22: 1.10, 23: 0.90,
      },
    },
  ...
  storage: {
    ...
    /* 🔋 …AND THE BUILDABLE BATTERY. The idea rejected at the top of this block
       was a flat city-wide buffer INDEPENDENT of what the player built — the
       one that made the first Power Station carry every peak. This one is not
       independent of it: capacity is BOUGHT, one Grid Battery at a time, so a
       city with a bigger buffer is a city that spent more on the buffer.
       Sized so one battery carries a small load for a couple of hours, not a
       whole night. A solar city that wants to run after dark builds several —
       and still has to generate the daytime surplus to fill them, which is the
       decision the building exists to create. */
    perBatteryUnitMin: 600,
    dischargePerMinPerBattery: 6.0,
  },
  ...
  transmission: {
    enforce: true,
    /* 🔌 A GENERATOR MUST REACH THE GRID CONNECTOR TO FEED THE CITY.
       The Highway Connection modal has always told the player to "drag power
       lines from the pole on the north-west verge to generators, turbines,
       substations", and until this round that was decoration — an unwired
       turbine hall produced exactly as much as a wired one, so there was no
       reason to draw a single span and the whole tool was optional scenery.
       With this true, an unlinked plant reports `out: 0` and a `why` the panel
       prints; it is not deleted and it still seeds its own neighbours, so
       islanding a generator costs the city its OUTPUT and not the building.
       🔴 IT IS ANDed WITH `enforce` AND THE PER-CITY `wired` LATCH, so it can
          never touch a save written before transmission mattered. It also does
          nothing at all when the host supplies no connector seeds — a headless
          test, or a host too old to have /src/power/lines.js, must not lose its
          power because a pole it has never heard of is unreachable.
       Set false for a balance round that wants transmission to matter without
       demanding one particular topology. */
  ...
  lines: {
    ...
    wire: {
      ...
      /* 🔴 WIDENED AND LIT, and the old numbers are worth keeping in view:
         `w: 0.028` with `col: 0x2b2b30` was a realistic cable — a few
         millimetres of near-black wire. The note above already conceded that a
         REAL cable "would vanish at the aerial camera this game is played
         from", and 0.028 was chosen as the narrowest that still reads. It was
         too narrow: the report was "the power lines are not doing the same
         thing as the pipes… I want the same for the lines", beside a screenshot
         of the water main — a fat, bright cyan run visible across the whole
         city — next to a power line that from the same camera is a row of dark
         poles with nothing legible between them.
         A water main is a painted band on a plane; this is a ribbon in the
         world at crossarm height, so it cannot be the same object. It CAN be
         the same READ: wide enough to follow at a glance and lit rather than
         shadowed. MeshBasicMaterial is unlit, so this colour is what shows —
         gold, the same family as the LV cable in the overlay and the surge that
         runs along it, so the map view and the world agree about what a power
         line looks like.
         ⚠ NOT WHITE, and not a neon. It sits over grass, road and rooftops at
           night; a pure white ribbon blooms into the street lights and stops
           reading as a wire at all. */
      w: 0.085,
      col: 0xffd24a,
    },
    ...
    say: {
      /* ⚠ RETIRED, AND KEPT AS A TOMBSTONE. This refusal no longer exists — a
         line crosses roads and buildings now, exactly as a water main does, and
         quote() stopped filling `blocked` with occupied cells. The string stays
         because `say` is read by key and a missing key would print `undefined`
         if any path still reached for it; it should never be shown again, and
         if it is, something re-added a refusal this round deliberately removed. */
      occupied: 'A line may cross a road or a building — if you are seeing this, a placement rule came back.',
  ...
  overlay: {
    /* ⚡ THE SURGE — the moving light on a cable that is carrying current.
       Deliberately the SAME THREE NUMBERS /src/water/tuning.js gives its pipe
       flow (`flowDash`, `flowSpeed`, and a live/not-live rule), because the two
       animations must read as one idea in two colours: blue water moving,
       yellow current moving. A player who has learned one has learned the
       other, and matching the cadence is most of that.
       `on: false` is the kill switch — the picture goes back to static cables
       and the rAF loop never starts.
       ⚠ speed is TILES PER SECOND and dash is TILES. Water runs 1.6 and 0.9;
         current is faster than water in every intuition anyone has, so it runs
         a little quicker and no further apart. */
    surge: { on: true, speed: 2.2, dash: 0.9 },
  ...
  col: {
    ...
    /* ⚡ THE SURGE COLOURS — brighter than the cable they run on, because a
       dash the same value as its wire reads as texture rather than movement.
       LV surges near-white gold: the request was "a yellow surge just like the
       pipes surge blue", and on a #ffd24a cable the thing that reads as
       ELECTRICITY is a hotter yellow moving over a cooler one. HV keeps the
       cyan family so the two voltages stay told apart while both are alive. */
    lvSurge:     '#fff8d0',
    hvSurge:     '#dffbff',
