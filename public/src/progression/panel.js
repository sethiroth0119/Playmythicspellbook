...
#ncprog .pgcard{width:min(1180px,calc(100vw - 40px));height:min(760px,calc(100vh - var(--topbarh) - 40px));
  margin-top:var(--topbarh);background:var(--panel-solid);border:1px solid var(--edge);border-radius:6px;
...
#ncprog .pgtab:hover{color:var(--bone);border-color:rgba(198,160,74,.55);}
...
#ncprog .pgnode{position:absolute;width:${NODEW}px;min-height:${NODEH}px;border-radius:5px;padding:7px 9px;
  border:1px solid var(--edge);background:rgba(28,24,19,.92);cursor:pointer;box-sizing:border-box;
  transition:border-color .12s,background .12s;}
#ncprog .pgnode:hover{border-color:rgba(198,160,74,.5);}
...
#ncprog .pglvl{color:#ffb86b;}
#ncprog .pgbuy{margin-left:12px;padding:5px 11px;border-radius:5px;cursor:pointer;font-family:'Cinzel',serif;font-size:10px;
  letter-spacing:.12em;text-transform:uppercase;border:1px solid rgba(127,224,160,.5);background:rgba(40,120,70,.22);color:#9fe0b0;}
#ncprog .pgbuy:hover{background:rgba(40,120,70,.36);}
...
#ncprog .pgrow{display:flex;gap:12px;align-items:flex-start;padding:10px 12px;border:1px solid var(--edge);
  border-radius:5px;margin-bottom:8px;background:rgba(28,24,19,.72);}
...
#ncprog .pgrsrc{font-size:10.5px;color:#8d8272;margin-top:5px;font-style:italic;}
...
#ncprog .pgsrcs{margin-top:16px;border-top:1px solid var(--edge);padding-top:10px;font-size:11px;color:#8d8272;line-height:1.6;}
...
export function makePanel(api, host) {
  ...
  function onClick(ev) {
    ...
    const buy = t.closest('[data-act="buypts"]');
    if (buy) { api.buyPoints(); return; }
  }
    ...
    for (const n of ns) {
      ...
      for (const r of n.req) {
        ...
        lines += '<path d="M' + x1 + ' ' + y1 + ' H' + mx + ' V' + y2 + ' H' + x2 + '" fill="none" stroke="' +
                 (lit ? 'rgba(76,175,122,.75)' : 'rgba(198,160,74,.32)') + '" stroke-width="' + (lit ? 2 : 1.4) + '"/>';
      }
    }
    let cards = '';
    for (const n of ns) {
      ...
      cards += '<div class="pgnode st-' + n.state + (sel === n.id ? ' sel' : '') + '" data-id="' + esc(n.id) +
        ...
        '<div class="pgnm">' +
          ...
          (n.level > 1 ? '<span class="pglvl" title="Requires city level ' + n.level + '">🏙 Lv ' + n.level + '</span>' : '') +
  ...
  function drawDetail(v) {
    ...
    if (n.level > 1) {
      h += '<div class="pgsec"><div class="pgsh">City level</div>';
      const L = n.levelState;
      if (L && L.met === true) h += '<div class="pgsdesc" style="font-size:12px;margin:0;color:#8fd8ae">✔ Level ' + n.level + ' needed — this city is level ' + L.have + '.</div>';
      else if (L && L.met === false) h += '<div class="pgsdesc" style="font-size:12px;margin:0;color:#e08a80">Level ' + n.level + ' needed — this city is level ' + L.have + '. The city levels up on the Cinder its buildings earn; the bar is on the Vital Signs card.</div>';
      else h += '<div class="pgsdesc" style="font-size:12px;margin:0">Level ' + n.level + ' needed. Nothing here can read the city level this session, so it is not enforced.</div>';
      h += '</div>';
    }
  ...
  function drawMilestones(v) {
    ...
    for (const m of v.milestones) {
      ...
      h += '<div class="pgrow ' + cls + '">' +
        ...
        /* ⚠ `m.source` IS THE PLAYER'S CAPTION AND ONLY THAT. It used to be
           the maintainer's grep string as well, so this line printed
           "Population — node-city cityPop(), handed over as ctx.pop()" at a
           player. The line stays — a milestone that stops saying where its
           number came from is the rule in milestones.js being broken, not
           fixed — and milestones.js now hands over prose. The grep string
           lives in `trace`, which report() never copies this far. */
  ...
  function drawAchievements(v) {
    let h = '<div class="pgcat">🏆 Achievements</div>' +
      '<div class="pgcatb">Things this city did. They pay no development points, and the card rewards for them ' +
      'are not switched on yet — see the panel on the right.</div>';
    for (const a of v.achievements) {
      ...
      h += '<div class="pgrow ' + cls + '">' +
        ...
        /* Same split as the milestone caption above, and the `trigger:` label
           went with it: it announced a developer field to a player, and what
           followed it was `builtCount() ≥ 1`. `a.how` is prose now and reads
           as its own sentence, so the prefix has nothing left to introduce. */
        '<div class="pgrsrc">' + esc(a.how) + '</div></div>' +
        '<div style="text-align:right">' +
          (st.ok
  ...
  function draw() {
    ...
    if (tab === 'dev') {
      ...
      const pk = v.pack || { usd: 5, points: 2 };
      sideHtml = '<div class="pgsh">Points</div>' +
        '<div class="pgsdesc">Development points come from the milestones on the left, and from the bank: ' +
        pk.points + ' ⬡ for $' + pk.usd + ', held by your account and spendable in any of your cities. ' +
        'Milestone points are spent first.</div>' +
        ...
        '<div class="pgul">' + v.points.spent + ' ⬡ spent <small>· on ' + v.unlockedCount + ' nodes' + (v.points.bankSpent ? ', ' + v.points.bankSpent + ' of them from the bank' : '') + '</small></div>' +
        '<div class="pgul">' + v.points.available + ' ⬡ available <small>· ' + v.points.own + ' from milestones + ' + v.points.bank + ' in the bank</small></div>' +
        (v.cityLevel != null ? '<div class="pgul">🏙 City level ' + v.cityLevel + ' <small>· of ' + v.cityLevelMax + ' — some nodes need a level</small></div>' : '') +
        (v.canBuy ? '<button class="pgbtn2" data-act="buypts">💳 Buy ' + pk.points + ' ⬡ for $' + pk.usd + '</button>' : '') +
        '</div>' +
      ...
      /* 🔴 REWORDED, NOT DELETED. This is the ONLY place a player is told that
         the card rewards are not live yet; every row on the left still says
         "reward: reserved", and without this panel that phrase reads like a
         card they have earned and cannot find. The old copy said it in the
         module's own vocabulary — onAchievement(fn), "the field a card id
         goes in" — which is the bug this batch is fixing, so it says the same
         two facts in the game's nouns instead. The seam itself is unchanged
         and documented in milestones.js for whoever wires it up. */
      sideHtml = '<div class="pgsh">About rewards</div>' +
        '<div class="pgsdesc">Achievements are meant to unlock cards one day. That part is not switched on yet: your ' +
        'card collection lives elsewhere in the game, and this screen can only report the moment an achievement is earned.</div>' +
        '<div class="pgsec"><div class="pgul">Earning one is announced as it happens <small>· once, and never again on a later visit</small></div>' +
        '<div class="pgul">No card is handed over yet <small>· which is why every row reads "reward: reserved"</small></div></div>';
    }

    /* 🔴 THE SCROLL FIX. This panel is redrawn from scratch every 4s tick
       and on every click, and innerHTML resets every scroll position inside
       it — so the Milestones list "shot back to the top" the moment a player
       scrolled down. Remember the three scrolling panes, and skip the write
       entirely when nothing on screen changed. */
    const keep = ['.pgmain', '.pgside', '.pgrail'].map((sel) => { const el = root.querySelector(sel); return el ? el.scrollTop : 0; });
    const html =
      '<div class="pgcard">' +
      ...
      '</div>' +
      '<div class="pgfoot"><div class="pgpts">Available development points<b>' + v.points.available + ' ⬡</b>' +
        (v.points.bank ? ' <small style="color:var(--mist)">· ' + v.points.bank + ' in the bank</small>' : '') + '</div>' +
      (v.canBuy ? '<button class="pgbuy" data-act="buypts" title="' + (v.pack ? v.pack.points + ' points for $' + v.pack.usd : '') + ', held by your account">💳 Buy points</button>' : '') +
      '<div class="pgnote">' + esc(v.footNote) + '</div></div>' +
      '</div>';
    if (html === lastHtml) return;
    lastHtml = html;
    root.innerHTML = html;
    ['.pgmain', '.pgside', '.pgrail'].forEach((sel, i) => { const el = root.querySelector(sel); if (el && keep[i]) el.scrollTop = keep[i]; });
  }
