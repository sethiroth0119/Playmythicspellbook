...
export function makeState(ctx) {
  const S = {
    ...
    /* ⬡ How much of `spent` came out of the ACCOUNT'S BANK (bought points,
       ctx.bankPoints / ctx.spendBank) rather than out of this city's
       milestones. Needed so `available` can be re-derived: earned − (spent
       − bankSpent) is what the milestones still have to give. Saved as `b`. */
    bankSpent: 0,
  ...
  /* ⬡ THE BANK — points the account BOUGHT (2 ⬡ for $5), read live from the
     host and never cached here: the parent is the ledger, this module only
     asks. 0 when the host hands nothing over (standalone, or an older host),
     and then the tree behaves exactly as it did before the bank existed. */
  function bankPoints() {
    try { return (typeof ctx.bankPoints === 'function') ? Math.max(0, ctx.bankPoints() | 0) : 0; } catch (e) { return 0; }
  }
  function points() {
    const e = earnedPoints();
    const own = Math.max(0, e - Math.max(0, S.spent - S.bankSpent));
    const bank = bankPoints();
    return { earned: e, spent: S.spent, bankSpent: S.bankSpent, own, bank, available: own + bank };
  }

  /* ── the city-level question, asked of the host and never guessed ─────── */
  function levelState(need) {
    need = need | 0;
    if (need <= 1) return null;
    /* ⚠ FAILS OPEN. A host that hands over no level reader (an older build,
       standalone) must not lock every deep node behind a number nobody can
       see. The blocker names the level only when it can read one. */
    if (typeof ctx.cityLevel !== 'function') return { need, have: null, met: null };
    let have = null;
    try { have = ctx.cityLevel(); } catch (e) { have = null; }
    if (!Number.isFinite(+have)) return { need, have: null, met: null };
    return { need, have: +have, met: +have >= need };
  }
  ...
  function licenceState(key) {
    ...
    if (typeof ctx.hasLicence !== 'function') {
      /* Player prose: this string is printed verbatim by panel.js drawDetail()
         and by the blocker line below, and it used to name ctx.hasLicence. */
      return { key, held: null, why: 'nothing here can read your City Hall licences this session' };
    }
  ...
  function status(node) {
    ...
    const lv = levelState(node.level);
    if (lv && lv.met === false) {
      blockers.push({ kind: 'level', id: node.level | 0, level: lv,
        text: 'Requires city level ' + (node.level | 0) + ' — this city is level ' + lv.have + '. The city levels up on the Cinder its buildings earn.' });
    }
    const p = points();
    if (p.available < (node.cost | 0)) {
      blockers.push({ kind: 'points', text: 'Costs ' + (node.cost | 0) + ' development points — you have ' + p.available +
        (p.bank ? ' (' + p.bank + ' from the bank)' : '') });
    }
    return { state: blockers.length ? 'locked' : 'available', blockers, licence: lic, level: lv };
  }
  ...
  function unlock(id) {
    ...
    /* ⬡ MILESTONE POINTS FIRST, THE BANK FOR THE REMAINDER — and the bank is
       asked to actually hand it over. If the host takes less than asked (a
       balance that moved under us on another device), nothing is unlocked
       and nothing is recorded: the bank is the ledger, not this counter. */
    const cost = n.cost | 0;
    const p = points();
    const fromBank = Math.max(0, cost - p.own);
    if (fromBank > 0) {
      let took = 0;
      try { took = (typeof ctx.spendBank === 'function') ? (ctx.spendBank(fromBank) | 0) : 0; } catch (e) { took = 0; }
      if (took !== fromBank) return { ok: false, reason: 'blocked', blockers: [{ kind: 'points', text: 'The point bank could not cover ' + fromBank + ' ⬡ — refresh and try again.' }] };
      S.bankSpent += fromBank;
    }
    S.unlocked.add(id);
    S.spent += cost;
    fire('change');
  ...
  function readMetrics() {
    ...
    for (const k in METRICS) {
      const m = METRICS[k];
      let r;
      /* The thrown message can carry a call signature verbatim, and this `why`
         is printed to the player — so it goes to the console instead, exactly
         as milestones.js readerThrew() does. */
      try { r = m.read(ctx); } catch (e) {
        try { console.warn('[Progress] the ' + k + ' metric threw:', e); } catch (x) {}
        r = { ok: false, why: 'this figure could not be read this session' };
      }
  ...
  function tick() {
    ...
    for (const a of ACHIEVEMENTS) {
      ...
      let r;
      try { r = a.test(ctx, view); } catch (e) {
        try { console.warn('[Progress] achievement ' + a.id + ' threw:', e); } catch (x) {}
        r = { ok: false, why: 'this achievement could not be checked this session' };
      }
  ...
  function load(p) {
    ...
    S.bankSpent = Math.min(S.spent, Math.max(0, Math.round(+p.b || 0)));
    ...
    if (S.spent !== real) {
      ...
      S.spent = real;
      S.bankSpent = Math.min(S.bankSpent, real);
    }
    return { legacy: false, unlocked: S.unlocked.size, reached: S.reached.size };
  }

  return {
    S, has, unlock, status, points, earnedPoints, bankPoints, levelState,
