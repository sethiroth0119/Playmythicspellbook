...
/* 🛢 WHAT CANNOT BE BUILT AT ALL, only bought.
   ──────────────────────────────────────────────────────────────────────────
   A tanker is a VEHICLE. It is bought at Prince Portfolios' Truck Yard (the
   Oil & Fuel tab) or from another player on the P2P vehicle market, and it
   then shows up in the Cracking Yard because St.count('truck') reads the lot.
   Fabricating one out of camp metal made the refinery its own vehicle factory
   and left the P2P market with nothing to sell a refinery owner.
   ⚠ ENFORCED IN commission() TOO, not only here. canBuild() is what the UI
     greys out; commission() is the authority, and a rule that only exists in
     the disabled state of a button is not a rule. */
export const BUY_ONLY = { truck: 'Tanker Truck' };
export function buyOnly(id) { return !!BUY_ONLY[id]; }

export function canBuild(id) {
  const e = EQUIPMENT[id];
  if (!e || buyOnly(id) || St.count(id) >= e.max) return false;
...
export function commission(id) {
  ...
  if (!e) return false;
  if (buyOnly(id)) {
    St.toast('🛢️ A ' + BUY_ONLY[id] + ' is bought, not built — Prince Portfolios → Haul Rigs → Oil & Fuel, ' +
      'or the P2P vehicle market. It parks here the moment it is yours.', 6000);
    return false;
  }
