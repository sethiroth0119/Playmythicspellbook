...
export function init(canvasHost, opts) {
  ...
  /* 🧍 …AND THE OPERATOR SPECIFICALLY, because preloadAll() resolving is not
     the same as the character being ready. Measured: the yard opened with the
     GLB fetched, parsed and every clip present in the registry — and the
     player still wearing the procedural figure, because the one rebuild()
     that preloadAll() triggers had already run by the time the skinned mesh
     finished parsing. The model was fine; the avatar was simply never rebuilt
     again, and nothing anywhere reported a problem.
     Rebuilding just the avatar is cheap (it does not touch the yard) and
     idempotent, so doing it once more on the character's own promise costs
     nothing and closes the race. */
  Models.preload('character')
    .then(() => { if (!disposed && player) Walk.rebuildAvatar(player); })
    .catch(() => {});
...
export function rebuild() {
  ...
  /* Parked trucks: the ones not out on the road, so the fleet is countable
     without opening a panel. See TRUCK_YAW / truckBlockers below. */
  ...
  for (let i = 0; i < Math.max(0, (E.truck | 0) - out); i++) {
    ...
    t.position.set(p.x, 0, p.z); t.rotation.y = TRUCK_YAW;
    t.traverse(o => { if (o.isMesh) o.castShadow = true; });
    root.add(t);
    /* ⚠ BEFORE MEASURING. setFromObject reads world matrices, and a node added
       this frame still carries the identity until they are refreshed — the box
       would come back the model's raw local size, unrotated. */
    t.updateMatrixWorld(true);
    truckBlockers(blockers, t, p.x, p.z);
  }
...
/* ── BUILD PLOTS ──────────────────────────────────────────────────────────
   ...
   opening anything. */
/* 🚚 WHICH WAY A PARKED TANKER POINTS, and its footprint.
   ──────────────────────────────────────────────────────────────────────────
   At the model slot's yaw 0 the truck faces +Z, so its long axis runs along Z;
   this turns it to run across X instead, which is the orientation that reads
   as parked-square-on rather than angled away down the yard's spine.

   The collision was ONE circle of r 2.6 on a vehicle 10.19 m long, so the
   middle of the truck stopped you and both ends did not — you walked straight
   through the cab and through the back of the trailer. A single circle big
   enough to cover a 10 m truck (r 5.1) would instead wall off three metres of
   empty apron on either side.
   So the footprint is a CHAIN of circles down the truck's own axis: a capsule,
   in the only shape the walker understands. Adjacent circles overlap, so the
   union has no gap to slip through, and it hugs the body closely enough to
   walk past. truckBlockers() MEASURES that axis off the placed object — see
   the note there for why deriving it from this yaw was wrong. */
const TRUCK_YAW = Math.PI / 2;
/* Fallback dimensions, used only when the object cannot be measured — i.e. the
   procedural mock-up, before the real model has finished loading. */
const TRUCK_LEN = 10.2;
const TRUCK_HALF_W = 1.35;

/* 🔴 THE FOOTPRINT IS MEASURED OFF THE PLACED OBJECT, NOT DERIVED FROM THE
   YAW — and the first version got exactly that wrong. It laid the chain along
   `(sin(yaw), cos(yaw))`, which is the axis a model pointing down +Z would
   have. This model's long axis is along +X at yaw 0, so once the truck was
   turned 90° the barrier ran ACROSS it: a 10 m wall through the truck's waist,
   nothing along its body, and a player could still walk the length of it.
   Reported, correctly, as "I can still walk through truck".

   Measuring the object closes that whole class of bug. Rotate the truck, swap
   the model, change its export orientation — the footprint follows, because it
   is read from the same world-space box the renderer draws. */
function truckBlockers(blockers, obj, x, z) {
  let hx = TRUCK_LEN / 2, hz = TRUCK_HALF_W;      // fallback: the procedural mesh
  try {
    const b = new T.Box3().setFromObject(obj);
    const s = b.getSize(new T.Vector3());
    if (s.x > 0.2 && s.z > 0.2) { hx = s.x / 2; hz = s.z / 2; }
  } catch (e) {}

  // Lay the capsule down whichever horizontal axis is actually the longer one.
  const alongX = hx >= hz;
  const half = alongX ? hx : hz;                  // half the LENGTH
  const rad = Math.max(0.6, alongX ? hz : hx);    // half the WIDTH = the radius

  /* Pulled in by one radius at each end so the capsule's ROUND CAPS land on
     the bumpers instead of a radius beyond them. */
  const span = Math.max(0, half - rad);
  /* ⚠ EVENLY DISTRIBUTED, not stepped from one end. A `d += step` loop leaves
     the last circle wherever the stride happens to stop, which put the chain
     off centre — overshooting the tail and under-covering the nose. */
  const n = Math.max(2, Math.ceil((span * 2) / (rad * 1.4)) + 1);
  for (let i = 0; i < n; i++) {
    const d = -span + (span * 2) * (i / (n - 1));
    blockers.push({ x: x + (alongX ? d : 0), z: z + (alongX ? 0 : d), r: rad });
  }
}
...
function buildPads(interactables) {
  for (const p of Build.openPlots()) {
    ...
    root.add(g);
    pads.push(g);
    /* 🛢 A BUY-ONLY PAD DOES NOT ADVERTISE A BUILD. It still stands there and
       is still walkable — the panel behind it lists the trucks on the pad and
       says where to buy another — but a prompt reading "Build Tanker Truck #3
       · Short of materials" over a unit that can never be built from materials
       is the UI telling the player to do something the game will refuse. */
    const buyOnly = Build.buyOnly(p.id);
    interactables.push({
      id: 'pad:' + p.id,
      label: buyOnly ? p.label + ' pad' : 'Build ' + p.label + ' #' + p.next,
      hint: buyOnly ? 'Bought, not built — see inside' : (p.ready ? 'Materials on site' : 'Short of materials'),
      x: p.x, z: p.z, r: 2.6, act: 'build', buildId: p.id, ready: buyOnly ? true : p.ready,
    });
  }
}
