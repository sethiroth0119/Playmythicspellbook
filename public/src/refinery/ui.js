  ...
  return '<div class="hp-top">' +
    ...
    '<div class="hp-body">' +
      ...
      '<div class="hp-yard hp-yard-full">' +
        ...
        /* 🔭 The Overview button was removed on request. The yard is walked,
           and a detached camera let the player inspect and interact with a
           site they were not standing in — which is the one thing the walking
           rebuild existed to stop. Yard.setOverview() is still there and still
           works; nothing calls it, so the mode is unreachable rather than
           deleted, and re-adding a button is a one-line change. */
...
function bindYardHud() {
  /* 🔭 No Overview binding — the button is gone (see the note where it used
     to be rendered). Guarded rather than assumed: if anything re-adds an
     element with that id, it gets its old behaviour back rather than being
     a dead control. */
...
function paneBoard(s) {
  ...
  for (const o of s.offers) {
    ...
    h += '<div class="hp-offer' + (o.rush ? ' rush' : '') + (o.own ? ' own' : '') + '">' +
      ...
      (o.own ? ' · <b style="color:#7bc043">your station — 55% Cinder, the rest lands in your pumps</b>' : '') + '</div>' +
...
function paneStock(s) {
  ...
  for (const id of all) {
    ...
    h += '<div style="margin-bottom:9px"><div class="hp-barlbl"><span>' + (meta.ico || '') + ' ' + esc(meta.name) + note + '</span><b>' + fmt(v) + ' L</b></div>' +
      ...
      /* 🛢 DRUM IT INTO THE CAMP STORES. Selling turns a cut into Cinder; this
         turns it into the RESOURCE, which is a different decision — the cuts
         are real game resources now, so they can be banked in the Foundation
         Reserve, posted on the resource market, spent by city buildings, or
         carried to the hospital, which needs ethanol for a vaccine.
         ⚠ Only for a cut with a stash id. Everything in STREAMS and COMPONENTS
           has one today, but the guard is what stops a future yard-only
           intermediate offering a button that would refuse. */
      (St.stashIdFor(id) ? '<div class="hp-row" style="margin-top:4px;gap:5px">' +
        '<span class="hp-err" style="flex:1">' + St.LITRES_PER_UNIT + ' L = 1 ' +
          esc((St.stashIdFor(id) || '')) + ' in your stores</span>' +
        '<button class="hp-btn sm" data-drawcut="' + id + '" data-drawn="1"' +
          (v < St.LITRES_PER_UNIT ? ' disabled' : '') + '>Drum 1</button>' +
        '<button class="hp-btn sm" data-drawcut="' + id + '" data-drawn="5"' +
          (v < St.LITRES_PER_UNIT * 5 ? ' disabled' : '') + '>Drum 5</button></div>' : '') +
    '</div>';
  }
  ...
  for (const [cat, label] of cats) {
    ...
    for (const e of EQUIP_LIST.filter(x => x.cat === cat)) {
      ...
      h += '<div class="hp-offer"><div class="hd"><b>' + e.ico + ' ' + esc(e.name) + '</b>' +
        ...
        /* 🛢 A buy-only unit shows WHERE, not what it costs in metal. Leaving
           the bill of materials up next to a dead button was the version that
           read as a bug rather than a rule. */
        (Build.buyOnly(e.id)
          ? '<div class="sub" style="margin-top:5px;color:#e8a13a">🛢️ Bought, not built. Prince Portfolios → Haul Rigs → <b>Oil &amp; Fuel</b>, or the P2P vehicle market. It parks here the moment it is yours.</div>'
        ...
        '<div class="act">' +
          (Build.buyOnly(e.id) || maxed ? '' : '<button class="hp-btn pri sm" data-plan="' + e.id + '">' + (ready ? '🏗 Commission' : 'See what it needs') + '</button>') +
...
function buildHtml(id) {
  ...
  /* 🛢 A BUY-ONLY UNIT GETS A DIFFERENT PANEL ENTIRELY, not a greyed button on
     a bill of materials it can never satisfy. The plot is still walkable and
     still tells the player what parks there — it just sends them to the yard
     that sells it. Listing the trucks they already own is the proof the route
     works: a tanker bought from another player shows up in this list. */
  if (Build.buyOnly(id)) {
    let mine = [];
    try { mine = St.tankers(); } catch (e2) {}
    const rows = mine.length
      ? mine.map(t => '<div class="hp-bom"><span class="ic">🛢️</span>' +
          '<span class="nm">' + esc(t.name) + '</span>' +
          '<span class="qt">' + (t.litres > 0 ? fmt(t.litres) + ' L' : '') + '</span>' +
          '<span class="hv">' + esc(t.condition) + '</span></div>').join('')
      : '<div class="hp-muted">Nothing on the pad yet.</div>';
    return '<div class="hp-modal-hd"><b>' + e.ico + ' ' + esc(e.name) + '</b>' +
        '<span class="hp-muted">' + owned + ' parked</span>' +
        '<button class="hp-btn sm" data-close>✕ Close</button></div>' +
      '<div class="hp-modal-body">' +
        '<div class="hp-card"><div class="hp-muted">' + esc(e.desc) + '</div></div>' +
        '<div class="hp-card"><h3>🛢️ Bought, not built</h3>' +
          '<div class="hp-muted">A tanker is a vehicle. Buy one at <b>Prince Portfolios → Haul Rigs → ' +
          'Oil &amp; Fuel</b>, or from another player on the <b>P2P vehicle market</b>. It parks here ' +
          'the moment it is on your lot — there is no transfer step.</div></div>' +
        '<div class="hp-card"><h3>🚚 On the pad</h3>' + rows + '</div>' +
        '<div class="hp-row"><button class="hp-btn sm" data-close>Close</button></div>' +
      '</div>';
  }
...
function wire(pane, s) {
  ...
  on('[data-ship]', n => {
    ...
    const job = s.contracts.find(c => c.batchId === b.id);
    if (C.dispatch(job, b, n.dataset.pri)) {
      B.removeBatch(b.id); job.batchId = 'sent'; paint();
      /* 📦 The operator carries the load out to the bay. Purely visual, and
         timed rather than tracked: the drums are on the truck when dispatch
         returns, so there is no state to follow — the walk cycle just shows
         what the player's character was doing for the next few seconds.
         Guarded end to end; with no carry clip this is a no-op. */
      try {
        const p = Yard.getPlayer && Yard.getPlayer();
        if (p) {
          Walk.setCarrying(p, true);
          setTimeout(() => { try { Walk.setCarrying(p, false); } catch (e) {} }, 6000);
        }
      } catch (e) {}
    }
  });
  ...
  /* 🛢 Drum a cut into the camp stores. Reports the refusal rather than doing
     nothing, because "not enough in the tanks" and "your vault is full" are
     both things the player can act on and neither is visible from the button. */
  on('[data-drawcut]', n => {
    const r = St.drawToStash(n.dataset.drawcut, parseInt(n.dataset.drawn, 10) || 1);
    if (r && r.ok) {
      St.toast('🛢 Drummed ' + r.units + ' × ' + r.resource + ' (' + r.litres.toLocaleString() + ' L) into your stores.', 3600);
      paint();
    } else {
      St.toast('⚠ ' + ((r && r.error) || 'That could not be drummed.'), 4200);
    }
  });
