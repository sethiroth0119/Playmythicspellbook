...
/* 🚶 MOVEMENT SPEED, RETUNED AGAINST THE REAL ANIMATION.
   These were 7.4 and 13.2 — and the yard is in metres, so that is 7.4 m/s at a
   "walk", roughly Olympic sprint pace. It never showed while the operator was
   a procedural figure whose legs were swung by an arbitrary phase. With a real
   character it is glaring: the Guardian's own clips measure 1.90 m/s walking
   and 3.81 m/s running (clipGroundSpeed reports this at load), so the feet had
   to slide four strides for every one they took.
   Halved, which keeps the yard brisk to cross while leaving the playback rate
   near 2× rather than 4× — fast-paced, but with the feet planted.
   ⚠ Change these and the animation follows automatically: the playback rate is
     derived from the clip's own stride, not hard-coded. Nothing else to retune. */
const SPEED_WALK = 4.2;      // metres / second
const SPEED_RUN  = 7.8;
const ACCEL      = 12.0;
...
function buildAvatar(self) {
  ...
  if (model.userData.custom) {
    ...
    const carry = Models.clipFor('character', 'carry');
    if (idle || walk || run || carry) {
      ...
      if (carry) self.actions.carry = self.mixer.clipAction(carry);
      for (const k in self.actions) { self.actions[k].loop = T.LoopRepeat; self.actions[k].enabled = true; }
      /* 👣 Measure each locomotion clip's own ground speed once, here, so the
         per-frame code is a division rather than a guess. Idle is excluded:
         it has no stride and must always play at 1×. */
      self.clipSpeed = {};
      ['walk', 'run', 'carry'].forEach((k) => {
        if (!self.actions[k]) return;
        const sp = clipGroundSpeed(self, self.actions[k].getClip());
        if (sp > 0) self.clipSpeed[k] = sp;
      });
      /* 🧍 NO IDLE CLIP IS THE COMMON CASE, not an edge one — a generator that
         exports one file per animation rarely produces a standing pose, and
         the shipped Guardian has walk / run / carry only. Falling through to
         `walk` is right (step() slows it to 0.55× when stationary, so the
         operator shifts their weight rather than marching on the spot), but
         only because that timeScale floor exists. Do not remove it. */
      play(self, self.actions.idle ? 'idle' : (self.actions.walk ? 'walk' : (self.actions.run ? 'run' : 'carry')));
    }
  } else {
...
/* 👣 HOW FAST THE GROUND GOES BY IN A CLIP — the number that stops the feet
   from sliding.
   ═══════════════════════════════════════════════════════════════════════════
   A walk cycle is authored IN PLACE: the character strides, the root never
   moves, and it is the game that slides the whole thing along the floor. The
   feet only look planted when the clip is played at the rate its own stride
   implies. Play a 1.2 m/s walk while moving at 7.4 m/s — which is what this
   yard does, since SPEED_WALK is a brisk jog in metres — and the character
   skates.

   So the stride is measured rather than guessed: step through the clip, watch
   the foot joints travel forward and back in the character's own frame, and
   take the peak-to-peak excursion. That is one step. A cycle is two steps, so
   the implied ground speed is (2 × step) / duration.

   Returns metres per second at timeScale 1, or 0 when it cannot be measured —
   in which case the caller falls back to the old rough ratio. */
function clipGroundSpeed(self, clip) {
  try {
    if (!clip || !clip.duration || !self.model) return 0;
    const skin = [];
    self.model.traverse((n) => { if (n.isSkinnedMesh && n.skeleton) skin.push(n); });
    if (!skin.length) return 0;
    const bones = skin[0].skeleton.bones || [];
    // Foot / toe joints by name, with a fallback to the lowest bones in the rig.
    let feet = bones.filter((b) => /foot|toe|ankle/i.test(b.name || ''));
    if (!feet.length) {
      feet = bones.slice().sort((a, b) => a.position.y - b.position.y).slice(0, 2);
    }
    if (!feet.length) return 0;

    const probe = new T.AnimationMixer(self.model);
    ...
    /* ⚠ MEASURE IN THE PLAYER GROUP'S FRAME, NOT THE MODEL'S.
       The normalisation scale lives on the model wrapper, and this rig's bone
       space is ~100× its mesh space, so model-local coordinates are in bone
       units: the first version of this read a 1.9 m/s walk as 186 m/s and every
       clip came out unplayably fast. The player group carries position and
       rotation but no scale, so its frame is metres — the same units as
       self.speed, which is the whole point of the comparison. */
    const frame = self.group || self.model;
    for (let i = 0; i < steps; i++) {
      probe.update(dt);
      (self.model.parent || self.model).updateMatrixWorld(true);
      inv.copy(frame.matrixWorld).invert();
      for (const f of feet) {
        v.setFromMatrixPosition(f.matrixWorld).applyMatrix4(inv);
        if (v.z < minZ) minZ = v.z;
        if (v.z > maxZ) maxZ = v.z;
      }
    }
    act.stop(); probe.stopAllAction(); probe.uncacheRoot(self.model);
    skin.forEach((m) => { if (m.skeleton) { m.skeleton.pose(); m.skeleton.update(); } });

    const step = maxZ - minZ;
    if (!(step > 0.05)) return 0;          // not a locomotion clip
    return (step * 2) / clip.duration;
  } catch (e) { return 0; }
}

/* 📦 Is the operator hauling something? Drives the `carry` clip in step().
   Exposed rather than inferred so the yard can turn it on around an action
   that actually involves carrying — loading a bay, moving a drum — instead of
   this file having to guess from position. Safe to call with no carry clip
   loaded: step() checks for the action before using it. */
export function setCarrying(self, on) {
...
export function step(self, dt, bounds) {
  ...
  /* 📦 CARRYING SLOWS YOU DOWN — and it has to, for the animation to read.
     The carry clip is a laden trudge: it covers 0.79 m/s of ground against the
     walk's 1.90, so playing it at walking pace would run it at 5× and the
     drum-carrying stagger would look like a sprint. 0.42 puts its playback
     rate alongside the other two, and a hauler who moves slower than an
     empty-handed one is what the player expects anyway. */
  const carryFactor = (self.carrying && self.actions && self.actions.carry) ? 0.42 : 1;
  const target = (mag > 0.06 ? (running ? SPEED_RUN : SPEED_WALK) : 0) * Math.min(1, mag) * carryFactor;
  ...
  if (self.mixer) {
    ...
    /* 📦 CARRYING BEATS EVERYTHING BUT STANDING STILL. An operator hauling a
       drum does not break into a run, so `carry` covers both walk and run
       speeds while it is set — the timeScale below still stretches it, so
       moving faster reads as a faster trudge rather than a different gait.
       Set through setCarrying(); with no carry clip in the model this whole
       branch collapses to the old walk/run behaviour. */
    const carrying = !!self.carrying && !!self.actions.carry;
    const moving = self.speed >= 0.4;
    play(self, !moving ? (carrying ? 'carry' : (self.actions.idle ? 'idle' : 'walk'))
             : (carrying ? 'carry' : (self.speed > SPEED_WALK * 1.12 ? 'run' : 'walk')));

    /* 👣 PLAY THE CLIP AT THE RATE ITS OWN STRIDE IMPLIES.
       clipSpeed[k] is how fast the ground would pass at timeScale 1, measured
       from the clip's actual foot travel (see clipGroundSpeed). Dividing the
       real speed by it is what plants the feet: the character now covers
       exactly one stride per stride, at any speed, for any rig.
       ⚠ The old line was `speed / SPEED_WALK`, which silently assumed every
         clip walks at SPEED_WALK — 7.4 m/s here, a sprint. That mismatch WAS
         the skating.
       ⚠ Smoothed, because timeScale reacts instantly and the raw speed steps
         on every keypress; without this the legs stutter on start and stop. */
    const a = self.actions[self.current];
    if (a) {
      if (self.current === 'idle') {
        a.timeScale = 1;
      } else if (!moving) {
        // Standing with no idle clip: hold the walk almost still rather than
        // marching on the spot.
        a.timeScale = 0.35;
      } else {
        const ref = (self.clipSpeed && self.clipSpeed[self.current]) || 0;
        const want = ref > 0 ? (self.speed / ref) : clamp(self.speed / SPEED_WALK, 0.55, 1.9);
        self._ts = (self._ts == null) ? want : self._ts + (want - self._ts) * Math.min(1, dt * 9);
        a.timeScale = clamp(self._ts, 0.35, 3.2);
      }
    }
