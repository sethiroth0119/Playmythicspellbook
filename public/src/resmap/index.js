...
const API = {
  ...
  /* 🩹 THE ONE THING ALLOWED TO OVERRIDE THE PIN, and it exists because the
     pin can latch a WRONG id. `pinned` freezes the first answer the host
     gives, which is exactly right when the host is stable — and node-city's
     was not: it fell back to a storage key that changes as identity
     resolves, so a city could be pinned to any of five strings depending on
     how far boot had got. A frozen wrong answer is still wrong.
     /src/water/network.js holds the oldest surviving evidence of the real
     one — the id the mains were stamped with when they were laid — and hands
     it here. Nothing else calls this.
   ⚠ IT IS NOT A SETTER. It refuses anything empty, refuses a no-op, and
     invalidates the field cache so the ground is redrawn where the pipes
     say it was. Returns whether it actually moved anything. */
  adoptCityId(id) {
    const s = String(id == null ? '' : id);
    if (!s || s === cityId) return false;
    try { console.warn('[resmap] adopting the ground id the mains were laid under: ' + s + ' (was ' + (cityId || 'unset') + ')'); } catch (e) {}
    cityId = s; pinned = true;
    Fields.invalidate(); Overlay.repaintNext();
    return true;
  },
