  ...
  try {
    for (const c of cells) {
      ...
      if (t) {
        ...
        /* 🙂 PLOT MOOD. A CONVERSION IS THE SEAM NOTHING ELSE COVERS: it
           rewrites `t.rc` on a tile that already stood, so tryPlace is never
           called, no tile is created and computeLinks() has nothing to
           recompute. What it does move is the weighted road meter
           (roadUsed(), via capWeight), which is the one real number a class
           carries — see /src/plotmood's header for why that, and not an
           invented per-plot "frontage quality", is what the glyph reads. */
        try { if (typeof window !== 'undefined' && window.MythicPlotMood) window.MythicPlotMood.invalidate('road-class'); } catch (e) {}
        converted++;
        continue;
      }
