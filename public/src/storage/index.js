...
export async function openOffice() {
  ...
  /* 🏢 WHO YOU KEEP THIS FOR. A player hired as a corporation's Warehouse
     Worker owns nothing here, so the office greeted them with "You do not own
     a Warehouse yet" and nothing else — technically true, and it reads exactly
     like the hire did nothing at all. Their position is a real, server-held
     fact (corp_members.role, set by corp_hire), so say it.
     ⚠ Guarded on the bridge entry EXISTING, not just on its answer: this
       module has to keep working against a host that has not been reloaded. */
  const corp = (() => { try { return (b.corpDesk && b.corpDesk()) || null; } catch (e) { return null; } })();
  const staffLine = (corp && /^(warehouse worker|ceo|corp ceo)$/i.test(corp.role || ''))
    ? '<div style="font-size:.78rem;color:#cfe6ff;border:1px solid rgba(120,200,255,.22);'
      + 'background:rgba(120,200,255,.06);border-radius:8px;padding:.5rem .65rem;margin-bottom:.8rem">'
      + '🏢 You keep the stores for <b>' + esc(corp.name) + '</b>'
      + (corp.tag ? ' [' + esc(corp.tag) + ']' : '') + ' as <b>' + esc(corp.role) + '</b>. '
      + 'The ceiling below is your own — walk the corporation’s yard from Just Business → Guild &amp; Hiring.</div>'
    : '';
