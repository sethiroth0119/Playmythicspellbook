...
const ctx = {
  ...
  /* ⏳ How long a listing lives, in ms, from index.html's RES_MARKET_TTL_MS —
     the SAME number it passes to rl_expire_mine, so the countdown drawn on a
     row and the sweep that removes it cannot disagree. 0 (or an older
     index.html with no such bridge entry) means "not known", and render.js
     draws no countdown at all rather than inventing one. */
  listingTtlMs: () => { const b = B(); try { return (b && b.listingTtlMs ? b.listingTtlMs() : 0) | 0; } catch (e) { return 0; } },
