...
/* ⏳ HOW LONG THIS LISTING HAS LEFT.
   ══════════════════════════════════════════════════════════════════════════
   Feature board (Gary Brooks): "when you list resources for sale you cannot
   tell how long before the listing is removed". The clock was real the whole
   time — a listing is swept at created_at + TTL, by rl_expire_mine server-side
   and by the same number client-side — it was simply never drawn, so a seller
   watching their goods come home had no way to know it was a deadline and not
   the market losing them.

   ⚠ THE TTL IS NOT WRITTEN DOWN HERE. It arrives through the bridge
     (ctx.listingTtlMs) from the one constant index.html also hands to
     rl_expire_mine. A second copy in this module is a copy that can disagree
     with the sweep that actually removes the row, and a countdown that
     disagrees with the deadline is worse than no countdown.
   ⚠ UNKNOWN PRINTS NOTHING. No TTL, no created_at, or an unparseable one, and
     this returns an empty string rather than guessing 72h. A market may be
     silent about a deadline; it may never state one it cannot stand behind.
   ⚠ THE ABSOLUTE TIME IS IN THE TOOLTIP, not in the row: "expires in 3h 12m"
     is what a seller acts on, and the wall-clock instant is what they check
     when they want to be sure. Both come from the same computed number. */
export function expiryOf(row, ctx) {
  try {
    const ttl = (ctx && typeof ctx.listingTtlMs === 'function') ? (ctx.listingTtlMs() | 0) : 0;
    if (!(ttl > 0)) return null;
    const made = row && row.created_at ? Date.parse(row.created_at) : NaN;
    if (!isFinite(made)) return null;
    const at = made + ttl;
    return { at, left: at - Date.now(), ttl };
  } catch (e) { return null; }
}

/* "3h 12m", "2d 4h", "8m". Deliberately two units at most: a countdown with
   seconds in it re-renders as a lie the moment the page sits still. */
...
/* The chip itself. `mine` only changes the words: on someone else's listing it
   is how long you have to decide, on your own it is how long before the goods
   come back to your stash. */
export function expiryHtml(row, ctx, mine) {
  const e = expiryOf(row, ctx);
  if (!e) return '';
  let abs;
  try { abs = new Date(e.at).toLocaleString(); } catch (x) { abs = new Date(e.at).toISOString(); }
  if (e.left <= 0) {
    return `<span class="small-text" style="color:#e0879a" title="Expired ${esc(abs)}">⌛ expired${
      mine ? ' — coming back to your stash' : ''}</span>`;
  }
  // Under six hours is the point where a seller might want to re-price rather
  // than let it lapse, so it stops being grey.
  const urgent = e.left < 6 * 3600000;
  return `<span class="small-text${urgent ? '' : ' ink-dim'}"${urgent ? ' style="color:#ffd166"' : ''
    } title="${esc(mine ? 'This listing is removed and the goods return to your stash at ' + abs
                        : 'This listing leaves the market at ' + abs)}">⌛ ${
    mine ? 'expires in ' : 'ends in '}${esc(fmtLeft(e.left))}</span>`;
}
...
export function rowHtml(row, ctx, mine) {
  ...
  const head = `
      <div style="flex:1;min-width:190px">
        ...
        <div class="small-text ink-dim">${mine ? 'your listing' : 'from ' + esc(row.seller_name || 'a survivor')} · ${stock}${cur === 'trade' ? ' · <span style="color:#7fd1e6">swap</span>' : ''}${
          (() => { const x = expiryHtml(row, ctx, mine); return x ? ' · ' + x : ''; })()}</div>
      </div>`;
...
export default { rowHtml, listHtml, formHtml, formPreview, takeSummary, askLabel, cannotWhy, esc, expiryOf, expiryHtml, fmtLeft };
