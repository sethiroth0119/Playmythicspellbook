...
const CODES = {
  ...
  /* 🚛 THE DEPOT GATE (sql/075). The rule is the Transport Depot's own shipped
     description — "a city without one cannot receive freight" — and it is
     enforced by a BEFORE INSERT trigger on transport_contracts, so this code
     arrives as a raise rather than in a `{ok:false}` envelope. That is why
     rpcFail() looks a bare message up in this table: without that lookup the
     player would read the word `no_depot_at_destination` and nothing else.
     ⚠ `d.to_node` COMES FROM THE TRIGGER'S DETAIL PAYLOAD and may be absent —
       parseDetail() answers {} for a raise that carried prose, or none. The
       sentence has to stand up without it, because "no depot at undefined" is
       the failure mode this whole table exists to prevent. */
  no_depot_at_destination: (d) => ({
    why: (d && d.to_node ? String(d.to_node) : 'That destination') +
         ' has no finished Transport Depot, so there is nowhere for a rig to unload.',
    fix: 'Ship to a node that has one — the route picker marks them — or have the city there build a Transport Depot. A depot still under construction does not count until it finishes.',
  }),
  out_of_reach: (d) => ({
...
function rpcFail(name, sent, e) {
  ...
  /* 4. A BARE REFUSAL CODE RAISED BY A TRIGGER *INSIDE* AN RPC.
        failCoded() already does this for the two direct-INSERT paths
        (createCompany / registerRig), because sql/073's §2b guards refuse those
        by raising. sql/075's depot gate is a BEFORE INSERT trigger on
        transport_contracts, so it raises inside transport_dispatch() — an RPC —
        and lands here instead, where branch 5 would have printed
        "The freight service failed: no_depot_at_destination" and stopped.
        Same table, same lookup, same rule: on a MISS nothing is invented and
        the message falls through to branch 5 verbatim. */
  const code = msg.trim();
  if (CODE_RE.test(code) && Object.prototype.hasOwnProperty.call(CODES, code)) {
    const m = explain(code, parseDetail(e && e.details));
    return Object.assign(base, { code: code, why: m.why, fix: m.fix });
  }
  return Object.assign(base, {
...
/* ═══ THE DEPOT GATE — a READ, and deliberately not an enforcement ═════════
   sql/075 keeps a server-side index of which nodes hold a finished Transport
   Depot and refuses a contract INSERT to any other destination. THAT is the
   rule. This is the shop window onto it: the route picker marks the nodes that
   can take a load, and dispatch() refuses locally BEFORE the manifest is
   escrowed, so an ordinary player never meets the raw exception.

   🔴 THE DEGRADED ANSWER IS "ALLOW", AND IT IS THE ONLY SAFE ONE HERE. If the
      RPC is missing (075 not applied), or the read fails, or the player is
      offline, `required` comes back false and nothing in this file refuses —
      the server decides, as it always did. The opposite default would mean one
      unapplied migration silently blockades every haul in the game from the
      client side, which is a bigger outage than the rule is worth, and it would
      look exactly like the feature being broken. A client that fails open in
      front of a server that fails closed is the correct pair: the client can
      only ever be WRONG BY BEING PERMISSIVE, and the trigger is still there.

   ⏱ Cached for 60s, keyed on nothing: it is one row-set for the whole world,
      it changes only when somebody finishes a depot (hours of build time), and
      the panel repaints on a ticker. `force` is for the depot's own refresh
      button, so a player who just built one is not told to wait a minute. */
const GATE_TTL_MS = 60000;
let _gateVal = null, _gateAt = 0, _gateWait = null;
export async function depotGate(force) {
  const now = Date.now();
  if (!force && _gateVal && (now - _gateAt) < GATE_TTL_MS) return _gateVal;
  // Collapse concurrent callers onto one round trip — routes.js and index.js
  // both ask during the same repaint.
  if (_gateWait) return await _gateWait;
  const c = client();
  if (!c) return { required: false, nodes: null, unknown: true, why: offWhy() };
  _gateWait = (async () => {
    const r = await rpc(c, 'transport_depot_gate', {});
    let out;
    if (!r.ok) {
      out = { required: false, nodes: null, unknown: true, why: r.why || r.error || 'depot map unavailable' };
    } else {
      const d = r.data || {};
      const list = Array.isArray(d.nodes) ? d.nodes : [];
      out = {
        required: d.required === true,
        // A Set, because the only question ever asked of it is membership, and
        // the caller must be able to tell "no depots anywhere" (an empty Set)
        // from "we could not ask" (null). Those two are the same array and
        // opposite answers.
        nodes: new Set(list.map((n) => String((n && n.node) != null ? n.node : n))),
        levels: list.reduce((m, n) => { if (n && n.node != null) m[String(n.node)] = n.lvl | 0; return m; }, {}),
        unknown: false,
      };
    }
    _gateVal = out; _gateAt = Date.now(); _gateWait = null;
    return out;
  })();
  return await _gateWait;
}
/* Synchronous membership for a renderer, off whatever the last read left
   behind. Answers null — never false — when nothing has been read yet, so a
   picker can draw "unknown" instead of libelling every node on the map as
   unreachable during the first paint. */
export function depotKnownAt(node) {
  if (!_gateVal || !_gateVal.nodes) return null;
  return _gateVal.nodes.has(String(node));
}
export function depotGateCached() { return _gateVal; }
...
export async function dispatch(carrierId, rigId, fromNode, toNode, cargo, route) {
  ...
  /* 🚛 THE DEPOT PRE-FLIGHT, AND IT HAS TO BE ABOVE THE ESCROW.
     sql/075's trigger refuses this contract at the INSERT, inside
     transport_dispatch — which rolls the wallet charge back with it, so the
     player's Cinder is never at risk. Their CARGO is: takeRes() below debits
     the manifest from the local save before the RPC is called, and the unwind
     for an RPC refusal is `held.refund()` further down. That path works, but it
     means a haul that was never going to be accepted takes the goods out of the
     stash and puts them back, and any save in between records the hole. Asking
     first costs one cached read.
     ⚠ `gate.nodes` NULL IS NOT "NO NODES". It is "we could not ask" — see
       depotGate(). Only a positive answer refuses. */
  const gate = await depotGate();
  if (gate.required && gate.nodes && toNode != null && !gate.nodes.has(String(toNode))) {
    const m = explain('no_depot_at_destination', { to_node: String(toNode) });
    return { ok: false, missing: false, offline: false, row: null,
             code: 'no_depot_at_destination', error: 'no_depot_at_destination', why: m.why, fix: m.fix };
  }
