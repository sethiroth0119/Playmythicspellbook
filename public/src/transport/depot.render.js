...
/* 🚛 WHERE FREIGHT CAN ACTUALLY BE UNLOADED (sql/075).
   The Transport Depot's own description has always told players "a city
   without one cannot receive freight"; sql/075 is what makes that true, with a
   BEFORE INSERT trigger on transport_contracts. This is the panel's half: name
   the destinations that work BEFORE the player fills in a form and pays for a
   quote against one that does not.

   🔴 IT DRAWS NOTHING IN THREE OF THE FOUR STATES, and that is the design.
      `gate` absent (not asked yet), `gate.unknown` (075 not applied, or the
      read failed) and `required === false` (the operator has the rule switched
      off) all render '' — because a warning about a rule that is not in force,
      or that this client could not read, is a false statement about what the
      server will do. Only a positive, current answer is allowed to say
      anything. Same stance as index.js's `nodes: null` vs an empty array. */
function depotGateNote(v) {
  const g = obj(v.gate);
  if (!v.gate || g.unknown === true || g.required !== true) return '';
  const nodes = arr(g.nodes);
  const to = str(obj(v.form).to).trim();

  if (!nodes.length) {
    return banner('warn',
      'No city has built a Transport Depot yet, so there is nowhere for a rig to unload — every haul will be refused.',
      'Build a Transport Depot in a city (it is in the city build palette) and it can receive freight as soon as it finishes.');
  }
  /* The list is capped for the SAME reason the rate board caps itself: a world
     with 39 nodes on it prints a paragraph nobody reads. The count is always
     honest, so "+ 12 more" is never mistaken for the whole list. */
  const shown = nodes.slice(0, 12).join(', ') + (nodes.length > 12 ? ', +' + (nodes.length - 12) + ' more' : '');
  const bad = to && nodes.indexOf(to) < 0;
  return (bad
    ? banner('bad',
        to + ' has no finished Transport Depot, so a haul there will be refused when you dispatch it.',
        'Pick one of the nodes listed below, or have the city at ' + to + ' build a Transport Depot. One still under construction does not count until it finishes.')
    : '')
    + note('Freight can only be unloaded at a city with a Transport Depot. Accepting freight now (' + nodes.length + '): ' + shown);
}
