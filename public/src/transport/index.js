...
import {
  PP_RIGS, PP_RIGS_BY_ID, RIG_RARITIES, rollRig, rigById, rarityIndex, CARGO_CLASSES, cargoClassOf,
  cargoClassIds,
...
const S = {
  ...
  /* 🚛 sql/075's destination rule, as READ FROM THE SERVER — never decided
     here. `{ required, nodes:Set|null, unknown }`. null is the honest starting
     state: nothing has been asked yet, and a renderer must be able to tell that
     apart from "asked, and no node in the world has a depot". */
  gate: null,
};
...
function buildView() {
  ...
  return {
    ...
    /* 🚛 THE DEPOT RULE, FLATTENED FOR A PURE RENDERER. The Set becomes a
       sorted ARRAY here rather than in depot.render.js: that file's first rule
       is that it reads nothing but its argument and returns a string, and
       handing it a live Set would hand it a structure it could mutate — the
       same reason `form` above is copied rather than passed.
       THREE STATES, and the renderer draws three different things:
         gate === null           → not asked yet. Say nothing.
         gate.unknown === true   → asked, no answer (075 not applied, or
                                   offline). Say nothing — a rule nobody can
                                   read is not a rule to warn a player about.
         gate.required           → the server WILL refuse a destination that is
                                   not in `nodes`. */
    gate: S.gate ? {
      required: S.gate.required === true,
      unknown: S.gate.unknown === true,
      nodes: S.gate.nodes ? Array.from(S.gate.nodes).sort() : null,
    } : null,
...
async function refresh() {
  ...
  const [co, rigs, cars, cons, gate] = await Promise.all([
    acall(myCompany, { ok: false }),
    acall(listMyRigs, { ok: false }),
    acall(listCarriers, { ok: false }),
    acall(listContracts, { ok: false }),
    /* 🚛 The depot map. A FIFTH LEG, not a fifth reason to fail: it is read
       with the same acall() wrapper as the other four so an unapplied sql/075
       costs the exchange its "these nodes can receive freight" line and nothing
       else. It is deliberately NOT passed through classify() below — a missing
       transport_depot_gate() must not set S.missing and print "the freight
       tables are not installed" over a depot screen whose four real tables
       loaded perfectly. */
    acall(depotGate, { required: false, nodes: null, unknown: true }),
  ]);
  ...
  // Kept even when it came back `unknown` — the renderer needs the difference
  // between "not asked yet" (null) and "asked, could not tell" (unknown), and
  // overwriting one with the other loses it.
  S.gate = gate || null;
  S.loading = false;
...
const api = {
  ...
  /* 🗣 ONE PLACE A REFUSAL CODE BECOMES A SENTENCE, published so index.html can
     use it too. The Haulage Board in index.html posts to `haul_post` with its
     own `Cloud.client.rpc` call and toasts `e.message` — which, now that
     sql/075 puts a BEFORE INSERT trigger on haul_requests, is the bare string
     `no_depot_at_destination`. The alternative was a second copy of the
     sentence in index.html, and contracts.js's error table says in its own
     header that NOTHING ELSE decides what a code means. So the table is lent
     out rather than duplicated.
     Takes the supabase error object itself — message AND details — because the
     node id the sentence names lives in the raise's DETAIL payload, and a
     caller that had to unpack that first would be re-implementing the half of
     this that is easy to get wrong.
     Answers null on a message that is not a known bare code, so the caller
     keeps printing the database's own words rather than a guess. */
  explainError: (e) => {
    try {
      const f = failCoded(e);
      return (f && f.code) ? { code: f.code, why: f.why, fix: f.fix, text: f.why + ' ' + f.fix } : null;
    } catch (err) { return null; }
  },
  /* The depot map behind sql/075's destination rule, for panels outside this
     module (the Haulage Board's To field). Cached and shared with the depot
     screen's own read. */
  depotGate: (force) => depotGate(force),
  ...
  /* An optional cargo class id rolls within that ladder only; omitted, it rolls
     the whole table exactly as it always did. The ids are the keys of
     CARGO_CLASSES — 'freight', 'oil', 'feed', 'livestock' as of this writing,
     and deliberately NOT listed as a closed set here, because a class added to
     that table must not need this comment edited to become rollable. */
  rollRig: (cargoClass) => call(rollRig, null, cargoClass),
  /* The class table and the default-aware reader, so index.html can build the
     yard's filter tabs without a second copy of the class list. */
  cargoClasses: () => CARGO_CLASSES,
  cargoClassOf: (rig) => call(cargoClassOf, 'freight', rig),
  /* The ids in TAB ORDER. index.html renders one filter tab per entry and
     stocks the floor per entry, so a class added to CARGO_CLASSES appears in
     the Truck Yard with no edit to index.html at all. The fallback is the two
     classes that existed before this list did: a bridge that answers nothing
     must still leave the yard with its original tabs, never with none. */
  cargoClassIds: () => call(cargoClassIds, ['freight', 'oil']),
