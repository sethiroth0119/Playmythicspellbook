/* ════════════════════════════════════════════════════════════════════════════
   ...
   ----------------------------------------------------------------------------
   Trucks that roll onto the ORDINARY Prince Portfolios auction floor, in TWO
   CARGO CLASSES: six freight rigs, one per game rarity, for Transportation
   Companies, and five tankers (common → legendary) for refineries. The class
   is what the Truck Yard's filter tabs sort on, and it is also the reason a
   refinery no longer builds its own trucks — see CARGO_CLASSES below.
   A haul-class listing is a normal PP vehicle with
...
/* 🧊 THE 3D MODELS, and why they are shared DOWN a class rather than one per rig.
   ────────────────────────────────────────────────────────────────────────────
   A rig row may carry `model: { url, scale, rotY }`. The Truck Yard reads it
   through `_ppModelOf()` (index.html) and renders the same orbit viewer the
   auction floor already uses for cars, so a listing with a model shows the
   truck instead of an emoji and the placeholder that used to read
   "[VEHICLE PHOTO · drop a 3/4 render]".

   ONE MODEL PER CLASS, NOT PER TIER. A tier is a spec sheet — more runs, more
   tonnes, a bigger tank — not a different vehicle, and three packed exports of
   the same silhouette would be three megabytes to say the same thing. The tier
   is already on the card in the rarity chip and the numbers beside it.

   🔴 THE URL IS A PATH INTO THE DEPLOYED BUNDLE AND IT IS ON AN ALLOW-LIST.
      public/.assetsignore blocks EVERY .glb in the bundle by default — the
      guard that stops an oversized drop aborting a whole deploy — so each of
      these has its own re-include line there. Add a model here without adding that line and the file simply
      404s in production while working perfectly in local dev, which is the
      worst shape a mistake can take. Packed by `tools/glbprop.mjs` from ~95 MB
      / ~3M-triangle Meshy exports down to 0.86 MB and 1.31 MB; masters live in
      assets-source/glb-masters/trucks/, never under public/.
   ⚠ `scale` and `rotY` are the viewer's own knobs (rotY in DEGREES — the
...
/* The tanker is the one existing model with no ambiguity about which rows it
   belongs to: every oil-class rig IS a tanker, and this is the same file the
   Cracking Yard already parks (see `truck:` in /src/refinery/models.js). */
const TANKER_MODEL = { url: '/models/trucks/tanker.glb', scale: 1, rotY: 0 };
...
/* 🛢 CARGO CLASS — which business a rig is bought FOR.
   ────────────────────────────────────────────────────────────────────────────
   'freight' hauls other players' cargo for a Transportation Company.
   'oil'     hauls crude and product for a refinery, and is the ONLY way a
             tanker enters the game: the Cracking Yard's build path was removed
             so that a fuel truck is always something a player bought, from the
             yard or from another player on the P2P market.

   'feed'    carries bulk animal feed TO a farm, and 'livestock' carries the
             animals themselves. Two classes and not one, which is the whole
             decision in this pair: a feed bulker and a stock deck are not two
             tiers of one truck, they are opposite ends of one supply chain, and
             a single 'farm' class would make rollRig() hand a player who wants
             to move cattle a hopper full of grain at random. The yard filters
             on this field, so one class per JOB is what makes the tab useful.
             ⏳ FORWARD DATA, STATED PLAINLY: the farm business these serve is
             not built yet (the city's Livestock Farm building exists; the
             business does not). They are bought, parked, sold and rendered like
             any other rig today, and `tonnes`/`head` are authored so the
             numbers are ratified in one place before dispatch reads them —
             exactly the position `litres` was in before the Cracking Yard.

   🔴 THE DEFAULT IS 'freight' AND IT IS READ, NOT ASSUMED. Every saved rig row
      predates this field, so anything reading it must treat a missing value as
      freight — `cargoClassOf()` below is the one place that decision lives.
      An `undefined` class silently failing a `=== 'freight'` filter would make
      every truck a player already owns vanish from the yard's default tab.
   ⚠ AND IT IS NOW A SET MEMBERSHIP TEST, NOT A CHAIN OF ===. The two-class
     version read `c === 'oil' ? 'oil' : 'freight'`, which is correct for two
     and silently wrong for four: every feed and livestock rig would have come
     back 'freight' and landed in the freight tab. A row whose class this table
     does not carry is still freight — an unknown id must degrade to the
     default, never vanish off every tab at once.
   📎 `sub` is the one-line the Truck Yard's tab prints under the label; `blurb`
     is the longer sentence. Both live here so index.html holds no second copy
     of what a cargo class is called — its TABS array is derived from this. */
export const CARGO_CLASSES = {
  freight:   { id: 'freight',   label: 'Freight',      ico: '📦', sub: 'Transportation Companies', blurb: 'Hauls cargo for Transportation Companies.' },
  oil:       { id: 'oil',       label: 'Oil & Fuel',   ico: '🛢️', sub: 'Refineries · Cracking Yard', blurb: 'Crude and product for refineries. Parks in the Cracking Yard.' },
  feed:      { id: 'feed',      label: 'Bulk Feed',    ico: '🌾', sub: 'Farms · feed and grain', blurb: 'Bulk animal feed and grain, delivered to a farm in tonnes.' },
  livestock: { id: 'livestock', label: 'Livestock',    ico: '🐄', sub: 'Farms · animal transport', blurb: 'Live animals, moved by the head between pens, market and slaughterhouse.' },
};
export function cargoClassOf(rig) {
  const c = rig && rig.cargoClass;
  return (typeof c === 'string' && Object.prototype.hasOwnProperty.call(CARGO_CLASSES, c)) ? c : 'freight';
}
/* Every class id, in the order the yard's tabs should print them. Derived, so
   adding a class above is the whole change — index.html asks for this list
   through the bridge rather than keeping its own. */
export function cargoClassIds() { return Object.keys(CARGO_CLASSES); }

export const PP_RIGS = [
  {
    id: 'haul_flatbed', name: 'Roachback Flatbed', type: 'Flatbed', cargoClass: 'freight',
  ...
  {
    id: 'haul_boxhauler', name: 'Mule Box Hauler', type: 'Box Hauler', cargoClass: 'freight',
  ...
  {
    id: 'haul_freighter', name: 'Kettledrum Freighter', type: 'Freighter', cargoClass: 'freight',
  ...
  {
    id: 'haul_longhaul', name: 'Ashgate Longhaul', type: 'Longhaul', cargoClass: 'freight',
  ...
  {
    id: 'haul_roadtrain', name: 'Saint Corvid Roadtrain', type: 'Roadtrain', cargoClass: 'freight',
  ...
  {
    id: 'haul_cinderline', name: 'The Cinder Line', type: 'Roadtrain', cargoClass: 'freight',
  ...
  /* ── 🛢 OIL & FUEL CLASS ───────────────────────────────────────────────────
     Tankers. These are the refinery's fleet, and buying one is now the ONLY
     way to get one: the Cracking Yard's "build a tanker truck" path was
     removed, so a fuel truck is always a vehicle somebody paid for — here at
     fixed price, or from another player on the P2P market.

     🔴 `litres` IS THE FIELD THE REFINERY READS, and it is the reason these
        are not simply freight rigs with a different picture. A refinery cares
        how much product a truck moves per run, not how much generic cargo it
        can carry; `cargo` stays because the shared listing code reads it, but
        the Cracking Yard sizes its haul off `litres`.
     ⚠ MIRROR: the base tanker is 9,000 L, which is the figure the refinery's
       own truck already carried (TRUCK_L in /src/refinery/state.js). The
       cheapest tanker here matches it exactly so that removing the build path
       changes what a truck COSTS and where it comes from, never what it does.
       If TRUCK_L moves, this row moves with it. */
  {
    id: 'haul_bowser', name: 'Ratchet Bowser', type: 'Tanker', cargoClass: 'oil',
    rarity: 'common', haul: true, baseValue: 62000,
    runs: 3, cargo: 1.00, risk: 0, speed: 1.00, litres: 9000,
    lotSlots: 1, weight: 40,
    emoji: '🛢️', accent: RIG_ACCENT,
    model: TANKER_MODEL,
    desc: 'Nine thousand litres and a hand pump. Every refinery starts here.',
  },
  {
    id: 'haul_pipsqueak', name: 'Pipsqueak Hauler', type: 'Tanker', cargoClass: 'oil',
    rarity: 'uncommon', haul: true, baseValue: 138000,
    runs: 4, cargo: 1.25, risk: -3, speed: 1.05, litres: 13000,
    lotSlots: 1, weight: 26,
    emoji: '🛢️', accent: RIG_ACCENT,
    model: TANKER_MODEL,
    desc: 'Baffled tank. The load stops trying to steer for you on the bends.',
  },
  {
    id: 'haul_cistern', name: 'Ashwater Cistern', type: 'Tanker', cargoClass: 'oil',
    rarity: 'rare', haul: true, baseValue: 320000,
    runs: 5, cargo: 1.60, risk: -8, speed: 1.12, litres: 19000,
    lotSlots: 1, weight: 18,
    emoji: '🛢️', accent: RIG_ACCENT,
    model: TANKER_MODEL,
    desc: 'Insulated barrel. Product arrives the temperature it left at.',
  },
  {
    id: 'haul_titan', name: 'Midnight Fuel Titan', type: 'Tanker', cargoClass: 'oil',
    rarity: 'epic', haul: true, baseValue: 690000,
    runs: 6, cargo: 2.10, risk: -14, speed: 1.22, litres: 28000,
    lotSlots: 2, weight: 10,
    emoji: '🛢️', accent: RIG_ACCENT,
    model: TANKER_MODEL,
    desc: 'Runs the dead stretches at night because the tank likes the cold.',
  },
  {
    id: 'haul_leviathan', name: 'Cradle Leviathan', type: 'Tanker', cargoClass: 'oil',
    rarity: 'legendary', haul: true, baseValue: 1480000,
    runs: 8, cargo: 2.80, risk: -22, speed: 1.35, litres: 42000,
    lotSlots: 2, weight: 6,
    emoji: '🛢️', accent: RIG_ACCENT,
    model: TANKER_MODEL,
    desc: 'Two trailers of product and a driver who has never lost a litre.',
  },

  /* ── 🌾 BULK FEED CLASS ────────────────────────────────────────────────────
     Hoppers. Feed and grain in bulk, blown into a farm's silo rather than
     unloaded by hand, which is why the defining number is TONNES and not the
     generic `cargo` multiplier the shared listing code reads.

     ⏳ The farm business is not written yet — see the CARGO_CLASSES header.
        These sell, park, resell and render today; nothing consumes `tonnes`.
     🔴 SHORT LADDER ON PURPOSE, three tiers rather than six. A class with one
        row makes the yard's tab show the same truck forever, which reads as
        broken; six would be six tiers of invented economy for a business that
        has no dispatch maths to balance them against. Three is the smallest
        number that is still a choice. auditRigs() checks the ladder it claims,
        not the full rarity list — same rule the oil class runs under. */
  {
    id: 'haul_chaffhopper', name: 'Chaff Hopper', type: 'Bulk Feeder', cargoClass: 'feed',
    rarity: 'common', haul: true, baseValue: 52000,
    runs: 3, cargo: 1.00, risk: 0, speed: 1.00, tonnes: 12,
    lotSlots: 1, weight: 55,
    emoji: '🌾', accent: RIG_ACCENT, model: FEED_MODEL,
    desc: 'One hopper, one auger. Twelve tonnes of feed and a tarp that mostly holds.',
  },
  {
    id: 'haul_ryebelly', name: 'Ryebelly Bulker', type: 'Bulk Feeder', cargoClass: 'feed',
    rarity: 'rare', haul: true, baseValue: 240000,
    runs: 5, cargo: 1.65, risk: -8, speed: 1.12, tonnes: 26,
    lotSlots: 1, weight: 32,
    emoji: '🌾', accent: RIG_ACCENT, model: FEED_MODEL,
    desc: 'Sealed bays and a blower that fills a silo without opening the lid.',
  },
  {
    id: 'haul_silotrain', name: 'Threshing Silo Train', type: 'Bulk Feeder', cargoClass: 'feed',
    rarity: 'epic', haul: true, baseValue: 560000,
    runs: 6, cargo: 2.15, risk: -14, speed: 1.22, tonnes: 44,
    lotSlots: 2, weight: 13,
    emoji: '🌾', accent: RIG_ACCENT, model: FEED_MODEL,
    desc: 'Two hoppers on one spine. Feeds a whole valley and turns in its own length.',
  },

  /* ── 🐄 LIVESTOCK CLASS ────────────────────────────────────────────────────
     Stock decks. The cargo is alive, so the defining number is HEAD — how many
     animals ride — and it is the field a farm will size a run off. `cargo`
     stays because the shared listing code reads it; nothing else should.

     ⏳ Forward data, same as the feed class above. */
  {
    id: 'haul_penfold', name: 'Penfold Stockwagon', type: 'Livestock Hauler', cargoClass: 'livestock',
    rarity: 'common', haul: true, baseValue: 58000,
    runs: 3, cargo: 1.00, risk: 0, speed: 1.00, head: 18,
    lotSlots: 1, weight: 55,
    emoji: '🐄', accent: RIG_ACCENT, model: STOCK_MODEL,
    desc: 'Slatted sides, straw floor, eighteen head. They arrive noisy and alive.',
  },
  {
    id: 'haul_drover', name: 'Drover Double-Deck', type: 'Livestock Hauler', cargoClass: 'livestock',
    rarity: 'rare', haul: true, baseValue: 265000,
    runs: 5, cargo: 1.60, risk: -8, speed: 1.10, head: 42,
    lotSlots: 1, weight: 32,
    emoji: '🐄', accent: RIG_ACCENT, model: STOCK_MODEL,
    desc: 'Two decks and a ramp one drover can work alone. Forty-two, penned in fours.',
  },
  {
    id: 'haul_abattoirline', name: 'Abattoir Line Hauler', type: 'Livestock Hauler', cargoClass: 'livestock',
    rarity: 'epic', haul: true, baseValue: 610000,
    runs: 6, cargo: 2.05, risk: -14, speed: 1.20, head: 76,
    lotSlots: 2, weight: 13,
    emoji: '🐄', accent: RIG_ACCENT, model: STOCK_MODEL,
    desc: 'Ventilated, watered, and it runs the killing floor to a timetable.',
  },
];
...
/* `cargoClass` is OPTIONAL and omitting it keeps the original behaviour of
   rolling across the whole table — index.html's generator calls this with no
   argument for the ordinary auction floor, where a tanker is as valid a
   listing as anything else. The truck yard passes a class so each tab restocks
   from its own ladder. */
export function rollRig(cargoClass) {
  const pool = cargoClass
    ? PP_RIGS.filter((r) => cargoClassOf(r) === cargoClass)
    : PP_RIGS;
  if (!pool.length) return null;
  let total = 0;
  for (const r of pool) { const w = +r.weight; if (w > 0) total += w; }
  ...
  for (let i = pool.length - 1; i >= 0; i--) if (+pool[i].weight > 0) return pool[i];
  return pool[0];
}
...
export function auditRigs(rarityIds, condMult, vehicleNames = PP_VEHICLE_NAMES_MIRROR) {
  ...
  /* 1. One rig per rarity PER CARGO CLASS, no more, no fewer. The count is
        derived from the ladder, never a literal, so adding a seventh rarity
        upstream shows up here as a missing rig instead of passing silently.

     ⚠ THE RULE IS PER CLASS BECAUSE THE FLOOR FILTERS BY CLASS. It used to be
       a flat "one rig per rarity" over the whole table, which was right when
       every rig was freight. With the oil class added, a flat rule would both
       fail on the total AND stop noticing a rarity missing from one class —
       the freight tab could lose its Legendary and the audit would still pass
       because a tanker held that rarity. The oil ladder is deliberately SHORT
       (no mythic): the class tops out at legendary, so it is checked against
       the rarities it actually claims rather than the full ladder. */
  /* ⚠ DERIVED FROM CARGO_CLASSES, NEVER A LITERAL PAIR. This was
     `{ freight: [], oil: [] }` when there were two classes; a third class added
     above would then have pushed onto `undefined` and thrown INSIDE the audit —
     the one function whose job is to notice that kind of change. Every declared
     class gets a bucket, including an empty one, so "no rigs at all in class X"
     below is reachable rather than a crash. */
  const byClass = {};
  cargoClassIds().forEach((cls) => { byClass[cls] = []; });
  PP_RIGS.forEach((r) => { byClass[cargoClassOf(r)].push(r); });

  if (byClass.freight.length !== ladder.length) {
    problems.push(`expected ${ladder.length} freight rigs (one per rarity), found ${byClass.freight.length}`);
  }
  Object.keys(byClass).forEach((cls) => {
    const seen = Object.create(null);
    byClass[cls].forEach((r) => {
      if (ladder.indexOf(r.rarity) < 0) problems.push(`${r.id}: rarity "${r.rarity}" is not in the game's rarity ladder`);
      if (seen[r.rarity]) problems.push(`${cls}: rarity "${r.rarity}" is used by both ${seen[r.rarity]} and ${r.id}`);
      seen[r.rarity] = r.id;
    });
    if (cls === 'freight') {
      ladder.forEach((rid) => { if (!seen[rid]) problems.push(`no freight rig for rarity "${rid}"`); });
    } else if (!byClass[cls].length) {
      problems.push(`no rigs at all in cargo class "${cls}" — the yard's ${cls} tab would be empty`);
    }
  });

  /* THE CLASS CAPACITY FIELD. Each of these classes has ONE number that is the
     reason it is a separate class at all — the tanker's litres, the feed
     bulker's tonnes, the stock deck's head count — and a row missing it renders
     a blank where the card's headline figure goes. Table-driven so a new class
     declares its field beside itself rather than growing another forEach. */
  const CAPACITY_FIELD = { oil: 'litres', feed: 'tonnes', livestock: 'head' };
  Object.keys(CAPACITY_FIELD).forEach((cls) => {
    const f = CAPACITY_FIELD[cls];
    (byClass[cls] || []).forEach((r) => {
      if (!(r[f] > 0)) problems.push(`${r.id}: a ${cls}-class rig must carry a positive ${f} capacity`);
    });
  });

  /* Every model path a rig claims must look like a bundle path. This cannot
     see a 404 — nothing in a pure data module can — but it does catch the two
     mistakes that have a shape: a bare filename (loads relative to whatever
     page is open) and an off-origin URL (blocked, and a model the game does not
     own). The `.assetsignore` allow-list is the other half and is checked by
     the smoke, which can read the file. */
  PP_RIGS.forEach((r) => {
    if (!r.model) return;
    const u = r.model.url;
    if (typeof u !== 'string' || !u.startsWith('/models/') || !/\.glb$/i.test(u)) {
      problems.push(`${r.id}: model.url "${u}" must be an absolute /models/… .glb path in this bundle`);
    }
  });
  ...
  /* ⚠ PER CLASS, because rollRig() normalises WITHIN a class — a freight roll
     never sees a tanker. Summing the whole table would demand 100 across two
     independent ladders, which is not a number that means anything. */
  Object.keys(byClass).forEach((cls) => {
    if (!byClass[cls].length) return;
    const total = byClass[cls].reduce((s, r) => s + (+r.weight || 0), 0);
    if (Math.abs(total - 100) > 1e-9) problems.push(`${cls} roll weights sum to ${total}, expected 100`);
  });
  ...
  /* 5. The ladder must actually ladder. Sorted by rarity index, runs and
     ...
     ⚠ WITHIN A CLASS. Comparing across classes made every tanker look like a
       dominated freight rig ("haul_bowser: 3 runs does not beat haul_flatbed's
       3") when in fact they are two separate ladders a player chooses between
       by BUSINESS, not by tier — a refinery cannot use a box hauler at all. */
  Object.keys(byClass).forEach((cls) => {
    const ordered = byClass[cls].slice().sort((a, b) => rarityIndex(a.rarity) - rarityIndex(b.rarity));
    for (let i = 1; i < ordered.length; i++) {
      ...
      // …and the class's own capacity number must ladder too, or a dearer tier
      // hauls less of the only thing its class exists to haul.
      const cf = CAPACITY_FIELD[cls];
      if (cf && !(hi[cf] > lo[cf])) problems.push(`${hi.id}: ${hi[cf]} ${cf} does not beat ${lo.id}'s ${lo[cf]} ${cf}`);
    }
  });
