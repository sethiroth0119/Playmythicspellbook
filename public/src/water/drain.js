...
export async function place(x, z) {
  ...
  const p = price();
  if (p == null) {
    return { ok: false, why: '🌊 The Sea Drain has no price in this city, so nothing was placed. ' +
                             '(The city never handed the water system its cost settings.)' };
  }
  busy = true;
  try {
    let paid = false;
...
#nwd-hint{position:absolute;left:50%;transform:translateX(-50%);bottom:92px;z-index:7;display:none;
  ...
  border-radius:6px;padding:7px 12px;color:var(--bone,#e9e0cc);font-size:12px;line-height:1.4;
...
#nwd-hint .nwdhint{color:var(--mist,#9d907a);font-size:11px}
#nwd-hint .nwdx{background:none;border:0;color:var(--mist,#9d907a);cursor:pointer;font-size:13px;
