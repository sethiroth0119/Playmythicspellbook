...
export function solve(snap) {
  ...
  /* What those same plants WOULD have drawn from a clean source, so the panel
     can say what share of the draw contamination costs. See rejectedShare. */
  let rejectedIdeal = 0;
  ...
  return {
    ...
    /* 🔴 THE VOLUME ALONE READS BACKWARDS UNDER A PURITY METER (bug-mu5agw8r:
       "the XXX m3/minute rejected at the plant figure is working in reverse …
       the purer I got my water supply, the higher this figure got").
       It is not reversed — it is an ABSOLUTE volume, so it scales with how much
       the plants draw and with the yield of the ground under them. Measured on
       the real solver: a 0.823-purity well rejects 3.47 while a dirtier
       0.789-purity one rejects 2.92, because the cleaner one sits on better
       ground and draws more. Under a PURITY meter that looks like the number
       is inverted.
       So the panel also states the SHARE of what these plants could have drawn
       from clean ground, which is the figure that actually follows purity:
       0 when nothing is rejected, and it falls as purity rises for a given
       network. Both are the same arithmetic the tick charged — no second
       derivation (the rule this file's header sets out). */
    rejectedShare: rejectedIdeal > 0 ? Math.max(0, Math.min(1, rejected / rejectedIdeal)) : 0,
    rejectedIdeal,
