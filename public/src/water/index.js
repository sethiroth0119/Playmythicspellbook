...
/* ?v= on these two because bug-mu2r4z2p changed the contract between them and
   this file (Panel.setFocus / Overlay.footprint): the node-city bump refreshes
   index.js, and without a bump of their own a cached panel.js would pair with
   the new index.js and closePanel() would throw on a missing setFocus. */
import * as Panel from './panel.js?v=wf1';
import * as Overlay from './overlay.js?v=wf1';
...
let mounted = false;
let grid = 24;
/* 📍 The host's camera rig, kept ONLY for bug-mu2r4z2p's "show me where this
   source is". `controls` is OrbitControls (its `.object` is the camera) and
   arrives in the same mount ctx the pipe tool already reads it from. */
let camRig = null;
...
/* 🙂 PLOT MOOD — the invalidation ping, once, for all four network editors
   below. A pipe or a drain changes WHICH TILES ARE SERVED and creates no
   tile at all, so node-city's tryPlace wrapper never sees it; without this
   the mood glyph over a newly plumbed house would not clear until something
   unrelated happened to invalidate the board.
   ⚠ It hangs off the SETTERS, not off refresh(): refresh() also runs on the
     solve tick, and a per-tick invalidate would turn a change-driven layer
     into a polling one. O(1), guarded, and a no-op with no window at all —
     network.js is deliberately node-importable. */
function moodPing(why) {
  try { if (typeof window !== 'undefined' && window.MythicPlotMood) window.MythicPlotMood.invalidate(why); } catch (e) {}
}
...
const API = {
  ...
  mount(h) {
    ...
    try {
      ...
      camRig = (h && h.controls) ? { controls: h.controls, toast: h.toast } : (h && h.toast ? { toast: h.toast } : null);
      Panel.mount(h, { onLayers: () => { Overlay.repaintNext(); NetUI.repaintNext(); refresh(); }, close: () => API.closePanel(),
                       onFocus: (id) => API.focusSource(id) });
      Overlay.mount(h);
      ...
      NetUI.mount(h, {
        ...
        state: () => state,
        netState: () => net,
        /* 🙂 PLOT MOOD RIDES THE PIPE COMMIT TAIL, NOT THE FAÇADE.
           ⚠ THIS LINE IS THE WHOLE FIX FOR A REAL, PROVEN HOLE. The ping used
             to hang only off API.pipes/API.drains below — a console-and-driver
             surface the player never touches. The shipped pipe DRAG commits in
             netui.js commit() and the Sea Drain button in drain.js place(), and
             BOTH of them end at after() -> api.onEdit(), i.e. exactly here. A
             driven MythicWater.drains.place() moved the drain count 0 -> 1 and
             left the mood generation counter frozen at 67, so the glyph over a
             newly drained block only caught up on the 1 s invalidate('tick') —
             by accident, and a second later than the player expects.
           ⚠ AND THE ID IS DISTINCT PER TAIL (water-edit). One shared id would make
             a seam test unable to tell which of the two player paths fired,
             which is the confusion that hid the hole in the first place. */
        onEdit: () => { NetUI.repaintNext(); refresh(); moodPing('water-edit'); },
      });
      ...
      Drain.mount(h, {
        ...
        grid: () => grid,
        netState: () => net,
        /* 🙂 PLOT MOOD RIDES THE SEA DRAIN COMMIT TAIL, NOT THE FAÇADE.
           ⚠ THIS LINE IS THE WHOLE FIX FOR A REAL, PROVEN HOLE. The ping used
             to hang only off API.pipes/API.drains below — a console-and-driver
             surface the player never touches. The shipped pipe DRAG commits in
             netui.js commit() and the Sea Drain button in drain.js place(), and
             ...
             newly drained block only caught up on the 1 s invalidate('tick') —
             by accident, and a second later than the player expects.
           ⚠ AND THE ID IS DISTINCT PER TAIL (drain-edit). One shared id would make
             a seam test unable to tell which of the two player paths fired,
             which is the confusion that hid the hole in the first place. */
        onEdit: () => { NetUI.repaintNext(); refresh(); moodPing('drain-edit'); },
      });
      ...
      try {
        ...
        if (shelf && shelf.register) {
          ...
          shelf.register('waterMains', { save: () => Net.save(),
                                         load: (b) => {
                                           Net.load(b, grid);
                                           /* 🗺 THE PIPES CAN CORRECT THE GROUND. If the saved stamp
                                              disagreed with the id the host derived this boot, Net.load
                                              adopts the saved one — and the aquifers have to move back
                                              with it or the mains end up dug to water that is no longer
                                              there. Both this module and /src/resmap are re-pinned; the
                                              resmap call is guarded because a 404 on that module must
                                              cost the overlay and not the pipes. */
                                           const adopted = Net.adoptedId && Net.adoptedId();
                                           if (adopted) {
                                             pinned = false; setCityId(adopted);
                                             try {
                                               const rm = (typeof window !== 'undefined') && window.MythicResourceMap;
                                               if (rm && rm.adoptCityId) rm.adoptCityId(adopted);
                                             } catch (e) {}
                                           }
                                           NetUI.repaintNext(); Drain.syncMeshes();
                                         } });
        } else {
  ...
  pipes: {
    ...
    add(list) { const n = Net.add(list); if (n) { NetUI.repaintNext(); refresh(); moodPing('water-pipe-add'); } return n; },
    remove(list) { const n = Net.remove(list); if (n) { NetUI.repaintNext(); refresh(); moodPing('water-pipe-remove'); } return n; },
  ...
  drains: {
    ...
    add(list) { const n = Net.addDrain(list); if (n) { Drain.syncMeshes(); NetUI.repaintNext(); refresh(); moodPing('drain-add'); } return n; },
    remove(list) { const n = Net.removeDrain(list); if (n) { Drain.syncMeshes(); NetUI.repaintNext(); refresh(); moodPing('drain-remove'); } return n; },
  ...
  /* Closing the view drops the focus: the info view is a mode (see refresh()),
     and a yellow outline that outlived the panel would be paint with no key. */
  closePanel() { Panel.hide(); Overlay.hide(); Overlay.setFocus(null); Panel.setFocus(null); return true; },

  /* 📍 bug-mu2r4z2p — "is there a way for these to be highlighted on the city
     map if you select them?" `id` is what the panel's source row carries:
     'basin:<i>', 'surface' or 'sea'. Selecting outlines that source's tiles on
     the overlay and slides the camera so the body is in the middle of the
     screen; selecting the same row again clears it. Returns the footprint so a
     driver can check it without reading pixels.
     ⚠ THE CAMERA MOVES BY TRANSLATION ONLY — target and camera shifted by the
       same vector, exactly what the WASD pan does — so the player's zoom and
       angle survive. A lookAt-style jump that reset the orbit would lose the
       view they had set up to compare this basin against something.
     ⚠ AN EMPTY FOOTPRINT SAYS SO. The coast can sit wholly off the plate in a
       city whose sea never reaches a buildable column; highlighting nothing
       and moving nothing would be the original bug again. */
  focusSource(id) {
    if (!mounted) return null;
    const next = (id && Overlay.focused() !== id) ? id : null;
    if (!next) {
      Overlay.setFocus(null); Panel.setFocus(null);
      Panel.render(state, caps(), true); refresh();
      return { id: null, tiles: [] };
    }
    let tiles = [];
    try { tiles = Overlay.footprint(H(), next, grid); } catch (e) { tiles = []; }
    if (!tiles.length) {
      try { camRig && camRig.toast && camRig.toast('📍 That source does not reach any square of this city.', 'bad'); } catch (e) {}
      return { id: next, tiles: [] };
    }
    Overlay.setFocus({ id: next, tiles });
    Panel.setFocus(next);
    const c = Overlay.centreOf(tiles);
    let panned = false, at = null;
    try {
      const ctl = camRig && camRig.controls, cam = ctl && ctl.object;
      if (ctl && ctl.target && cam && c) {
        // node-city's tile → world mapping is `x - HALF + .5` (overlay.js's header).
        const half = grid / 2;
        const dx = (c.x - half + 0.5) - ctl.target.x, dz = (c.z - half + 0.5) - ctl.target.z;
        ctl.target.x += dx; ctl.target.z += dz;
        cam.position.x += dx; cam.position.z += dz;
        if (typeof ctl.update === 'function') ctl.update();
        panned = true;
        at = { x: ctl.target.x, z: ctl.target.z };
      }
    } catch (e) { panned = false; }
    Panel.render(state, caps(), true);
    refresh();
    return { id: next, tiles, centre: c, panned, at };
  },
