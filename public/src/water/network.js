...
/* Set by load() when the saved stamp overrode the derived id, so the host can
   realign the ground with the pipes. Read once and cleared by the caller. */
let adopted = null;
export function adoptedId() { const a = adopted; adopted = null; return a; }
...
export function load(blob, grid) {
  ...
  /* 🔴 A MISMATCH USED TO BIN THE ENTIRE NETWORK, AND THAT IS WHAT PLAYERS
     SAW AS "our pipes are deleting". The guard was written against one
     hazard — a mayor restoring somebody else's city — but the blob it
     refuses does not arrive from another city. It arrives on the save
     shelf, out of THIS city's own save file, whose ownership is already
     stamped and checked before any of it is read (see cityKey's _owner
     note). Within one save file the pipes are definitionally this city's.

     What the mismatch actually meant was that the HOST derived a different
     ground id this boot than it did when the pipes were laid — the
     cityGroundId drift fixed in node-city. The cost of being wrong was
     asymmetric and nobody had priced it: refusing lost every main the
     player had ever laid, and the next autosave wrote the empty network
     over the top, so it could never come back.

   ⭐ SO THE SAVED ID WINS, which is the rule /src/resmap and /src/water's
     own hydrology slice already follow ("a city founded on one id keeps its
     ground even if the host would derive a different one today"). This was
     the one place doing the opposite. Adopting also puts the GROUND back:
     the pipe stamp is the oldest surviving evidence of what this city was
     seeded from, so the aquifers return to where the mains were dug.
   ⚠ THE ADOPTION IS RECORDED, not silent — index.js reads it back and
     re-pins the resource map, and a silent realignment of the whole world
     under a city is not something to do without saying so. */
  adopted = null;
  if (blob.cityId && cityId && String(blob.cityId) !== cityId) {
    try { console.warn('[water/mains] this city was seeded as ' + blob.cityId + ' and the host derived ' + cityId + ' — adopting the saved id and keeping the ' + ((blob.p || '').split(' ').filter(Boolean).length) + ' mains'); } catch (e) {}
    adopted = String(blob.cityId);
  }
  if (blob.cityId) cityId = String(blob.cityId);
...
export default { solve, idle, components, attachOf, pathBetween, add, remove, has, keys,
                 count, reset, save, load, setCity, adoptedId, pipeKey, setDomain, domain,
