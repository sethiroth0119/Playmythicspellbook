...
/* 📍 EVERY SOURCE ROW IS A BUTTON TO ITS PLACE ON THE MAP (bug-mu2r4z2p: "I
   can't locate where these sources are on the map (nothing brings any
   information up) — is there a way for these to be highlighted on the city
   map if you select them?"). The row wraps the name line AND its sub-line so
   the whole block is the target, not a 12-px word. Clicking the focused row
   again clears it. index.js decides what a focus means (outline + camera);
   this file only reports which id was clicked and draws the selected state. */
let focusId = null;
export function setFocus(id) { focusId = id || null; }
function srcOpen(id, name) {
  const on = focusId === id;
  return '<div class="wtsrcitem' + (on ? ' on' : '') + '" role="button" tabindex="0" data-wtsrc="' + esc(id) +
    '" aria-pressed="' + on + '" title="' + (on ? 'Clear the highlight' : 'Show ' + esc(name) + ' on the map') + '">';
}
function srcClose() { return '</div>'; }
function pin(id) { return '<i class="wtpin">' + (focusId === id ? '📍' : '⌖') + '</i>'; }

function sources(s) {
  let h = '<div class="wtsec">SOURCES<span class="wtsecv wthint">click one to find it on the map</span></div><div class="wtsrc">';
  for (const b of s.basins) {
    const st = STATUS[b.status] || STATUS.recharging;
    h += srcOpen('basin:' + b.i, b.name) + '<div class="wtsrow">' +
      '<span class="wtsn">' + pin('basin:' + b.i) + '🕳 ' + esc(b.name) + (b.springfed ? ' <i class="wtspring" title="Fed by surface water">⛲</i>' : '') + '</span>' +
      ...
      '<div class="wtsub2">' + fmtVol(b.stock) + ' of ' + fmtVol(b.volume) +
        ' · pumping ' + fmtQ(b.pump) + ' against ' + fmtQ(b.recharge) + ' of recharge</div>' + srcClose('basin:' + b.i);
  }
  ...
  if (s.sea) {
    h += srcOpen('sea', s.sea.name) + '<div class="wtsrow">' +
      '<span class="wtsn">' + pin('sea') + '🌊 ' + esc(s.sea.name) + ' <i class="wtspring" title="Salt water">🧂</i></span>' +
      ...
      '<div class="wtsub2">drawing ' + fmtQ(s.sea.draw) + ' · the ' + esc(s.sea.side) +
        ...
        ' coastal · salt is not pollution and does not clean up</div>' + srcClose('sea');
  }
  if (s.surface.river || s.surface.lakes) {
    const bad = s.surface.purity < WATER.purity.warnBelow;
    h += srcOpen('surface', s.surface.river ? 'the river' : 'the lakes') + '<div class="wtsrow">' +
      '<span class="wtsn">' + pin('surface') + '🌊 ' + (s.surface.river ? 'River' : '') +
      ...
      '<div class="wtsub2">drawing ' + fmtQ(s.surface.draw) +
        (s.surface.river ? ' · open water carries pollution downstream to every intake below it' : '') + '</div>' + srcClose('surface');
  }
  return h + '</div>';
}
...
function html(s, caps) {
  ...
  /* 🔴 SHARE FIRST, VOLUME SECOND (bug-mu5agw8r). The volume is an absolute
     rate: it grows with the number of plants and with the yield of the ground
     they stand on, so under this meter it could rise while purity rose. The
     share is the part of the draw contamination costs, and it follows purity.
     See the note beside rejectedShare in hydro.js. */
  h += '<div class="wtends"><span>' + (s.rejected > 0.0005
        ? '<b class="dn">' + pct(s.rejectedShare || 0) + '</b> of the draw rejected at the plant · <b class="dn">' + fmtQ(s.rejected) + '</b>'
...
#ncwtr .wtsrcitem{cursor:pointer;border-radius:5px;margin:0 -4px;padding:0 4px;border:1px solid transparent}
#ncwtr .wtsrcitem:hover{background:rgba(255,224,102,.07)}
#ncwtr .wtsrcitem:focus-visible{outline:2px solid #ffe066;outline-offset:1px}
#ncwtr .wtsrcitem.on{background:rgba(255,224,102,.12);border-color:rgba(255,224,102,.55)}
#ncwtr .wtpin{font-style:normal;display:inline-block;width:14px;color:#ffe066;opacity:.8}
#ncwtr .wthint{color:var(--mist);font-size:10px;opacity:.8}
...
export function mount(h, a) {
  ...
  root.addEventListener('click', (ev) => {
    ...
    const src = ev.target.closest('[data-wtsrc]');
    if (src && api.onFocus) api.onFocus(src.dataset.wtsrc);
  });
  // Keyboard parity for the role="button" rows — Enter/Space, as a button.
  root.addEventListener('keydown', (ev) => {
    if (ev.key !== 'Enter' && ev.key !== ' ') return;
    const src = ev.target.closest && ev.target.closest('[data-wtsrc]');
    if (!src || !api.onFocus) return;
    ev.preventDefault(); api.onFocus(src.dataset.wtsrc);
  });
...
export function render(state, caps, force) {
  if (!root || !open) return;
  let hot = false;
  /* 📍 `.wtsrcitem:hover` for the same reason as the legend rows: a source row
     rebuilt between mousedown and mouseup never receives its click. A FORCED
     render (the focus just changed) goes through anyway — that repaint is the
     answer to the click, and the pointer is necessarily still on the row. */
  try { hot = !force && (!!root.querySelector('.wtrow:hover, .wtsrcitem:hover') ||
    // A clicked source row keeps keyboard focus; that must not freeze the panel
    // for the rest of the session the way a focused checkbox rightly does.
    !!(document.activeElement && root.contains(document.activeElement) && !document.activeElement.matches('.wtsrcitem'))); }
  catch (e) { hot = false; }
  if (hot) return;
