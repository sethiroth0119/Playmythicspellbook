...
import { itemCount, toast, ready, addGems, craftedBook as bridgeBook, confirmAsync } from './ws.bridge.js';
...
import { armoury, armouryDetail, provenance, slotsFor, equip, heroList, carriedBy, vaultDesk, deliverToVault } from './armoury.js';
...
function pickerView(s) {
  ...
  const schemPanel = schem
    ...
    : '';
  /* ⚠ THIS ELEMENT WAS MISSING AND IT MADE A WHOLE MESSAGE LAYER DEAD CODE.
     buildView and forgeView both emit `.wsb-msg`; pickerView did not — yet
     pickerView is the ONLY view that contains the armoury (vault delivery),
     the contract board and the blueprint claims. Every one of those handlers
     assigns `_msg` and calls paint(), and paint() writes `_msg` into a node
     that this view never rendered. Measured in a browser: a refused vault
     delivery produced a ZERO-BYTE screen delta — the player clicked Deliver,
     confirmed, and nothing whatsoever happened. The sharpest case was
     `vault_unreadable`, the refusal that exists to stop a blind delivery
     overwriting the guild's stack: the one message that could never be seen. */
...
function armouryView() {
  ...
  const hs = heroList();
  /* 🏦 The corporation, if the smith is in one. Read ONCE for the whole list
     rather than per weapon — it is the same answer for every row, and this
     function repaints on every bench action. `null` hides the delivery path
     entirely: offering "Deliver to vault" to an independent smith would be a
     button whose only possible outcome is an error. */
  const desk = vaultDesk();
  ...
  const rows = list.map((def) => {
    ...
    return `<div class="wsb-st filled" style="min-height:0;padding:.65rem">
      ...
      ${vaultBtn(def, p, holder, desk)}
    </div>`;
  }).join('');

  return `<div class="wsb-panel" style="margin-top:1rem"><h3>🏛 Armoury</h3>
    <div class="wsb-sub" style="margin-bottom:.5rem">What you have built, and what went into it.${
      desk ? ' Anything finished can be delivered to <b>' + esc(desk.name) + '</b>’s vault.' : ''}</div>
    <div class="wsb-stations">${rows}</div></div>`;
}

/* 🏦 The delivery control for one weapon. Renders NOTHING when the smith is in
   no corporation, and renders a REASON rather than a dead button when the
   weapon cannot go — a disabled control with no explanation is the same defect
   as a missing one, just quieter. Both refusals mirror deliverToVault(), which
   is where the rule actually lives; this only says it out loud. */
function vaultBtn(def, p, holder, desk) {
  if (!desk) return '';
  if (p.unverified) {
    return `<div class="wsb-sub" style="margin-top:.3rem;color:#e8a09f">Unverified — it was minted offline, so it cannot go into a shared vault.</div>`;
  }
  if (holder) {
    return `<div class="wsb-sub" style="margin-top:.3rem">${esc(holder.hero.name)} is carrying this — unequip it before delivering.</div>`;
  }
  return `<div style="margin-top:.3rem"><button class="wsb-btn" style="padding:.15rem .45rem;font-size:.68rem"
      data-vault="${esc(def.id)}">🏦 Deliver to ${esc(desk.name)}’s vault</button></div>`;
}
...
function bind(s) {
  ...
  /* 🏦 Deliver to the corporation vault. Confirmed first — this is the one
     action on the bench that moves a weapon somewhere the smith cannot take it
     back from on their own, and an accidental click on a two-minute build is
     not recoverable from here. The button is disabled for the round trip so a
     double-click cannot try to deliver the same weapon twice. */
  el.querySelectorAll('[data-vault]').forEach((b) => {
    b.onclick = async () => {
      const id = b.getAttribute('data-vault');
      const bk = bridgeBook() || {};
      const nm = (bk[id] && bk[id].name) || id;
      const desk = vaultDesk();
      if (!desk) { _msg = 'You are not in a corporation.'; paint(); return; }
      const ok = await confirmAsync('Deliver ' + nm + ' to ' + desk.name +
        '’s vault? It leaves your armoury and any member of the corporation can draw it.');
      if (!ok) return;
      b.disabled = true;
      const r = await deliverToVault(id);
      _msg = (r && r.ok)
        ? ('Delivered ' + (r.name || nm) + ' to ' + (r.corp || desk.name) + '’s vault.')
        : ({
            equipped:   (r && r.hero) ? (r.hero + ' is carrying it — unequip it first.') : 'It is equipped — unequip it first.',
            unverified: 'That weapon was minted offline and cannot go into a shared vault.',
            not_held:   'You are not holding that weapon.',
            no_corp:    'You are not in a corporation.',
            offline:    'The vault needs a connection — nothing was moved.',
            network:    'The vault could not be reached. Check the vault before delivering again.',
            /* The vault could not be READ. Refusing is deliberate: the degrade
               path writes an absolute quantity, so delivering blind would
               overwrite whatever the guild already has in that row. */
            vault_unreadable: 'Could not read the corporation vault, so nothing was moved — it is still yours. Try again in a moment.',
            no_bridge:  'The bench is not connected to the game.',
          }[(r && r.error) || ''] || ('Delivery refused: ' + ((r && r.error) || 'unknown') + '. The weapon is still yours.'));
      /* Toast AS WELL as the inline line, and only for a refusal. paint()
         re-renders the armoury and a refused delivery leaves the row exactly
         where it was, so the only thing that changed is one line of text the
         player's eye is not on — these are precisely the outcomes that must
         not be missed. A success is self-evident: the row disappears. */
      if (!(r && r.ok)) { try { toast('🏦 ' + _msg, 5200); } catch (e) {} }
      paint();
    };
  });
