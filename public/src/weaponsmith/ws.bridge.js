...
/* 🏦 THE CORPORATION VAULT. corpDesk() is read-only and returns null when the
   smith belongs to no corporation — the armoury hides the delivery path in
   that case rather than offering one that can only fail.
   deliverToCorpVault() is the whole operation, take and unwind together: like
   buyBlueprintAza above, the module is never handed the half that removes a
   weapon without the half that gives it back. index.html owns the refund
   because index.html owns the inventory. */
export const corpDesk           = ()       => { const b = bridge(); try { return b && b.corpDesk ? b.corpDesk() : null; } catch (e) { return null; } };
export const deliverToCorpVault = async (id) => {
  const b = bridge();
  try { return (b && b.deliverToCorpVault) ? await b.deliverToCorpVault(id) : { ok: false, error: 'no_bridge' }; }
  catch (e) { return { ok: false, error: 'threw' }; }
};
