...
/* 👷 HOW MANY POSTS A BUILDING OFFERS — 10 to 200, by the size of the
   ...
   ⚠ POSTS ARE CAPACITY, NOT A PROMISE OF WORKERS. The city's crew is capped
     at CREW_MAX units in total, so a 200-post stadium reads 3 / 187 filled
     for most cities; that is the point of the number — room to grow into. */
  ...
  return '<div class="pbox"><h3>👷 Who works the ' + esc(def ? def.name : k) + '?</h3>' +
    ...
    '<div class="pfoot">' +
      (canEnlistMore()
        ...
        : '<button class="pbtn" id="pk-enlist" disabled>🛏 No beds left — build Housing or a Resting House</button>') +
      '<button class="pbtn" id="pk-close">Close</button></div></div>';
}
...
function enlistFlow(toTile) {
  if (!canEnlistMore()) {
    try { CTX.toast('🛏 No beds left — build Housing or a Resting House first.', 'bad'); } catch (e) {}
    return;
  }
