...
/* 🛏 RETUNED FOR bug-mu4mc2c8 ("Post a worker doesn't work … the numbers look
   out by a factor of ~100"). Measured before the change, over the 98 buildings
   that take a crew: posts ran 18 (Farm) to 159 (Nuclear Plant), 7,522 at Lv 1
   across the set — a mid city of ~25 workplaces offered ~1,700 posts against a
   bed cap of AT MOST 20. The player's reading was exact: "I have hundreds of
   beds free, and still no more cards" — CREW_MAX bound long before housing did.
   Posts are now a handful per building (see CARD_POSTS below) and the cap
   rises to match, so a mid city can staff what it builds:
     before  base 3 · +2 per Housing level · +2 per Resting House level · max 20
     after   base 3 · +2 per Housing level · +5 per Resting House level · max 60
   The Resting House step is 5 so a Tier-4 House adds 20 — the same "20" its
   own tier ladder prints for beds, which is the number the player quoted.
   ⚖ OWNER-ADJUSTABLE: CREW_MAX and CREW_PER_RESTHOUSE are balance choices, not
     derived ones. What limits a big crew now is its UPKEEP (CREW_EAT_PER_MIN —
     60 units eat 6 rations/min, about seven Restaurants' worth), which is the
     intended brake, rather than a flat cap no amount of building could move.
   ⚠ The O(crew × jobs) note above still holds: autoFill at 60 × ~100 open
     posts is ~6k workPower calls per placement, once per click. */
export const CREW_BASE = 3;
export const CREW_PER_HOUSING = 2;
export const CREW_PER_RESTHOUSE = 5;
export const CREW_MAX = 60;
...
/* Is there a bed free? Both the roster footer and the post picker ask this, so
   they can never disagree about whether enlisting is possible. */
export function canEnlistMore() { return state().length < crewCap(); }
...
/* 🔴 bug-mu4mc2c8: "The message says to build housing — I have hundreds of
   beds free, and still no more cards." At the hard cap more Housing does
   nothing, so the sentence has to say which wall the player hit. */
export function bedsFullMsg() {
  return capParts().capped
    ? 'The work crew is at the city maximum of ' + CREW_MAX + ' — more Housing will not add beds'
    : 'No beds left — build Housing or a Resting House';
}
...
export function slotsAt(k) {
  ...
  /* A building the tick does not multiply cannot use a crew, whatever the work
     ...
     🔴 …AND THERE IS NOW A THIRD WAY TO BE MULTIPLIED. When this guard was
       written, `gen` and `svc` were the only two things a tick could scale.
       Patronage added a third: a shop with neither now earns Cinder from
       residents who walk in, and a posted unit lifts exactly that. Left as it
       was, the guard silently refused a crew at every building on the high
       street — the shops are the ONLY ones a player watches make money, so
       "nowhere to put my units" would have survived the whole work-table
       round with nothing on screen explaining it.
     ⚠ THE HOST ANSWERS THIS, NOT THIS FILE. earnsFromCustomers is handed in
       at mount and is backed by patronage's own NEEDS table, so the list of
       paying shops has exactly one definition. A copy here would drift the
       first time a need gained a venue. Absent host ⇒ the old behaviour. */
  let paidByCustomers = false;
  try { paidByCustomers = !!(CTX.earnsFromCustomers && CTX.earnsFromCustomers(t.type)); } catch (e) {}
  if (!def.gen && !def.svc && !paidByCustomers) return 0;
  return cardPostCapacity(t, def);
}
/* 👥 NPC SEATS — the 10–200 figure below, for NAMED RESIDENTS (node-city's
   npcSeatsAt). Same gate as slotsAt, so a building seats residents exactly
   when it could take a card; a different count, because the two are
   different populations — see the bug-mu4mc2c8 note on CARD_POSTS. */
export function seatsAt(k) {
  if (!(slotsAt(k) > 0)) return 0;
  const t = G().tiles[k];
  return postCapacity(t, defOf(t.type), k);
}
/* 👷 CARD POSTS — how many of the player's UNITS a building takes.
   🔴 bug-mu4mc2c8. For a round these shared the 10–200 resident-seat figure
   below, which was sized for a named roster of up to 400 residents, not for a
   crew capped at 20 cards. A Nuclear Plant read "0 / 159 posted", a Purifier
   "0 / 34", and the Posts row printed red until filled — so every building in
   the city looked "massively penalised" with no way to ever clear it. (It was
   never an actual output penalty: an empty post is ×1.00, the crew only adds.
   But the screen said otherwise, on every building.)
   THE RULE, back to what this file's POSTS header always promised: a building
   takes `def.crew` units — its own staffed-jobs figure, 1 (Gas Station) to 6
   (Nuclear Plant), so the bigger business still holds more — plus one per
   level above the first, clamped to CARD_POSTS.MAX. No seeded jitter: at a
   scale of 2 a ±20% roll is a coin flip between 2 and 3, not "rng a player
   can plan around".
     before  Farm 18 · Purifier 34 · Restaurant 75 · Nuclear 159 (Lv 1)
             sum over the 98 crewable buildings: 7,522
     after   Farm 2  · Purifier 2  · Restaurant 2  · Nuclear 6  (Lv 1; +1/level)
             sum over the 98 crewable buildings: ~195
   Why a handful is enough: BOOST_CAP (work.js) doubles a building with ~2
   good units, so a third or fourth post is the most a player could ever use. */
export const CARD_POSTS = { MIN: 1, MAX: 8, PER_LEVEL: 1 };
export function cardPostCapacity(t, def) {
  const base = Math.max(CARD_POSTS.MIN, (def && +def.crew) | 0);
  const lvl = CARD_POSTS.PER_LEVEL * Math.max(0, ((t && t.lvl) | 0) - 1);
  return Math.max(CARD_POSTS.MIN, Math.min(CARD_POSTS.MAX, base + lvl));
}
/* 👷 HOW MANY POSTS A BUILDING OFFERS — 10 to 200, by the size of the
   business. "the bigger the business the more Posts filled it can hold."
   Size is the building's own Cinder price on a log scale (a 14 🔥 Farm is
   small, a 600 🔥 Stadium is big, 3,000+ is the top), then a ±20% roll that
   is SEEDED on the city and the tile so it is the same number every time the
   card is opened and after every reload — "rng" that a player can plan
   around rather than one that changes under them — and a quarter more per
   level above the first.
   🔴 bug-mu4mc2c8: THIS IS NOW THE RESIDENT-SEAT COUNT ONLY (seatsAt →
     node-city npcSeatsAt). It used to be the card-post count too, and "a
     200-post stadium reads 3 / 187 filled" — the line this note used to end
     on — is exactly what players reported as broken. Cards use
     cardPostCapacity above. */
export const POSTS_MIN = 10, POSTS_MAX = 200;
function hash01(str) {
  let h = 2166136261 >>> 0;
  for (let i = 0; i < str.length; i++) { h ^= str.charCodeAt(i); h = Math.imul(h, 16777619) >>> 0; }
  return (h >>> 0) / 4294967296;
}
export function postCapacity(t, def, k) {
  const cost = Math.max(1, +((def && def.cost && def.cost.cinder)) || 1);
  const size = Math.max(0, Math.min(1, (Math.log(cost) - Math.log(10)) / (Math.log(3000) - Math.log(10))));
  const base = POSTS_MIN + (POSTS_MAX - POSTS_MIN) * size;
  const jitter = 0.8 + hash01('posts|' + salt() + '|' + k + '|' + (t && t.type)) * 0.4;
  const lvl = 1 + 0.25 * Math.max(0, ((t && t.lvl) | 0) - 1);
  return Math.max(POSTS_MIN, Math.min(POSTS_MAX, Math.round(base * jitter * lvl)));
}
...
export function enlist(cardId) {
  ...
  if (crew.length >= crewCap()) return bedsFullMsg() + '.';
...
#crewpick .orem{display:inline-block;margin-top:7px;font-size:10px;padding:3px 9px;border-radius:6px;
  border:1px solid rgba(224,138,128,.5);color:#e08a80;background:rgba(224,138,128,.10);cursor:pointer;}
#crewpick .orem:hover{border-color:#e08a80;background:rgba(224,138,128,.22);}
  ...
  return '<div class="cbox">' +
    ...
    '<div class="cfoot2">' +
      '<button class="pbtn" id="crew-add"' + (crew.length >= cap ? ' disabled' : '') + '>' +
        (crew.length >= cap ? '🛏 ' + esc(bedsFullMsg()) : '➕ Enlist a unit') + '</button>' +
  ...
  return '<div class="pbox"><h3>👷 Who works the ' + esc(def ? def.name : k) + '?</h3>' +
    ...
    (cands.length ? cands.map(o =>
      '<button class="opt' + (o.here ? ' here' : o.ok ? ' good' : '') + '"' + (o.ok ? '' : ' disabled') +
        ...
        /* 🔴 A VISIBLE WAY OFF THE JOB. Clicking a posted row already recalled
           the unit, but nothing on screen said so — the row read "posted
           here" and looked like every other pickable option, so the only way
           to discover it was to click the thing that looks like it posts.
           An affordance the player has to guess at is not one.
           ⚠ A SPAN, NOT A BUTTON. The row itself is a <button>; nesting one
             inside another is invalid HTML and the parser silently unnests
             it, which would drop this control out of its row entirely. */
        (o.here ? '<span class="orem" role="button" tabindex="0" data-unpost="' + esc(o.card) +
          '">✕ Remove from this building</span>' : '') +
      '</button>').join('')
      : '<div class="psub">Nobody is on the work crew yet — enlist one of your units below and it will be posted here.</div>') +
    /* 🔴 THE DEAD END THIS FIXES, AND IT IS THE WHOLE REPORTED BUG.
       Reported: "the system to add units from the players collection to work in
       buildings is not working, I cannot add my units."
       Nothing was broken. Every part worked in isolation — the collection
       reaches the city, the roster renders, the picker lists cards, enlist and
       post both succeed. What did not exist was a way to enlist FROM HERE.
       The player's natural route is: building → 'Post a worker here' → this
       dialog → 'Nobody is on the work crew. Enlist someone first.' → and the
       only control was CLOSE. It told them what to do and gave them no way to
       do it. The enlist button lived on a side card in the left column that
       nothing on this path mentions.
       ⚠ AND IT ENLISTS *AND POSTS* IN ONE ACTION when opened from a building.
         Standing in front of a farm asking for a worker, the player has
         already made both decisions; making them enlist, close, find the
         roster and post again is asking twice for one answer.
       ⚠ The button states the bed limit rather than vanishing at it — a
         control that disappears teaches nothing, and 'no beds' is a thing the
         player can go and fix. */
    '<div class="pfoot">' +
      (canEnlistMore()
        ? '<button class="pbtn good" id="pk-enlist">➕ Enlist a unit from your collection</button>'
        : '<button class="pbtn" id="pk-enlist" disabled>🛏 ' + esc(bedsFullMsg()) + '</button>') +
      '<button class="pbtn" id="pk-close">Close</button></div></div>';
}
...
/* ⚠ MODULE SCOPE, AND IT HAS TO BE. This was first inserted just above the
   `crew-add` branch — which sits INSIDE the panel's click handler, so the
   function ended up nested in it. Nested declarations are legal, so nothing
   complained at parse time; but onPickClick could not see it, and the
   `export default { … enlistFlow … }` at the bottom threw a ReferenceError
   during module evaluation. The whole crew module then failed to mount and
   CREW was null — i.e. the fix for 'I cannot add my units' broke the feature
   outright, in a way modcheck's parse pass cannot catch and only loading the
   page reveals. */
/* 🔴 ONE ENLIST FLOW, AND BOTH ENTRY POINTS USE IT. The roster's button and
   the building picker's button were about to be two copies of the same twelve
   lines, which is how the sixth-argument bug below gets fixed in one of them
   and not the other.
   `toTile` is the building that asked, or null when the roster asked. */
function enlistFlow(toTile) {
  if (!canEnlistMore()) {
    try { CTX.toast('🛏 ' + bedsFullMsg() + '.', 'bad'); } catch (e) {}
    return;
  }
  /* The picker shows each card's WORK profile, not just its power. Choosing a
     ...
     timber.
     ⚠ THE FLAG IS THE SIXTH ARGUMENT IN THIS TREE, NOT THE FOURTH. The picker
       here already takes (title, filter, onPick, emptyMsg, onClose); passing
       `true` in slot four — as the branch this came from did, against its own
       three-argument picker — would set the EMPTY MESSAGE to "true" and never
       show a work profile at all. Nulls are explicit so the shape is obvious. */
  /* 🔴 STEP ASIDE, THEN COME BACK — the collection opened BEHIND this dialog.
     Reported: "the modal does not disappear and the list appears behind it."
     #crewpick is position:fixed at z-index 70; the host's #cardpicker is
     z-index 45 by deliberate design (it has to sit under the inspect panel).
     So the picker this button opens was rendering *below* the dialog that
     opened it: the player clicked Enlist, the dialog stayed put, and their
     collection was a dim smear behind it.
     ⚠ HIDDEN, NOT CLOSED. closePick() nulls PICK, and PICK is what remembers
       which building asked — tearing it down here is how enlisting from a
       farm would land the unit nowhere. Hiding keeps that state intact.
     ⚠ RESTORED ON *EVERY* EXIT. openCardPicker funnels its three ways out
       (pick, Close, backdrop) through one dismiss() that always fires
       onClose, so passing restore there covers cancelling too. Restoring only
       on a successful pick would leave a player who changed their mind
       staring at a dimmed city with no dialog and no way back.
     The z-index bump is local to THIS instance: the global #cardpicker rule
     stays 45 for every other caller, because that ordering is load-bearing
     elsewhere and this is the one caller opening it from a modal that
     outranks it. 9840 sits under #toasts (9850) so confirmations still land
     on top. */
  const host = document.getElementById('crewpick');
  if (host) host.style.display = 'none';
  const restore = () => { const h = document.getElementById('crewpick'); if (h) h.style.display = ''; };
  try {
    CTX.openCardPicker('Enlist a unit onto the work crew', c => c.type !== 'Structure', (c) => {
      ...
      if (toTile) {
        /* Enlisted BECAUSE a building asked. Post it there now — and if it
           cannot work there, say why rather than silently leaving it idle:
           the player picked this unit for this job and deserves the reason. */
        const why = post(c.id, toTile);
        if (why) {
          after('👷 ' + c.name + ' joined the work crew, but ' + why, true);
          openPostPicker(c.id);
        } else {
          after('👷 ' + c.name + ' is working here now.');
          PICK = { forTile: toTile };
          renderPick();
        }
        return;
    ...
    try { const cp = document.getElementById('cardpicker'); if (cp) cp.style.zIndex = '9840'; } catch (e) {}
  } catch (e) { restore(); }
}
...
function onPickClick(ev) {
  ...
  if (ev.target.id === 'pk-enlist') {
    /* 🔴 REMEMBER WHICH BUILDING ASKED, before the picker is torn down. PICK is
       cleared by closePick(), and the whole point of enlisting from here is to
       land the unit in THIS building — reading it afterwards gets null and the
       player is dropped back into the same 'nobody is on the crew' dialog they
       just tried to escape. */
    const forTile = PICK && PICK.forTile;
    enlistFlow(forTile);
    return;
  }
  ...
  /* ⚠ BEFORE the row handler, and it stops the event. The control lives
     inside the .opt row, so without this the click bubbles on and the row's
     own "already posted here → recall" branch runs a second unpost against a
     unit that is already idle. */
  const rem = ev.target.closest('[data-unpost]');
  if (rem) {
    ev.stopPropagation();
    const rid = rem.dataset.unpost, rc = cardOf(rid);
    unpost(rid);
    after('👷 ' + ((rc && rc.name) || rid) + ' is off duty.');
    renderPick(); render();
    return;
  }
  ...
  if (cardBtn && PICK && PICK.forTile) {
    ...
    // ⚠ render() as well as renderPick(): the crew roster behind this dialog
    //   shows each unit's posting, and leaving it stale means the panel and
    //   the dialog disagree about where the same unit is standing.
    if (postOf(id) === PICK.forTile) { unpost(id); after('👷 ' + ((c && c.name) || id) + ' is off duty.'); renderPick(); render(); return; }
...
export function mount(ctx) {
  ...
  const api = {
    ...
    // posts
    slotsAt, seatsAt, takesWork, postsAt, freeSlotsAt, postOf, idleIds, canWorkAt, post, unpost,
