...
function paint() {
  ...
  const inp = inputs();
  /* 🦠 The illness report rides the same card, so the card shows whenever
     the city has a case even before it has a grave. */
  let virus = null, worldVirus = null;
  try { virus = ctx.plague ? ctx.plague() : null; } catch (e) { virus = null; }
  try { worldVirus = ctx.worldVirus ? ctx.worldVirus() : null; } catch (e) { worldVirus = null; }
  const show = inp.armed || S.unburied > 0 || S.deaths > 0 || !!(virus && virus.cases > 0) || !!worldVirus;
