...
/* 🦠 THE VIRUS, ON THE SAME CARD. `v` is /src/plague's cityReport() for this
   city (null when that module is absent or not yet mounted), `wv` the world
   virus event. Every figure is that module's own — cases, stages, pressure,
   the output drag — printed, never re-derived. */
function virusLines(v, wv) {
  const L = [];
  if (wv && wv.def) {
    L.push('🦠 <b>World virus</b> ' + esc(wv.def.icon || '') + ' ' + esc(wv.def.name) +
      ' <span class="mut">— loose in every city at once' + (wv.def.line ? '. ' + esc(wv.def.line) : '') + '</span>');
  }
  if (!v) {
    if (!wv) L.push('🦠 <b>Virus</b> <span class="mut">no illness is being tracked in this city this session</span>');
    return L;
  }
  const cases = v.cases | 0;
  if (!cases) {
    L.push('🦠 <b>Virus</b> <span class="ok">no active cases</span> · infection pressure <b class="' +
      (v.pressure >= 0.5 ? 'bad' : v.pressure >= 0.25 ? 'warn' : '') + '">' + pct(v.pressure || 0) + '</b>' +
      (v.pressure >= 0.25 ? ' <span class="mut">— health, water and food coverage keep this down; a well-built city reaches ~0</span>' : ''));
    return L;
  }
  L.push('🦠 <b>Virus</b> <b class="bad">' + cases + '</b> of ' + (v.roster | 0) + ' residents sick (' + pct(v.share || 0) + ')' +
    ' · pressure <b>' + pct(v.pressure || 0) + '</b>' +
    (v.healthDrag > 0 ? ' · city output <b class="bad">−' + pct(v.healthDrag) + '</b>' : ''));
  for (const s of (v.strains || []).slice(0, 3)) {
    const st = s.strain || {};
    L.push('&nbsp;&nbsp;↳ <b>' + esc(st.name || 'unnamed strain') + '</b>' +
      (st.isolate ? ' <span class="mut">' + esc(st.isolate) + '</span>' : '') +
      ' · ' + s.cases + ' case' + (s.cases === 1 ? '' : 's') +
      ' <span class="mut">(' + (s.stages.incubating | 0) + ' incubating · ' + (s.stages.symptomatic | 0) + ' symptomatic · ' +
      (s.stages.critical | 0) + ' critical)</span>' +
      (st.curedAt ? ' <span class="ok">cure administered</span>' : ''));
  }
  L.push('<span class="mut">A cure is mixed in the Research Facility\'s Containment Lab; clinics, med labs and coverage slow the spread.</span>');
  return L;
}
...
export function render(f, x) {
  ...
  if (!f.armed) {
    ...
    for (const line of virusLines(x.virus, x.worldVirus)) L.push(line);
    return L.join('<br>');
  }
