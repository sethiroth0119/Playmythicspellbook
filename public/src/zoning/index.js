...
export function mount(ctx) {
  ...
  /* 🛤 A ZONE UNDER A ROAD IS DROPPED — the other half of bug-mu2q24h4: "those
     zoned tiles show up on the Develop button (even though nothing can happen)".
     A road drawn over zoned land left the zone underneath it, plan() found a
     tile there and counted it as an occupied plot, and the overlay drew the
     film over the carriageway. Roads are never zoned (setZone refuses the
     paint), so a zone found under one is stale by definition and is removed —
     here, where every redraw and every plan passes, rather than in the road
     tool, so it also cleans up saves written before this. Returns how many. */
  function pruneRoadZones() {
    let n = 0;
    try {
      for (const k of Object.keys(G.zones || {})) {
        const c = k.split(','), x = +c[0], z = +c[1];
        if (!isFinite(x) || !isFinite(z) || !isRoad(x, z)) continue;
        delete G.zones[k];
        try { specWrite(x, z, k, null, null); } catch (e) {}
        n++;
      }
      if (n) saveSoon();
    } catch (e) {}
  ...
  function setZone(x, z, id, opt) {
    ...
    /* ⚠ BUT A ROAD MAY ALWAYS BE UN-ZONED — bug-mu2q24h4, "if you accidentally
       zone on a road, you can't use the right click to un-zone". This test used
       to run before the id was looked at, so it refused the CLEAR as well as
       the paint, and a zone that ended up under a road (painted before the road
       was drawn over it) could never be removed. Only painting is refused. */
    if (id != null && isRoad(x, z)) return false;
  ...
  function plan(only) {
    ...
    pruneRoadZones();   // a zone under a road is not a plot — see below
