...
const CACHE_VERSION = 'mythic-v121v168-graveart';
...
const SHELL_CACHE = 'mythic-shell';
const SHELL_KEY = '/__shell__';
function _timeout(p, ms) {
  return Promise.race([p, new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), ms))]);
}
async function serveShell(event) {
  let cache = null, cached = null, cachedVer = '';
  try {
    cache = await caches.open(SHELL_CACHE);
    cached = await cache.match(SHELL_KEY);
    cachedVer = (cached && cached.headers.get('x-shell-version')) || '';
  } catch (e) {}
  let liveVer = '';
  if (cachedVer) {
    try {
      const v = await _timeout(fetch('/version.txt?sw=' + Date.now(), { cache: 'no-store' }), 4000);
      if (v && v.ok) liveVer = (await v.text()).trim();
    } catch (e) {}
    if (liveVer && liveVer === cachedVer) return cached;
  }
  try {
    const fresh = await fetch('/', { cache: 'no-cache', credentials: 'same-origin' });
    if (fresh && fresh.status === 200 && fresh.type === 'basic' && cache) {
      const copy = fresh.clone();
      event.waitUntil((async () => {
        try {
          const html = await copy.text();
          const m = /window\.BUILD_VERSION\s*=\s*'([^']+)'/.exec(html);
          if (!m) return;
          const headers = new Headers(copy.headers);
          headers.set('x-shell-version', m[1]);
          headers.delete('content-length');
          headers.delete('content-encoding');
          await cache.put(SHELL_KEY, new Response(html, { status: 200, headers }));
        } catch (e) {}
      })());
    }
    return fresh;
  } catch (e) {
    if (cached) return cached;        // offline → last known build
    throw e;
  }
}
...
self.addEventListener('fetch', (event) => {
  ...
  /* 🚀 THE APP SHELL, VERSION-CHECKED. Owner: "the game is loading slow".
     Measured on a phone profile (150 ms RTT, 1.6 Mbps): a REPEAT visit spent
     14 s re-downloading the 2.7 MB index.html. The edge serves "/" with no
     ETag (every other file has one), so the browser can never get a 304 and
     `no-cache` means a full download every single open.
     The shell is therefore kept here, tagged with the BUILD_VERSION baked into
     its own bytes, and served only when the tiny version.txt says that is
     still the live build. Any other answer — a new build, version.txt
     unreachable-but-online, a tag that cannot be read — goes to the network
     exactly as before, so a deploy is still visible on the next open.
     ⚠ Top-level "/" and "/index.html" ONLY. Iframe documents keep stepping
       aside (the redirected-iframe failure described above).
     ⚠ Always fetches "/" itself, never `req`: /index.html answers with a 307,
       and a redirected response cannot answer a navigation.
     ⚠ SHELL_CACHE is not version-named, so the activate reaper leaves it; the
       version tag, not the cache name, is what makes it fresh. */
  ...
  /* 📖 …AND /handbook/, FOR A DIFFERENT REASON WITH THE SAME CURE.
     The rules book loads handbook.json, gamedata.json and art.json by plain
     relative path, so none of them carry a ?v= either. Under cache-first they
     pin on a reader's device until CACHE_VERSION moves — which means a rules
     CORRECTION would be invisible to exactly the players it was written for,
     and the book's whole promise is that it is current.
     ⚠ THE PAGE'S OWN `cache: 'no-cache'` CANNOT FIX THIS. The service worker
       answers before the HTTP cache is consulted, so a fetch option set inside
       the page never gets a say. The fix has to be here.
     ⚠ Still falls back to cache when offline, so the book stays readable on a
       train. A few KB of JSON; the round-trip is worth being right. */
