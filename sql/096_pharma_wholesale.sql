-- ════════════════════════════════════════════════════════════════════════════
-- 096 — PHARMA WHOLESALE: lots of finished medicine one Medical Corporation
...
-- 🔴 SCOPED TO 'listed', AND THAT CLAUSE IS LOAD-BEARING. Without it this
--    policy also admits the seller to the sold -> received transition, because
--    the trigger permits that move for anyone RLS let through — so a seller
--    could mark a buyer's lot delivered before the haul landed. Receipt is the
--    BUYER's to record (pl_upd_recv below); once a lot is sold the seller has
--    no further business writing to the row at all.
