-- ===========================================================================
-- MYTHIC SPELLBOOK - RUN ALL PENDING SQL
-- Generated 2026-08-05. Project ktsiasyjusesawtrwrjc.
--
-- Verified against live before generating: all 10 community tables return 404
-- and both RPCs return PGRST202, so every section below is genuinely needed.
--
-- Plain ASCII on purpose. Emoji and box-drawing get mangled when SQL is pasted
-- through a console and the editor then reports invented 42P01 errors on lines
-- that were never SQL.
--
-- Run TOP TO BOTTOM. Every section is idempotent and safe to re-run, and each
-- ends with a verify query - read the output before moving on.
--
-- ORDER IS BY URGENCY, then by dependency:
--   A. Bank of Ethos atomic balance  - closes a LIVE Cinder-minting race
--   B. Economy reset RPC             - the wipe that half-fired on 2026-08-02
--   C. Community 001 -> 002 -> 003 -> 005 (001 MUST run first)
--
-- !! NOT INCLUDED ON PURPOSE: STEP 3 of economy_reset.sql (the directive
--    insert). It ALREADY RAN twice on 2026-08-02. Running it again would make
--    everyone who already paid the wipe pay it a second time. Do not add it.
-- ===========================================================================


-- ###########################################################################
-- SECTION A - BANK OF ETHOS ATOMIC BALANCE  (sql/004)
-- Fixes the lost-update race that let withdrawals mint Cinder. Expect has_rpc 1.
-- ###########################################################################
...
-- ###########################################################################
-- SECTION B - ECONOMY RESET RPC  (STEP 2 of economy_reset.sql ONLY)
-- The server half of the 2026-08-02 wipe, which never landed. No new directive.
-- ###########################################################################

-- Accounts still carrying the durable hg_economy_owed flag repair themselves
-- on their next tick once this function exists. No directive is needed.
  ...
  begin
    update public.user_profiles
       set forge = (coalesce(forge, '{}'::jsonb)
                    ...
                    - '__ownedDiceSkins__' - '__equippedDiceSkin__'
...
-- verify: expect 1
select count(*) as economy_reset_row_exists
from pg_proc p join pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public' and p.proname = 'economy_reset_row';


-- ###########################################################################
-- SECTION C - 001_community_core.sql
-- Communities + membership + roles. Expect tables 2, policies 8, helpers 2, rpcs 2.
-- ###########################################################################
...
-- ###########################################################################
-- SECTION C - 002_community_corps.sql
-- Corp affiliation. Expect tbl 1, policies 4, one_active_index 1.
-- ###########################################################################
...
-- ###########################################################################
-- SECTION C - 003_community_ledger.sql
-- Contribution ledger + audit. Expect tables 2, policies 3, rpc 1.
-- ###########################################################################
...
-- ###########################################################################
-- SECTION C - 005_community_phase2.sql
-- Announcements, votes, objectives, rewards. Expect tables 5, policies 12, rpcs 4.
-- ###########################################################################
