...
begin
  ...
  if v_slot is null then
    /* ⚠ MONEY PATH — this guard must UNDO the fee before it returns.
       The entry fee is taken ~45 lines above. A plain `return` in plpgsql is a
       normal exit, not an abort: the transaction COMMITS, so returning here
       without reversing the payment charges a ticket or 3 real AZA and hands
       back nothing. (A `raise` would roll back instead, but it would also
       destroy the {ok:false, reason} contract every other failure in this
       function returns, and the client switches on `reason`.)

       The advisory lock above should make this unreachable — a run selected
       with count < max_players always has a free slot in 0..max_players-1,
       and slots are unique per run. It is still fixed, for the reason written
       into that lock's own comment: the concurrency assumptions here were
       measured wrong once already (four entrants, four separate worlds, 12
       rounds out of 12). A dead guard on a real-money path is exactly the one
       that must fail safe, because the day it stops being dead nobody is
       watching.

       The ledger stays APPEND-ONLY per CLAUDE.md: the refund is a compensating
       +v_cost line, never a deletion or a rewrite of the debit. */
    if p_pay = 'ticket' then
      update public.warpath_tickets set tickets = tickets + 1, updated_at = now()
        where user_id = v_uid;
    elsif p_pay = 'aza' then
      update public.bank_of_ethos set aza = aza + v_cost, updated_at = now()
        where user_id = v_uid;
      insert into public.boe_ledger (user_id, kind, aza, note)
        values (v_uid, 'warpath_refund', v_cost, 'Warpath entry refunded — run full');
    end if;
    return jsonb_build_object('ok', false, 'reason', 'run_full');
  end if;
