...
/* 🔴 A CONSTANT IS EXTRACTED BY NAME, NEVER BY VALUE, and this suite crashed
   for weeks because one was pinned by value. `lit('const BOE_VAULT_CAP =
   8000;')` was never asserting that the cap IS 8000 — nothing else in this
   file reads that number. It was pulling the declaration so the extracted
   functions have something to close over. So when the vault cap was legitimately
   raised to 15,500 the anchor stopped matching, boemint.mjs threw before its
   first check, and run.mjs counted it as a CRASHED round: every invariant in
   it unmeasured, including the ones that guard against minting ◈ out of thin
   air.
   Matching the declaration and taking whatever value is there keeps the seam
   honest AND lets the number move. If a test ever needs the cap to be a
   specific figure, that belongs in a check that says so out loud — not in an
   extraction anchor, where the failure looks like a crash rather than a
   disagreement. */
function constDecl(name) {
  /* ⚠ No `^…m` anchor. The name is unique in the file, and anchoring bought
     nothing while quietly failing to match at all. */
  const m = SRC.match(new RegExp('const\\s+' + name + '\\s*=\\s*[^;\\n]+;'));
  if (!m) throw new Error('BOEMINT: const ' + name + ' no longer declared in index.html');
  return m[0].trim();
}
...
/* 🔴 THE VAULT CAP AND FEE ARE PART OF THE SEAM, and leaving them out killed
   this file for 59 commits.
   `boeDepositRes` grew a pre-flight cap check and a flat Cinder fee at
   e0d736abe7 ("Ops Vault: 8,000-unit cap and a 500 Cinder deposit fee"). The
   extraction list below was not updated, so the very first line of the
   extracted function called an undeclared `_boeVaultUsed` and threw
   ReferenceError before §1 reached a single assertion. MEASURED: boemint is
   green on e0d736abe7~1 and throws on e0d736abe7 — that one commit is the
   whole regression, and it went unnoticed because run.mjs only counts a
   non-zero exit as "a round failed" and prints no ROUND banner for a crash, so
   a CRASHED sub-gauntlet is indistinguishable in the summary from one that ran
   and failed a check.
   ⚠ The lesson is the extraction contract, not these two names: anything
   boeDepositRes calls that is not mocked in `env` below has to be listed here,
   and the failure mode is a crash that looks like a test result. */
let SEAM = [
  lit('let _boeResChain = Promise.resolve();'),
  lit('let _boeResEpoch = 0;'),
  constDecl('BOE_VAULT_CAP'),
  constDecl('BOE_DEPOSIT_FEE'),
  fn('_boeVaultUsed'),
  fn('_boeResReset'),
  fn('_boeResTx', 'async '),
...
function makeWorld() {
  const w = {
    ...
    toasts: [],
    /* 💸 Cinder, for the deposit fee. It has to be MOCKED rather than left out:
       the fee is `typeof spendGems === 'function' && !spendGems(...)`, so an
       absent mock does not fail — it SKIPS, and the file would then measure a
       deposit path that no player ever runs. Start well above the fee so the
       fee is charged rather than refusing the deposit outright, and count both
       legs so a fee that is taken twice or refunded twice is visible. */
    gems: 100000,
    gemSpent: 0,
    gemRefunded: 0,
...
console.log('\n§5  no reset: the deposit stands');
{
  ...
  /* 💸 THIS CHECK EXISTS TO PROVE THE FEE PATH IS REACHED AT ALL.
     The fee is behind `typeof spendGems === 'function'`, so if the mock is ever
     dropped from `env` the fee silently does nothing and every section above
     keeps passing while measuring a deposit no player can make. Asserting the
     charge landed is what makes that removal visible. It is pinned in §5 and
     NOT in §1–§3 deliberately: those run under the sabotage switches, whose
     "9 failures each, the SAME 9" baseline in this file's header is a
     documented measurement, and a fee assertion on an unwind path would move
     that number and make the two switches disagree. */
  ok(w.gemSpent === 500 && w.gemRefunded === 0,
     'the deposit fee was really charged once and not refunded (the fee path is live)',
     '→ spent ' + w.gemSpent + ', refunded ' + w.gemRefunded);
}
