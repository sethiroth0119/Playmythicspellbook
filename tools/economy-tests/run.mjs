   ...
   ════════════════════════════════════════════════════════════════════════════ */
{
  ...
  if (!CITY_MAP) {
    ...
    /* ── 🔁 CYCLE-DARK IS NOT PRODUCER-DARK ────────────────────────────────
       `reach()` calls a cycle "not a root". That is correct for a walk that
       has to terminate, but it means an id lands on this list for two very
       different reasons and only ONE of them is a missing building:

         • PRODUCER-DARK — something below it has no ECO_BUILDING_MAP row at
           all. Nobody in the city can make it, at any price. This is the
           thing this round exists to drive down, and the ceiling is on it.

         • CYCLE-DARK — every input below it HAS a producer, but the walk
           re-enters a real graph cycle. ECONOMY.md names two of these as
           deliberate (steel⇄recycledMetal, electricity⇄its fuels), says
           plainly that "a real economy is circular", and `topoOrder()` breaks
           them on purpose. A third, steel⇄metalAlloys, is the same shape.

       CONFLATING THEM MAKES THE METRIC PUNISH ADDING A PRODUCER. Giving a
       cyclic id a building moves it INTO `MAKEABLE`, where it immediately
       lands dark and the total goes UP — even though the city can now make
       one more thing than it could before. The automotive chain hit exactly
       that: 19 new producer buildings took the raw count 19 → 21 while adding
       ZERO ids that nobody can make. Measured at that commit: 12 cycle-dark,
       10 producer-dark.

       ⚠ THE CEILING IS STILL A LITERAL AND STILL ONLY EVER GOES DOWN. This
         splits the population being measured; it does not soften the test.
         `reachCyc` differs from `reach` in ONE line — a re-entered id is
         treated as satisfiable — so an id with a genuinely absent producer
         below it is dark under BOTH and still counts against the ceiling. */
    const memoCyc = new Map();
    function reachCyc(id, stack) {
      if (R.isDeposit(id)) return true;
      if (R.isByproduct(id)) return false;
      if (memoCyc.has(id)) return memoCyc.get(id);
      if (stack.has(id)) return true;               // ← the only difference
      if (!MAKEABLE.has(id)) { memoCyc.set(id, false); return false; }
      stack.add(id);
      const leg = R.legsOf(id)[0] || { in: {} };
      let ok = true;
      for (const inp in (leg.in || {})) if (!reachCyc(inp, stack)) { ok = false; break; }
      stack.delete(id);
      memoCyc.set(id, ok);
      return ok;
    }

    /* ── ♻️ BYPRODUCT-DARK IS NOT PRODUCER-DARK EITHER ──────────────────
       Same argument as the cycle split above, one category further, and it is
       not this file's opinion — IT IS THE SHIPPED SIM'S. sim.js:754 carries

         const BYPRODUCT_OK = new Set([...])
         'Byproducts arrive from the city's own activity rather than from a
          supplier, so a recycling step is never blocked for want of a waste
          producer.'

       so the running economy treats waste as AVAILABLE while `reach()` above
       returns false for it. The test and the thing it tests disagreed, and the
       test was the one that was wrong: every id in the recycling chain was
       counted as 'nobody can make this' when the sim will happily run it.

       This is the failure the cycle note predicts — 'CONFLATING THEM MAKES THE
       METRIC PUNISH ADDING A PRODUCER'. Giving the city a Trash Crusher, which
       is a real producer for two real ids, moved recycledPlastic and
       recycledGlass into MAKEABLE where they landed straight on the dark list
       and pushed the count 9 → 11. The ceiling did its job and stopped it; the
       answer is the split, not a bigger ceiling.
       ⚠ THE CEILING DOES NOT MOVE. It is 10, it has only ever gone down, and
         this change takes the measured number DOWN (recycledMetal leaves the
         list with its two siblings) rather than making room. If this had
         needed the literal raised it would have been the wrong fix.
       ⚠ IT IS STILL A ONE-LINE DIFFERENCE from `reach`, exactly as reachCyc
         is, so an id with a genuinely absent producer below it is dark under
         ALL THREE walks and still counts. */
    const memoByp = new Map();
    function reachByp(id, stack) {
      if (R.isDeposit(id)) return true;
      if (R.isByproduct(id)) return true;           // ← the only difference
      if (memoByp.has(id)) return memoByp.get(id);
      if (stack.has(id)) return false;
      if (!MAKEABLE.has(id)) { memoByp.set(id, false); return false; }
      stack.add(id);
      const leg = R.legsOf(id)[0] || { in: {} };
      let ok = true;
      for (const inp in (leg.in || {})) if (!reachByp(inp, stack)) { ok = false; break; }
      stack.delete(id);
      memoByp.set(id, ok);
      return ok;
    }
    ...
    const cycleDark     = dark.filter(id => reachCyc(id, new Set()));
    const byproductDark = dark.filter(id => !reachCyc(id, new Set()) && reachByp(id, new Set()));
    const producerDark  = dark.filter(id => !reachCyc(id, new Set()) && !reachByp(id, new Set()));

    console.log('\n  ' + lit.length + ' of ' + MAKEABLE.size + ' mapped outputs reach the ground.');
    console.log('  🔁 dark only because the walk re-enters a real cycle (' + cycleDark.length + '):');
    console.log('    ' + (cycleDark.join(', ') || '— none —'));
    console.log('  ♻️ dark only because something below is a BYPRODUCT, which sim.js supplies (' + byproductDark.length + '):');
    console.log('    ' + (byproductDark.join(', ') || '— none —'));
    console.log('  🏗️ dark because something below has NO producer at all (' + producerDark.length + '):');
    console.log('    ' + (producerDark.join(', ') || '— none —') + '\n');
    ...
    const DARK_CEILING = 10;
    chk('nothing below a mapped output is UNMAKEABLE — ceiling ' + DARK_CEILING,
        producerDark.length <= DARK_CEILING, producerDark.length + ' producer-dark ids');
    /* Printed, not asserted. Driving this one down means breaking a real cycle
       in the recipe graph, which is a pricing decision (prices.js derives base
       price off the primary leg) and not a building one. */
    console.log('   (cycle-dark, reported only: ' + cycleDark.length + ')');
    console.log('   (byproduct-dark, reported only: ' + byproductDark.length + ')');
    /* Anti-vacuity: the split must not have swallowed the population. If
       reachByp ever grounded everything, producerDark would be 0 and the
       ceiling would be unfalsifiable — the exact tautology this package exists
       to remove. There ARE ids nobody can make; assert some remain visible. */
    chk('the split did not empty the metric — producer-dark is still a real, non-zero population',
        producerDark.length > 0, 'producer-dark is 0; reachByp is grounding too much');
   ...
   ════════════════════════════════════════════════════════════════════════════ */
{
  ...
  /* 🛣 ROAD_CLASSES — lifted because the road guards below stopped being
     self-contained. `allRoadKeys`/`isRoad` used to compare `t.type === 'road'`
     inline; the road-class work refactored both to route through
     isRoadTile → isRoadType → roadClassOf → ROAD_CLASSES, and this harness
     assembles those functions into a bare `new Function`, so the three new
     callees were free identifiers and every §6/§7 probe died with
     `ReferenceError: isRoadTile is not defined` — the whole suite aborted
     before it reached §8. Lifting the shipped functions verbatim is what this
     section already does for the other twelve; re-implementing the road test
     here would be asserting about a fiction.
     ⚠ THE NULL PROTOTYPE IS PART OF THE GUARD, not a detail. ROAD_CLASSES ships
     as Object.assign(Object.create(null), {…}) precisely so a tile typed
     'constructor' or 'toString' is not a road. lit() evaluates the object
     LITERAL, which has Object.prototype — so re-strip the prototype here, or
     this harness would silently answer "yes, a road" to exactly the input §5
     exists to catch one line up. */
  const ROAD_CLASSES_LIT = lit('const ROAD_CLASSES');
  const ROAD_CLASSES  = ROAD_CLASSES_LIT
    ? Object.assign(Object.create(null), ROAD_CLASSES_LIT) : null;
  const FN = {};
  for (const [k, decl] of Object.entries({
    ...
    roadClassOf: 'function roadClassOf(type)', isRoadType: 'function isRoadType(type)',
    isRoadTile: 'function isRoadTile(t)',
  ...
  const got =
    ...
    /* The road guards are now indirect (isRoadTile → ROAD_CLASSES). If this
       table is ever renamed, every §6/§7 road probe answers "no roads" — which
       is a plausible-looking green, not a crash. Name it here. */
    chk('read ROAD_CLASSES, and it kept its null prototype (a tile typed ' +
        "'constructor' must not be a road)",
        !!ROAD_CLASSES && Object.keys(ROAD_CLASSES).length > 0 &&
        Object.getPrototypeOf(ROAD_CLASSES) === null && !!ROAD_CLASSES.road,
        ROAD_CLASSES ? Object.keys(ROAD_CLASSES).join(',') +
          ' | proto=' + String(Object.getPrototypeOf(ROAD_CLASSES)) : 'NULL') &
    ...
    chk('read the fifteen shipped agent/road guards + BUILDINGS + AGENTS + WEATHER + TRUCK_STOPS + the ops loop',
      ...
      const api = new Function(
        ...
        Object.entries(FN).map(([k, b]) => {
          const args = { weatherTwinType: 'type', twinTileType: 'type', isTruckStop: 'ty',
            roadClassOf: 'type', isRoadType: 'type', isRoadTile: 't',
        ...
        (city, bldSite, opsKeyOf, Object.keys(OP_BP), BLD, TRUCK_STOPS,
         ...
         { morale: 50 }, AGENTS_LIT, ROAD_CLASSES);
   ...
   ════════════════════════════════════════════════════════════════════════════ */
{
  ...
  /* The baseline, packed several ids to a row purely so 261 numbers stay
  ...
  'aerospaceAluminum:43.973653 agriculturalMachinery:24.789101 aluminum:6.8920429',
  'aluminumOre:2.0228571 animalFeed:0.91375884 anomalousEnergy:219.14286',
  'anomalousMatter:255.66667 anomalySensors:140.72752 appliances:21.664285',
  ...
  'bread:1.812369 brick:1.2361633 buses:118.35978 cannedFood:2.8753919 cardBoxes:47.516575',
  'cardStock:3.1244433 cardboard:2.3419063 cars:71.545327 cement:1.7912744 cheese:15.493517',
  ...
  'communicationEquipment:39.824601 compositeMaterials:13.738718 compost:0.2655',
  'computerComponents:49.967544 computers:82.456878 concrete:2.1236238',
  'constructionComponents:9.6989099 constructionEquipment:42.155538',
  ...
  'dataStorageHardware:84.067089 deliveryVehicles:76.939082 diagnosticEquipment:97.293603',
  'diesel:3.6174495 dimensionalMaterial:438.28571 displays:31.072665 droneComponents:72.232696',
  'eggs:1.6729544 electricVehicles:120.79491 electricalComponents:9.8166196 electricity:0.25',
  'electronicComponents:16.709527 electronicWaste:0.0375 emergencyEquipment:29.83679',
  'emergencyFood:4.33016 emergencySupplies:8.9217064 engines:19.664144 fabric:4.3908501',
  'factoryEquipment:55.660623 fertilizer:5.4768835 fiberOpticCable:11.813466 flour:1.110262',
  'freightVehicles:119.20297 freshFish:1.0325 freshWater:0.54575 frozenFood:3.2111626',
  'fruit:0.826 furniture:8.3819265 furnitureComponents:4.5996706 gasoline:3.8039851',
  'generators:19.450881 glass:2.2793239 goldOre:12.586667 gravel:0.38123077',
  'hazardousMaterialEquipment:99.678009 hazardousWaste:0.0375 heavyMachinery:38.825905',
  ...
  'householdGoods:8.9944092 hydrogen:3.328898 industrialChemicals:8.6128014',
  'industrialFuel:3.7663208 industrialGas:2.9236515 industrialMachinery:24.756563',
  'industrialRobots:187.02438 industrialVehicles:63.13841 industrialWaste:0.0375',
  ...
  'lumber:1.4203267 luxuryGoods:12.389771 machineParts:11.191669 maintenanceParts:14.921151',
  'meat:3.7511366 medicalChemicals:14.003373 medicalEquipment:27.591385',
  'medicalSupplies:9.0690974 medicalWaste:0.0375 medicine:16.288482 metalAlloys:9.9476688',
  'metalComponents:9.0343684 microchips:77.135205 miningEquipment:45.198976',
  ...
  'poultry:3.1887602 prefabricatedComponents:12.045535 premiumPaper:7.1042187',
  ...
  'protectiveEquipment:15.545745 pumps:13.9087 quantumComponents:117.60093 quartz:1.4576471',
  ...
  'reinforcedConcrete:4.6074206 reinforcedContainmentMaterials:177.97682',
  'relayComponents:55.224305 researchChemicals:25.718649 researchEquipment:157.79398',
  'residentialWaste:0.0375 restaurantSupplies:3.2747088 rice:0.6195 robotics:103.33642',
  'rubber:6.8652713 sand:0.413 satelliteComponents:105.55551 satelliteSystems:325.27674',
  'seafood:1.18 seaweed:0.85448276 secureElectronics:113.60715 securityEquipment:33.618989',
  'seeds:1.239 semiconductorChemicals:25.254676 semiconductorMaterials:62.055083',
  'sensors:36.152988 servers:296.8389 sheetMetal:9.918069 shellfish:1.9061538 shoes:5.3817367',
  ...
  'sportingGoods:8.4791665 starterDecks:14.537948 steel:7.5632459 stone:0.51625',
  'structuralSteel:11.755945 sugar:1.7457198 sugarCrops:0.58305882 surgicalSupplies:16.446494',
  'surveillanceEquipment:79.704619 syntheticFiber:7.3362476 timber:0.68833333 tires:9.7773829',
  'titanium:7.08 tournamentProducts:45.828122 toys:7.5384717 trucks:105.17855',
  'tungsten:10.298182 turbines:37.471083 vegetables:0.708 vehicleParts:15.445084',
  'wastewater:0.0375 wheat:0.55066667 wiring:13.437501 wood:0.72882353 woodPanels:2.3785225',
  'woodPulp:2.6176727 zincOre:3.3317647',
  /* 🛢 RE-BASELINED when the refinery cuts entered the ledger. These three are
     INPUTS to alternate feedstock legs with no producer inside the city graph —
     the refinery mini-game makes them — so rawBase() gives them the raw floor,
     which is the honest price for something the derivation cannot cost. They
     are additive: the same commit moved 0 of the existing 258 prices, because
     legsOf() prices off the PRIMARY leg and these only appear on alternates. */
  /* ⚙️🪵 RE-BASELINED when the two unreachable-input defects were fixed — 45 ids,
     and the round's own failure message asks for exactly this, in this commit,
     with the percentages recorded. They are:

       · FERROUS, 2-6% CHEAPER. steel[blast] required `metalAlloys` while every
         metalAlloys leg requires steel — a closed loop with no entry, so
         neither could make a first unit and twelve firms below them were dead.
         Removing that 0.04 removes a charge the chain could never actually have
         paid, so steel −5.97%, sheetMetal −5.56%, structuralSteel −5.07%,
         metalComponents −4.02%, and everything built out of them drifts down
         2-4% (machineParts, vehicleParts, engines, cars, trucks, the machinery
         families). Cheaper heavy industry is the correct consequence: the
         catalogue had been pricing in an input that no city could supply.

       · BOARD, 3-11% DEARER. woodPanels required raw `wood` on top of `lumber`
         and no building yields wood as a firm output, so it too was unbuildable.
         The woody input is unchanged in MASS (lumber 0.7 + wood 0.3 → lumber
         1.0) but sawn lumber costs more than logs, so woodPanels +11.47%,
         furnitureComponents +5.29%, furniture +3.36%. That direction is right
         as well: if the log route does not exist, board is made of lumber and
         should cost what lumber costs.

     ⚠ NOTHING WAS RE-BASELINED TO MAKE A NUMBER GO GREEN. Both edits removed an
       input that could never be satisfied; the prices moved because the recipes
       became payable, not because the check was in the way. See
       _ecoreach_smoke.mjs, which pins the reachability these two unblocked. */
  'heavyOil:0.25 kerosene:0.25 naphtha:0.25',
  ];
  const BASELINE = {};
   ...
   ════════════════════════════════════════════════════════════════════════════ */
{
  ...
  if (!gotAll) {
    ...
    /* 🛢 THE THIRD SET: ids promoted with a producer that is not a BUILDING.
       The Cracking Yard drums its cuts into the camp stores, so they are real,
       made, and not derivable from ECO_BUILDING_MAP — see MINIGAME_IDS in
       production.data.js.
       ⚠ IT IS AN IMPORTED LITERAL, NEVER A FILTER OF RESOURCES. The header
         above records a draft that computed its expectation FROM the thing it
         was checking and therefore could not fail; taking this list from the
         module keeps the assertion falsifiable — add an id to RESOURCES that
         is in none of the three sets and this still goes red, which is the
         whole point. */
    const MINIGAME = (PD.MINIGAME_IDS || []).slice();
    /* 🧱 THE FOURTH SET — ids promoted out of node-city's CITY_STOCK. Same
       discipline as the other three: an imported literal, never a filter of
       RESOURCES, so an id that is in none of the four still goes red. */
    const STOCKP = (PD.PROMOTED_STOCK_IDS || []).slice();
    chk('index.html RESOURCES = the 14 legacy ids + the derivation + the mini-game ids, in that order (' + RIDS.length + ')',
        same(RIDS, LEGACY14.concat(STOCKP).concat(DERIVED).concat(MINIGAME)),
        RIDS.length + ' ids vs ' + (LEGACY14.length + STOCKP.length + DERIVED.length + MINIGAME.length) + ' expected');
    ...
    const orphan = RIDS.filter(id => !LEGACY14.includes(id) && !STOCKP.includes(id) && !DERIVED.includes(id) && !MINIGAME.includes(id));
    ...
    /* 🛢 A MINI-GAME PRODUCER COUNTS. The rule is "no id is inert", not "every
       id comes from a tile" — the Cracking Yard is a producer, it is just not a
       building. The list is the curated one from production.data.js, so this
       cannot be used to wave through an id that nothing makes. */
    const noProd = RIDS.filter(id => !produced.has(id) && !MINIGAME.includes(id));
   ...
   ════════════════════════════════════════════════════════════════════════════ */
{
  ...
  const fnText = (src, name) => {
    ...
    /* Keep a leading `async` - A3 made saveNow() async and a lift that starts at
       the word `function` hands the sandbox an await inside a plain function,
       which is a SyntaxError in new Function and took the whole round with it.
       The round0r lifter already did this; the two copies now agree. */
  ...
  if (!gotAll) {
    ...
    try {
      ...
      /* 🛢 "City-made" means MADE BY THE PLAYER rather than found — which the
         refinery's cuts are, even though the thing that makes them is a
         mini-game and not a tile. Leaving them out classified fifteen live
         ledger ids as neither lootable nor made, and this check exists
         precisely because that state once "shipped 56 undecided ids into three
         reward tables". */
      /* 🧱 city-made also covers what was promoted out of CITY_STOCK. */
      CHAIN_IDS = promoted.concat(pd.MINIGAME_IDS || []).concat(pd.PROMOTED_STOCK_IDS || []);
    ...
    if (SALV && rollFn) {
      ...
      /* 🔴 THE TRAP ABOVE IS NO LONGER LATENT — IT WENT LIVE. This used to pin
         exoticVsPool === exoticVsLedger, certifying that the two subtractions
         gave the same answer and that the fix above was pure insurance. The
         refinery promotion pushed the ledger past the loot pool and those two
         numbers came apart for real, so the old pin can never be green again
         and keeping it would be a permanent red that teaches the reader to
         skim. What replaces it is the thing the pin was protecting: the gap is
         now exactly how many exotic salvage rows would VANISH from the drop
         table if that line regressed to RESOURCE_IDS — asserted positive, and
         printed so the number is in front of whoever next touches it. */
      const wouldVanish = exoticVsPool - exoticVsLedger;
      chk('…and the ledger has now outgrown the loot pool, so that fix is load-bearing: ' + wouldVanish + ' exotic rows would vanish if it regressed',
   ...
   ════════════════════════════════════════════════════════════════════════════ */
{
  ...
  const SRC0 = {
    ...
    /* A12 (per-node operation sites) and B2 (a mayor siting their Construction
       Co.) added these three and opsReconcile now calls them. LIFTED, like every
       other row here - a stub would grade a made-up answer. Without them the
       sandbox threw ReferenceError at round0r and the suite died there, 332
       passes reported instead of 1045, read as green because the five known
       failures had already printed. See _checkall.mjs minPasses. */
    opsSiteHere:    fnText(NC, 'opsSiteHere'),
    opsAdoptLegacySites: fnText(NC, 'opsAdoptLegacySites'),
    opsMayorMaySite: fnText(NC, 'opsMayorMaySite'),
    ...
    /* 🏛 …AND THE THIRD STANDING, for the same reason as bldNodeCo above:
       bldHasCo() is now a union of three, and lifting two of them
       ReferenceErrors the moment a gate runs. This one reads `gov`, which the
       sandbox does not declare — its own try/catch answers false for that, so
       the sandbox default is "not a mayor", which is the state every refusal
       below is written against. Caught by the full gate on the round that
       added it: the fast tier does not run this file, so a missing lift here
       surfaces as a crash in tryPlace rather than a failed assertion. */
    bldMayorCo:     fnText(NC, 'bldMayorCo'),
  ...
  /* 🛣 THE ROAD PREDICATES, LIFTED — not stubbed. demolishClick's very first
     ...
     the road-class work moved it behind isRoadTile → isRoadType → roadClassOf
     → ROAD_CLASSES, and this sandbox is a bare `new Function`, so those three
     became free identifiers and §3f died with `ReferenceError: isRoadTile is
     not defined` — taking §4 onward with it. A stub here would be worse than
     the crash: `t.type === 'road'` hand-typed back in would answer "not a
     road" for every road CLASS the table has since gained, and grade its own
     copy instead of the shipped one.
     ⚠ ROAD_CLASSES travels as its whole shipped DECLARATION, prototype and
     all. It is `Object.assign(Object.create(null), {…})` on purpose — a tile
     typed 'constructor' must not be a road — and rebuilding it from a literal
     here would quietly restore Object.prototype and lose that. */
  const ROADCLS  = constDecl(NC, 'ROAD_CLASSES');
  const ROADFNS  = ['roadClassOf', 'isRoadType', 'isRoadTile'].map(n => fnText(NC, n));
  /* 🙂 THE MOOD PINGS, LIFTED FOR THE SAME REASON AND ON THE SAME RULE.
     The plot-mood work added moodInvalidate('demolish') to demolishClick and
     moodInvalidate('upgrade')/'finish' to the two order paths, so this bare
     `new Function` gained three more free identifiers and §3f died at
     `ReferenceError: moodInvalidate is not defined` — again taking every
     section after it down, which is exactly the shape the road note above
     was written about.
     ⚠ THIS ONE IS SAFE TO LIFT VERBATIM AND WOULD NOT BE SAFE TO STUB. The
     shipped body is `if (window.MythicPlotMood) …invalidate(why)` inside a
     try/catch, and this sandbox already declares `const window = __ctx.win
     || {}` with no mood module on it — so the lifted function does here
     precisely what the shipped one does in a city where the module has not
     mounted: nothing. Hand-writing a no-op would look identical today and
     stop tracking the file the moment mood invalidation grows a side effect,
     which is the failure the road comment calls 'grading its own copy'. */
  const MOODFNS = ['moodInvalidate', 'moodBeat'].map(n => fnText(NC, n));
  ...
  const gotAll =
    ...
    /* Named explicitly: if ROAD_CLASSES or a predicate is ever renamed, the
       template interpolates '' and demolishClick ReferenceErrors again — loud,
       but 300 lines from the cause. Say it here instead. */
    chk('lifted ROAD_CLASSES and the three road predicates demolishClick calls',
        !!ROADCLS && ROADFNS.every(Boolean) && ROADCLS.indexOf('Object.create(null)') > 0,
        'ROAD_CLASSES=' + (ROADCLS ? ROADCLS.length + ' chars' : 'NULL') +
        ' fns=' + ROADFNS.map(f => f ? 'ok' : 'NULL').join('/')) &
    /* Named for the same reason as the road guards one line up: a rename here
       interpolates '' and §3f ReferenceErrors 300 lines from the cause. */
    chk('lifted the mood pings the order and demolish paths call',
        MOODFNS.every(Boolean),
        'fns=' + MOODFNS.map(f => f ? 'ok' : 'NULL').join('/')) &
  ...
  const mkCity = (ctx) => {
    const S = `
      ...
      // 🚫 A refusal is a modal in the city now (node-city refuse()); here it is
      // the same 'bad' toast the graders already read.
      function refuse(m) { toast(m, 'bad'); }
      // 👥 The named roster's lift (node-city npcWorkMult) — ×1 here, the same as
      // a city with nobody on the books, so the baseline stays comparable.
      function npcWorkMult() { return 1; } function npcSeatsAt() { return 0; } function npcNamedAt() { return 0; }
      ...
      /* 🔴 ADDED WHEN THE BEAT GREW A CALLER AND THIS DID NOT. autoStaffTick
         joined animate()'s 4-second block when citizens started taking jobs by
         themselves; BEAT_SRC is scraped from that block VERBATIM, so the eval
         threw "autoStaffTick is not defined" and took the whole run down —
         including round0p's promotion-drift check, which therefore has not
         reported for as long as the stub has been missing.
         ⚠ THIS IS THE COST OF SCRAPING LIVE SOURCE, and it is worth paying:
           the alternative is a copy of the beat that drifts from the real one
           silently. A new sibling in animate() means a new stub here, and the
           failure is loud, immediate and names the function. */
      function autoStaffTick() { spy.beat.push('autoStaffTick'); }
      function decayTick() { spy.beat.push('decayTick'); }
      function nodeXpTick() { spy.beat.push('nodeXpTick'); }
      /* Same story as autoStaffTick above — a later addition to animate()'s
         beat that never got a stub here. */
      function thriveDividendTick() { spy.beat.push('thriveDividendTick'); }
      ...
      /* A12 made a site belong to ONE node: opsSiteHere() compares site.nodeId to
         OPS.st.nodeId, and with no st.nodeId declares NOTHING here - the safe
         failure its own comment names. This stub builds st by hand and dropped
         the node, so every manifest below (all stamped nodeId n) read as sited
         elsewhere: reconcile touched no row, returned undefined, six checks
         graded a designed refusal as a lost licence, and the suite died at
         337/1045 on the TypeError that followed. The city under test is on
         node n, and st says so. */
      /* ...and it WRITES THE CACHE, as the real opsRefresh does (OPS.st = st).
         opsSiteHere() reads OPS.st, never the argument, so a stub that only
         returned st left the cache at null: the node id on the manifest was
         right and nothing read it. */
      async function opsRefresh() { const st = { nodeId: MANIFEST.nodeId || 'n', ops: __ctx.blindRegister ? [] : MANIFEST.ops, unavailable: !!__ctx.blindRegister, manager: false }; OPS.st = st; OPS.at = Date.now(); return st; }
  ...
  {
    const manifest = { nodeId: 'n', ops: [{ id: 'lic-1', type: 'construction', site: { nodeId: 'n', x: 5, y: 6, rot: 0, sitedAt: Date.now(), eff: 1 } }] };
    const spy = mkSpy();
  ...
  }
  {
    ...
    const manifest = { nodeId: 'n', ops: [{ id: 'lic-2', type: 'construction', site: { nodeId: 'n', x: 7, y: 7, rot: 0 } }] };
    const spy = mkSpy();
  ...
  {
    const manifest = { nodeId: 'n', ops: [{ id: 'lic-3', type: 'construction', site: { nodeId: 'n', x: 9, y: 9, rot: 0, sitedAt: Date.now(), eff: 1 } }] };
    const spy = mkSpy();
  ...
  }
  {
    ...
    const manifest = { nodeId: 'n', ops: [{ id: 'lic-4', type: 'construction', site: { nodeId: 'n', x: 9, y: 9, rot: 0 } }] };
    const spy = mkSpy();
  ...
  {
    const manifest = { nodeId: 'n', ops: [{ id: 'lic-5', type: 'construction', site: { nodeId: 'n', x: 11, y: 11, rot: 0 } }] };
    const spy = mkSpy();
  ...
  }
  {
    ...
    const manifest = { nodeId: 'n', ops: [{ id: 'lic-6', type: 'construction', site: { nodeId: 'n', x: 12, y: 12, rot: 0 } }] };
    const spy = mkSpy();
  ...
  {
    const manifest = { nodeId: 'n', ops: [{ id: 'lab-1', type: 'smuggling', site: { x: 3, y: 3, rot: 0 }, lab: { tier: 2 } }] };
    const spy = mkSpy();
    const api = mkCity({ spy: spy, B: BLDG, manifest: manifest });
    ...
    if (gotGate) {
      ...
      // ── (4) REFUSAL ONE — no free crew ──────────────────────────────────────
      {
        ...
        chk('§8 REFUSAL 1 — with every crew working, a new order is refused before anything is charged',
            /* ⚠ /crew|gang/, not /crew/. The refusal is produced by
               bldCrewBusyMsg(), whose wording was changed to "Every build gang
               is out" — so a check pinned to the word "crew" started failing on
               a gate that was working perfectly: no tile, nothing charged, one
               bad toast, exactly as required. A test that greps prose has to
               accept the prose being edited; what matters is that the refusal
               NAMES THE REASON, and either word does that. The three
               structural conditions either side are the real assertion. */
            !api.game.tiles['5,5'] && spy.paid.length === 0 && spy.toasts.length === 1 &&
            spy.toasts[0].kind === 'bad' && /crew|gang/i.test(spy.toasts[0].m),
   ...
   ════════════════════════════════════════════════════════════════════════════ */
{
     ...
     Prove it can fail: defer-noSave / defer-serverwrite / defer-nostamp. */
  {
    ...
    if (BRIDGE && VERDICT && POLICY && STAMP_LINE && RAISE_LINE) {
      ...
      /* A5 CHANGED THE RULE, AND THIS CHECK FOLLOWS IT. It used to want policy
         'full' and one server write here: a signed-in player with no row got
         an EMPTY city_state row founded on their first autosave. That is what
         left a phantom empty row on the OLD key within sixty seconds of
         city_move_node(), and what cityStateLoad's wrong-node retarget then
         had to choose between. _savePolicy now writes NOTHING for an empty grid
         the player never touched (verdict 'new' still - the tranche is still
         due), and the first real tile is what founds the row, by the player's
         own hand. Both halves are pinned: the untouched grid writes nothing,
         and one tile later the write is 'full' and lands exactly once. */
      chk('§3c …and the first trustworthy read decides it: new city, tranche due - but an untouched grid founds no row',
          a3.v === 'new' && a3.pol === 'none' && a3.P.serverWrites === 0,
          'verdict ' + a3.v + ', policy ' + a3.pol + ', server writes ' + a3.P.serverWrites);
      const a4 = await session('signs in, server has no city, places the first tile', { signedIn: true, uid: 'u-1', serverRow: null }, devA, [['1,1', { type: 'house' }]]);
      chk('§3c …and the FIRST REAL TILE founds the row: full policy, exactly one server write',
          a4.v === 'new' && a4.pol === 'full' && a4.P.serverWrites === 1,
          'verdict ' + a4.v + ', policy ' + a4.pol + ', server writes ' + a4.P.serverWrites);
       ...
       in. */
  {
    ...
    /* The header is REBUILT here, not lifted - so it has to say async when the
       shipped function does. A3 made saveNow() async (it awaits saveCity and
       reports only what the server accepted), and a rebuilt plain header put an
       await inside a non-async function: SyntaxError at new Function, the
       whole save round gone, 485 passes where 1045 are normal. */
    const declFn = (name) => {
      const b = srcBlockAfter(ncW, 'function ' + name + '()');
      const isAsync = ncW.indexOf('async function ' + name + '()') >= 0;
      return b ? (isAsync ? 'async ' : '') + 'function ' + name + '() ' + b : null;
    };
    ...
    if (SOON && NOW && POL) {
      ...
      /* 🛡 THE LOSS GUARD'S TWO NAMES. saveNow() gained
         `_lastGoodObjects = _cityObjectCount()` when the city-disappearing bug
         was fixed, and both live at node-city's MODULE scope — so the lifted
         copy threw ReferenceError and this round reported "THREW
         _cityObjectCount is not defined" instead of anything about saving. The
         §3e check above ("every function the save path calls is still
         DECLARED") stayed green throughout, because the function does exist;
         it is this synthetic scope that could not see it.
         ⚠ The counter is injected as a FAITHFUL COPY of the shipped one — tiles
           that are not the anchor — and NOT as a stub returning a constant. A
           stub would let a guard that mis-counts sail through the very round
           that exists to drive it. `_lastGoodObjects` is declared as a plain
           mutable binding, exactly as node-city declares it. */
      const mkWriters = new Function(
        'game', 'MythicCityBridge', 'console', 'setTimeout', 'clearTimeout', 'serialize',
        '_loadDone', '_loadFailed', '_cityVerdict', '_cityObjectCount',
        'let saveTimer = null; let _lastGoodObjects = -1;\n' + POL + '\n' + SOON + '\n' + NOW + '\n' +
      ...
      const drive = (v, loadDone, loadFailed, tiles, how) => {
        ...
        const W = mkWriters(game,
          ...
          () => { let n = 0; try { for (const k in game.tiles) { const t = game.tiles[k];
            if (t && t.type !== 'anchor') n++; } } catch (e) { return -1; } return n; });
        let threw = null;
        try {
   ...
   ════════════════════════════════════════════════════════════════════════════ */
{
  ...
  if (!gotAll) {
    ...
    /* 🛢 THE REFINERY CUTS ARE HAND-PRICED TOO. Their producer is the Cracking
       Yard, not a CITY_PRODUCTION row, so there is no yields/inputs pair to
       re-derive a price FROM — the parity rule below has nothing to divide.
       Treating them as derived would report thirteen ids with "no producer"
       and then compare their shipped price against NaN. */
    const MINIGAME_PRICED = (PD.MINIGAME_IDS || []);
    const DERIVED_IDS = RIDS.filter(id => !HAND_PRICED.includes(id) && !MINIGAME_PRICED.includes(id));
   ...
   ════════════════════════════════════════════════════════════════════════════ */
{
  ...
  const SRC = {
    ...
    /* D18 folded four hand-rolled ownership walks into one predicate and two
       helpers, and both _opsAllRows and the grant call them now. This sandbox
       has to carry them or every grant check dies with "_opIsType is not
       defined" — which is what happened: 558 passes against 1046. It was
       caught by _checkall's pass FLOOR and not by the failure count, because
       the five baseline reds print first, so got === expect and the suite
       looked green while checking half of itself.
       LIFTED, never stubbed: a stub would grade the sandbox's own idea of who
       owns what, in the round that exists to test exactly that. */
    opIsType:       blk('function _opIsType(o, type)'),
    heldRows:       blk('function _opsHeldRows()'),
    rowsOf:         blk('function _opsRowsOf(type)'),
    ownsOp:         blk('function _ownsOp(type)'),
  ...
  const missing = Object.keys(SRC).filter((k) => !SRC[k]);
  chk('round0u can read all ten shipped functions out of public/index.html',
  ...
  if (!missing.length && callerLine) {
    ...
    const build = (opts) => {
      ...
      const api = new Function(...names, `
        ...
        function _opIsType(o, type) ${src.opIsType}
        function _opsHeldRows() ${src.heldRows}
        function _opsRowsOf(type) ${src.rowsOf}
        function _ownsOp(type) ${src.ownsOp}
...
/* 🔴 AND A THIRD TIME — not a suite that was never wired in, but one that WAS
   wired in, ran every time, and had stopped being a test.
   This loop used to be `stdio: 'inherit'` + `if (r.status !== 0) bad++`, which
   cannot tell these two apart:
     • a suite that RAN and failed N checks              → exit 1
     • a suite that CRASHED before its first assertion   → exit 1
   boemint.mjs was the second one for 59 commits. `boeDepositRes` grew a vault
   cap at e0d736abe7 calling `_boeVaultUsed`, which boemint's extraction list
   did not pull, so it threw ReferenceError at the top of §1 — and the summary
   line said "4 round(s) failed", exactly as it would if a real invariant had
   gone red. Everyone read the count, saw the number they expected from the
   known weaponParts/gunOil failures, and moved on. A guard that cannot report
   its own death is worse than an absent one, because the number it produces
   looks like a measurement.
   So the output is CAPTURED and inspected rather than passed straight through:
   a fatal uncaught exception in Node ends with a `Node.js v…` epilogue, and a
   signal kill leaves r.signal set. Either is reported as CRASHED on its own
   line, and crashes are counted and printed separately from failures so the
   summary can never again show a dead suite as a failing one.
   The output is printed verbatim immediately after each suite, so this costs
   ordering within the run and nothing else. */
let crashed = 0;
const CRASH_EPILOGUE = /\r?\nNode\.js v\d+\./;
for (const f of ['gauntlet1.mjs', 'gauntlet2.mjs', 'gauntlet3.mjs', 'fuelarb.mjs', 'boemint.mjs', 'repairtrap.mjs', 'azapeg.mjs']) {
  ...
  const isCrash = !!r.signal || r.error != null || CRASH_EPILOGUE.test(out);
  if (isCrash) {
    crashed++; bad++;
    console.log('\n💥 ' + f + ' CRASHED — it did not run to its own summary, so every '
      + 'invariant in it is UNMEASURED. This is not a failing check; it is an absent test.'
...
console.log(bad
  ? '\n❌ ECONOMY GAUNTLET: ' + bad + ' round(s) failed'
      + (crashed ? ' — of which ' + crashed + ' CRASHED and measured nothing' : '')
